/*
 * recovered_stubs.c - Recovered stub implementations
 * Auto-recovered from Ghidra decompilation.
 */

#include "warlords2.h"

#ifndef CONCAT22
#define CONCAT22(hi, lo) ((int)(((unsigned int)(unsigned short)(hi) << 16) | (unsigned short)(lo)))
#endif
#ifndef CONCAT44
#define CONCAT44(hi, lo) ((long long)(((unsigned long long)(unsigned int)(hi) << 32) | (unsigned int)(lo)))
#endif
#ifndef ZEXT48
#define ZEXT48(x) ((unsigned long long)(unsigned int)(x))
#endif
#ifndef SUB41
#define SUB41(x, n) ((char)((x) >> ((n) * 8)))
#endif
#ifndef SUB42
#define SUB42(x, n) ((short)((x) >> ((n) * 8)))
#endif

/* Extern declarations */
extern long iRam10117700;
extern long iRam10117424;
extern long gCombatDisplayPieces;
extern long gFrontLineLookup;
extern long gSelectionArray;
extern long iRam101174ac;
extern char *pcRam10116398;
extern char *gSelectionCount;
extern int *piRam10115cf4;
extern int *gCityOwnership;
extern int *piRam10117470;
extern short *gHeroCount;
extern short *psRam1011677c;
extern short *psRam10115d84;
extern short *psRam10115d80;
extern unsigned int *puRam101176e0;
extern unsigned int *puRam10117360;
extern long iRam10115dc8;
extern long gSCENInfo;
extern unsigned int uRam10117604;
extern short *psRam10115ce0;
extern short *psRam10116304;
extern long iRam10115dd4;
extern long iRam10115e58;
extern long iRam10115e5c;
extern long iRam10115e60;
extern long iRam10115e64;
extern long gStdMoveCostTable;
extern long gTileWalkOffsets;
extern long gZoCNeighborDYOffsets;
extern long gZoCNeighborDXOffsets;
extern long gFlyingMoveCostTable;
extern long gPlayerColorTable;
extern long gPerPlayerArmyCount;
extern long gMinimapGWorld;
extern long gDisplayState;
extern long iRam1011773c;
extern int *piRam10115e34;
extern int *gMainGameWindow;
extern int *gAppObject;
extern int *piRam10116bc0;
extern int *piRam101174b0;
extern short *psRam10115ef8;
extern short *psRam10115f00;
extern short *gScrollLockFlag;
extern short *gUIShortGlobal;
extern short *gLastArmyTypeForCostMap;
extern short *psRam1011760c;
extern unsigned int *puRam10115edc;
extern unsigned int *puRam10115ee0;
extern unsigned int *puRam10115efc;
extern unsigned int *puRam101174b0;
extern unsigned int *puRam10117544;
extern unsigned int *puRam101175f0;
extern unsigned int *puRam101175f8;
extern unsigned int *gViewportOffset;
extern unsigned int uRam101175f8;
extern long iRam10115e30;
extern long gNeighborDYOffsets;
extern long gNeighborDXOffsets;
extern long gNeighborPatternTable;
extern long gSpriteTable;
extern long gArrowDirTable;
extern long iRam101173cc;
extern long iRam101173d0;
extern long iRam101173d4;
extern long iRam1011746c;
extern long iRam101175ec;
extern long iRam10117684;
extern long iRam10117738;
extern long gSoundPrefs;
extern char *pcRam10117378;
extern char *pcRam1011785c;
extern int *piRam10115d88;
extern int *piRam10115dc4;
extern int *piRam10115df0;
extern int *gUnitDataHandle;
extern int *piRam10115f64;
extern int *piRam10115f7c;
extern int *gOverviewWindow;
extern int *gScreenGWorld;
extern int *gInfoWindow;
extern int *gMainDocView;
extern int *gSidePanelWindow;
extern int *gActiveDocView;
extern int *gActiveView;
extern int *gSndHandle;
extern int *gSndChannel;
extern int *gTunePlayer;
extern int *piRam101165f4;
extern int *piRam101165f8;
extern int *gProgressView;
extern int *piRam10117114;
extern int *piRam101173e0;
extern int *piRam10117468;
extern int *piRam101174a8;
extern int *gPathCacheHandle;
extern int *gPathCachePtr;
extern int *gCostMapHandle;
extern int *gDistGridHandle;
extern int *gCostMapPtr;
extern int *piRam101175d0;
extern int *gMapGenData;
extern int *gMapGenConfig;
extern int *piRam101176b8;
extern int *piRam10117804;
extern short *psRam10115d18;
extern short *psRam10115dec;
extern short *psRam10115df4;
extern short *gPathfindingInitialized;
extern short *gAIModeFlag;
extern short *gSpriteCount;
extern short *psRam10115f68;
extern short *psRam101165d8;
extern short *psRam101165dc;
extern short *gCurrentMusicID;
extern short *psRam10117424;
extern short *psRam10117428;
extern short *gZoCPenalty;
extern short *gSearchRadius;
extern short *gMaxSearchRange;
extern unsigned int *puRam1011529c;
extern unsigned int *puRam101152c4;
extern unsigned int *puRam101152c8;
extern unsigned int *puRam101152cc;
extern unsigned int *puRam101152d4;
extern unsigned int *puRam101152d8;
extern unsigned int *puRam101152dc;
extern unsigned int *puRam10115308;
extern unsigned int *puRam10115314;
extern unsigned int *puRam10115330;
extern unsigned int *puRam10115378;
extern unsigned int *puRam10115380;
extern unsigned int *puRam10115390;
extern unsigned int *puRam101153e8;
extern unsigned int *puRam1011540c;
extern unsigned int *puRam10115480;
extern unsigned int *puRam101154a0;
extern unsigned int *puRam10115504;
extern unsigned int *puRam1011553c;
extern unsigned int *puRam10115540;
extern unsigned int *puRam10115544;
extern unsigned int *puRam1011556c;
extern unsigned int *puRam10115574;
extern unsigned int *puRam10115584;
extern unsigned int *puRam1011561c;
extern unsigned int *puRam10115630;
extern unsigned int *puRam1011563c;
extern unsigned int *puRam10115640;
extern unsigned int *puRam10115658;
extern unsigned int *puRam1011565c;
extern unsigned int *puRam10115688;
extern unsigned int *puRam101156b8;
extern unsigned int *puRam101156c0;
extern unsigned int *puRam101156c4;
extern unsigned int *puRam101156d0;
extern unsigned int *puRam101156fc;
extern unsigned int *puRam10115708;
extern unsigned int *puRam10115754;
extern unsigned int *puRam10115758;
extern unsigned int *puRam10115768;
extern unsigned int *puRam10115800;
extern unsigned int *puRam10115808;
extern unsigned int *puRam10115814;
extern unsigned int *puRam1011581c;
extern unsigned int *puRam1011583c;
extern unsigned int *puRam10115844;
extern unsigned int *puRam1011584c;
extern unsigned int *puRam10115858;
extern unsigned int *puRam1011585c;
extern unsigned int *puRam10115874;
extern unsigned int *puRam101158d0;
extern unsigned int *puRam101158d4;
extern unsigned int *puRam101158d8;
extern unsigned int *puRam101158dc;
extern unsigned int *puRam101158e0;
extern unsigned int *puRam101158ec;
extern unsigned int *puRam10115998;
extern unsigned int *puRam101159a0;
extern unsigned int *puRam10115a10;
extern unsigned int *puRam10115a30;
extern unsigned int *puRam10115ad8;
extern unsigned int *puRam10115af0;
extern unsigned int *puRam10115af4;
extern unsigned int *puRam10115b04;
extern unsigned int *puRam10115b10;
extern unsigned int *puRam10115b24;
extern unsigned int *puRam10115b2c;
extern unsigned int *puRam10115b4c;
extern unsigned int *puRam10115b70;
extern unsigned int *puRam10115b74;
extern unsigned int *puRam10115b78;
extern unsigned int *puRam10115d80;
extern unsigned int *gPatternDrawFlag;
extern unsigned int *puRam10115f40;
extern unsigned int *gProgressBar;
extern unsigned int *puRam10115fa8;
extern unsigned int *puRam10116208;
extern unsigned int *puRam101163ac;
extern unsigned int *puRam101165f0;
extern unsigned int *gTuneHeaderH;
extern unsigned int *gTuneDataH;
extern unsigned int *puRam10116910;
extern unsigned int *puRam10116914;
extern unsigned int *puRam101169c4;
extern unsigned int *puRam10116a40;
extern unsigned int *puRam10116bc0;
extern unsigned int *puRam10117098;
extern unsigned int *puRam10117170;
extern unsigned int *puRam1011735c;
extern unsigned int *puRam10117370;
extern unsigned int *puRam10117424;
extern unsigned int *puRam10117428;
extern unsigned int *puRam10117468;
extern unsigned int *gDistGridDeref;
extern unsigned int *gDistGridHandlePtr;
extern unsigned int *puRam101175ac;
extern unsigned int *gCurrentPathFlags;
extern unsigned int *puRam101175f4;
extern unsigned int *puRam1011760c;
extern unsigned int *gCurrentMoveMode;
extern unsigned int *gCombatSoundIDs;
extern unsigned int *puRam10117850;
extern unsigned int *puRam10117854;
extern unsigned int *puRam10117858;
extern unsigned int uRam10115ed8;
extern unsigned int uRam10116070;
extern unsigned int uRam101169c8;
extern unsigned int uRam10116a54;
extern unsigned int uRam10116bbc;
extern unsigned int uRam10116c54;
extern unsigned int uRam10116c60;
extern unsigned int uRam10116c7c;
extern unsigned int uRam10116f40;
extern unsigned int uRam10116ff0;
extern unsigned int uRam10117018;
extern unsigned int uRam101172f8;
extern unsigned int uRam101173c8;
extern unsigned int uRam101173d8;
extern unsigned int gItemNameBuffer;
extern unsigned int gColorPalette;
extern unsigned int uRam1011772c;
extern unsigned int gProgressCounterBase;
extern short *gAutoModeFlag;
extern unsigned int uRam101174f0;
extern unsigned int uRam101174d0;

/* Forward declarations */
void LockHandle_Thunk();
void ReleaseResource_Thunk();
void FUN_10000300();
void DisposeGeneric();
void FUN_10000390();
void ResizeHandle();
void MakeAlias();
int FUN_100006f0();
void CopyBits_Thunk();
void SndDisposeChannel_Thunk();
void CopyMask_Thunk();
void FUN_100013b0();
void FUN_100013c8();
void FUN_100013e0();
void FUN_100013f8();
void FUN_10001410();
void FUN_10001428();
void FUN_10001440();
void FUN_10001458();
int FUN_10001470();
void FUN_10001488();
int FUN_100014b8();
void FUN_100014d0();
void FUN_10001590();
void FUN_100015a8();
int SndPlay_Thunk();
void FUN_10001680();
void FUN_100016b0();
#ifdef MODERN_BUILD
void ResolveAlias();
#endif
void FUN_100018f0();
void FUN_10001908();
void FUN_10001920();
void FUN_100019f8();
#ifdef MODERN_BUILD
int GetString();
#endif
void DrawNumber();
void BlockMoveData_Thunk();
void FreeHandle();
void CopyHandleData();
int OpenDefaultComponent_Thunk();
int TuneSetHeader_Thunk();
void PaintRect_Thunk();
void SndDoImmediate_Thunk();
int Get1NamedResource_Thunk();
void CopyDataToBuffer();
#ifdef MODERN_BUILD
void AddResource();
#endif
void TuneSetVolume_Thunk();
void FUN_10002970();
int GetHandleDataSize();
void DisposeHandleFull();
int SndNewChannel_Thunk();
int TuneQueue_Thunk();
void TuneGetStatus_Thunk();
void TuneUnroll_Thunk();
int SndChannelStatus_Thunk();
void TuneStop_Thunk();
int TunePreroll_Thunk();
void FUN_10005b68();
void FUN_10007c50();
void FUN_100099cc();
int LockPixels_Wrapper();
void UnlockPixels_Wrapper();
void BlitTerrainPixel();
void EraseGWorld(int param_1,short param_2);
void DrawHLine(int param_1,short param_2,short param_3,short param_4,short param_5);
void DrawVLine(int param_1,short param_2,short param_3,short param_4,short param_5);
void FUN_1000c67c();
int FUN_1000d910(short param_1);
long long FUN_1000f6a0(short param_1);
long long CanArmyEnterCity(short param_1,short param_2);
/* short AI_GetRuinCapability(short param_1); -- clean version in ai/ai.c */
long long GetDirection(short param_1,short param_2,short param_3,short param_4);
/* void AI_ActivateBestGroup(); -- clean version in ai/ai.c */
void WriteCityRecords(long long param_1);
void ReadCityRecords(long long param_1);
void FUN_1001f5e8(short param_1,short param_2);
long long GetArmyInfo(short param_1,short *param_2);
long long ScoreUnitForHire(short param_1);
void FUN_10021200(int param_1,char param_2);
void StoreCoords(short *param_1,short param_2,short param_3);
void FUN_10025598();
void FUN_1002565c();
void FUN_10025f94();
void FUN_10026310();
void FUN_10026568();
void CopyResourceToDoc(int *param_1,long long param_2);
void FUN_10030454(long long param_1);
void FUN_10031f54(long long param_1);
void FUN_10031fc4();
void FUN_10032860(short param_1,short param_2,long long param_3,short param_4);
void FUN_10037fc4();
long long SerializeMoveData(int param_1,short *param_2);
void FUN_10038c60(short param_1,char param_2,short param_3,short param_4,
                 long long param_5);
void FUN_1003a230(int *param_1);
void FUN_1003a5f8();
void FUN_1003aeb0();
int FUN_1003c938();
void FUN_1003cac4(long long param_1,short param_2);
void FUN_1003ced4(long long param_1,long long param_2);
void FUN_1003d5d4();
void FUN_1003d6c4();
void FUN_1003e13c(long long param_1,short param_2);
void FUN_1003f674(short param_1,short param_2,short param_3);
void UpdateProgressBar(short param_1);
void InitDataStructures(long long param_1,long long param_2,long long param_3,long long param_4,
                 long long param_5,long long param_6,long long param_7);
void FUN_100423a0(short param_1);
void PostLoadCityInit();
void FUN_100424e4();
int FUN_100425c0(short param_1,short param_2,short param_3);
long long FUN_100426b4(short *param_1,short param_2);
long long FUN_100427cc(long long param_1,short param_2);
short FUN_10043248(short *param_1,short param_2);
void FUN_100439a4(short *param_1);
void FUN_10043c84(short param_1,short *param_2,short param_3);
long long FUN_10043e60(short *param_1);
unsigned char FUN_100445fc(int param_1,short param_2,short param_3,short param_4,short *param_5);
short FUN_10044728(short *param_1,short param_2,int param_3);
void InitCityNames();
void EnsureSpriteAndLock(short param_1);
void UnlockAndReleaseSprite(short param_1);
void EnsureSpriteLoaded(short param_1);
void InitArmyProduction();
int CreateUnitTemplate(short param_1,short *param_2,short *param_3,short *param_4,
                short *param_5,short *param_6);
void FUN_1004a178(short param_1,short *param_2,short *param_3,short *param_4,
                 short *param_5);
int FUN_1004a21c(short param_1);
void FUN_1004f620();
void GetScoreSnapshot(int param_1);
void ProgressCreate(long long param_1,long long param_2,short param_3);
void ProgressUpdate(int *param_1,short param_2);
void GenerateRandomMap();
void NotifyTileChanged();
void NotifyAllTilesChanged();
void ClampToMapBounds(short *param_1,short *param_2);
void RandomScatter(short *param_1,short *param_2,short param_3,short param_4);
long long FUN_10052184(long long param_1,short param_2,unsigned short param_3);
int * FUN_100522dc(int *param_1,unsigned short param_2,char param_3);
void StreamWrite();
void StreamSetPosition(int *param_1,long long param_2,short param_3);
long long ByteSwap32(int param_1);
void FUN_10052f6c();
void FUN_100534c8(short param_1);
void ReadSCNResource(int *param_1,int param_2);
void WriteSCNResource(int *param_1,int param_2);
void FUN_1005cc8c();
void ProgressDelay(short param_1);
void FUN_1005f1a0(short param_1,short param_2,long long param_3);
void FUN_1005f1d0(short param_1,short param_2,long long param_3);
void FUN_1005f200(short param_1,short param_2,long long param_3);
int FUN_1005f678(short param_1,short param_2);
int FUN_1005f6b0(short param_1,short param_2);
void DrawMovementPathArrow(short param_1,short param_2,short param_3);
void DrawSingleMapTile(short param_1,short param_2,short param_3);
unsigned long long SaveDrawingState(unsigned long long param_1);
void RestoreDrawingState(unsigned long long param_1,unsigned long long param_2);
void SetupDrawingEnvironment();
void FUN_10075ee0(int param_1,short param_2,short param_3);
void FUN_1007ec7c(int param_1,long long param_2);
void FUN_1007ed14(int param_1,long long param_2);
void FUN_1007edac(int param_1,long long param_2);
void FUN_1007f0a4(int param_1,short param_2);
void FUN_10082260(int *param_1);
void FUN_10082f24(int *param_1,short param_2,short param_3);
int * ConstructDialogBehavior(int *param_1);
void FUN_10090e0c(int *param_1,long long param_2,long long param_3,char param_4);
void FUN_10091334();
void FUN_10091338(short param_1,short param_2);
void HideMinimapCursor();
void ShowMinimapCursor();
int * FUN_10091488(int *param_1);
void CSoundHandler_Destroy(int *param_1,unsigned long long param_2);
void CSoundHandler_PlaySound(int *param_1,short param_2,char param_3);
void CSoundHandler_LoadSound(int param_1,short param_2);
short CSoundHandler_LoadSoundByName(int param_1);
void StopGlobalSound();
char IsGlobalSoundPlaying();
void PlayGlobalSound(short param_1,short param_2);
short ResolveSoundName();
unsigned long long IsMusicPlaying();
void FadeAndStopMusic();
void DisposeMusicResources();
void LoadMusic(long long param_1,long long param_2);
void PlayMusic(long long param_1,char param_2);
void YieldToEventLoop();
void SetMusicVolume(short param_1);
void RestoreMusic();
void StopMusic();
void PlayUISound(short param_1);
void PlayNotificationSound();
void PlayAttackHitSound();
void PlayAttackMissSound();
void PlayFanfareSound();
void InitCombatSounds();
int OpenResourceStream(int param_1,short param_2);
void CloseResourceStream(int param_1,unsigned long long param_2);
int FindResourceByType(int param_1,long long param_2,short param_3);
short ChebyshevDistance(int param_1,int param_2);
void ExpandPlayerZone(short param_1);
void ConnectAdjacentZones();
long long ShouldBeCoastal(short param_1,short param_2);
void ConvertToTerrainCodes();
long long HasNeighborOnLand(short param_1,short param_2);
long long HasNeighborWithTerrain(short param_1,short param_2,short param_3);
void ExpandCoastalTiles();
void AddExtraLandPatches();
void PlacePlayerStartZones();
void CalculateLandTarget();
short CheckLandTarget();
void GrowLandBlob();
void FillEnclosedWater();
void GenerateLandMasses();
void PlacePlayerCities();
void PlaceNeutralCities();
void AssignCityAlliances();
void DrawRoadBetweenNeutral(int param_1,int param_2);
void DrawRoadBetweenPlayer(int param_1,int param_2);
void GenerateRoadsBetweenCities();
void SmoothTerrainAroundCities();
void EnsureCityCoastalAccess();
void PlaceCitiesAndRoads();
void BuildRoadConnectivityGraph();
void FinalizeAndSave();
void ScatterSwampTiles(short param_1,short param_2);
void PlaceSwampCluster();
void PlaceSwampClusters();
void PlaceRuins();
void ErodeLandTiles();
void PlaceRiverWaterChannels();
void AssignPlayerColors();
void AssignPlayerStartPositions();
void SmoothMountainRegions();
void SmoothForestRegions();
void PlaceMountainForestChains();
void FUN_100afb5c(int param_1,unsigned long long param_2,unsigned long long param_3);
unsigned long long FUN_100b02d0(unsigned long long param_1);
int GetBitMapPtr();
long long FUN_100b057c(long long param_1,char param_2);
void FUN_100b06d4(short *param_1,short *param_2);
void GetScreenBounds(int *param_1,short *param_2);
char * FUN_100b0960(int param_1,long long param_2);
void FUN_100b0a18(int param_1,char *param_2);
int FUN_100b1058(int param_1,long long param_2,long long param_3,short param_4);
int FUN_100b1134(int param_1,int param_2,long long param_3,long long param_4,
                short param_5);
void FUN_100b1210();
unsigned int FUN_100b1b90(unsigned char *param_1,long long param_2,unsigned int param_3);
int ConvertToString();
void ConcatPStrings(long long param_1,short param_2,short param_3);
char * FUN_100b1d90(char *param_1,long long param_2);
void ConcatBuildName(unsigned char *param_1,unsigned char *param_2,unsigned char *param_3);
long long FUN_100b226c(short *param_1,short *param_2);
void FUN_100b2e74();
void FUN_100b4c68();
int * FUN_100beb7c(int *param_1);
void FUN_100becb8();
void FUN_100c0f10();
int FUN_100c2120(long long param_1);
int FUN_100c43ec();
int * FUN_100c43fc(int *param_1);
int * FUN_100c604c(int *param_1);
int * FUN_100c7d84(int *param_1);
int FUN_100c9950();
int * FUN_100c9960(int *param_1);
int FUN_100ca068();
int * FUN_100ca078(int *param_1);
int * FUN_100d7adc(int *param_1);
void FUN_100d7ba0(int *param_1,long long param_2,short param_3);
void FUN_100da260(int *param_1,long long param_2);
void CheckMemError();
void FUN_100e1674(int *param_1,int *param_2);
int InitEventSystem();
int * ConfigureEventSystem(int *param_1);
int * FUN_100e90bc(int *param_1);
int FUN_100e9e78();
void FUN_100ebbe0(int param_1);
void FUN_100ec5c8(int param_1,long long param_2);
void FUN_100ecf88(short *param_1);
unsigned long long IsHandleValid();
void FUN_100f06dc(short param_1,short *param_2);
void FUN_100f0d3c(int param_1,char param_2);
int FUN_100f1264();
long long FUN_100f2214(short param_1);
void FUN_100f25e0(long long param_1,long long param_2,long long param_3);
void FUN_100f2a54();
void FUN_100f56c0();
void FUN_100f5814();
int * FUN_100fb260(int *param_1);
void SetResourceType(int *param_1,long long param_2,short param_3);
int FUN_1010597c();
int FUN_1010d6a8();
int * FUN_1010d6b8(int *param_1);
int FUN_1010f044();
int * FUN_1010f054(int *param_1);
long long FUN_10115d3c(int param_1);
unsigned long long FUN_10115e4c(int param_1,unsigned int param_2);
unsigned long long FUN_10116338(unsigned long long param_1);
long long FUN_10116e14(short *param_1);
int * FUN_10117024(int *param_1,int *param_2);
int * FUN_101171bc(int *param_1,int *param_2);
void FUN_10004c54();
void DrawFullMapView();
void FUN_100090f0();
/* unsigned long long AI_FindArmyForHero(short param_1); -- clean version in ai/ai.c */
/* long long AI_PlaceNewUnit(short param_1,short param_2); -- clean version in ai/ai.c */
void FUN_1000f7bc(short param_1);
void FUN_1000fac4();
void RefreshMapAfterCombat(short param_1,short param_2);
void FUN_1000fccc(short param_1);
long long CalcArmyStrength(short param_1);
long long FUN_1000ffe0(short param_1,short param_2,int param_3,int param_4);
/* int AI_CullWeakUnits(short param_1,int param_2); -- clean version in ai/ai.c */
/* long long AI_ArmyManagement(short param_1,short param_2); -- clean version in ai/ai.c */
/* long long AI_CheckArmyOwnership(short param_1); -- clean version in ai/ai.c */
/* long long AI_EarlyGameAttack(short param_1,short param_2); -- clean version in ai/ai.c */
/* long long AI_SearchForRuins(); -- clean version in ai/ai.c */
/* void AI_AttackNeutralArmies(); -- clean version in ai/ai.c */
long long FUN_10012a8c(long long param_1,short param_2);
/* long long AI_AttackCity(short param_1,short param_2); -- clean version in ai/ai.c */
/* int AI_FindBestMovableArmy(short param_1); -- clean version in ai/ai.c */
/* long long AI_CheckRuinAccessible(short param_1); -- clean version in ai/ai.c */
/* long long AI_AttackEnemyArmy(short param_1,int param_2); -- clean version in ai/ai.c */
void FUN_10016cc4(int param_1,int param_2,int param_3,short param_4);
long long GetNeighborArmies(short param_1,int param_2,int param_3);
void FUN_1001a12c(short param_1,short param_2);
short ProcessSingleAssignment(short param_1,long long param_2);
/* void AI_CheckArmyPathsBlocked(); -- clean version in ai/ai.c */
short FUN_1001b584(long long param_1,short *param_2,unsigned short param_3,short param_4,int *param_5);
void FUN_1001b8e0(short param_1);
long long FUN_1001c2dc(short param_1,short *param_2,short param_3);
long long FUN_1001c6fc(short param_1,long long param_2,short param_3,short param_4);
long long AdvanceGroupToTarget(short param_1);
void InitSpecialLocations();
void MoveUnitToTarget(short *param_1,short param_2,short param_3);
long long MoveArmyToTarget(int param_1,short param_2,short param_3,unsigned short param_4);
long long CheckArmyEscort(short param_1);
void FUN_1001e564(short param_1,short param_2);
long long IsPositionReachable(short param_1,short param_2);
/* void AI_ResetProductionState(); -- clean version in ai/ai.c */
/* void AI_CheckNearbyThreats(short param_1); -- clean version in ai/ai.c */
/* void AI_PrecomputeThreats(); -- clean version in ai/ai.c */
int FUN_1002122c(short param_1,short param_2,short *param_3);
void FUN_1002279c();
void FUN_10025f2c();
void FUN_100275ec();
char IsMusicPrefsLoaded();
char IsSoundEnabled();
long long PostLoadInitialization();
long long FUN_10028b30(int *param_1,short param_2,short param_3);
short FUN_10028e10(long long param_1,short param_2,short param_3);
void FUN_1002b230(short param_1);
void FUN_1002bbd4();
void GetAIStateSnapshot();
int LookupRuinAtPos(short param_1,short param_2);
unsigned long long FUN_1002bf64(short param_1,short param_2,short param_3,short param_4);
int FUN_1002c200(short param_1,short param_2,short param_3);
void FUN_1002cbbc();
void FUN_1002ce38();
void FUN_1003357c(int param_1,short param_2);
void FUN_10035d7c();
void ClearCommandRecords();
void SaveTurnState();
void FUN_1003956c();
short FUN_10039c58(short param_1);
void UpdateArmyDisplay(short param_1);
void FUN_1003c068();
void FUN_1003cb84();
void FUN_1003d094();
void FUN_1003d734(int *param_1);
int FUN_10042a24(short param_1,short param_2);
void FUN_10042bb4(short param_1,short param_2);
void FUN_10044110(short param_1,short param_2);
void FUN_100445a8(short *param_1,int param_2);
void InitArmyData();
void FUN_100472f4(long long param_1,long long param_2,long long param_3,short param_4);
void FUN_10047de8(short param_1,int param_2,short *param_3);
void FUN_1004f664();
void FUN_10050ffc(long long param_1,long long param_2,long long param_3,long long param_4,
                 long long param_5,int param_6,int param_7);
void SetupAndGenerateRandomMap(short param_1,short param_2,short param_3,short param_4);
int FUN_1005310c(int param_1);
short * FUN_10053838(short param_1,short param_2,short param_3);
void FUN_100539e8(int param_1,short param_2,short param_3);
void FUN_10054824();
void LoadMapResources();
long long LoadScenario(short param_1);
void FUN_1005bbe8();
void DrawArmyOverlayOnMap(int *param_1,short param_2);
void UpdateMinimapAroundTile(short param_1,short param_2,short param_3);
void FUN_1007d168(int *param_1,long long param_2);
void FUN_1007ded4(int param_1,int *param_2,char param_3);
void FUN_1007e314(int param_1);
void FUN_1007e894(int param_1,int *param_2,char param_3);
void ScanForExistingCities();
int FindNextRoadSegment(int param_1,short param_2);
void BuildRoadSegment(int param_1);
void FUN_100eb91c(int param_1,int *param_2,long long param_3,long long param_4,char param_5);
void FUN_10114e58();
char IsMusicEnabled();
void FUN_1003c368();
void FUN_1003c838();
void FUN_10064850(short param_1,short param_2);

/* Address: 0x10000048 Size: 24 bytes */
void LockHandle_Thunk()

{

  ((void (*)())*puRam10115544)();
  return;
}

/* MoveTo_Thunk defined elsewhere */


/* LineTo_Thunk defined elsewhere */


/* Address: 0x100002a0 Size: 24 bytes */
void ReleaseResource_Thunk()

{

  ((void (*)())*puRam10115874)();
  return;
}

/* Address: 0x10000300 Size: 24 bytes */
void FUN_10000300()

{

  ((void (*)())*puRam101153e8)();
  return;
}

/* Address: 0x10000318 Size: 24 bytes */
void DisposeGeneric()

{

  ((void (*)())*puRam1011581c)();
  return;
}

/* Address: 0x10000390 Size: 24 bytes */
void FUN_10000390()

{

  ((void (*)())*puRam10115378)();
  return;
}

/* Address: 0x100004e0 Size: 24 bytes */
void ResizeHandle()

{

  ((void (*)())*puRam1011556c)();
  return;
}

/* Address: 0x100006d8 Size: 24 bytes */
void FUN_100006d8()

{

  ((void (*)())*puRam10115380)();
  return;
}

/* Address: 0x100006f0 Size: 24 bytes */
int FUN_100006f0()

{

  ((void (*)())*puRam10115800)();
  return 0;
}

/* FUN_10000708 defined elsewhere */


/* GetGWorld_Wrapper defined elsewhere */


/* SetGWorld_Wrapper defined elsewhere */


/* Address: 0x100008d0 Size: 24 bytes */
void FUN_100008d0()

{

  ((void (*)())*puRam10115754)();
  return;
}

/* Address: 0x10000900 Size: 24 bytes */
void FUN_10000900()

{

  ((void (*)())*puRam101158d8)();
  return;
}

/* HLock_Thunk_Sound defined elsewhere */


/* HLock_Thunk defined elsewhere */


/* Address: 0x100012f0 Size: 24 bytes */
void CopyMask_Thunk()

{

  ((void (*)())*puRam10115758)();
  return;
}

/* Address: 0x100013b0 Size: 24 bytes */
void FUN_100013b0()

{

  ((void (*)())*puRam1011565c)();
  return;
}

/* Address: 0x100013c8 Size: 24 bytes */
void FUN_100013c8()

{

  ((void (*)())*puRam10115504)();
  return;
}

/* Address: 0x100013e0 Size: 24 bytes */
void FUN_100013e0()

{

  ((void (*)())*puRam101159a0)();
  return;
}

/* Address: 0x100013f8 Size: 24 bytes */
void FUN_100013f8()

{

  ((void (*)())*puRam10115b04)();
  return;
}

/* Address: 0x10001410 Size: 24 bytes */
void FUN_10001410()

{

  ((void (*)())*puRam10115a30)();
  return;
}

/* Address: 0x10001428 Size: 24 bytes */
void FUN_10001428()

{

  ((void (*)())*puRam10115584)();
  return;
}

/* Address: 0x10001440 Size: 24 bytes */
void FUN_10001440()

{

  ((void (*)())*puRam101158ec)();
  return;
}

/* Address: 0x10001458 Size: 24 bytes */
void FUN_10001458()

{

  ((void (*)())*puRam10115480)();
  return;
}

/* Address: 0x10001470 Size: 24 bytes */
int FUN_10001470()

{

  ((void (*)())*puRam10115998)();
  return 0;
}

/* Address: 0x10001488 Size: 24 bytes */
void FUN_10001488()

{

  ((void (*)())*puRam10115688)();
  return;
}

/* Address: 0x100014b8 Size: 24 bytes */
int FUN_100014b8()

{

  ((void (*)())*puRam10115814)();
  return 0;
}

/* Address: 0x100014d0 Size: 24 bytes */
void FUN_100014d0()

{

  ((void (*)())*puRam101154a0)();
  return;
}

/* Address: 0x10001590 Size: 24 bytes */
void FUN_10001590()

{

  ((void (*)())*puRam10115af0)();
  return;
}

/* Address: 0x100015a8 Size: 24 bytes */
void FUN_100015a8()

{

  ((void (*)())*puRam10115574)();
  return;
}

/* Address: 0x10001638 Size: 24 bytes */
int FUN_10001638()

{

  ((void (*)())*puRam101158dc)();
  return 0;
}

/* Address: 0x10001680 Size: 24 bytes */
void FUN_10001680()

{

  ((void (*)())*puRam10115a10)();
  return;
}

/* Address: 0x100016b0 Size: 24 bytes */
void FUN_100016b0()

{

  ((void (*)())*puRam10115b70)();
  return;
}

/* Address: 0x10001758 Size: 24 bytes */
void FUN_10001758()

{

  ((void (*)())*puRam10115858)();
  return;
}

/* Address: 0x100018f0 Size: 24 bytes */
void FUN_100018f0()

{

  ((void (*)())*puRam10115390)();
  return;
}

/* Address: 0x10001908 Size: 24 bytes */
void FUN_10001908()

{

  ((void (*)())*puRam10115b10)();
  return;
}

/* Address: 0x10001920 Size: 24 bytes */
void FUN_10001920()

{

  ((void (*)())*puRam10115308)();
  return;
}

/* FUN_100019c8 defined elsewhere */


/* Address: 0x100019f8 Size: 24 bytes */
void FUN_100019f8()

{

  ((void (*)())*puRam101156fc)();
  return;
}

/* FUN_10001a70 defined elsewhere */


/* FUN_10001b18 defined elsewhere */


/* Address: 0x10001bd8 Size: 24 bytes */
/* Renamed from GetString to avoid conflict with Mac Toolbox GetString(short) */
int Game_GetString()

{

  ((void (*)())*puRam1011561c)();
  return 0;
}

/* Address: 0x10001dd0 Size: 24 bytes */
void FUN_10001dd0()

{

  ((void (*)())*puRam10115314)();
  return;
}

/* Address: 0x10001e78 Size: 24 bytes */
void BlockMoveData_Thunk()

{

  ((void (*)())*puRam10115330)();
  return;
}

/* Address: 0x10001f50 Size: 24 bytes */
void FreeHandle()

{

  ((void (*)())*puRam10115540)();
  return;
}

/* Address: 0x10001f98 Size: 24 bytes */
void CopyHandleData()

{

  ((void (*)())*puRam10115b24)();
  return;
}

/* Address: 0x100020d0 Size: 24 bytes */
int FUN_100020d0()

{

  ((void (*)())*puRam1011540c)();
  return 0;
}

/* Address: 0x10002100 Size: 24 bytes */
int FUN_10002100()

{

  ((void (*)())*puRam101152c4)();
  return 0;
}

/* Address: 0x10002148 Size: 24 bytes */
void FUN_10002148()

{

  ((void (*)())*puRam101156d0)();
  return;
}

/* Address: 0x10002178 Size: 24 bytes */
void FUN_10002178()

{

  ((void (*)())*puRam101158d0)();
  return;
}

/* Address: 0x10002238 Size: 24 bytes */
int FUN_10002238()

{

  ((void (*)())*puRam1011583c)();
  return 0;
}

/* Address: 0x10002340 Size: 24 bytes */
void CopyDataToBuffer()

{

  ((void (*)())*puRam10115b78)();
  return;
}

/* FUN_100023b8 defined elsewhere */


/* Address: 0x10002568 Size: 24 bytes */
void FUN_10002568()

{

  ((void (*)())*puRam1011585c)();
  return;
}

/* Address: 0x100026b8 Size: 24 bytes */
void FUN_100026b8()

{

  ((void (*)())*puRam101152d4)();
  return;
}

/* Address: 0x10002970 Size: 24 bytes */
void FUN_10002970()

{

  ((void (*)())*puRam10115b4c)();
  return;
}

/* Address: 0x10002a60 Size: 24 bytes */
int GetHandleDataSize()

{

  ((void (*)())*puRam10115ad8)();
  return 0;
}

/* DisposeHandle_Thunk defined elsewhere */


/* FUN_10002b20 defined elsewhere */


/* Address: 0x10002ce8 Size: 24 bytes */
void FUN_10002ce8()

{

  ((void (*)())*puRam1011584c)();
  return;
}

/* Address: 0x10002d00 Size: 24 bytes */
int FUN_10002d00()

{

  ((void (*)())*puRam101158d4)();
  return 0;
}

/* SetMacRect defined elsewhere */


/* Address: 0x10003228 Size: 24 bytes */
int FUN_10003228()

{

  ((void (*)())*puRam101152c8)();
  return 0;
}

/* Address: 0x100032e8 Size: 24 bytes */
void FUN_100032e8()

{

  ((void (*)())*puRam101152cc)();
  return;
}

/* HUnlock_Thunk defined elsewhere */


/* Address: 0x100035a0 Size: 24 bytes */
void FUN_100035a0()

{

  ((void (*)())*puRam101152dc)();
  return;
}

/* Address: 0x100035b8 Size: 24 bytes */
int FUN_100035b8()

{

  ((void (*)())*puRam101158e0)();
  return 0;
}

/* Address: 0x10003660 Size: 24 bytes */
void FUN_10003660()

{

  ((void (*)())*puRam1011529c)();
  return;
}

/* Address: 0x10003678 Size: 24 bytes */
int FUN_10003678()

{

  ((void (*)())*puRam101152d8)();
  return 0;
}

/* Address: 0x10005b68 Size: 452 bytes */
void FUN_10005b68()

{
  unsigned int uVar1;
  int iVar2;
  int iVar3;
  int iVar4;
  short *psVar5;
  short *psVar6;
  int iVar7;
  int iVar8;
  short sVar9;
  
  psVar6 = psRam10117428;
  psVar5 = psRam10117424;
  if (*(int *)(psRam10117428 + 0x14) < 0) {
    if ((*psRam10117428 != *psRam10117424) || (psRam10117428[1] != psRam10117424[1])) {
      iVar7 = AbsShort((long long)*psRam10117428 - (long long)*psRam10117424);
      if (1 < iVar7) {
        return;
      }
      iVar7 = AbsShort((long long)psVar6[1] - (long long)psVar5[1]);
      if (1 < iVar7) {
        return;
      }
      FUN_1002c970(*psVar5,psVar5[1],*psVar6,psVar6[1]);
      FUN_10004ed8();
    }
    iVar7 = 0;
    if (0 < psVar5[2]) {
      do {
        iVar8 = 0;
        if (0 < psVar5[3]) {
          do {
            iVar4 = *(int *)(psVar6 + 0x12) +
                    (iVar7 - psVar6[0xf]) * (((int)psVar6[0x10] - (int)psVar6[0xe]) + 1) * 10 +
                    psVar6[0xe] * -10;
            iVar2 = iVar8 * 10;
            uVar1 = *(unsigned int *)(iVar4 + iVar2);
            if ((int)uVar1 < 0) {
              sVar9 = psVar5[0xe];
              iVar3 = *(int *)(psVar5 + 0x12) +
                      (iVar7 - psVar5[0xf]) * (((int)psVar5[0x10] - (int)sVar9) + 1) * 10 +
                      (sVar9 * 4 + (int)sVar9 & 0x7fffffffU) * -2;
              if ((*(int *)(iVar2 + iVar3 + 4) == *(int *)(iVar4 + iVar2 + 4)) &&
                 (*(unsigned int *)(iVar3 + iVar2) == uVar1)) {
                *(unsigned int *)(iVar3 + iVar2) = *(unsigned int *)(iVar3 + iVar2) & 0x7fffffff;
              }
            }
            sVar9 = (short)iVar8 + 1;
            iVar8 = (int)sVar9;
          } while (sVar9 < psVar5[3]);
        }
        iVar7 = (int)(short)((short)iVar7 + 1);
      } while (iVar7 < psVar5[2]);
    }
  }
  return;
}

/* Address: 0x10007c50 Size: 388 bytes */
void FUN_10007c50()

{
  int uVar1;
  unsigned int uVar4;
  int iVar5;
  short *puVar6;
  short *puVar7;
  short sVar8;
  int iVar9;
  int iVar10;
  int uVar2;
  int uVar3;
  
  puVar7 = puRam10117428;
  puVar6 = puRam10117424;
  *puRam10117428 = *puRam10117424;
  puVar7[1] = puVar6[1];
  puVar7[2] = puVar6[2];
  puVar7[3] = puVar6[3];
  uVar1 = *(int *)(puVar6 + 6);
  uVar2 = *(int *)(puVar6 + 8);
  uVar3 = *(int *)(puVar6 + 10);
  uVar4 = *(unsigned int *)(puVar6 + 0xc);
  *(int *)(puVar7 + 4) = *(int *)(puVar6 + 4);
  *(int *)(puVar7 + 6) = uVar1;
  *(int *)(puVar7 + 8) = uVar2;
  *(int *)(puVar7 + 10) = uVar3;
  *(unsigned int *)(puVar7 + 0xc) = uVar4 & 0xffff0000;
  FUN_10003a48(puVar7 + 0xe,puVar6 + 0xe);
  uVar4 = *(unsigned int *)(puVar6 + 0x14);
  *(unsigned int *)(puVar7 + 0x14) =
       (uVar4 >> 0x17 & 1) << 0x17 |
       (uVar4 >> 0x18 & 1) << 0x18 |
       (uVar4 >> 0x19 & 1) << 0x19 |
       (uVar4 >> 0x1a & 1) << 0x1a |
       (uVar4 >> 0x1b & 1) << 0x1b |
       (uVar4 >> 0x1c & 1) << 0x1c |
       (uVar4 >> 0x1d & 1) << 0x1d |
       (uVar4 >> 0x1e & 1) << 0x1e | uVar4 & 0x80000000 | *(unsigned int *)(puVar7 + 0x14) & 0x7fffff;
  puVar7[0x15] = puVar6[0x15];
  puVar7[0x16] = puVar6[0x16];
  puVar7[0x17] = puVar6[0x17];
  *(unsigned int *)(puVar7 + 0x14) = *(unsigned int *)(puVar7 + 0x14) | 0x80000000;
  iVar9 = 0;
  if (0 < (short)puVar7[2]) {
    do {
      iVar10 = 0;
      if (0 < (short)puVar7[3]) {
        do {
          iVar5 = *(int *)(puVar7 + 0x12) +
                  (iVar9 - (short)puVar7[0xf]) *
                  (((int)(short)puVar7[0x10] - (int)(short)puVar7[0xe]) + 1) * 10 +
                  (short)puVar7[0xe] * -10;
          *(unsigned int *)(iVar5 + iVar10 * 10) = *(unsigned int *)(iVar5 + iVar10 * 10) | 0x80000000;
          sVar8 = (short)iVar10 + 1;
          iVar10 = (int)sVar8;
        } while (sVar8 < (short)puVar7[3]);
      }
      iVar9 = (int)(short)((short)iVar9 + 1);
    } while (iVar9 < (short)puVar7[2]);
  }
  return;
}

/* Address: 0x100099cc Size: 4 bytes */
void FUN_100099cc()

{
  return;
}

/* Address: 0x10009a98 Size: 104 bytes */
int FUN_10009a98()

{
  int uVar1;
  int *puVar2;
  int iVar3;
  
  puVar2 = (int *)HLock_Thunk();
  DetachResource(puVar2);
  iVar3 = Render_ValidatePixMap(puVar2);
  if (iVar3 == 0) {
    EndFocus(0xfffffffffffffd66);
    uVar1 = *puVar2;
  }
  else {
    uVar1 = *puVar2;
  }
  return uVar1;
}









































































/* Address: 0x10009b00 Size: 72 bytes */
void FUN_10009b00()

{
  long long uVar1;
  
  uVar1 = HLock_Thunk();
  DisposeHandle_Thunk();
  HUnlock_Thunk(uVar1);
  return;
}





































/* Address: 0x10009fa8 Size: 96 bytes - stubbed: callers pass 6 args vs 4 params */
void FUN_10009fa8()

{
  return;
}

/* Address: 0x1000a008 Size: 140 bytes */
void FUN_1000a008(int param_1,short param_2)

{
  int local_18;
  int local_14 [5];
  
  if (param_1 != 0) {
    GetGWorld_Wrapper(local_14,&local_18);
    SetGWorld_Wrapper(*(int *)(param_1 + 0x10),0);
    SetDrawingColor(param_2);
    PaintRect_Thunk((unsigned long long)*(unsigned int *)(param_1 + 0x10) + 0x10);
    SetGWorld_Wrapper(local_14[0],local_18);
  }
  return;
}





















































































/* Address: 0x1000a714 Size: 184 bytes */
void FUN_1000a714(int param_1,short param_2,short param_3,short param_4,short param_5
                 )

{
  int local_28;
  int local_24 [9];
  
  if (param_1 != 0) {
    GetGWorld_Wrapper(local_24,&local_28);
    if (*(int *)(param_1 + 0x10) != 0) {
      SetGWorld_Wrapper(*(int *)(param_1 + 0x10),0);
    }
    SetDrawingColor(param_5);
    MoveTo_Thunk(param_2,param_3);
    LineTo_Thunk(param_4 + -1,0);
    SetGWorld_Wrapper(local_24[0],local_28);
  }
  return;
}









































































/* Address: 0x1000a7cc Size: 184 bytes */
void FUN_1000a7cc(int param_1,short param_2,short param_3,short param_4,short param_5
                 )

{
  int local_28;
  int local_24 [9];
  
  if (param_1 != 0) {
    GetGWorld_Wrapper(local_24,&local_28);
    if (*(int *)(param_1 + 0x10) != 0) {
      SetGWorld_Wrapper(*(int *)(param_1 + 0x10),0);
    }
    SetDrawingColor(param_5);
    MoveTo_Thunk(param_2,param_3);
    LineTo_Thunk(0,param_4 + -1);
    SetGWorld_Wrapper(local_24[0],local_28);
  }
  return;
}









































































/* Address: 0x1000c67c Size: 168 bytes */
void FUN_1000c67c()

{
  int *puVar1;
  int *puVar2;
  int iVar3;
  unsigned long long uVar4;
  int iVar5;
  
  iVar3 = iRam1011746c;
  puVar2 = puRam10117468;
  if (*psRam10115d18 == 0) {
    FUN_1000c724(0);
  }
  uVar4 = 7;
  do {
    puVar1 = *(int **)((int)((uVar4 & 0xffffffff) << 2) + iVar3);
    if (puVar1 != (int *)0x0) {
      DetachResource(puVar1);
    }
    *puVar2 = *puVar1;
    FUN_10020ae8(uVar4);
    iVar5 = *(int *)((int)((uVar4 & 0xffffffff) << 2) + iVar3);
    if (iVar5 != 0) {
      DisposeHandle_Thunk(iVar5);
    }
    iVar5 = (int)uVar4;
    uVar4 = (long long)(short)((short)uVar4 + -1);
  } while (iVar5 != 0);
  return;
}

























































































































/* Address: 0x1000d910 Size: 156 bytes */
int FUN_1000d910(short param_1)

{
  int uVar1;
  int *puVar2;
  int *ppuVar3;
  int **local_2c;
  
  ppuVar3 = 0 /* TVect base */;
  if (*psRam10115d18 == 0) {
    ppuVar3 = 0 /* TVect base */;
    FUN_1000c724(1);
  }
  if ((*(short *)(*(int*)((char*)ppuVar3 - 0x69d)) == 0) || (param_1 != *(short *)(*(int *)(*(int*)((char*)ppuVar3 - 0x151)) + 0x110)))
  {
    puVar2 = *(int **)((*(int*)((char*)ppuVar3 - 0x10d)) + param_1 * 4);
    if (puVar2 != (int *)0x0) {
      DetachResource(puVar2);
      ppuVar3 = local_2c;
    }
    uVar1 = *puVar2;
    *(int *)(*(int*)((char*)ppuVar3 - 0x10a)) = uVar1;
  }
  else {
    uVar1 = *(int *)(*(int*)((char*)ppuVar3 - 0x10e));
  }
  return uVar1;
}

/* Address: 0x1000f6a0 Size: 104 bytes */
long long FUN_1000f6a0(short param_1)

{
  short sVar1;
  int iVar2;
  
  sVar1 = *(short *)(*piRam10117468 + 0x24a);
  if (sVar1 != 0) {
    do {
      sVar1 = sVar1 + -1;
      iVar2 = *piRam10117468 + sVar1 * 0x5c;
      if ((*(short *)(iVar2 + 0x24c) != 0) && (*(short *)(iVar2 + 0x250) == param_1)) {
        return 1;
      }
    } while (sVar1 != 0);
  }
  return 0;
}

/* Address: 0x1000f708 Size: 180 bytes */
long long FUN_1000f708(short param_1,short param_2)

{
  char cVar1;
  short sVar2;
  int iVar3;
  
  sVar2 = *(short *)(*piRam10117468 + 0x24a);
  if (sVar2 != 0) {
    do {
      sVar2 = sVar2 + -1;
      iVar3 = *piRam10117468 + sVar2 * 0x5c;
      if ((*(short *)(iVar3 + 0x24c) != 0) && (*(short *)(iVar3 + 0x250) == param_1)) {
        return 0;
      }
    } while (sVar2 != 0);
  }
  cVar1 = *(char *)(*piRam10117468 + (int)param_1 + 0x56);
  if ((cVar1 != '\b') && (cVar1 != '\x05')) {
    if ((param_2 != 0) && (*(char *)(*piRam10117468 + (int)param_1 + 0x56) == '\x06')) {
      return 1;
    }
    return 0;
  }
  return 1;
}

/* Address: 0x10015980 Size: 72 bytes */
/* Address: 0x10015980 Size: 72 bytes */
/* AI_GetRuinCapability - clean version in ai/ai.c */

/* Address: 0x100184dc Size: 152 bytes */
long long GetDirection(short param_1,short param_2,short param_3,short param_4)

{
  if (param_1 != param_3) {
    if (param_2 == param_4) {
      if (param_3 < param_1) {
        return 6;
      }
      return 2;
    }
    if (param_1 <= param_3) {
      if (param_4 < param_2) {
        return 1;
      }
      return 3;
    }
    if (param_4 < param_2) {
      return 7;
    }
    return 5;
  }
  if (param_2 == param_4) {
    return 0xff;
  }
  if (param_4 < param_2) {
    return 0;
  }
  return 4;
}

/* Address: 0x1001aea0 Size: 152 bytes */
/* Address: 0x1001aea0 Size: 152 bytes */
/* AI_ActivateBestGroup - clean version in ai/ai.c */

/* Address: 0x1001e330 Size: 188 bytes */
void FUN_1001e330(long long param_1)

{
  unsigned long long uVar1;
  int iVar2;
  long long uVar3;
  short sVar4;
  
  uVar1 = ZEXT48(0 /* TVect base */);
  sVar4 = 0;
  do {
    uVar3 = FUN_1000d910(sVar4);
    StreamWrite(param_1,uVar3,0x42c);
    ReleaseCityData(sVar4);
    iVar2 = (int)uVar1;
    sVar4 = sVar4 + 1;
  } while (sVar4 < 8);
  FlushCityCache();
  StreamWrite(param_1,**(int **)(iVar2 + -0x430),0x4b0);
  if (**(short **)(iVar2 + -0x1a74) == 0) {
    if (**(int **)(iVar2 + -0x3fc) != 0) {
      DisposeHandle_Thunk(**(int **)(iVar2 + -0x3fc));
    }
  }
  return;
}


























































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1001e3ec Size: 196 bytes */
void FUN_1001e3ec(long long param_1)

{
  unsigned long long uVar1;
  int iVar2;
  long long uVar3;
  short sVar4;
  
  uVar1 = ZEXT48(0 /* TVect base */);
  sVar4 = 0;
  do {
    uVar3 = FUN_1000d910(sVar4);
    StreamRead(param_1,uVar3,0x42c);
    ReleaseCityData(sVar4);
    iVar2 = (int)uVar1;
    sVar4 = sVar4 + 1;
  } while (sVar4 < 8);
  FlushCityCache();
  StreamRead(param_1,**(int **)(iVar2 + -0x430),0x4b0);
  if (**(short **)(iVar2 + -0x1a74) == 0) {
    if (**(int **)(iVar2 + -0x3fc) != 0) {
      DisposeHandle_Thunk(**(int **)(iVar2 + -0x3fc));
    }
  }
  PostLoadCityInit();
  return;
}









































































































/* EvaluatePosition defined elsewhere */


/* Address: 0x1001f5e8 Size: 96 bytes */
void FUN_1001f5e8(short param_1,short param_2)

{
  SetupInfluenceMap(0xf,param_1,param_2,*gCurrentMoveMode,*gCurrentPathFlags);
  FUN_1001f220(param_1,param_2,1);
  return;
}

/* Address: 0x1001f648 Size: 272 bytes */
long long FUN_1001f648(short param_1,short *param_2)

{
  char cVar1;
  int *piVar2;
  int *ppuVar3;
  long long uVar4;
  int iVar5;
  int iVar6;
  
  piVar2 = piRam10117468;
  ppuVar3 = 0 /* TVect base */;
  iVar6 = (int)param_1;
  uVar4 = FUN_1001e794(iVar6,3,0xffffffffffffffff,1);
  if ((int)uVar4 == -1) {
    uVar4 = 0;
  }
  else {
    iVar5 = FUN_10047d80(iVar6,uVar4);
    *param_2 = (short)iVar5;
    iVar5 = *(int *)(*(int*)((char*)ppuVar3 - 0x151)) + iVar6 * 0x42 + iVar5;
    cVar1 = *(char *)(iVar5 + 0x1622);
    iVar6 = (int)cVar1;
    if (*(char *)(*(int *)(*(int*)((char*)ppuVar3 - 0x14f)) + *(char *)(iVar5 + 0x161a) * 6 + 5) == '\x01') {
      iVar6 = (int)(short)(cVar1 + 2);
    }
    if ((*(short *)(*piVar2 + 2) < 9) || (3 < iVar6)) {
      if ((*(short *)(*piVar2 + 2) < 5) || (2 < iVar6)) {
        uVar4 = 1;
      }
      else {
        uVar4 = 0;
      }
    }
    else {
      uVar4 = 0;
    }
  }
  return uVar4;
}

/* Address: 0x10020f94 Size: 620 bytes */
long long FUN_10020f94(short param_1)

{
  short sVar2;
  int iVar1;
  short sVar3;
  short sVar4;
  short sVar5;
  short sVar6;
  long long lVar7;
  short sVar8;
  long long lVar9;
  char auStack_64 [22];
  short local_4e;
  short local_4c;
  short local_48;
  short local_46;
  short local_44;
  short local_3c;
  short local_3a;
  short local_38;
  short local_36;
  short local_34;
  short local_32;
  short local_2e;
  short local_2c;
  short local_2a;
  
  lVar7 = -1;
  sVar8 = 0;
  sVar4 = 0;
  if (param_1 == 0) {
    sVar5 = 800;
    sVar6 = 4;
  }
  else {
    sVar5 = 0x5dc;
    sVar6 = 5;
  }
  while( true ) {
    lVar9 = 0;
    do {
      GetUnitStats(lVar9,auStack_64);
      if ((((local_34 == 0) && ((int)lVar9 != 0x1c)) && ((int)lVar9 != 5)) &&
         ((-1 < local_46 && (local_46 <= sVar5)))) {
        sVar3 = local_4e - local_4c;
        if (sVar6 <= local_4e) {
          if ((int)local_46 <= sVar5 + -200) {
            sVar3 = sVar3 + 1;
          }
          if ((int)local_46 <= sVar5 + -400) {
            sVar3 = sVar3 + 1;
          }
          if (0xb < local_48) {
            sVar3 = sVar3 + 1;
          }
          if (0xf < local_48) {
            sVar3 = sVar3 + 1;
          }
          if (local_2e != 0) {
            sVar3 = sVar3 + 1;
          }
          if ((local_2c != 0) || (local_2a != 0)) {
            sVar3 = sVar3 + 1;
          }
          if (((local_36 != 0) || (local_38 != 0)) || (local_3a != 0)) {
            sVar3 = sVar3 + 1;
          }
          if (local_3c != 0) {
            sVar3 = sVar3 + 1;
          }
          if (local_44 != 0) {
            sVar3 = sVar3 + 1;
          }
          if (local_32 != 0) {
            sVar3 = sVar3 + 2;
          }
        }
        if (1 < sVar3) {
          sVar2 = RandomRange(1,sVar3 << 1,0);
          sVar3 = sVar3 + sVar2;
        }
        if ((sVar8 < sVar3) ||
           ((sVar3 == sVar8 && (iVar1 = RandomRange(1,2,0xffffffffffffffff), iVar1 != 0)))) {
          lVar7 = lVar9;
          sVar8 = sVar3;
        }
      }
      sVar3 = (short)lVar9 + 1;
      lVar9 = (long long)sVar3;
    } while (sVar3 < 0x1d);
    if (((int)lVar7 != -1) || (3 < sVar4)) break;
    sVar4 = sVar4 + 1;
    sVar5 = sVar5 + 500;
    sVar6 = sVar6 + -1;
    if (sVar6 < 1) {
      sVar6 = 1;
    }
  }
  return lVar7;
}

/* Address: 0x10021200 Size: 44 bytes */
void FUN_10021200(int param_1,char param_2)

{
  param_2 = *(char *)(param_1 + 8) + param_2;
  *(char *)(param_1 + 8) = param_2;
  if ('\t' < param_2) {
    *(char *)(param_1 + 8) = 9;
  }
  return;
}

/* Address: 0x1002269c Size: 20 bytes */
void FUN_1002269c(short *param_1,short param_2,short param_3)

{
  *param_1 = param_2;
  param_1[1] = param_3;
  return;
}

/* Address: 0x10025598 Size: 80 bytes */
void FUN_10025598()

{
  if (*puRam10115d80 < 5) {
    return;
  }
  return;
}

/* Address: 0x1002565c Size: 252 bytes */
void FUN_1002565c()

{
  int *piVar1;
  int iVar2;
  int *piVar3;
  int *ppuVar4;
  long long uVar5;
  int **local_34;
  
  piVar3 = piRam10115d88;
  piVar1 = (int *)*piRam10115d88;
  ppuVar4 = 0 /* TVect base */;
  if (piVar1 != (int *)0x0) {
    ResourceRead_Dispatch((int)piVar1 + (int)*(short *)(*piVar1 + 0x328));
    uVar5 = FUN_1007a894(0);
    FUN_1007a95c(uVar5,*piVar3);
    ResourceRead_Dispatch(*piVar3 + (int)*(short *)(*(int *)*piVar3 + 0x200),uVar5);
    ppuVar4 = local_34;
  }
  *piVar3 = 0;
  *(short *)(*(int*)((char*)ppuVar4 - 0x688)) = 0;
  FUN_1003f674(1,1,1);
  iVar2 = *(int *)(*(int*)((char*)ppuVar4 - 0x151));
  if (*(short *)(iVar2 + 0x12e) != 0) {
    if ((*(unsigned int *)(iVar2 + 0x134) >> 0x1a & 1) == 0) {
      *(unsigned int *)(iVar2 + 0x134) = *(unsigned int *)(iVar2 + 0x134) | 0x4000000;
      FUN_1005f6b0(0x19,2);
      FUN_10040040();
    }
  }
  return;
}

/* Address: 0x10025f94 Size: 4 bytes */
void FUN_10025f94()

{
  return;
}

/* Address: 0x10026310 Size: 576 bytes */
void FUN_10026310()

{
  long long uVar1;
  int *piVar2;
  int *piVar3;
  short sVar4;
  unsigned long long uVar5;
  short local_148 [8];
  short local_138 [8];
  short local_128 [2];
  int local_124;
  int local_120;
  char auStack_11c [284];
  
  uVar1 = ResourceRead_Dispatch(*piRam10115dc4 + (int)*(short *)(*(int *)*piRam10115dc4 + 0x7f8));
  FocusObject();
  local_124 = 7;
  local_120 = 0x14;
  piVar2 = (int *)FUN_10078a30(uVar1,0x7262616b,0x10cd,&local_124,0);
  piVar3 = (int *)ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x308),0x67726964);
  if (piVar3 != (int *)0x0) {
    ResourceRead_Dispatch((int)*(short *)(*piVar3 + 0x4d0) + (int)piVar3);
  }
  FUN_1002606c(local_128,local_138,local_148);
  uVar5 = 0;
  if (0 < local_128[0]) {
    do {
      piVar3 = (int *)ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x308),uVar5 + 0x73696431
                                  );
      if (piVar3 != (int *)0x0) {
        if (*(short *)((int)piVar3 + 0x82) != 3) {
          *(short *)((int)piVar3 + 0x82) = 3;
        }
        sVar4 = *(short *)((int)local_138 + (int)((uVar5 & 0xffffffff) << 1));
        if (sVar4 != *(short *)(piVar3 + 0x20)) {
          *(short *)(piVar3 + 0x20) = sVar4;
          ResourceRead_Dispatch((int)piVar3 + (int)*(short *)(*piVar3 + 0x4d0));
        }
      }
      uVar1 = FUN_1005f678(0x6b,*(short *)((int)local_148 + (int)((uVar5 & 0xffffffff) << 1)));
      BuildPascalString(auStack_11c,uVar1);
      FUN_10090e0c(piVar2,uVar5 + 0x616d6231,auStack_11c,1);
      sVar4 = (short)uVar5 + 1;
      uVar5 = (unsigned long long)sVar4;
    } while (sVar4 < local_128[0]);
  }
  if ((int)uVar5 < 8) {
    do {
      piVar3 = (int *)ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x308),uVar5 + 0x73696431
                                  );
      if (piVar3 != (int *)0x0) {
        ResourceRead_Dispatch((int)piVar3 + (int)*(short *)(*piVar3 + 0x660),0,0);
      }
      sVar4 = (short)uVar5 + 1;
      uVar5 = (unsigned long long)sVar4;
    } while (sVar4 < 8);
  }
  return;
}




























































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10026568 Size: 4 bytes */
void FUN_10026568()

{
  return;
}

/* Address: 0x10029dcc Size: 184 bytes */
void FUN_10029dcc(int *param_1,long long param_2)

{
  long long uVar1;
  int local_120;
  short local_11c [2];
  char local_118 [280];
  
  local_118[0] = 0;
  ResolveAlias(param_2,local_11c,&local_120,local_118);
  DisposeHandleFull(param_2);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x2b8));
  uVar1 = ConvertToString(local_118);
  AddResource(param_2,local_120,local_11c[0],uVar1);
  MarkChanged();
  ReleaseResource_Thunk(param_2);
  return;
}


















































































































































































































































































































































































































































































































































































/* FUN_1002c92c defined elsewhere */


/* Address: 0x10030454 Size: 60 bytes */
void FUN_10030454(long long param_1)

{
  if (*psRam10115dec != 0) {
    FUN_1003cac4(param_1,5);
  }
  return;
}

/* Address: 0x10031f54 Size: 112 bytes */
void FUN_10031f54(long long param_1)

{
  short sVar1;
  int *ppuVar2;
  char auStack_108 [264];
  
  ppuVar2 = 0 /* TVect base */;
  sVar1 = *psRam10115df4;
  *psRam10115df4 = sVar1 + 1;
  BuildPascalString(auStack_108,param_1);
  FUN_10090e0c(*(int *)(*(int*)((char*)ppuVar2 - 0x6ac)),(long long)sVar1 + 0x6c696e31,auStack_108,1);
  return;
}

/* Address: 0x10031fc4 Size: 124 bytes */
void FUN_10031fc4()

{
  int *piVar1;
  int *piVar2;
  
  piVar2 = piRam10115df0;
  if (*piRam10115df0 != 0) {
    ResourceRead_Dispatch(*piRam10115df0 + (int)*(short *)(*(int *)*piRam10115df0 + 0x810));
    piVar1 = (int *)*piVar2;
    ResourceRead_Dispatch((int)piVar1 + (int)*(short *)(*piVar1 + 0x738));
    *piVar2 = 0;
  }
  return;
}

/* Address: 0x10032860 Size: 336 bytes */
void FUN_10032860(short param_1,short param_2,long long param_3,short param_4)

{
  unsigned int uVar1;
  char auStack_168 [6];
  short local_162;
  char auStack_160 [2];
  short local_15e;
  char auStack_158 [256];
  short local_58;
  short local_56;
  short local_52;
  int local_50;
  int local_4c;
  int local_48;
  unsigned int local_44;
  int local_40;
  int local_3c;
  int local_38;
  int local_34;
  
  FUN_10001b18(&local_58);
  BuildPascalString(auStack_158,param_3);
  ConvertToString(auStack_158);
  uVar1 = FUN_100006f0();
  local_34 = uVar1 + (int)param_1;
  local_38 = (int)param_2 + (int)(short)(local_58 + local_56 + local_52);
  local_40 = (int)param_2;
  local_3c = (int)param_1;
  if (param_4 == 1) {
    local_4c = ((int)uVar1 >> 1) + (unsigned int)((int)uVar1 < 0 && (uVar1 & 1) != 0);
    local_50 = 0;
    FUN_100e1674(&local_40,&local_50);
  }
  else if (param_4 == -1) {
    local_48 = 0;
    local_44 = uVar1;
    FUN_100e1674(&local_40,&local_48);
  }
  FUN_100e12b8(&local_40,auStack_160);
  FUN_100e12b8(&local_40,auStack_168);
  FUN_10032208(local_15e,local_162 + 2,param_2,param_3,0xfffffffffffffffe);
  return;
}












































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10037fc4 Size: 32 bytes */
void FUN_10037fc4()

{
  FUN_10037f68();
  return;
}

/* Address: 0x1003865c Size: 516 bytes */
long long FUN_1003865c(int param_1,short *param_2)

{
  int iVar1;
  int iVar2;
  unsigned int *puVar3;
  int iVar4;
  short sVar5;
  long long lVar6;
  int iVar8;
  unsigned long long uVar7;
  unsigned long long uVar9;
  short local_48 [36];
  
  puVar3 = puRam1011735c;
  lVar6 = 0;
  iVar8 = 0;
  do {
    local_48[iVar8 * 2 + 1] = 0;
    local_48[iVar8 * 2] = 0;
    sVar5 = (short)iVar8 + 1;
    iVar8 = (int)sVar5;
  } while (sVar5 < 8);
  iVar8 = 0;
  do {
    if ('\0' < *(char *)(*puRam1011735c + iVar8 * 0x2c + 0x1422)) {
      lVar6 = (long long)(short)((short)lVar6 + 1);
      local_48[iVar8 * 2] = 1;
    }
    sVar5 = (short)iVar8 + 1;
    iVar8 = (int)sVar5;
  } while (sVar5 < 8);
  iVar8 = 0;
  do {
    if (('\x01' < *(char *)(*puRam1011735c + iVar8 * 0x2c + 0x1422)) && ((int)lVar6 < 10)) {
      lVar6 = (long long)(short)((short)lVar6 + 1);
      local_48[iVar8 * 2 + 1] = 1;
    }
    sVar5 = (short)iVar8 + 1;
    iVar8 = (int)sVar5;
  } while (sVar5 < 8);
  iVar8 = 0;
  do {
    if ((local_48[iVar8 * 2] != 0) && (local_48[iVar8 * 2 + 1] == 0)) {
      *(char *)(*puVar3 + iVar8 * 0x2c + 0x1422) = 1;
    }
    sVar5 = (short)iVar8 + 1;
    iVar8 = (int)sVar5;
  } while (sVar5 < 8);
  uVar7 = 0;
  iVar8 = 0;
  do {
    uVar9 = 0;
    do {
      iVar1 = (int)((uVar9 & 0x7fffffff) << 1);
      if (*(short *)((int)local_48 + (int)((uVar7 & 0xffffffff) << 2) + iVar1) != 0) {
        *(char *)(param_1 + iVar8 * 0x16) = (char)uVar7;
        iVar2 = (int)(uVar7 * 0x2c);
        iVar4 = param_1 + iVar8 * 0x16;
        *(char *)(iVar4 + 1) = *(char *)(*puVar3 + iVar2 + (int)uVar9 + 0x1423);
        *(short *)(iVar4 + 2) = *(short *)(iVar2 + *puVar3 + iVar1 + 0x1426);
        *(short *)(iVar4 + 4) = *(short *)(iVar2 + *puVar3 + iVar1 + 0x142a);
        BlockMoveData_Thunk(iVar4 + 6,
                     uVar7 * 0x2c + (unsigned long long)*puVar3 + (uVar9 & 0xfffffff) * 0x10 + 0x142e);
        iVar8 = (int)(short)((short)iVar8 + 1);
      }
      sVar5 = (short)uVar9 + 1;
      uVar9 = (unsigned long long)sVar5;
    } while (sVar5 < 2);
    sVar5 = (short)uVar7 + 1;
    uVar7 = (unsigned long long)sVar5;
  } while (sVar5 < 8);
  *param_2 = (short)lVar6 * 0x16;
  return lVar6;
}





























































































































































































































































































































































































































































/* Address: 0x10038c60 Size: 300 bytes */
void FUN_10038c60(short param_1,char param_2,short param_3,short param_4,
                 long long param_5)

{
  int bVar1;
  char cVar2;
  int iVar3;
  int iVar4;
  unsigned int *puVar5;
  unsigned long long uVar6;
  
  puVar5 = puRam1011735c;
  iVar3 = *puRam1011735c + param_1 * 0x2c;
  cVar2 = *(char *)(iVar3 + 0x1422);
  uVar6 = (unsigned long long)cVar2;
  if (cVar2 < '\x02') {
    *(char *)(iVar3 + 0x1422) = cVar2 + '\x01';
  }
  else {
    iVar3 = *puRam1011735c + param_1 * 0x2c;
    bVar1 = *(char *)(iVar3 + 0x1423) <= *(char *)(iVar3 + 0x1424);
    uVar6 = (unsigned long long)bVar1;
    if (*(char *)(*puRam1011735c + param_1 * 0x2c + (unsigned int)bVar1 + 0x1423) <= param_2) {
      return;
    }
  }
  iVar3 = (int)((long long)param_1 * 0x2c);
  *(char *)(*puVar5 + iVar3 + (int)uVar6 + 0x1423) = param_2;
  iVar4 = (int)((uVar6 & 0xffffffff) << 1);
  *(short *)(iVar3 + *puVar5 + iVar4 + 0x1426) = param_3;
  *(short *)(iVar3 + *puVar5 + iVar4 + 0x142a) = param_4;
  if ((int)param_5 == 0) {
    *(char *)(*puVar5 + param_1 * 0x2c + (int)((uVar6 & 0xffffffff) << 4) + 0x142e) =
         *(char *)(iRam10115e30 + 0x60);
  }
  else {
    BlockMoveData_Thunk((long long)param_1 * 0x2c + (unsigned long long)*puVar5 + (uVar6 & 0xfffffff) * 0x10 + 0x142e
                 ,param_5);
  }
  return;
}

















































































































































































































































/* Address: 0x1003a230 Size: 968 bytes */
void FUN_1003a230(int *param_1)

{
  char cVar1;
  char cVar2;
  short uVar3;
  int iVar4;
  unsigned int *puVar5;
  int uVar6;
  int *piVar8;
  long long uVar7;
  short sVar9;
  short sVar10;
  unsigned long long uVar11;
  int iVar12;
  long long uVar13;
  unsigned long long uVar14;
  char auStack_258 [256];
  char auStack_158 [254];
  short local_5a [45];
  
  uVar6 = uRam1011772c;
  puVar5 = puRam1011735c;
  uVar13 = 0;
  sVar10 = 8;
  iVar12 = 0;
  do {
    sVar9 = (short)iVar12 + 1;
    local_5a[iVar12 + 1] = sVar10;
    sVar10 = sVar10 + 1;
    iVar12 = (int)sVar9;
  } while (sVar10 < 0x16);
  uVar14 = 1;
  if (1 < sVar9) {
    do {
      if (0 < (int)uVar14) {
        iVar12 = (int)((uVar14 & 0xffffffff) << 1);
        iVar4 = *puVar5 + *(short *)((int)local_5a + iVar12 + 2) * 0x1e;
        iVar12 = *puVar5 + *(short *)((int)local_5a + iVar12) * 0x1e;
        cVar1 = *(char *)(iVar12 + 0xd26);
        cVar2 = *(char *)(iVar4 + 0xd26);
        uVar11 = uVar14;
        if ((cVar2 < cVar1) ||
           ((cVar2 == cVar1 && (*(char *)(iVar4 + 0xd27) < *(char *)(iVar12 + 0xd27))))) {
          while( true ) {
            iVar12 = (int)((uVar11 & 0x7fffffff) << 1);
            uVar3 = *(short *)((int)local_5a + iVar12 + 2);
            *(short *)((int)local_5a + iVar12 + 2) = *(short *)((int)local_5a + iVar12);
            *(short *)((int)local_5a + iVar12) = uVar3;
            uVar11 = (unsigned long long)(short)((short)uVar11 + -1);
            if ((long long)uVar11 < 1) break;
            iVar12 = (int)((uVar11 & 0xffffffff) << 1);
            iVar4 = *puVar5 + *(short *)((int)local_5a + iVar12 + 2) * 0x1e;
            iVar12 = *puVar5 + *(short *)((int)local_5a + iVar12) * 0x1e;
            cVar1 = *(char *)(iVar12 + 0xd26);
            cVar2 = *(char *)(iVar4 + 0xd26);
            if ((cVar1 <= cVar2) &&
               ((cVar2 != cVar1 || (*(char *)(iVar12 + 0xd27) <= *(char *)(iVar4 + 0xd27))))) break;
          }
        }
      }
      sVar10 = (short)uVar14 + 1;
      uVar14 = (unsigned long long)sVar10;
    } while (sVar10 < sVar9);
  }
  uVar14 = 0;
  if (0 < sVar9) {
    do {
      piVar8 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308),
                                   uVar14 + 0x69746530);
      if (piVar8 != (int *)0x0) {
        uVar11 = (unsigned long long)*(short *)((int)local_5a + (int)((uVar14 & 0xffffffff) << 1) + 2);
        BuildPascalString(auStack_158,
                     (unsigned long long)*puVar5 + ((uVar11 & 0xfffffff) * 0x10 - uVar11 & 0x7fffffff) * 2 +
                     0xd12);
        ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x7c8),auStack_158,1);
      }
      if (*(char *)(*puVar5 + *(short *)((int)local_5a + (int)((uVar14 & 0xffffffff) << 1) + 2) *
                              0x1e + 0xd26) == '\x01') {
        uVar13 = 1;
      }
      else if (*(char *)(*puVar5 + *(short *)((int)local_5a + (int)((uVar14 & 0xffffffff) << 1) + 2)
                                   * 0x1e + 0xd26) == '\x02') {
        uVar13 = 2;
      }
      else if (*(char *)(*puVar5 + *(short *)((int)local_5a + (int)((uVar14 & 0xffffffff) << 1) + 2)
                                   * 0x1e + 0xd26) == '\x05') {
        uVar13 = 3;
      }
      else if (*(char *)(*puVar5 + *(short *)((int)local_5a + (int)((uVar14 & 0xffffffff) << 1) + 2)
                                   * 0x1e + 0xd26) == '\x06') {
        uVar13 = 4;
      }
      else if (*(char *)(*puVar5 + *(short *)((int)local_5a + (int)((uVar14 & 0xffffffff) << 1) + 2)
                                   * 0x1e + 0xd26) == '\a') {
        uVar13 = 5;
      }
      uVar7 = FUN_1005f678(0xa8,uVar13);
      DrawNumber(uVar6,uVar7,
                   *(char *)
                    (*puVar5 + *(short *)((int)local_5a + (int)((uVar14 & 0xffffffff) << 1) + 2) *
                               0x1e + 0xd27));
      piVar8 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308),
                                   uVar14 + 0x626f6e30);
      if (piVar8 != (int *)0x0) {
        BuildPascalString(auStack_258,uVar6);
        ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x7c8),auStack_258,1);
      }
      sVar10 = (short)uVar14 + 1;
      uVar14 = (unsigned long long)sVar10;
    } while (sVar10 < sVar9);
  }
  return;
}


















































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1003a5f8 Size: 4 bytes */
void FUN_1003a5f8()

{
  return;
}

/* Address: 0x1003aeb0 Size: 120 bytes - stubbed: callers pass 4 args vs 1 param */
void FUN_1003aeb0()

{
  return;
}

/* Address: 0x1003c938 Size: 396 bytes - stubbed: callers pass 0 args */
int FUN_1003c938()

{
  return 0;
}

/* Address: 0x1003cac4 Size: 192 bytes */
void FUN_1003cac4(long long param_1,short param_2)

{
  unsigned int uVar1;
  int iVar2;
  int *puVar3;
  int local_2c;
  
  iVar2 = ResourceRead_Dispatch(*gOverviewWindow + (int)*(short *)(*(int *)*gOverviewWindow + 0x308),0x696e666f);
  if (iVar2 != 0) {
    puVar3 = (int *)
             (*(short *)(**(int **)(local_2c + -0x544) +
                         *(short *)(**(int **)(local_2c + -0x544) + 0x110) * 2 + 0xa0) * 6 +
             *(int *)(local_2c + -0x1d4));
    uVar1 = puVar3[1];
    *(int *)(iVar2 + 0x9e) = *puVar3;
    *(unsigned int *)(iVar2 + 0xa2) = uVar1 & 0xffff0000;
    FUN_10081040(iVar2,param_1);
    if (0 < param_2) {
      ProgressDelay(param_2);
    }
  }
  return;
}

/* Address: 0x1003ced4 Size: 448 bytes */

void FUN_1003ced4(long long param_1,long long param_2)

{
  int *puVar1;
  int iVar2;
  int local_35c;
  char auStack_338 [256];
  char auStack_238 [256];
  int *local_138;
  char auStack_134 [256];
  short local_34;
  int local_30;
  int local_2c;
  int local_28;
  int local_24;
  
  puVar1 = puRam10117370;
  local_138 = (int *)0x0;
  local_34 = 0;
  local_30 = 0;
  local_28 = 0;
  local_24 = 0;
  local_2c = *puRam10117370;
  *puRam10117370 = auStack_134;
  iVar2 = TrySetjmp(auStack_134);
  if (iVar2 != 0) {
    LockHandleRange(local_34,local_30);
    return;
  }
  if ((int)param_1 != 0) {
    local_138 = (int *)ResourceRead_Dispatch(**(int **)(local_35c + -0x1e4) +
                                    (int)*(short *)(*(int *)**(int **)(local_35c + -0x1e4) + 200),
                                    0x3fc,0);
    if (local_138 == (int *)0x0) goto LAB_1003d038;
    ResourceRead_Dispatch((int)*(short *)(*local_138 + 800) + (int)local_138);
    BuildPascalString(auStack_238,param_1);
    FUN_10090e0c(local_138,0x32737431,auStack_238,1);
    BuildPascalString(auStack_338,param_2);
    FUN_10090e0c(local_138,0x32737432,auStack_338,1);
  }
  if (local_138 != (int *)0x0) {
    ResourceRead_Dispatch((int)local_138 + (int)*(short *)(*local_138 + 0x810));
    ResourceRead_Dispatch((int)local_138 + (int)*(short *)(*local_138 + 0x738));
  }
LAB_1003d038:
  *puVar1 = local_2c;
  return;
}

/* Address: 0x1003d5d4 Size: 240 bytes */
void FUN_1003d5d4()

{
  int *piVar1;
  int *ppuVar2;
  int *piVar3;
  int **local_2c;
  
  piVar3 = gActiveDocView;
  ppuVar2 = 0 /* TVect base */;
  if (*gActiveDocView != 0) {
    *(char *)(*gActiveDocView + 0xbe) = 1;
    piVar1 = (int *)*piVar3;
    ResourceRead_Dispatch((int)piVar1 + (int)*(short *)(*piVar1 + 0x4d0));
    piVar3 = (int *)*piVar3;
    ResourceRead_Dispatch((int)piVar3 + (int)*(short *)(*piVar3 + 0x4c8));
    ppuVar2 = local_2c;
  }
  piVar3 = *(int **)(*(int*)((char*)ppuVar2 - 0x541));
  if ((piVar3 != (int *)0x0) &&
     (piVar3 = (int *)ResourceRead_Dispatch((int)*(short *)(*piVar3 + 0x308) + (int)piVar3,0x6f766572),
     piVar3 != (int *)0x0)) {
    *(char *)(piVar3 + 0x25) = 1;
    ResourceRead_Dispatch((int)*(short *)(*piVar3 + 0x4d0) + (int)piVar3);
    ResourceRead_Dispatch((int)*(short *)(*piVar3 + 0x4c8) + (int)piVar3);
  }
  return;
}

/* Address: 0x1003d6c4 Size: 112 bytes */
void FUN_1003d6c4()

{
  int *piVar1;
  
  piVar1 = (int *)ResourceRead_Dispatch(*gOverviewWindow + (int)*(short *)(*(int *)*gOverviewWindow + 0x308),
                               0x696e666f);
  if (piVar1 != (int *)0x0) {
    *(char *)(piVar1 + 0x2a) = 0;
    ResourceRead_Dispatch((int)*(short *)(*piVar1 + 0x488) + (int)piVar1);
  }
  return;
}

/* Address: 0x1003e13c Size: 5420 bytes */
void FUN_1003e13c(long long param_1,short param_2)

{
  int uVar1;
  int bVar3;
  unsigned int *puVar4;
  unsigned long long uVar5;
  char *puVar7;
  unsigned int *puVar8;
  char uVar24;
  int *piVar9;
  unsigned int uVar10;
  int *puVar11;
  long long uVar6;
  int iVar12;
  int uVar13;
  int iVar14;
  int iVar15;
  int *piVar16;
  int *piVar17;
  int *piVar18;
  unsigned int uVar19;
  int *puVar20;
  int *puVar21;
  short sVar23;
  int uVar22;
  int bVar25;
  unsigned long long uVar26;
  int *puVar27;
  int *puVar28;
  int *puVar29;
  int *puVar30;
  int *puVar31;
  int iVar32;
  short *psVar33;
  int iVar34;
  int *piVar35;
  int *piVar36;
  int bVar37;
  int bVar38;
  short sStack0000001e;
  unsigned int local_56c;
  int local_548 [8];
  unsigned int local_528;
  int local_408;
  int iStack_404;
  int iStack_400;
  int iStack_3fc;
  int local_3f8 [2];
  int local_3f0;
  int local_3ec;
  int local_3e8;
  int local_3e4;
  int local_3e0;
  int local_3dc;
  int local_3d8;
  int local_3d4;
  int local_3d0 [2];
  int local_3c8;
  int local_3c4;
  int local_3c0;
  int local_3bc;
  int local_3b8 [2];
  int local_3b0;
  int local_3ac;
  int local_3a8;
  int local_3a4;
  int local_3a0;
  int local_39c;
  short local_398;
  short local_396;
  int local_390;
  int local_38c;
  int local_388;
  int local_384;
  int local_380;
  int local_37c;
  int local_378;
  int iStack_374;
  int local_370;
  int local_36c [21];
  int local_318;
  int local_314;
  int local_310;
  int local_30c;
  char auStack_308 [80];
  char local_2b8;
  char local_2b7;
  char local_2b6;
  char auStack_2b4 [8];
  int local_2ac;
  int *local_2a8;
  int local_2a4;
  int local_2a0;
  char auStack_29c [8];
  int local_294;
  int local_290;
  char auStack_28c [8];
  int local_284;
  int local_280;
  char auStack_27c [8];
  int local_274;
  int local_270;
  char auStack_26c [8];
  char auStack_264 [8];
  int *local_25c;
  int local_258;
  int local_254;
  int local_250;
  int local_24c;
  char auStack_248 [8];
  int local_240;
  int local_23c;
  int local_238;
  int iStack_234;
  int iStack_230;
  int iStack_22c;
  int local_228;
  int uStack_224;
  char auStack_220 [8];
  int local_218;
  int local_214;
  int local_210;
  int local_208;
  int local_204;
  int local_200;
  int local_1fc;
  int local_1f8;
  int local_1f0;
  int local_1ec;
  int local_1e8;
  int local_1e4;
  int local_1e0;
  int local_1d8;
  int local_1d4;
  char auStack_1d0 [8];
  int local_1c8;
  int local_1c4;
  int local_1c0;
  int local_1b8;
  int local_1b4;
  int local_1b0;
  int local_1ac;
  int local_1a8;
  int local_1a0;
  int local_19c;
  int local_198;
  int local_194;
  int local_190;
  int local_188;
  int local_184;
  int local_180;
  int local_17c;
  int local_178;
  int local_170;
  int local_16c;
  int local_168;
  int local_164;
  int local_160;
  int local_158;
  int local_154;
  int local_150;
  int local_14c;
  int local_148;
  int local_140;
  int local_13c;
  int local_138;
  int local_134;
  int local_130;
  int local_12c;
  int local_128;
  int local_124;
  int local_120;
  int local_11c;
  int local_118;
  int local_114;
  int local_110;
  int local_10c;
  int local_108;
  int local_104;
  int local_100;
  int local_fc;
  int local_f8;
  int local_f4;
  int local_f0;
  int local_ec;
  int local_e8;
  int local_e4;
  int local_e0;
  int local_dc;
  int local_d8;
  int local_d4;
  int local_d0;
  int local_cc;
  int local_c8;
  int local_c4;
  int local_c0;
  int local_bc;
  int local_b8;
  int local_b4;
  int local_b0;
  int local_ac;
  int local_a8;
  int local_a4;
  int local_a0;
  int local_9c;
  int local_98;
  int local_94;
  int local_90;
  int local_8c;
  int local_88;
  int local_7c;
  int local_78;
  int local_6c;
  int local_68;
  int local_64;
  char auStack_60 [8];
  char auStack_58 [8];
  int *local_50;
  int uVar2;
  
  puVar4 = puRam10116208;
  puVar11 = (int *)(((char*)0) + -0x548);
  puVar21 = (int *)(((char*)0) + -0x3f8);
  puVar27 = (int *)(((char*)0) + -0x3f0);
  puVar28 = (int *)(((char*)0) + -0x3e0);
  piVar18 = (int *)(((char*)0) + -0x3d0);
  puVar29 = (int *)(((char*)0) + -0x3c8);
  piVar17 = (int *)(((char*)0) + -0x3b8);
  puVar30 = (int *)(((char*)0) + -0x3b0);
  puVar31 = (int *)(((char*)0) + -0x3a8);
  psVar33 = (short *)(((char*)0) + -0x398);
  puVar20 = (int *)(((char*)0) + -0x390);
  piVar16 = (int *)(((char*)0) + -0x388);
  piVar35 = (int *)(((char*)0) + -0x380);
  piVar36 = (int *)(((char*)0) + -0x370);
  *puVar11 = 0x3c;
  sStack0000001e = param_2;
  Game_GetString((unsigned long long)*puRam101169c4 + 0x32,puVar11);
  uVar19 = local_56c;
  EndFocus();
  bVar37 = local_528 < 0x300000;
  GetScreenBounds(psVar33,(unsigned long long)*(unsigned int *)(uVar19 - 0xb0) + 0x56);
  if (bVar37) {
    if (0x280 < local_396) {
      local_396 = 0x280;
    }
    if (0x1e0 < *psVar33) {
      *psVar33 = 0x1e0;
    }
  }
  uVar26 = (unsigned long long)local_396;
  uVar5 = (unsigned long long)(0x27f < uVar26);
  bVar3 = 0x27f < uVar26 != (!CARRY8(uVar26,uVar5) && !CARRY8(uVar26,uVar26 + uVar5));
  uVar22 = *(int *)(**(int **)(uVar19 - 0x554) + 0x88);
  local_318 = 0;
  local_314 = 0;
  local_310 = 0;
  local_30c = 0;
  *puVar28 = 0;
  *puVar29 = 0;
  *puVar27 = 0;
  puVar7 = ((char*)0) + -0x308;
  OpenResourceStream(puVar7,0x80);
  puVar8 = (unsigned int *)FindResourceByType(puVar7,0x57696e64,1000);
  bVar38 = puVar8 != (unsigned int *)0x0;
  uVar10 = (unsigned int)bVar38;
  local_2b8 = '\x01';
  local_2b7 = '\x01';
  local_2b6 = '\x01';
  if (puVar8 == (unsigned int *)0x0) goto LAB_1003e634;
  *puVar21 = puVar8;
  uVar24 = GetHandleFlags(puVar8);
  DetachResource(*puVar21);
  uVar19 = *puVar8;
  local_318 = *(int *)(uVar19 + 8);
  local_314 = *(int *)(uVar19 + 0xc);
  local_310 = *(int *)(uVar19 + 0x10);
  local_30c = *(int *)(uVar19 + 0x14);
  uVar19 = *puVar8;
  uVar1 = *(int *)(uVar19 + 0x1c);
  uVar2 = *(int *)(uVar19 + 0x20);
  uVar13 = *(int *)(uVar19 + 0x24);
  *puVar28 = *(int *)(uVar19 + 0x18);
  *(int *)(((char*)0) + -0x3dc) = uVar1;
  *(int *)(((char*)0) + -0x3d8) = uVar2;
  *(int *)(((char*)0) + -0x3d4) = uVar13;
  uVar19 = *puVar8;
  local_2b8 = *(char *)(uVar19 + 0x28);
  uVar1 = *(int *)(uVar19 + 0x30);
  uVar2 = *(int *)(uVar19 + 0x34);
  uVar13 = *(int *)(uVar19 + 0x38);
  *puVar29 = *(int *)(uVar19 + 0x2c);
  *(int *)(((char*)0) + -0x3c4) = uVar1;
  *(int *)(((char*)0) + -0x3c0) = uVar2;
  *(int *)(((char*)0) + -0x3bc) = uVar13;
  uVar19 = *puVar8;
  local_2b6 = *(char *)(uVar19 + 0x3c);
  uVar1 = *(int *)(uVar19 + 0x44);
  uVar2 = *(int *)(uVar19 + 0x48);
  uVar13 = *(int *)(uVar19 + 0x4c);
  *puVar27 = *(int *)(uVar19 + 0x40);
  *(int *)(((char*)0) + -0x3ec) = uVar1;
  *(int *)(((char*)0) + -1000) = uVar2;
  *(int *)(((char*)0) + -0x3e4) = uVar13;
  local_2b7 = *(char *)(*puVar8 + 0x50);
  piVar9 = (int *)FUN_10001ef0();
  uVar19 = local_56c;
  if ((*(char *)*puVar8 != '\0') && (*(int *)(*piVar9 + 0x1e) == 0)) {
    puVar7 = ((char*)0) + -0x2b4;
    GetScreenBounds(puVar7,(unsigned long long)*(unsigned int *)(local_56c - 0xb0) + 0x56);
    uVar10 = FUN_100b04f0((unsigned long long)*puVar8 + 2,puVar7);
  }
  bVar38 = false;
  if (uVar10 != 0) {
    local_2ac = FUN_10003360();
    local_2a8 = &local_318;
    uVar19 = local_56c;
    puVar11 = puVar31;
    if ((((char*)0) != (char *)0x3a8) ||
       (puVar11 = (int *)NewPtr_Thunk(0x10), puVar11 != (int *)0x0)) {
      *puVar11 = *local_2a8;
      puVar11[1] = local_2a8[1];
      puVar11[2] = local_2a8[2];
      puVar11[3] = local_2a8[3];
    }
    local_2a4 = 3;
    local_2a0 = 3;
    FUN_100e16b8(puVar31);
    if (uVar10 == 0) {
LAB_1003e4ac:
      bVar38 = false;
    }
    else {
      FUN_100e12b8(puVar31);
      uVar6 = GetBitMapPtr(((char*)0) + -0x29c);
      iVar12 = FUN_10001e60(uVar6,local_2ac);
      uVar19 = local_56c;
      if (iVar12 == 0) goto LAB_1003e4ac;
      bVar38 = true;
    }
    *puVar31 = *puVar28;
    *(int *)(((char*)0) + -0x3a4) = *(int *)(((char*)0) + -0x3dc);
    *(int *)(((char*)0) + -0x3a0) = *(int *)(((char*)0) + -0x3d8);
    *(int *)(((char*)0) + -0x39c) = *(int *)(((char*)0) + -0x3d4);
    local_294 = 3;
    local_290 = 3;
    FUN_100e16b8(puVar31);
    if (bVar38) {
      FUN_100e12b8(puVar31);
      uVar6 = GetBitMapPtr(((char*)0) + -0x28c);
      iVar12 = FUN_10001e60(uVar6,local_2ac);
      uVar19 = local_56c;
      if (iVar12 == 0) goto LAB_1003e51c;
      bVar38 = true;
    }
    else {
LAB_1003e51c:
      bVar38 = false;
    }
    *puVar31 = *puVar29;
    *(int *)(((char*)0) + -0x3a4) = *(int *)(((char*)0) + -0x3c4);
    *(int *)(((char*)0) + -0x3a0) = *(int *)(((char*)0) + -0x3c0);
    *(int *)(((char*)0) + -0x39c) = *(int *)(((char*)0) + -0x3bc);
    local_284 = 3;
    local_280 = 3;
    FUN_100e16b8(puVar31);
    if (bVar38) {
      FUN_100e12b8(puVar31);
      uVar6 = GetBitMapPtr(((char*)0) + -0x27c);
      iVar12 = FUN_10001e60(uVar6,local_2ac);
      uVar19 = local_56c;
      if (iVar12 == 0) goto LAB_1003e58c;
      bVar38 = true;
    }
    else {
LAB_1003e58c:
      bVar38 = false;
    }
    *puVar31 = *puVar27;
    *(int *)(((char*)0) + -0x3a4) = *(int *)(((char*)0) + -0x3ec);
    *(int *)(((char*)0) + -0x3a0) = *(int *)(((char*)0) + -1000);
    *(int *)(((char*)0) + -0x39c) = *(int *)(((char*)0) + -0x3e4);
    local_274 = 3;
    local_270 = 3;
    FUN_100e16b8(puVar31);
    if (bVar38) {
      FUN_100e12b8(puVar31);
      uVar6 = GetBitMapPtr(((char*)0) + -0x26c);
      iVar12 = FUN_10001e60(uVar6,local_2ac);
      uVar19 = local_56c;
      if (iVar12 != 0) {
        bVar38 = true;
        goto LAB_1003e604;
      }
    }
    bVar38 = false;
  }
LAB_1003e604:
  if (((char*)0) != (char *)0x3f8) {
    RestoreHandleFlags(*puVar21,uVar24);
    uVar19 = local_56c;
  }
LAB_1003e634:
  ReleaseHandle_Mapgen(puVar8);
  if (bVar37) {
    sVar23 = 10;
  }
  else {
    sVar23 = 0;
  }
  uVar13 = ResourceRead_Dispatch(**(int **)(uVar19 - 0x1e4) +
                        (int)*(short *)(*(int *)**(int **)(uVar19 - 0x1e4) + 200),sVar23 + 0x3ea,
                        uVar22);
  **(int **)(local_56c - 0x1504) = uVar13;
  uVar19 = local_56c;
  FocusObject();
  if (bVar3) {
    iVar12 = 0x70;
  }
  else {
    iVar12 = 0xe0;
  }
  iVar32 = (local_396 - iVar12) + -3;
  *puVar20 = 0x9c;
  *puVar30 = 0x138;
  uVar13 = **(int **)(uVar19 - 0x1504);
  if (bVar38) {
    puVar7 = ((char*)0) + -0x264;
    FUN_100e1a64(puVar7,puVar28);
    iVar14 = FUN_100e11fc(puVar20,puVar7);
    if (bVar37) {
      iVar14 = 1;
    }
    local_25c = puVar20;
    puVar21 = puVar30;
    if (iVar14 != 0) {
      local_25c = puVar30;
      puVar21 = puVar20;
    }
    uVar6 = FUN_100e1580(puVar28,0);
    FUN_1007723c(uVar13,uVar6,puVar21,local_25c,0);
  }
  else {
    local_258 = *(int *)(**(int **)(uVar19 - 0x1504) + 0x2c);
    if ((bVar37) || (bVar3)) {
      bVar25 = true;
    }
    else {
      bVar25 = false;
    }
    puVar21 = puVar30;
    if (bVar25) {
      puVar21 = puVar20;
    }
    if ((bVar37) || (bVar3)) {
      bVar25 = true;
    }
    else {
      bVar25 = false;
    }
    puVar28 = puVar20;
    if (bVar25) {
      puVar28 = puVar30;
    }
    local_254 = iVar32;
    FUN_1007723c(uVar13,((char*)0) + -600,puVar21,puVar28,0);
  }
  uVar13 = ResourceRead_Dispatch(**(int **)(uVar19 - 0x1504) +
                        (int)*(short *)(*(int *)**(int **)(uVar19 - 0x1504) + 0x308),0x6f766572);
  **(int **)(local_56c - 0x169c) = uVar13;
  uVar19 = local_56c;
  FocusObject();
  iVar14 = ResourceRead_Dispatch(**(int **)(uVar19 - 0x1e4) +
                        (int)*(short *)(*(int *)**(int **)(uVar19 - 0x1e4) + 200),0x3ec,uVar22);
  piVar9 = *(int **)(local_56c - 0x16a0);
  *piVar9 = iVar14;
  uVar19 = local_56c;
  FocusObject();
  local_250 = 0x27;
  local_24c = 0x70;
  *puVar20 = 0x27;
  *(int *)(((char*)0) + -0x38c) = 0x70;
  iVar14 = *piVar9;
  uVar13 = *(int *)(iVar14 + 0x38);
  *puVar30 = *(int *)(iVar14 + 0x34);
  *(int *)(((char*)0) + -0x3ac) = uVar13;
  if (bVar38) {
    puVar7 = ((char*)0) + -0x248;
    FUN_100e1a64(puVar7,puVar27);
    iVar15 = FUN_100e11fc(puVar20,puVar7);
    puVar21 = puVar30;
    if (iVar15 != 0) {
      puVar21 = puVar20;
      puVar20 = puVar30;
    }
    uVar6 = FUN_100e1580(puVar27,0);
    FUN_1007723c(iVar14,uVar6,puVar21,puVar20,0);
  }
  else {
    if (bVar3) {
      sVar23 = (short)*(int *)(**(int **)(uVar19 - 0x1504) + 0x2c) +
               (short)*(int *)(**(int **)(uVar19 - 0x1504) + 0x34) + 0x10;
    }
    else {
      sVar23 = (short)*(int *)(**(int **)(uVar19 - 0x16a0) + 0x2c);
    }
    local_240 = (int)sVar23;
    puVar21 = puVar30;
    if (*psVar33 < 0x1e0) {
      puVar21 = puVar20;
    }
    if (*psVar33 < 0x1e0) {
      puVar20 = puVar30;
    }
    local_23c = iVar32;
    FUN_1007723c(iVar14,((char*)0) + -0x240,puVar21,puVar20,0);
  }
  iVar14 = ResourceRead_Dispatch(**(int **)(uVar19 - 0x1e4) +
                        (int)*(short *)(*(int *)**(int **)(uVar19 - 0x1e4) + 200),0x3eb,uVar22);
  piVar9 = *(int **)(local_56c - 0x18fc);
  *piVar9 = iVar14;
  uVar19 = local_56c;
  FocusObject();
  iVar15 = *(int *)(**(int **)(uVar19 - 0x16a0) + 0x2c) +
           *(int *)(**(int **)(uVar19 - 0x16a0) + 0x34);
  iVar34 = iVar15 + 0x10;
  *piVar16 = iVar34;
  iVar14 = *piVar9;
  local_228 = *(int *)(iVar14 + 0x34);
  uStack_224 = *(int *)(iVar14 + 0x38);
  *piVar18 = 0x81;
  *piVar17 = 0x78;
  if (bVar38) {
    piVar16 = (int *)FUN_100e1580(puVar29,0);
    iVar32 = piVar16[1];
    *piVar36 = *piVar16;
    *(int *)(((char*)0) + -0x36c) = iVar32;
    puVar7 = ((char*)0) + -0x220;
    FUN_100e1a64(puVar7,puVar29);
    iVar32 = FUN_100e11fc(((char*)0) + -0x228,puVar7);
    if (iVar32 == 0) {
      puVar7 = ((char*)0) + -0x1d0;
      FUN_100e1a64(puVar7,puVar29);
      iVar32 = FUN_100e11fc(piVar18,puVar7);
      if (iVar32 == 0) {
        FUN_100e10b0(((char*)0) + -0x180,piVar36,piVar17);
        local_178 = *piVar36;
        local_170 = local_180;
        local_16c = *(int *)(((char*)0) + -0x17c);
        *piVar35 = local_178;
        *(int *)(((char*)0) + -0x37c) = local_36c[0];
        *(int *)(((char*)0) + -0x378) = local_180;
        *(int *)(((char*)0) + -0x374) = local_16c;
        FUN_100e10b0(((char*)0) + -0x168,piVar36,((char*)0) + -0x228);
        local_408 = *piVar36;
        local_158 = local_168;
        iStack_3fc = *(int *)(((char*)0) + -0x164);
        iStack_404 = local_36c[0];
        iStack_400 = local_168;
        local_160 = local_408;
        local_154 = iStack_3fc;
        FUN_100e10b0(((char*)0) + -0x150,piVar36,piVar17);
        local_238 = *piVar36;
        local_140 = local_150;
        iStack_22c = *(int *)(((char*)0) + -0x14c);
        iStack_234 = local_36c[0];
        iStack_230 = local_150;
        local_148 = local_238;
        local_13c = iStack_22c;
      }
      else {
        FUN_100e10b0(((char*)0) + -0x1c8,piVar36,piVar18);
        local_1c0 = *piVar36;
        local_1b8 = local_1c8;
        local_1b4 = *(int *)(((char*)0) + -0x1c4);
        *piVar35 = local_1c0;
        *(int *)(((char*)0) + -0x37c) = local_36c[0];
        *(int *)(((char*)0) + -0x378) = local_1c8;
        *(int *)(((char*)0) + -0x374) = local_1b4;
        FUN_100e10b0(((char*)0) + -0x1b0,piVar36,piVar17);
        local_408 = *piVar36;
        local_1a0 = local_1b0;
        iStack_3fc = *(int *)(((char*)0) + -0x1ac);
        iStack_404 = local_36c[0];
        iStack_400 = local_1b0;
        local_1a8 = local_408;
        local_19c = iStack_3fc;
        FUN_100e10b0(((char*)0) + -0x198,piVar36,((char*)0) + -0x228);
        local_238 = *piVar36;
        local_188 = local_198;
        iStack_22c = *(int *)(((char*)0) + -0x194);
        iStack_234 = local_36c[0];
        iStack_230 = local_198;
        local_190 = local_238;
        local_184 = iStack_22c;
      }
    }
    else {
      FUN_100e10b0(((char*)0) + -0x218,piVar36,((char*)0) + -0x228);
      local_210 = *piVar36;
      local_208 = local_218;
      local_204 = *(int *)(((char*)0) + -0x214);
      *piVar35 = local_210;
      *(int *)(((char*)0) + -0x37c) = local_36c[0];
      *(int *)(((char*)0) + -0x378) = local_218;
      *(int *)(((char*)0) + -0x374) = local_204;
      FUN_100e10b0(((char*)0) + -0x200,piVar36,piVar18);
      local_408 = *piVar36;
      local_1f0 = local_200;
      iStack_3fc = *(int *)(((char*)0) + -0x1fc);
      iStack_404 = local_36c[0];
      iStack_400 = local_200;
      local_1f8 = local_408;
      local_1ec = iStack_3fc;
      FUN_100e10b0(((char*)0) + -0x1e8,piVar36,piVar17);
      local_238 = *piVar36;
      local_1d8 = local_1e8;
      iStack_22c = *(int *)(((char*)0) + -0x1e4);
      iStack_234 = local_36c[0];
      iStack_230 = local_1e8;
      local_1e0 = local_238;
      local_1d4 = iStack_22c;
    }
  }
  else if (bVar3) {
    FUN_100e10b0(((char*)0) + -0x138,piVar16,piVar17,0x70,((char*)0) + -0x238);
    local_130 = *piVar16;
    local_128 = local_138;
    local_124 = *(int *)(((char*)0) + -0x134);
    *piVar35 = local_130;
    *(int *)(((char*)0) + -0x37c) = iVar32;
    *(int *)(((char*)0) + -0x378) = local_138;
    *(int *)(((char*)0) + -0x374) = local_124;
    local_12c = iVar32;
    uVar6 = FUN_100e1580(piVar35,1);
    FUN_100e10f8(((char*)0) + -0x120,uVar6,((char*)0) + -0x228);
    piVar17 = (int *)FUN_100e1580(piVar35,1);
    local_118 = local_120;
    iStack_404 = *(int *)(((char*)0) + -0x11c);
    iStack_400 = *piVar17;
    iStack_3fc = piVar17[1];
    local_408 = local_120;
    local_114 = iStack_404;
    local_110 = iStack_400;
    local_10c = iStack_3fc;
    uVar6 = FUN_100e1580(piVar35,1);
    FUN_100e10f8(((char*)0) + -0x108,uVar6,piVar18);
    piVar18 = (int *)FUN_100e1580(piVar35,1);
    local_100 = local_108;
    iStack_234 = *(int *)(((char*)0) + -0x104);
    iStack_230 = *piVar18;
    iStack_22c = piVar18[1];
    local_238 = local_108;
    iVar34 = *psVar33 + 0xd;
    local_fc = iStack_234;
    local_f8 = iStack_230;
    local_f4 = iStack_22c;
  }
  else if (iVar15 + 3 < ((int)*psVar33 - *(int *)(**(int **)(uVar19 - 0x18fc) + 0x34)) + -4) {
    FUN_100e10b0(((char*)0) + -0xf0,piVar16,piVar18);
    local_e8 = *piVar16;
    local_e0 = local_f0;
    local_dc = *(int *)(((char*)0) + -0xec);
    *piVar35 = local_e8;
    *(int *)(((char*)0) + -0x37c) = iVar32;
    *(int *)(((char*)0) + -0x378) = local_f0;
    *(int *)(((char*)0) + -0x374) = local_dc;
    local_e4 = iVar32;
    uVar6 = FUN_100e1580(piVar35,1);
    FUN_100e10f8(((char*)0) + -0xd8,uVar6,piVar17);
    piVar18 = (int *)FUN_100e1580(piVar35,1);
    local_d0 = local_d8;
    iStack_404 = *(int *)(((char*)0) + -0xd4);
    iStack_400 = *piVar18;
    iStack_3fc = piVar18[1];
    local_408 = local_d8;
    local_cc = iStack_404;
    local_c8 = iStack_400;
    local_c4 = iStack_3fc;
    uVar6 = FUN_100e1580(piVar35,1);
    FUN_100e10f8(((char*)0) + -0xc0,uVar6,((char*)0) + -0x228);
    piVar18 = (int *)FUN_100e1580(piVar35,1);
    local_b8 = local_c0;
    iStack_234 = *(int *)(((char*)0) + -0xbc);
    iStack_230 = *piVar18;
    iStack_22c = piVar18[1];
    local_238 = local_c0;
    iVar34 = *psVar33 + 0xd;
    local_b4 = iStack_234;
    local_b0 = iStack_230;
    local_ac = iStack_22c;
  }
  else {
    iVar34 = ((int)*psVar33 - *(int *)(**(int **)(uVar19 - 0x18fc) + 0x34)) + -4;
    local_a4 = *(int *)(**(int **)(uVar19 - 0x18fc) + 0x30);
    *piVar16 = iVar34;
    *(int *)(((char*)0) + -900) = local_a4;
    local_a8 = iVar34;
    FUN_100e10b0(((char*)0) + -0xa0,piVar16,((char*)0) + -0x228);
    local_98 = *piVar16;
    local_90 = local_a0;
    local_8c = *(int *)(((char*)0) + -0x9c);
    *piVar35 = local_98;
    *(int *)(((char*)0) + -0x37c) = iVar32;
    *(int *)(((char*)0) + -0x378) = local_a0;
    *(int *)(((char*)0) + -0x374) = local_8c;
    local_408 = local_378 - *piVar18;
    iStack_3fc = local_37c + 0xe0;
    iStack_404 = local_37c;
    iStack_400 = local_378;
    local_238 = local_378 - *piVar17;
    iStack_22c = local_37c + 0x70;
    iStack_234 = local_37c;
    iStack_230 = local_378;
    local_94 = iVar32;
    local_88 = local_408;
    local_7c = iStack_3fc;
    local_78 = local_238;
    local_6c = iStack_22c;
  }
  piVar18 = (int *)**(int **)(uVar19 - 0x18fc);
  ResourceRead_Dispatch((int)piVar18 + (int)*(short *)(*piVar18 + 0x350),piVar35,0);
  iVar15 = *(int *)(((char*)0) + -0x37c);
  iVar14 = *(int *)(((char*)0) + -0x378);
  iVar32 = *(int *)(((char*)0) + -0x374);
  piVar18[0x32] = *piVar35;
  piVar18[0x33] = iVar15;
  piVar18[0x34] = iVar14;
  piVar18[0x35] = iVar32;
  piVar18[0x36] = local_408;
  piVar18[0x37] = iStack_404;
  piVar18[0x38] = iStack_400;
  piVar18[0x39] = iStack_3fc;
  piVar18[0x3a] = local_238;
  piVar18[0x3b] = iStack_234;
  piVar18[0x3c] = iStack_230;
  piVar18[0x3d] = iStack_22c;
  uVar19 = ResourceRead_Dispatch(**(int **)(local_56c - 0x1e4) +
                        (int)*(short *)(*(int *)**(int **)(local_56c - 0x1e4) + 200),0x3e9,uVar22);
  *puVar4 = uVar19;
  FocusObject();
  if (bVar38) {
    ResourceRead_Dispatch((int)*puVar4 + (int)*(short *)(*(int *)*puVar4 + 0x350),((char*)0) + -0x318,0
                );
  }
  else {
    local_64 = (local_396 - iVar12) + -9;
    piVar18 = (int *)*puVar4;
    local_68 = (iVar34 - piVar18[0xb]) + -0x10;
    ResourceRead_Dispatch((int)piVar18 + (int)*(short *)(*piVar18 + 0x378),((char*)0) + -0x68,0);
  }
  if (bVar37) {
    piVar18 = (int *)*puVar4;
    iVar12 = *piVar18;
    puVar20 = (int *)FUN_100e0f78(((char*)0) + -0x60,(unsigned long long)*puVar4 + 0x34);
    puVar21 = (int *)FUN_100e0f78(((char*)0) + -0x58,piVar18 + 0xd);
    ResourceRead_Dispatch((int)piVar18 + (int)*(short *)(iVar12 + 0x7c8),*puVar21,*puVar20);
  }
  iVar12 = ResourceRead_Dispatch((int)*puVar4 + (int)*(short *)(*(int *)*puVar4 + 0x308),0x6d617020);
  piVar18 = *(int **)(local_56c - 0x14f4);
  *piVar18 = iVar12;
  FocusObject();
  iVar12 = ResourceRead_Dispatch(*piVar18 + (int)*(short *)(*(int *)*piVar18 + 0x5f0),1);
  uVar5 = (unsigned long long)local_56c;
  if (iVar12 != 0) {
    *(char *)(*(int *)(iVar12 + 0x90) + 0x48) = 0;
    *(char *)(*(int *)(iVar12 + 0x90) + 0x49) = 0;
    *(char *)(*(int *)(iVar12 + 0x94) + 0x48) = 0;
    *(char *)(*(int *)(iVar12 + 0x94) + 0x49) = 0;
  }
  iVar12 = 0;
  piVar17 = *(int **)(local_56c - 0x554);
  local_50 = piVar18;
  do {
    FUN_10075ee0(*piVar17,iVar12,*(short *)(*piVar17 + iVar12 * 2 + 0x8c));
    uVar19 = (unsigned int)uVar5;
    sVar23 = (short)iVar12 + 1;
    iVar12 = (int)sVar23;
  } while (sVar23 < 4);
  if (sStack0000001e != 0) {
    FUN_1003c838();
    FUN_1003d4dc();
    FUN_10064850(1,1);
    FUN_1003c368();
  }
  FUN_1003d5d4();
  if ((!bVar38) || (local_2b8 != '\0')) {
    piVar18 = *(int **)(uVar19 - 0x1504);
    ResourceRead_Dispatch(*piVar18 + (int)*(short *)(*(int *)*piVar18 + 800));
    *(char *)(*piVar18 + 0xa4) = 1;
    ResourceRead_Dispatch(*piVar18 + (int)*(short *)(*(int *)*piVar18 + 0x4d0));
    ResourceRead_Dispatch(*piVar18 + (int)*(short *)(*(int *)*piVar18 + 0x4c8));
    uVar19 = local_56c;
  }
  if ((!bVar38) || (local_2b6 != '\0')) {
    piVar18 = *(int **)(uVar19 - 0x18fc);
    ResourceRead_Dispatch(*piVar18 + (int)*(short *)(*(int *)*piVar18 + 800));
    *(char *)(*piVar18 + 0xa4) = 1;
    ResourceRead_Dispatch(*piVar18 + (int)*(short *)(*(int *)*piVar18 + 0x4d0));
    ResourceRead_Dispatch(*piVar18 + (int)*(short *)(*(int *)*piVar18 + 0x4c8));
    uVar19 = local_56c;
  }
  if ((!bVar38) || (local_2b7 != '\0')) {
    piVar18 = *(int **)(uVar19 - 0x16a0);
    ResourceRead_Dispatch(*piVar18 + (int)*(short *)(*(int *)*piVar18 + 800));
    *(char *)(*piVar18 + 0xa4) = 1;
    ResourceRead_Dispatch(*piVar18 + (int)*(short *)(*(int *)*piVar18 + 0x4d0));
    ResourceRead_Dispatch(*piVar18 + (int)*(short *)(*(int *)*piVar18 + 0x4c8));
  }
  ResourceRead_Dispatch((int)*puVar4 + (int)*(short *)(*(int *)*puVar4 + 800));
  ResourceRead_Dispatch((int)*puVar4 + (int)*(short *)(*(int *)*puVar4 + 0x4d0));
  ResourceRead_Dispatch((int)*puVar4 + (int)*(short *)(*(int *)*puVar4 + 0x4c8));
  ResourceRead_Dispatch((int)*puVar4 + (int)*(short *)(*(int *)*puVar4 + 0x780));
  ResourceRead_Dispatch(**(int **)(local_56c - 0xedc) +
               (int)*(short *)(*(int *)**(int **)(local_56c - 0xedc) + 0x338),
               *(int *)(*puVar4 + 0x98));
  **(short **)(local_56c - 0x1bc0) = 1;
  FUN_1003d6c4();
  FullDisplayRefresh();
  if (sStack0000001e != 0) {
    ResourceRead_Dispatch((int)*puVar4 + (int)*(short *)(*(int *)*puVar4 + 0x4d0));
    uVar19 = local_56c;
    FUN_1003e0b0();
    if (*(short *)(**(int **)(uVar19 - 0x544) + 0x124) == 0) {
      RefreshDisplay(1,1,0);
      MapRefreshAndCombat();
    }
    else {
      piVar18 = *(int **)(uVar19 - 0x14f4);
      *(char *)(*piVar18 + 0xbe) = 1;
      ResourceRead_Dispatch(*piVar18 + (int)*(short *)(*(int *)*piVar18 + 0x4d0));
      ResourceRead_Dispatch(*piVar18 + (int)*(short *)(*(int *)*piVar18 + 0x4c8));
    }
    ResourceRead_Dispatch((int)*puVar4 + (int)*(short *)(*(int *)*puVar4 + 0x4c8));
    iVar12 = **(int **)(local_56c - 0x554);
    *(char *)(iVar12 + 0x9a) = 1;
    uVar22 = GetTickCount();
    *(int *)(iVar12 + 0x9c) = uVar22;
  }
  CloseResourceStream(((char*)0) + -0x308,2);
  return;
}











































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1003f674 Size: 116 bytes */
void FUN_1003f674(short param_1,short param_2,short param_3)

{
  FullDisplayRefresh();
  if (param_1 != 0) {
    MapRefreshAndCombat();
  }
  if (param_2 != 0) {
    RefreshDisplay(1,1,0);
  }
  if (param_3 != 0) {
    FUN_1005cc8c();
  }
  return;
}

/* Address: 0x1004039c Size: 108 bytes */
void FUN_1004039c(short param_1)

{
  long long uVar1;
  
  uVar1 = ResourceRead_Dispatch(*gOverviewWindow + (int)*(short *)(*(int *)*gOverviewWindow + 0x308),0x696e666f);
  if ((int)uVar1 != 0) {
    FUN_10080fe4(uVar1,param_1);
  }
  return;
}

/* Address: 0x10040408 Size: 852 bytes */
void FUN_10040408(long long param_1,long long param_2,long long param_3,long long param_4,
                 long long param_5,long long param_6,long long param_7)

{
  int *piVar1;
  int *piVar2;
  char auStack_738 [256];
  char auStack_638 [256];
  char auStack_538 [256];
  char auStack_438 [256];
  char auStack_338 [256];
  char auStack_238 [568];
  
  piVar1 = piRam10115f7c;
  piVar2 = (int *)ResourceRead_Dispatch(*piRam10115f7c + (int)*(short *)(*(int *)*piRam10115f7c + 0x308),
                               0x73747231);
  if (piVar2 != (int *)0x0) {
    BuildPascalString(auStack_238,param_2);
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x7c8),auStack_238,0);
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x4d0));
  }
  piVar2 = (int *)ResourceRead_Dispatch(*piVar1 + (int)*(short *)(*(int *)*piVar1 + 0x308),0x73747232);
  if (piVar2 != (int *)0x0) {
    BuildPascalString(auStack_338,param_3);
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x7c8),auStack_338,0);
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x4d0));
  }
  piVar2 = (int *)ResourceRead_Dispatch(*piVar1 + (int)*(short *)(*(int *)*piVar1 + 0x308),0x73747233);
  if (piVar2 != (int *)0x0) {
    BuildPascalString(auStack_438,param_4);
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x7c8),auStack_438,0);
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x4d0));
  }
  piVar2 = (int *)ResourceRead_Dispatch(*piVar1 + (int)*(short *)(*(int *)*piVar1 + 0x308),0x73747234);
  if (piVar2 != (int *)0x0) {
    BuildPascalString(auStack_538,param_5);
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x7c8),auStack_538,0);
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x4d0));
  }
  piVar2 = (int *)ResourceRead_Dispatch(*piVar1 + (int)*(short *)(*(int *)*piVar1 + 0x308),0x73747235);
  if (piVar2 != (int *)0x0) {
    BuildPascalString(auStack_638,param_6);
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x7c8),auStack_638,0);
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x4d0));
  }
  piVar2 = (int *)ResourceRead_Dispatch(*piVar1 + (int)*(short *)(*(int *)*piVar1 + 0x308),0x73747236);
  if (piVar2 != (int *)0x0) {
    BuildPascalString(auStack_738,param_7);
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x7c8),auStack_738,0);
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x4d0));
  }
  ResourceRead_Dispatch(*piVar1 + (int)*(short *)(*(int *)*piVar1 + 0x4c8));
  return;
}

/* Address: 0x100423a0 Size: 236 bytes */
void FUN_100423a0(short param_1)

{
  int bVar1;
  int *piVar2;
  int *piVar3;
  int *piVar4;
  int iVar5;
  int iVar6;
  
  piVar4 = gPathCachePtr;
  piVar3 = gPathCacheHandle;
  piVar2 = (int *)*gPathCacheHandle;
  if (piVar2 != (int *)0x0) {
    DetachResource(piVar2);
  }
  *piVar4 = *piVar2;
  iVar5 = 199;
  do {
    iVar6 = param_1 * 0xd6;
    *(char *)(*piVar4 + iVar6 + iVar5 + 0xe) = 0;
    bVar1 = iVar5 != 0;
    iVar5 = (int)(short)((short)iVar5 + -1);
  } while (bVar1);
  *(short *)(iVar6 + *piVar4) = 0;
  *(short *)(*piVar4 + iVar6 + 2) = 0;
  *(short *)(*piVar4 + iVar6 + 4) = 0;
  *(short *)(*piVar4 + iVar6 + 6) = 0;
  *(short *)(iVar6 + *piVar4 + 8) = 0;
  *(short *)(iVar6 + *piVar4 + 10) = 0;
  *(short *)(iVar6 + *piVar4 + 0xc) = 0;
  if (*piVar3 != 0) {
    DisposeHandle_Thunk(*piVar3);
  }
  return;
}













































































































































/* Address: 0x1004248c Size: 88 bytes */
void FUN_1004248c()

{
  long long lVar1;
  int iVar2;
  
  if (*gPathfindingInitialized != 0) {
    lVar1 = 0x13;
    do {
      FUN_100423a0(lVar1);
      iVar2 = (int)lVar1;
      lVar1 = (long long)(short)((short)lVar1 + -1);
    } while (iVar2 != 0);
  }
  return;
}

/* Address: 0x100424e4 Size: 220 bytes */
void FUN_100424e4()

{
  int *piVar1;
  int *piVar2;
  int *piVar3;
  int *piVar4;
  int iVar5;
  int **local_34;
  
  piVar4 = gDistGridHandle;
  piVar3 = gCostMapHandle;
  piVar2 = gPathCacheHandle;
  piVar1 = (int *)*gDistGridHandle;
  if (piVar1 == (int *)0x0) {
    local_34 = 0 /* TVect base */;
    iVar5 = AllocateHandle(0x8880,0);
    *piVar4 = iVar5;
  }
  else {
    if (*piVar1 == 0) {
      ResizeHandle(piVar1,0x8880);
      CheckMemError();
    }
    LockHandle_Thunk(*piVar4);
  }
  if (*piVar3 == 0) {
    iVar5 = AllocateHandle(0x4440,0,0);
    *piVar3 = iVar5;
  }
  if (*piVar2 == 0) {
    iVar5 = AllocateHandle(0x10b8,0,0);
    *piVar2 = iVar5;
  }
  *(short *)local_34[-0x67f] = 1;
  PostLoadCityInit();
  return;
}































































































































































































































/* Address: 0x100425c0 Size: 236 bytes */
int FUN_100425c0(short param_1,short param_2,short param_3)

{
  int *piVar1;
  int iVar2;
  short sVar3;
  int iVar4;
  short local_20;
  short local_1e [15];
  
  piVar1 = gPathCachePtr;
  iVar4 = *gPathCachePtr + param_3 * 0xd6;
  local_1e[0] = *(short *)(iVar4 + 6);
  local_20 = *(short *)(iVar4 + 8);
  iVar4 = 0;
  do {
    if (*(char *)(*piVar1 + param_3 * 0xd6 + iVar4 + 0xe) == -1) {
      return -1;
    }
    if ((local_1e[0] == param_1) && (local_20 == param_2)) {
      return iVar4;
    }
    iVar2 = ApplyDirectionOffset(local_1e,&local_20,*(char *)(*piVar1 + param_3 * 0xd6 + iVar4 + 0xe))
    ;
    if (iVar2 == 0) {
      return -1;
    }
    sVar3 = (short)iVar4 + 1;
    iVar4 = (int)sVar3;
  } while (sVar3 < 200);
  return -1;
}

/* Address: 0x100426b4 Size: 280 bytes */
long long FUN_100426b4(short *param_1,short param_2)

{
  int *piVar1;
  long long uVar2;
  int iVar3;
  int iVar4;
  short sVar5;
  
  piVar1 = gPathCachePtr;
  if (param_1[2] == *(short *)(param_2 * 0xd6 + *gPathCachePtr + 10)) {
    uVar2 = 0;
    iVar3 = *gPathCachePtr + param_2 * 0xd6;
    if ((((param_1[3] == *(short *)(iVar3 + 0xc)) && (param_1[5] == *(short *)(iVar3 + 2))) &&
        (param_1[4] == *(short *)(iVar3 + 4))) &&
       (iVar3 = FUN_100425c0(*param_1,param_1[1],param_2), iVar3 != -1)) {
      iVar4 = 0;
      if (iVar3 < 200) {
        do {
          *(char *)(*(int *)(param_1 + 6) + iVar4) =
               *(char *)(*piVar1 + param_2 * 0xd6 + iVar3 + 0xe);
          sVar5 = (short)iVar3 + 1;
          iVar3 = (int)sVar5;
          iVar4 = (int)(short)((short)iVar4 + 1);
        } while (sVar5 < 200);
      }
      *(short *)(*piVar1 + param_2 * 0xd6) = 2;
      uVar2 = 1;
    }
  }
  else {
    uVar2 = 0;
  }
  return uVar2;
}

/* Address: 0x100427cc Size: 272 bytes */
long long FUN_100427cc(long long param_1,short param_2)

{
  int *puVar1;
  int *piVar2;
  int *piVar3;
  int *puVar4;
  int *ppuVar5;
  int iVar6;
  short sVar7;
  long long uVar8;
  int **local_44;
  
  puVar4 = gDistGridDeref;
  piVar3 = gDistGridHandle;
  piVar2 = gPathCacheHandle;
  puVar1 = (int *)*gPathCacheHandle;
  uVar8 = 0;
  ppuVar5 = 0 /* TVect base */;
  if (puVar1 != (int *)0x0) {
    DetachResource(puVar1);
    ppuVar5 = local_44;
  }
  *(int *)(*(int*)((char*)ppuVar5 - 0xc5)) = *puVar1;
  puVar1 = (int *)*piVar3;
  if (puVar1 != (int *)0x0) {
    DetachResource(puVar1);
    ppuVar5 = local_44;
  }
  *puVar4 = *puVar1;
  *(int **)(*(int*)((char*)ppuVar5 - 0xc1)) = puVar4;
  if (param_2 == 0) {
    iVar6 = FUN_100426b4(param_1,0);
    if (iVar6 != 0) {
      uVar8 = 1;
    }
  }
  else {
    sVar7 = 1;
    do {
      iVar6 = FUN_100426b4(param_1,sVar7);
      if (iVar6 != 0) {
        uVar8 = 1;
        break;
      }
      sVar7 = sVar7 + 1;
    } while (sVar7 < 0x14);
  }
  iVar6 = *piVar3;
  if (iVar6 != 0) {
    DisposeHandle_Thunk(iVar6);
  }
  if (*piVar2 != 0) {
    DisposeHandle_Thunk(*piVar2);
  }
  return uVar8;
}

































































































































































































































/* Address: 0x10043248 Size: 1884 bytes */
short FUN_10043248(short *param_1,short param_2)

{
  unsigned char bVar1;
  unsigned char bVar2;
  short sVar3;
  short sVar4;
  int iVar5;
  int iVar6;
  int bVar7;
  int bVar8;
  int iVar9;
  int iVar10;
  int *puVar11;
  int *piVar12;
  short *psVar13;
  short sVar14;
  short *psVar15;
  short sVar17;
  int iVar16;
  short sVar18;
  short sVar20;
  int iVar19;
  short sVar21;
  long long lVar22;
  unsigned short local_88;
  short local_86;
  short local_84;
  short local_80;
  short local_7a;
  short local_74;
  short local_6e;
  short local_68;
  short local_62;
  short local_58;
  
  psVar13 = gZoCPenalty;
  piVar12 = gCostMapPtr;
  puVar11 = gDistGridHandlePtr;
  iVar10 = gNeighborDXOffsets;
  iVar9 = gNeighborDYOffsets;
  local_86 = 1;
  local_84 = 3;
  bVar7 = false;
  if ((*gAIModeFlag == 0) || (param_2 == 0)) {
    if ((param_1[4] & 0x10U) == 0) {
      if (param_2 == 0) {
        sVar17 = 0;
      }
      else {
        sVar17 = *gSearchRadius;
      }
      *gSearchRadius = sVar17;
      if (param_2 == 0) {
        local_80 = 6;
      }
      else if (*gAIModeFlag == 0) {
        local_80 = 0x32;
      }
      else {
        local_80 = 0x14;
      }
    }
    else {
      *gSearchRadius = 0;
      local_80 = *gMaxSearchRange;
    }
    local_7a = *param_1;
    if (param_1[2] < *param_1) {
      local_7a = param_1[2];
    }
    local_7a = local_7a - local_80;
    if (local_7a < 0) {
      local_7a = 0;
    }
    local_74 = param_1[1];
    if (param_1[3] < param_1[1]) {
      local_74 = param_1[3];
    }
    local_74 = local_74 - local_80;
    if (local_74 < 0) {
      local_74 = 0;
    }
    local_6e = *param_1;
    if (*param_1 < param_1[2]) {
      local_6e = param_1[2];
    }
    local_6e = local_6e + local_80;
    if (0x6f < local_6e) {
      local_6e = 0x6f;
    }
    local_68 = param_1[1];
    if (param_1[1] < param_1[3]) {
      local_68 = param_1[3];
    }
    local_68 = local_68 + local_80;
    if (0x9b < local_68) {
      local_68 = 0x9b;
    }
    sVar17 = *param_1;
    sVar3 = param_1[1];
    do {
      bVar8 = false;
      if (((param_1[4] & 0x10U) != 0) && (*gSearchRadius == local_80)) {
        return local_86;
      }
      local_62 = param_1[2] + *gSearchRadius;
      if (0x6f < local_62) {
        local_62 = 0x6f;
      }
      sVar14 = param_1[3] + *gSearchRadius;
      if (0x9b < sVar14) {
        sVar14 = 0x9b;
      }
      if (local_6e <= local_62) {
        local_62 = local_6e;
      }
      sVar21 = local_68;
      if (sVar14 < local_68) {
        sVar21 = sVar14;
      }
      if (local_74 < 0) {
        local_58 = 0;
      }
      else {
        local_58 = local_74;
      }
      lVar22 = (long long)(short)(param_1[2] - *gSearchRadius);
      if (lVar22 < 0) {
        lVar22 = 0;
      }
      if ((int)lVar22 <= (int)local_7a) {
        lVar22 = (long long)local_7a;
      }
      if ((int)lVar22 <= (int)local_62) {
        do {
          iVar6 = (int)lVar22;
          iVar5 = *(int *)*puVar11 + iVar6 * 0x138;
          sVar14 = param_1[3] - *gSearchRadius;
          if ((short)(param_1[3] - *gSearchRadius) < local_58) {
            sVar14 = local_58;
          }
          for (; sVar14 <= sVar21; sVar14 = sVar14 + 1) {
            sVar4 = *(short *)(iVar5 + sVar14 * 2);
            if (sVar4 < 1) {
              bVar1 = *(unsigned char *)(*piVar12 + iVar6 * 0x9c + (int)sVar14);
              *(short *)(sVar14 * 2 + iVar5) = -sVar4;
              bVar8 = true;
              if (*gAIModeFlag == 0) {
                sVar20 = 0;
              }
              else {
                sVar20 = 9;
              }
              iVar16 = (int)sVar20;
              if (sVar14 == 0) {
                if (iVar6 == 0) {
                  iVar16 = 1;
                }
                else if (iVar6 == 0x6f) {
                  iVar16 = 3;
                }
                else {
                  iVar16 = 2;
                }
              }
              else if (sVar14 == 0x9b) {
                if (iVar6 == 0) {
                  iVar16 = 6;
                }
                else if (iVar6 == 0x6f) {
                  iVar16 = 8;
                }
                else {
                  iVar16 = 7;
                }
              }
              else if (iVar6 == 0) {
                iVar16 = 4;
              }
              else if (iVar6 == 0x6f) {
                iVar16 = 5;
              }
              iVar16 = (int)*(short *)(iVar16 * 2 + gNeighborPatternTable);
              sVar20 = *(short *)(iVar16 * 2 + iVar10);
              while (sVar20 != -10) {
                sVar20 = (short)lVar22 + sVar20;
                iVar19 = (int)(short)((short)iVar16 + 1);
                sVar18 = sVar14 + *(short *)(iVar16 * 2 + iVar9);
                iVar16 = *(int *)*puVar11 + sVar20 * 0x138;
                psVar15 = (short *)(iVar16 + sVar18 * 2);
                if (*(short *)(iVar16 + sVar18 * 2) != 0x7531) {
                  bVar2 = *(unsigned char *)(*piVar12 + sVar20 * 0x9c + (int)sVar18);
                  local_88 = bVar2 & 7;
                  if (((int)sVar20 == (int)sVar17) && (sVar18 == sVar3)) {
                    if (((param_1[5] != 1) || ((bVar1 & 0x10) != 0)) ||
                       (((bVar2 & 0x10) != 0 || ((bVar2 & 8) == (bVar1 & 8))))) {
                      local_88 = 1;
LAB_10043894:
                      sVar20 = -*psVar15;
                      if (-*psVar15 < *psVar15) {
                        sVar20 = *psVar15;
                      }
                      if ((int)-sVar4 + (int)(short)local_88 < (int)sVar20) {
                        *psVar15 = sVar4 - local_88;
                      }
                    }
                  }
                  else {
                    if (param_1[5] == 2) {
                      if (((bVar2 & 8) == 0) || ((bVar2 & 0x10) != 0)) {
                        if ((bVar2 & 7) == 0) {
                          local_88 = 2;
                        }
                        if (2 < local_88) {
                          local_88 = 2;
                        }
                      }
                      else {
                        local_88 = 2;
                      }
                      goto LAB_10043894;
                    }
                    if (param_1[5] == 0) {
                      if (((bVar2 & 8) != 0) || ((bVar2 & 0x80) != 0)) goto LAB_10043894;
                      *psVar15 = 0x7531;
                    }
                    else if ((((bVar1 & 0x10) != 0) || ((bVar2 & 0x10) != 0)) ||
                            ((bVar2 & 8) == (bVar1 & 8))) {
                      if (((param_1[4] != 0) && (2 < local_88)) &&
                         ((((param_1[4] & 1U) != 0 && ((bVar2 & 0x20) != 0)) ||
                          (((param_1[4] & 2U) != 0 && ((bVar2 & 0x40) != 0)))))) {
                        local_88 = 2;
                      }
                      if (((((bVar1 & 8) == 0) || ((bVar1 & 0x10) != 0)) && ((bVar2 & 8) != 0)) &&
                         ((bVar2 & 0x10) == 0)) {
                        local_88 = local_88 + *psVar13;
                      }
                      goto LAB_10043894;
                    }
                  }
                }
                iVar16 = iVar19;
                sVar20 = *(short *)(iVar19 * 2 + iVar10);
              }
              if (((iVar6 == sVar17) && (sVar14 == sVar3)) && ((param_1[4] & 0x10U) == 0)) {
                bVar7 = true;
              }
            }
          }
          sVar14 = (short)lVar22 + 1;
          lVar22 = (long long)sVar14;
        } while (sVar14 <= local_62);
      }
      if ((bVar7) && (local_84 != 0)) {
        local_84 = local_84 + -1;
      }
      *gSearchRadius = *gSearchRadius + 1;
      if ((!bVar8) && (local_84 = 0, !bVar7)) {
        local_86 = 0;
      }
    } while (local_84 != 0);
  }
  else {
    local_86 = 0;
  }
  return local_86;
}

/* Address: 0x100439a4 Size: 736 bytes */
void FUN_100439a4(short *param_1)

{
  unsigned char bVar1;
  short sVar2;
  int iVar3;
  short *psVar4;
  int *puVar5;
  int *piVar6;
  unsigned long long uVar7;
  short sVar9;
  int iVar8;
  short sVar10;
  short sVar11;
  unsigned int unaff_r15;
  unsigned short unaff_r16;
  unsigned long long uVar12;
  unsigned char bVar13;
  unsigned long long uVar14;
  int iVar15;
  int iVar16;
  unsigned int uVar17;
  int iVar18;
  unsigned long long uVar19;
  int iVar20;
  unsigned int local_6c;
  short local_48;
  
  piVar6 = gCostMapPtr;
  puVar5 = gDistGridHandlePtr;
  psVar4 = gAIModeFlag;
  sVar9 = AbsShort(*(short *)(*(int *)*gDistGridHandlePtr + *param_1 * 0x138 + param_1[1] * 2));
  uVar7 = (unsigned long long)local_6c;
  local_48 = 0;
  uVar14 = (long long)param_1[1];
  iVar15 = (int)*param_1;
  do {
    bVar13 = 0;
    if ((iVar15 == param_1[2]) && ((int)uVar14 == (int)param_1[3])) {
      return;
    }
    if (param_1[5] == 1) {
      bVar13 = *(unsigned char *)(*piVar6 + iVar15 * 0x9c + (int)uVar14);
      unaff_r16 = bVar13 & 8;
      bVar13 = bVar13 & 0x10;
    }
    iVar8 = GetDirection(iVar15,uVar14,param_1[2],param_1[3]);
    uVar12 = 0xffffffffffffffff;
    iVar16 = -1;
    iVar18 = 0;
    do {
      iVar3 = (int)uVar7;
      uVar17 = iVar8 + *(short *)(iVar18 * 2 + *(int *)(iVar3 + -0x19d8));
      uVar17 = (unsigned int)(short)((short)uVar17 -
                            (short)(((long long)((int)uVar17 >> 3) +
                                     (unsigned long long)((int)uVar17 < 0 && (uVar17 & 7) != 0) & 0xffffffff)
                                   << 3));
      sVar10 = (short)iVar15 + *(short *)(uVar17 * 2 + *(int *)(iVar3 + -0x19d4));
      iVar20 = (int)sVar10;
      sVar11 = (short)uVar14 + *(short *)(uVar17 * 2 + *(int *)(iVar3 + -0x19d0));
      uVar19 = (unsigned long long)sVar11;
      if (((((*psVar4 != 0) && ((uVar17 & 1) != 0)) &&
           (((*(unsigned char *)(*piVar6 + sVar10 * 0x9c + (int)sVar11) & 8) == 0 ||
            (*(char *)(**(int **)(iVar3 + -0x544) +
                       (*(unsigned int *)(**(int **)(iVar3 + -0x548) +
                                  (int)(((uVar19 & 0x1fffffff) * 8 - uVar19 & 0xffffffff) << 5) +
                                 iVar20 * 2) >> 0x18) + 0x711) == '\x01')))) ||
          (((sVar10 < 0 || (0x6f < sVar10)) || (sVar11 < 0)))) ||
         (((0x9b < sVar11 ||
           (sVar2 = *(short *)(*(int *)*puVar5 + iVar20 * 0x138 + (int)((uVar19 & 0xffffffff) << 1))
           , sVar2 == 30000)) || (sVar2 == 0x7531)))) goto LAB_10043c20;
      if (((bVar13 == 0) &&
          (bVar1 = *(unsigned char *)(*piVar6 + sVar10 * 0x9c + (int)sVar11), (bVar1 & 0x10) == 0)) &&
         (param_1[5] == 1)) {
        if (unaff_r16 == 0) {
          if ((*(unsigned char *)(*piVar6 + iVar20 * 0x9c + (int)sVar11) & 8) == 0) goto LAB_10043bc8;
        }
        else if ((bVar1 & 8) != 0) goto LAB_10043bc8;
      }
      else {
LAB_10043bc8:
        sVar10 = AbsShort(*(short *)
                               (*(int *)*puVar5 + iVar20 * 0x138 + (int)((uVar19 & 0xffffffff) << 1)
                               ));
        uVar7 = (unsigned long long)local_6c;
        if ((sVar10 < sVar9) || (((*psVar4 != 0 && (iVar16 == -1)) && (sVar10 == sVar9)))) {
          uVar12 = uVar19;
          unaff_r15 = uVar17;
          sVar9 = sVar10;
          iVar16 = iVar20;
        }
      }
LAB_10043c20:
      sVar10 = (short)iVar18 + 1;
      iVar18 = (int)sVar10;
    } while (sVar10 < 8);
    if ((local_48 < 0xc6) && (iVar16 != -1)) {
      *(char *)(*(int *)(param_1 + 6) + (int)local_48) = (char)unaff_r15;
      local_48 = local_48 + 1;
      uVar14 = uVar12;
      iVar15 = iVar16;
    }
    else {
      param_1[2] = (short)iVar15;
      param_1[3] = (short)uVar14;
    }
  } while( true );
}

/* Address: 0x10043c84 Size: 476 bytes */
void FUN_10043c84(short param_1,short *param_2,short param_3)

{
  int bVar1;
  int *piVar2;
  int *piVar3;
  int *piVar4;
  short sVar5;
  int iVar6;
  short sVar7;
  int iVar8;
  int iVar9;
  
  piVar4 = gPathCachePtr;
  piVar3 = gPathCacheHandle;
  piVar2 = (int *)*gPathCacheHandle;
  iVar9 = -1;
  sVar7 = 10000;
  bVar1 = true;
  if (piVar2 != (int *)0x0) {
    DetachResource(piVar2);
  }
  *piVar4 = *piVar2;
  if (param_3 == 0) {
    iVar6 = 0;
  }
  else {
    iVar8 = 1;
    do {
      iVar6 = iVar8;
      if (*(short *)(*piVar4 + iVar8 * 0xd6) == 0) break;
      iVar6 = *piVar4 + iVar8 * 0xd6;
      sVar5 = CalcDistance(*(short *)(iVar6 + 10),*(short *)(iVar6 + 0xc),
                           *(short *)(iVar6 + 6),*(short *)(iVar6 + 8));
      if (sVar5 < sVar7) {
        iVar9 = iVar8;
        sVar7 = sVar5;
      }
      sVar5 = (short)iVar8 + 1;
      iVar8 = (int)sVar5;
      iVar6 = -1;
    } while (sVar5 < 0x14);
    if (iVar6 == -1) {
      bVar1 = iVar9 != -1;
      if (sVar7 < param_1) {
        iVar6 = iVar9;
      }
      if (iVar6 == -1) {
        bVar1 = false;
      }
    }
  }
  if (bVar1) {
    iVar9 = 199;
    do {
      *(char *)(*piVar4 + iVar6 * 0xd6 + iVar9 + 0xe) =
           *(char *)(*(int *)(param_2 + 6) + iVar9);
      bVar1 = iVar9 != 0;
      iVar9 = (int)(short)((short)iVar9 + -1);
    } while (bVar1);
    iVar6 = iVar6 * 0xd6;
    *(short *)(*piVar4 + iVar6) = 2;
    *(short *)(iVar6 + *piVar4 + 2) = param_2[5];
    *(short *)(iVar6 + *piVar4 + 4) = param_2[4];
    *(short *)(iVar6 + *piVar4 + 6) = *param_2;
    *(short *)(iVar6 + *piVar4 + 8) = param_2[1];
    *(short *)(iVar6 + *piVar4 + 10) = param_2[2];
    *(short *)(iVar6 + *piVar4 + 0xc) = param_2[3];
  }
  if (*piVar3 != 0) {
    DisposeHandle_Thunk(*piVar3);
  }
  return;
}





































































































































































































































































































/* Address: 0x10043e60 Size: 688 bytes */
long long FUN_10043e60(short *param_1)

{
  int bVar1;
  int *puVar2;
  int *piVar3;
  int *piVar4;
  int *piVar5;
  int *puVar6;
  int *puVar7;
  int *piVar8;
  int *ppuVar9;
  long long uVar10;
  int iVar11;
  short sVar12;
  int iVar13;
  long long uVar14;
  
  piVar8 = gCostMapPtr;
  puVar7 = gDistGridHandlePtr;
  puVar6 = gDistGridDeref;
  piVar5 = gDistGridHandle;
  piVar4 = gCostMapHandle;
  ppuVar9 = 0 /* TVect base */;
  uVar10 = CalcDistance(param_1[2],param_1[3],*param_1,param_1[1]);
  uVar14 = 0;
  if (((((short)param_1[2] < 0) || ((short)param_1[3] < 0)) || (0x6f < (short)param_1[2])) ||
     (0x9b < (short)param_1[3])) {
    return 0;
  }
  if (*(short *)(*(int*)((char*)ppuVar9 - 0x67f)) == 0) {
    FUN_100424e4();
  }
  iVar13 = 199;
  do {
    *(char *)(*(int *)(param_1 + 6) + iVar13) = 0xff;
    bVar1 = iVar13 != 0;
    iVar13 = (int)(short)((short)iVar13 + -1);
  } while (bVar1);
  iVar13 = (int)uVar10;
  if ((param_1[4] & 0x10) == 0) {
    iVar11 = FUN_100427cc(param_1,0);
    if (iVar11 != 0) {
      return 1;
    }
    if ((0xe < iVar13) && (iVar11 = FUN_100427cc(param_1,1), iVar11 != 0)) {
      return 1;
    }
    if ((iVar13 == 1) && (iVar11 = FUN_100428dc(param_1), iVar11 != 0)) {
      return 1;
    }
  }
  puVar2 = (int *)*piVar5;
  if (puVar2 != (int *)0x0) {
    DetachResource(puVar2);
  }
  *puVar6 = *puVar2;
  *puVar7 = puVar6;
  piVar3 = (int *)*piVar4;
  if (piVar3 != (int *)0x0) {
    DetachResource(piVar3);
  }
  *piVar8 = *piVar3;
  FUN_10042ee4(param_1,uVar10);
  if ((*(short *)(*(int *)*puVar7 + (short)param_1[2] * 0x138 + (short)param_1[3] * 2) != 0x7531) ||
     ((*(unsigned char *)(*piVar8 + (short)param_1[2] * 0x9c + (int)(short)param_1[3]) & 0x80) != 0)) {
    *(short *)(*(int *)*puVar7 + (short)param_1[2] * 0x138 + (short)param_1[3] * 2) = 0xffff;
    sVar12 = 0;
    do {
      uVar14 = FUN_10043248(param_1,sVar12);
      sVar12 = sVar12 + 1;
      if ((int)uVar14 != 0) break;
    } while (sVar12 < 2);
    if (((int)uVar14 == 1) && ((param_1[4] & 0x10) == 0)) {
      FUN_100439a4(param_1);
      if (iVar13 < 0xf) {
        FUN_10043c84(uVar10,param_1,0);
      }
      else {
        FUN_10043c84(uVar10,param_1,1);
      }
    }
  }
  FUN_10042d2c(param_1,0);
  iVar13 = *piVar5;
  if (iVar13 != 0) {
    DisposeHandle_Thunk(iVar13);
  }
  iVar13 = *piVar4;
  if (iVar13 != 0) {
    DisposeHandle_Thunk(iVar13);
  }
  return uVar14;
}





















































































































































































































































































































































































/* Address: 0x100445fc Size: 300 bytes */
unsigned char FUN_100445fc(int param_1,short param_2,short param_3,short param_4,short *param_5)

{
  unsigned char bVar1;
  unsigned char bVar2;
  
  bVar1 = *(unsigned char *)(*gCostMapPtr + param_2 * 0x9c + (int)param_3);
  bVar2 = bVar1 & 7;
  if (*param_5 != 0) {
    return 0x80;
  }
  if (*(short *)(param_1 + 10) == 2) {
    if (((bVar1 & 8) != 0) && ((bVar1 & 0x10) == 0)) {
      return 2;
    }
    if ((bVar1 & 7) == 0) {
      bVar2 = 2;
    }
    if (bVar2 < 3) {
      return bVar2;
    }
    return 2;
  }
  if (*(short *)(param_1 + 10) != 1) {
    return bVar2;
  }
  if ((bVar1 & 0x10) == 0) {
    if (param_4 == 0) {
      if ((bVar1 & 8) == 0) goto LAB_100446d8;
    }
    else if ((bVar1 & 8) != 0) goto LAB_100446d8;
    *param_5 = 1;
  }
LAB_100446d8:
  if (((*(unsigned short *)(param_1 + 8) != 0) && (2 < bVar2)) &&
     ((((*(unsigned short *)(param_1 + 8) & 1) != 0 && ((bVar1 & 0x20) != 0)) ||
      (((*(unsigned short *)(param_1 + 8) & 2) != 0 && ((bVar1 & 0x40) != 0)))))) {
    bVar2 = 2;
  }
  return bVar2;
}

/* Address: 0x10044728 Size: 444 bytes */
short FUN_10044728(short *param_1,short param_2,int param_3)

{
  int bVar1;
  unsigned short uVar2;
  int *puVar3;
  int *puVar4;
  int *piVar5;
  int *piVar6;
  int *ppuVar7;
  int iVar8;
  short sVar9;
  short sVar10;
  short sVar11;
  char uVar12;
  int iVar13;
  int **local_64;
  short local_3e;
  short local_3c;
  short local_3a [29];
  
  puVar4 = gDistGridDeref;
  piVar6 = gDistGridHandle;
  piVar5 = gCostMapHandle;
  local_3a[0] = *param_1;
  local_3c = param_1[1];
  uVar2 = param_1[4];
  local_3e = 0;
  sVar11 = 0;
  sVar10 = 0;
  if (param_3 != 0) {
    iVar13 = 99;
    do {
      *(char *)(param_3 + iVar13) = 0xff;
      bVar1 = iVar13 != 0;
      iVar13 = (int)(short)((short)iVar13 + -1);
    } while (bVar1);
  }
  puVar3 = (int *)*piVar6;
  ppuVar7 = 0 /* TVect base */;
  if (puVar3 != (int *)0x0) {
    DetachResource(puVar3);
    ppuVar7 = local_64;
  }
  *puVar4 = *puVar3;
  *(int **)(*(int*)((char*)ppuVar7 - 0xc1)) = puVar4;
  puVar4 = (int *)*piVar5;
  if (puVar4 != (int *)0x0) {
    DetachResource(puVar4);
    ppuVar7 = local_64;
  }
  *(int *)(*(int*)((char*)ppuVar7 - 0xc0)) = *puVar4;
  iVar13 = 0;
  do {
    if ((*(char *)(*(int *)(param_1 + 6) + iVar13) == -1) ||
       (iVar8 = ApplyDirectionOffset(local_3a,&local_3c,*(char *)(*(int *)(param_1 + 6) + iVar13)),
       iVar8 == 0)) break;
    sVar9 = FUN_100445fc(param_1,local_3a[0],local_3c,uVar2 & 8,&local_3e);
    sVar10 = sVar10 + sVar9;
    if ((param_3 != 0) && (iVar13 < 100)) {
      if (sVar10 < 0xff) {
        uVar12 = (char)sVar10;
      }
      else {
        uVar12 = 0xff;
      }
      *(char *)(param_3 + iVar13) = uVar12;
    }
    if (sVar10 <= param_2) {
      sVar11 = sVar11 + 1;
    }
    sVar9 = (short)iVar13 + 1;
    iVar13 = (int)sVar9;
  } while (sVar9 < 100);
  iVar13 = *piVar6;
  if (iVar13 != 0) {
    DisposeHandle_Thunk(iVar13);
  }
  iVar13 = *piVar5;
  if (iVar13 != 0) {
    DisposeHandle_Thunk(iVar13);
  }
  return sVar11;
}













































































































































































































































































































































/* SetupInfluenceMap defined elsewhere */


/* AllocInfluenceMap defined elsewhere */


/* FreeInfluenceMap defined elsewhere */






















































/* Address: 0x10045170 Size: 132 bytes */
void FUN_10045170()

{
  unsigned int uVar1;
  int iVar2;
  short *psVar3;
  short sVar4;
  int iVar5;
  
  psVar3 = gSpriteCount;
  iVar2 = gSpriteTable;
  FUN_100099cc();
  iVar5 = 0;
  if (0 < *psVar3) {
    do {
      uVar1 = *(unsigned int *)(iVar5 * 0x14 + iVar2);
      if (((uVar1 >> 0x1d & 1) != 0) && (uVar1 >> 0x1e != 0)) {
        Render_UnloadSprite(iVar5);
      }
      sVar4 = (short)iVar5 + 1;
      iVar5 = (int)sVar4;
    } while (sVar4 < *psVar3);
  }
  return;
}

/* Address: 0x100451f4 Size: 184 bytes */
void FUN_100451f4(short param_1)

{
  int iVar1;
  long long uVar2;
  int iVar3;
  unsigned long long uVar4;
  
  iVar1 = gSpriteTable;
  uVar4 = (unsigned long long)param_1;
  uVar2 = HLock_Thunk(*(int *)
                        (*(int *)((int)(((uVar4 & 0x3fffffff) * 4 + uVar4 & 0xffffffff) << 2) +
                                  gSpriteTable + 4) + 0x10));
  iVar3 = Render_ValidatePixMap(uVar2);
  if (iVar3 == 0) {
    Render_LoadSprite(uVar4);
    Render_ValidatePixMap(uVar2);
  }
  iVar3 = (int)(((uVar4 & 0x3fffffff) * 4 + uVar4 & 0x3fffffff) << 2);
  if ((*(unsigned int *)(iVar3 + iVar1) >> 0x1a & 1) != 0) {
    HLock_Thunk(*(int *)(*(int *)(iVar3 + iVar1 + 0xc) + 0x10));
    Render_ValidatePixMap();
  }
  return;
}

































































































/* Address: 0x1004530c Size: 228 bytes */
void FUN_1004530c(short param_1)

{
  int iVar1;
  int iVar2;
  int iVar3;
  unsigned long long uVar4;
  char auStack_148 [256];
  char auStack_48 [72];
  
  iVar3 = gSpriteTable;
  uVar4 = (unsigned long long)param_1;
  if (*(int *)(*(int *)((int)(((uVar4 & 0x3fffffff) * 4 + uVar4 & 0xffffffff) << 2) + gSpriteTable +
                       4) + 0x10) == 0) {
    DrawNumber(auStack_48,(unsigned long long)uRam10115ed8 + 0xec,uVar4);
    BuildPascalString(auStack_148,auStack_48);
    ConvertToString(auStack_148);
    FUN_100023b8();
  }
  else {
    iVar2 = (int)(((uVar4 & 0x3fffffff) * 4 + uVar4 & 0x3fffffff) << 2);
    iVar1 = iVar2 + gSpriteTable;
    HLock_Thunk(*(int *)(*(int *)(iVar1 + 4) + 0x10));
    HUnlock_Thunk();
    if ((*(unsigned int *)(iVar2 + iVar3) >> 0x1a & 1) != 0) {
      HLock_Thunk(*(int *)(*(int *)(iVar1 + 0xc) + 0x10));
      HUnlock_Thunk();
    }
  }
  return;
}





















































































































































































/* Address: 0x100462c8 Size: 212 bytes */
void FUN_100462c8(short param_1)

{
  short sVar1;
  int iVar2;
  unsigned long long uVar3;
  
  uVar3 = (unsigned long long)param_1;
  if ((*(unsigned int *)((int)(((uVar3 & 0x3fffffff) * 4 + uVar3 & 0xffffffff) << 2) + gSpriteTable) >> 0x1d
      & 1) == 0) {
    iVar2 = 0;
    if (0 < *gSpriteCount) {
      do {
        if ((*(unsigned int *)(iVar2 * 0x14 + gSpriteTable) >> 0x1d & 1) != 0) {
          if (*(char *)(iVar2 * 0x14 + gSpriteTable + 8) ==
              *(char *)((int)(((uVar3 & 0x3fffffff) * 4 + uVar3 & 0xffffffff) << 2) + gSpriteTable +
                       8)) {
            Render_UnloadSprite(iVar2);
            break;
          }
        }
        sVar1 = (short)iVar2 + 1;
        iVar2 = (int)sVar1;
      } while (sVar1 < *gSpriteCount);
    }
    Render_LoadSprite(uVar3);
  }
  else {
    Render_ValidateSprite(uVar3);
  }
  return;
}

/* SetDrawingColor defined elsewhere */


/* UnlockUnitData defined elsewhere */


/* Address: 0x10049e68 Size: 84 bytes */
void FUN_10049e68()

{
  int *piVar1;
  
  piVar1 = gUnitDataHandle;
  if (*gUnitDataHandle != 0) {
    ReleaseHandle_Mapgen(*gUnitDataHandle);
    *piVar1 = 0;
  }
  return;
}

/* Address: 0x1004a0c4 Size: 180 bytes */
int FUN_1004a0c4(short param_1,short *param_2,short *param_3,short *param_4,
                short *param_5,short *param_6)

{
  int *piVar1;
  short sVar2;
  int iVar3;
  int iVar4;
  
  piVar1 = piRam101175d0;
  LockUnitData();
  iVar4 = 0;
  do {
    iVar3 = *piVar1 + iVar4 * 0x3e;
    if (*(short *)(*piVar1 + iVar4 * 0x3e) == param_1) {
      *param_5 = *(short *)(iVar3 + 0x1e);
      *param_2 = *(short *)(iVar3 + 0x18);
      *param_3 = *(short *)(iVar3 + 0x16);
      *param_4 = *(short *)(iVar3 + 0x1a);
      *param_6 = *(short *)(iVar3 + 0x1c);
      break;
    }
    sVar2 = (short)iVar4 + 1;
    iVar4 = (int)sVar2;
  } while (sVar2 < 0x1d);
  UnlockUnitData();
  return iVar4;
}

/* Address: 0x1004a178 Size: 164 bytes */
void FUN_1004a178(short param_1,short *param_2,short *param_3,short *param_4,
                 short *param_5)

{
  int *piVar1;
  short sVar2;
  int iVar3;
  int iVar4;
  
  piVar1 = piRam101175d0;
  LockUnitData();
  iVar3 = 0;
  do {
    iVar4 = *piVar1 + iVar3 * 0x3e;
    if (*(short *)(*piVar1 + iVar3 * 0x3e) == param_1) {
      *param_2 = *(short *)(iVar4 + 0x18);
      *param_3 = *(short *)(iVar4 + 0x16);
      *param_4 = *(short *)(iVar4 + 0x1a);
      *param_5 = *(short *)(iVar4 + 0x1c);
      break;
    }
    sVar2 = (short)iVar3 + 1;
    iVar3 = (int)sVar2;
  } while (sVar2 < 0x1d);
  UnlockUnitData();
  return;
}

/* Address: 0x1004a21c Size: 140 bytes */
int FUN_1004a21c(short param_1)

{
  int *piVar1;
  int uVar2;
  short sVar3;
  int iVar4;
  
  uVar2 = gItemNameBuffer;
  piVar1 = piRam101175d0;
  LockUnitData();
  iVar4 = 0;
  do {
    if (*(short *)(*piVar1 + iVar4 * 0x3e) == param_1) {
      BlockMoveData_Thunk(uVar2,*piVar1 + iVar4 * 0x3e + 2);
      break;
    }
    sVar3 = (short)iVar4 + 1;
    iVar4 = (int)sVar3;
  } while (sVar3 < 0x1d);
  UnlockUnitData();
  return uVar2;
}





































































































































/* Address: 0x1004f620 Size: 68 bytes - stubbed: callers pass 3 args vs 2 params */
void FUN_1004f620()

{
  return;
}

/* Address: 0x10050bb0 Size: 92 bytes */
void FUN_10050bb0(int param_1)

{
  int iVar1;
  short sVar2;
  int iVar3;
  
  iVar1 = iRam101175ec;
  *puRam10115f40 = 4;
  FUN_1004f704();
  iVar3 = 0;
  do {
    *(short *)(iVar3 * 2 + param_1) = *(short *)(iVar3 * 2 + iVar1);
    sVar2 = (short)iVar3 + 1;
    iVar3 = (int)sVar2;
  } while (sVar2 < 8);
  return;
}

/* Address: 0x100513b4 Size: 292 bytes */
void FUN_100513b4(long long param_1,long long param_2,short param_3)

{
  int iVar1;
  int *piVar2;
  int *puVar3;
  int local_5c;
  short local_38;
  short local_36;
  int local_30;
  int local_2c;
  short local_28;
  short local_26;
  short local_24;
  short local_22;
  
  piVar2 = (int *)FUN_10000720(param_3);
  iVar1 = *piVar2;
  local_28 = *(short *)(iVar1 + 2);
  local_26 = *(short *)(iVar1 + 4);
  local_24 = *(short *)(iVar1 + 6);
  local_22 = *(short *)(iVar1 + 8);
  puVar3 = (int *)NewPtr_Thunk(0xdc);
  if (puVar3 != (int *)0x0) {
    FUN_100cdb5c(puVar3);
    *puVar3 = *(int *)(local_5c + -0x1948);
    *(short *)(puVar3 + 0x2c) = 0;
    *(short *)((int)puVar3 + 0xb2) = 0;
    *(short *)(puVar3 + 0x2d) = 0;
    puVar3[0x31] = 0;
    *(short *)(puVar3 + 0x32) = 0;
    puVar3[0x36] = 0;
  }
  GetScreenBounds(&local_38,&local_28);
  local_30 = (int)local_38;
  local_2c = (int)local_36;
  FUN_100d08c4(puVar3,param_1,param_2,&local_30,5,5,param_3);
  puVar3[6] = 0x70726f67;
  *(short *)((int)puVar3 + 0xb2) = 100;
  FUN_100861b8(puVar3,0,1);
  return;
}

/* Address: 0x100514d8 Size: 284 bytes */
void FUN_100514d8(int *param_1,short param_2)

{
  long long uVar1;
  int *piVar2;
  int local_14c;
  char auStack_128 [256];
  char auStack_28 [40];
  
  uVar1 = ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308),0x70726f67);
  if ((int)uVar1 != 0) {
    FUN_100861b8(uVar1,param_2,1);
  }
  piVar2 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308),0x73747235);
  if (piVar2 != (int *)0x0) {
    DrawNumber(auStack_28,*(int *)(local_14c + -0x194c),param_2);
    BuildPascalString(auStack_128,auStack_28);
    ResourceRead_Dispatch((int)*(short *)(*piVar2 + 0x7c8) + (int)piVar2,auStack_128,0);
    ResourceRead_Dispatch((int)*(short *)(*piVar2 + 0x4d0) + (int)piVar2);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x4c8));
  }
  return;
}





































































































































/* Address: 0x100516c0 Size: 312 bytes */
void FUN_100516c0()

{
  int *puVar1;
  
  puVar1 = gProgressBar;
  ProgressUpdate(*gProgressBar,0);
  LoadScenarioTemplate();
  InitializeMapToOcean();
  PlacePlayerStartZones();
  ProgressUpdate(*puVar1,10);
  PlaceCitiesAndRoads();
  ProgressUpdate(*puVar1,0x14);
  PlaceMountainForestChains();
  ProgressUpdate(*puVar1,0x1e);
  PlaceRiverWaterChannels();
  ProgressUpdate(*puVar1,0x28);
  GenerateLandMasses();
  ProgressUpdate(*puVar1,0x32);
  PlaceSwampClusters();
  ProgressUpdate(*puVar1,0x3c);
  RegisterCitiesAndAddNeutral();
  ProgressUpdate(*puVar1,0x46);
  PlaceRuins();
  ProgressUpdate(*puVar1,0x50);
  GenerateRoadNetwork();
  ProgressUpdate(*puVar1,0x5a);
  FinalizeAndSave();
  ProgressUpdate(*puVar1,100);
  return;
}

/* Address: 0x10051d60 Size: 4 bytes */
void FUN_10051d60()

{
  return;
}

/* Address: 0x10051d64 Size: 96 bytes */
void FUN_10051d64()

{
  short sVar1;
  short sVar2;
  
  sVar2 = 0;
  do {
    sVar1 = 0;
    do {
      NotifyTileChanged(sVar2,sVar1);
      sVar1 = sVar1 + 1;
    } while (sVar1 < 0x9c);
    sVar2 = sVar2 + 1;
  } while (sVar2 < 0x70);
  return;
}

































































































/* Address: 0x10051dc8 Size: 84 bytes */
void FUN_10051dc8(short *param_1,short *param_2)

{
  if (*param_1 < 1) {
    *param_1 = 0;
  }
  if (*param_2 < 1) {
    *param_2 = 0;
  }
  if (0x6f < *param_1) {
    *param_1 = 0x6f;
  }
  if (0x9b < *param_2) {
    *param_2 = 0x9b;
  }
  return;
}

/* Address: 0x10051e1c Size: 380 bytes */
void FUN_10051e1c(short *param_1,short *param_2,short param_3,short param_4)

{
  short sVar2;
  short sVar3;
  int iVar1;
  short sVar4;
  short sVar5;
  
  sVar2 = RandomRange(1,param_3,0);
  sVar3 = RandomRange(1,3,0xfffffffffffffffe);
  *param_1 = *param_1 + sVar3 * sVar2;
  sVar2 = RandomRange(1,param_3,0);
  sVar3 = RandomRange(1,3,0xfffffffffffffffe);
  *param_2 = *param_2 + sVar3 * sVar2;
  iVar1 = RandomRange(1,100,0);
  if ((iVar1 < 0x1e) && (param_4 != 0)) {
    sVar2 = *param_1;
    sVar3 = *param_2;
    sVar5 = 0x6f - sVar2;
    sVar4 = 0x9b - sVar3;
    if ((sVar2 <= sVar3) && ((sVar2 <= sVar5 && (sVar2 <= sVar4)))) {
      *param_1 = 0;
    }
    if (((sVar3 <= sVar2) && (sVar3 <= sVar5)) && (sVar3 <= sVar4)) {
      *param_2 = 0;
    }
    if (((sVar5 <= sVar3) && (sVar5 <= sVar2)) && (sVar5 <= sVar4)) {
      *param_1 = 0x6f;
    }
    if (((sVar4 <= sVar3) && (sVar4 <= sVar2)) && (sVar4 <= sVar5)) {
      *param_2 = 0x9b;
    }
  }
  ClampToMapBounds(param_1,param_2);
  return;
}

/* FUN_10051f98 defined elsewhere */


































































































































































































/* Address: 0x10052184 Size: 344 bytes */
long long FUN_10052184(long long param_1,short param_2,unsigned short param_3)

{
  int uVar1;
  long long uVar2;
  long long uVar3;
  char uVar5;
  long long uVar4;
  int local_44;
  short local_20 [2];
  char auStack_1c [28];
  
  uVar1 = uRam10116a54;
  uVar2 = FUN_10000420();
  FUN_10003450(0);
  uVar3 = GetResource_Thunk(param_1,param_2);
  if ((int)uVar3 == 0) {
    if (((param_3 & 1) != 0) || ((param_3 & 4) == 0)) {
      MarkChanged();
    }
  }
  FUN_10003450(uVar2);
  if (((int)uVar3 == 0) && ((param_3 & 4) != 0)) {
    uVar5 = FUN_100f1574(1);
    uVar2 = FUN_10000948(0);
    **(char **)(local_44 + -0x3c) = uVar5;
    FocusObject();
    uVar3 = FUN_100030a8(param_1);
    local_20[0] = (short)uVar3;
    uVar4 = ConvertToString(uVar1);
    AddResource(uVar2,param_1,uVar3,uVar4);
    MarkChanged();
  }
  else {
    ResolveAlias(uVar3,local_20,auStack_1c,uVar1);
    MarkChanged();
  }
  uVar2 = FUN_100fb260(0);
  SetResourceType(uVar2,param_1,local_20[0]);
  return uVar2;
}









































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100522dc Size: 204 bytes */
int * FUN_100522dc(int *param_1,unsigned short param_2,char param_3)

{
  int *piVar1;
  
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x118));
  EndFocus();
  if (param_3 == '\0') {
    piVar1 = (int *)FUN_100fa860(0);
    FUN_100fa8f8(piVar1,param_1);
  }
  else {
    piVar1 = (int *)FUN_1009cbcc(0,(param_2 & 2) != 0);
    FUN_100fa8f8(piVar1,param_1);
  }
  ResourceRead_Dispatch((int)piVar1 + (int)*(short *)(*piVar1 + 0xb0),0);
  return piVar1;
}

/* CloseStream defined elsewhere */


/* StreamRead defined elsewhere */


/* Address: 0x100524d4 Size: 68 bytes - stubbed: callers pass 3 args vs 1 param */
void FUN_100524d4()

{
  return;
}

/* Address: 0x10052518 Size: 96 bytes */
void FUN_10052518(int *param_1,long long param_2,short param_3)

{
  if (param_1 != (int *)0x0) {
    ResourceRead_Dispatch((int)*(short *)(*param_1 + 0xb0) + (int)param_1,param_2,param_3);
  }
  return;
}

/* Address: 0x100525a0 Size: 44 bytes */
short FUN_100525a0()

{
  return 0;
}

/* Address: 0x100525cc Size: 60 bytes */
long long FUN_100525cc(int param_1)

{
  char local_8;
  char uStack_7;
  char uStack_6;
  char uStack_5;
  
  uStack_5 = (unsigned char)param_1;
  uStack_6 = (unsigned char)((unsigned int)param_1 >> 8);
  uStack_7 = (unsigned char)((unsigned int)param_1 >> 0x10);
  local_8 = (unsigned char)((unsigned int)param_1 >> 0x18);
  return (unsigned long long)uStack_5 * 0x1000000 + (unsigned long long)uStack_6 * 0x10000 +
         (unsigned long long)uStack_7 * 0x100 + (unsigned long long)local_8;
}

/* Address: 0x10052f6c Size: 112 bytes - stubbed: callers disagree on arg count */
void FUN_10052f6c()

{
  return;
}

/* Address: 0x100534c8 Size: 880 bytes */
void FUN_100534c8(short param_1)

{
  int *piVar1;
  int *puVar2;
  int *piVar4;
  long long uVar3;
  int iVar5;
  int local_168;
  int local_164;
  char auStack_160 [16];
  int local_150;
  int local_14c;
  int local_148;
  int local_144;
  char auStack_140 [256];
  short local_40;
  int local_3c;
  int local_38;
  int local_34;
  int local_30;
  
  puVar2 = puRam10117370;
  piVar1 = piRam10115f64;
  piVar4 = (int *)ResourceRead_Dispatch(*piRam10115f64 + (int)*(short *)(*(int *)*piRam10115f64 + 0x308),
                               0x70696374);
  if (piVar4 != (int *)0x0) {
    ResourceRead_Dispatch((int)*(short *)(*piVar4 + 0xd8) + (int)piVar4,0);
  }
  piVar4 = (int *)ResourceRead_Dispatch(*piVar1 + (int)*(short *)(*(int *)*piVar1 + 0x308),0x646f6e65);
  if (piVar4 != (int *)0x0) {
    ResourceRead_Dispatch((int)*(short *)(*piVar4 + 0x660) + (int)piVar4,1,1);
    ResourceRead_Dispatch((int)*(short *)(*piVar4 + 0xd8) + (int)piVar4,1);
  }
  if (param_1 != 0) {
    piVar4 = (int *)ResourceRead_Dispatch(*piVar1 + (int)*(short *)(*(int *)*piVar1 + 0x308),0x74616b65);
    if (piVar4 == (int *)0x0) goto LAB_10053730;
    ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x660),1,1);
    ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0xd8),1);
  }
  if (piVar4 != (int *)0x0) {
    ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x618),auStack_160);
    local_168 = 0xfffffffc;
    local_164 = 0xfffffffc;
    FUN_100e16b8(auStack_160,&local_168);
    ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x350),auStack_160,0);
    local_150 = 4;
    local_14c = 4;
    local_148 = 4;
    local_144 = 4;
    ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x780),&local_150,0);
    uVar3 = FUN_100b2bb4(0);
    FUN_100b444c(uVar3,0x6f75746c,1);
    ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x3f0),uVar3,0x7f,1);
    iVar5 = ResourceRead_Dispatch(*piVar1 + (int)*(short *)(*(int *)*piVar1 + 0x7f8));
    if (iVar5 != 0) {
      *(int *)(iVar5 + 0x24) = piVar4[6];
    }
  }
LAB_10053730:
  if (*piVar1 != 0) {
    local_40 = 0;
    local_3c = 0;
    local_34 = 0;
    local_30 = 0;
    local_38 = *puVar2;
    *puVar2 = auStack_140;
    iVar5 = TrySetjmp(auStack_140);
    if (iVar5 == 0) {
      iVar5 = ResourceRead_Dispatch(*piVar1 + (int)*(short *)(*(int *)*piVar1 + 0x810));
      ResourceRead_Dispatch(*piVar1 + (int)*(short *)(*(int *)*piVar1 + 0x738));
      *piVar1 = 0;
      if (iVar5 == 0x646f6e65) {
        FUN_100532a4();
      }
      else {
        FUN_10053330();
      }
      *puVar2 = local_38;
    }
    else {
      piVar4 = (int *)*piVar1;
      if (piVar4 != (int *)0x0) {
        ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x738));
      }
      *piVar1 = 0;
      LockHandleRange(local_40,local_3c);
    }
  }
  return;
}

/* Address: 0x1005668c Size: 3240 bytes */
void FUN_1005668c(int *param_1,int param_2)

{
  unsigned int uVar1;
  int iVar2;
  short uVar4;
  char uVar5;
  unsigned int uVar3;
  short sVar6;
  unsigned long long uVar7;
  int iVar8;
  short *puVar9;
  int iVar10;
  
  FocusObject(param_1);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe0),param_2,0xa0);
  uVar7 = 0;
  do {
    iVar8 = param_2 + (int)((uVar7 & 0xffffffff) << 1);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    iVar2 = ByteSwap16();
    *(short *)(iVar8 + 0xa0) = (short)iVar2;
    if (iVar2 == 0) {
      *(short *)(iVar8 + 0xa0) = 0xff;
    }
    else {
      iVar8 = param_2 + (int)((uVar7 & 0xffffffff) << 1);
      if (*(short *)(iVar8 + 0xa0) == 0xf) {
        *(short *)(iVar8 + 0xa0) = 0;
      }
    }
    sVar6 = (short)uVar7 + 1;
    uVar7 = (unsigned long long)sVar6;
  } while (sVar6 < 8);
  uVar7 = 0;
  do {
    iVar8 = param_2 + (int)((uVar7 & 0xffffffff) << 1);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    iVar2 = ByteSwap16();
    *(short *)(iVar8 + 0xb0) = (short)iVar2;
    if (iVar2 == 0) {
      *(short *)(iVar8 + 0xb0) = 0xff;
    }
    else {
      iVar8 = param_2 + (int)((uVar7 & 0xffffffff) << 1);
      if (*(short *)(iVar8 + 0xb0) == 0xf) {
        *(short *)(iVar8 + 0xb0) = 0;
      }
    }
    sVar6 = (short)uVar7 + 1;
    uVar7 = (unsigned long long)sVar6;
  } while (sVar6 < 8);
  iVar8 = 0;
  do {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(param_2 + iVar8 * 2 + 0xc0) = uVar4;
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 8);
  iVar8 = 0;
  do {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(param_2 + iVar8 * 2 + 0xd0) = uVar4;
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 8);
  iVar8 = 0;
  do {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(param_2 + iVar8 * 2 + 0xe0) = uVar4;
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 8);
  iVar8 = 0;
  do {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(param_2 + iVar8 * 2 + 0xf0) = uVar4;
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 8);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe0),param_2 + 0x100,0x10);
  puVar9 = (short *)(param_2 + 0x110);
  sVar6 = 0;
  do {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *puVar9 = uVar4;
    sVar6 = sVar6 + 1;
    puVar9 = puVar9 + 1;
  } while (sVar6 < 0x12);
  uVar5 = ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe8));
  *(char *)(param_2 + 0x134) = uVar5;
  puVar9 = (short *)(param_2 + 0x136);
  sVar6 = 0;
  do {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *puVar9 = uVar4;
    sVar6 = sVar6 + 1;
    puVar9 = puVar9 + 1;
  } while (sVar6 < 0x27);
  iVar8 = 0;
  do {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    iVar2 = param_2 + iVar8 * 0x14;
    *(short *)(iVar2 + 0x184) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x186) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x188) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x18a) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x18c) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x18e) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 400) = uVar4;
    uVar5 = ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe8));
    *(char *)(iVar2 + 0x192) = uVar5;
    uVar5 = ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe8));
    *(char *)(iVar2 + 0x193) = uVar5;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x194) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x196) = uVar4;
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 8);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe0),param_2 + 0x224,800);
  puVar9 = (short *)(param_2 + 0x544);
  sVar6 = 0;
  do {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *puVar9 = uVar4;
    sVar6 = sVar6 + 1;
    puVar9 = puVar9 + 1;
  } while (sVar6 < 0x50);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe0),param_2 + 0x5e4,0x22c);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
  uVar4 = ByteSwap16();
  *(short *)(param_2 + 0x810) = uVar4;
  iVar8 = 0;
  do {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    iVar2 = param_2 + iVar8 * 0x20;
    *(short *)(iVar2 + 0x812) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x814) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe0),iVar2 + 0x816,0x17);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x82e) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x830) = uVar4;
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 0x28);
  iVar8 = 0;
  do {
    iVar2 = param_2 + iVar8 * 0x1e;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe0),iVar2 + 0xd12,0x17);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0xd2a) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0xd2c) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0xd2e) = uVar4;
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 0x16);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe0),param_2 + 0xfa6,0xa0);
  iVar8 = 0;
  do {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(param_2 + iVar8 * 2 + 0x1046) = uVar4;
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 10);
  iVar8 = 0;
  do {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(param_2 + iVar8 * 2 + 0x105a) = uVar4;
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 100);
  iVar8 = 0;
  do {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(param_2 + iVar8 * 2 + 0x1122) = uVar4;
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 8);
  iVar8 = 0;
  do {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(param_2 + iVar8 * 2 + 0x1132) = uVar4;
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 8);
  iVar8 = 0;
  do {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    iVar2 = param_2 + iVar8 * 0xc;
    *(short *)(iVar2 + 0x1142) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x1144) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x1146) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x1148) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x114a) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x114c) = uVar4;
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 8);
  iVar8 = 0;
  do {
    iVar2 = 0;
    do {
      iVar10 = 0;
      do {
        ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
        uVar4 = ByteSwap16();
        *(short *)(param_2 + iVar8 * 0x50 + iVar2 * 10 + iVar10 * 2 + 0x11a2) = uVar4;
        sVar6 = (short)iVar10 + 1;
        iVar10 = (int)sVar6;
      } while (sVar6 < 5);
      sVar6 = (short)iVar2 + 1;
      iVar2 = (int)sVar6;
    } while (sVar6 < 8);
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 8);
  iVar8 = 0;
  do {
    iVar2 = param_2 + iVar8 * 0x2c;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe0),iVar2 + 0x1422,3);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x1426) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x1428) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x142a) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar2 + 0x142c) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe0),iVar2 + 0x142e,0x20);
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 8);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe0),param_2 + 0x1582,0x40);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
  uVar4 = ByteSwap16();
  *(short *)(param_2 + 0x1602) = uVar4;
  iVar8 = 0;
  do {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    iVar10 = param_2 + iVar8 * 0x42;
    *(short *)(iVar10 + 0x1604) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar10 + 0x1606) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe0),iVar10 + 0x1608,0x26);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar10 + 0x162e) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe0),iVar10 + 0x1630,5);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar10 + 0x1636) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar10 + 0x1638) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar10 + 0x163a) = uVar4;
    iVar2 = ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe8));
    uVar1 = *(unsigned int *)(iVar10 + 0x163c);
    *(unsigned int *)(iVar10 + 0x163c) = iVar2 << 0x18 | uVar1 & 0xffffff;
    uVar3 = ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xe8));
    *(unsigned int *)(iVar10 + 0x163c) = (uVar3 & 0xff) << 0x10 | iVar2 << 0x18 | uVar1 & 0xffff;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar10 + 0x163e) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar10 + 0x1640) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar10 + 0x1642) = uVar4;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x100));
    uVar4 = ByteSwap16();
    *(short *)(iVar10 + 0x1644) = uVar4;
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 100);
  return;
}















































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10058c64 Size: 3864 bytes */
void FUN_10058c64(int *param_1,int param_2)

{
  int iVar1;
  long long uVar2;
  short sVar3;
  int iVar4;
  int iVar5;
  short *puVar6;
  int iVar7;
  
  FocusObject(param_1);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x170),param_2,0xa0);
  iVar5 = 0;
  do {
    sVar3 = *(short *)(param_2 + iVar5 * 2 + 0xa0);
    if (sVar3 == 0xff) {
      sVar3 = 0;
    }
    else if (sVar3 == 0) {
      sVar3 = 0xf;
    }
    iVar4 = *param_1;
    uVar2 = ByteSwap16(sVar3);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 8);
  iVar5 = 0;
  do {
    sVar3 = *(short *)(param_2 + iVar5 * 2 + 0xb0);
    if (sVar3 == 0xff) {
      sVar3 = 0;
    }
    else if (sVar3 == 0) {
      sVar3 = 0xf;
    }
    iVar4 = *param_1;
    uVar2 = ByteSwap16(sVar3);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 8);
  iVar5 = 0;
  do {
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(param_2 + iVar5 * 2 + 0xc0));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 8);
  iVar5 = 0;
  do {
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(param_2 + iVar5 * 2 + 0xd0));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 8);
  iVar5 = 0;
  do {
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(param_2 + iVar5 * 2 + 0xe0));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 8);
  iVar5 = 0;
  do {
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(param_2 + iVar5 * 2 + 0xf0));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 8);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x170),param_2 + 0x100,0x10);
  puVar6 = (short *)(param_2 + 0x110);
  sVar3 = 0;
  do {
    iVar5 = *param_1;
    uVar2 = ByteSwap16(*puVar6);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar5 + 400),uVar2);
    sVar3 = sVar3 + 1;
    puVar6 = puVar6 + 1;
  } while (sVar3 < 0x12);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x178),*(char *)(param_2 + 0x134));
  puVar6 = (short *)(param_2 + 0x136);
  sVar3 = 0;
  do {
    iVar5 = *param_1;
    uVar2 = ByteSwap16(*puVar6);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar5 + 400),uVar2);
    sVar3 = sVar3 + 1;
    puVar6 = puVar6 + 1;
  } while (sVar3 < 0x27);
  iVar5 = 0;
  do {
    iVar4 = *param_1;
    iVar7 = param_2 + iVar5 * 0x14;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x184));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x186));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x188));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x18a));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x18c));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x18e));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 400));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x178),*(char *)(iVar7 + 0x192));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x178),*(char *)(iVar7 + 0x193));
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x194));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x196));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 8);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x170),param_2 + 0x224,800);
  puVar6 = (short *)(param_2 + 0x544);
  sVar3 = 0;
  do {
    iVar5 = *param_1;
    uVar2 = ByteSwap16(*puVar6);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar5 + 400),uVar2);
    sVar3 = sVar3 + 1;
    puVar6 = puVar6 + 1;
  } while (sVar3 < 0x50);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x170),param_2 + 0x5e4,0x22c);
  iVar5 = *param_1;
  uVar2 = ByteSwap16(*(short *)(param_2 + 0x810));
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar5 + 400),uVar2);
  iVar5 = 0;
  do {
    iVar4 = *param_1;
    iVar7 = param_2 + iVar5 * 0x20;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x812));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x814));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x170),iVar7 + 0x816,0x17);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x82e));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x830));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 0x28);
  iVar5 = 0;
  do {
    iVar7 = param_2 + iVar5 * 0x1e;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x170),iVar7 + 0xd12,0x17);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0xd2a));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0xd2c));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0xd2e));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 0x16);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x170),param_2 + 0xfa6,0xa0);
  iVar5 = 0;
  do {
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(param_2 + iVar5 * 2 + 0x1046));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 10);
  iVar5 = 0;
  do {
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(param_2 + iVar5 * 2 + 0x105a));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 100);
  iVar5 = 0;
  do {
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(param_2 + iVar5 * 2 + 0x1122));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 8);
  iVar5 = 0;
  do {
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(param_2 + iVar5 * 2 + 0x1132));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 8);
  iVar5 = 0;
  do {
    iVar4 = *param_1;
    iVar7 = param_2 + iVar5 * 0xc;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x1142));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x1144));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x1146));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x1148));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x114a));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x114c));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 8);
  iVar5 = 0;
  do {
    iVar4 = 0;
    do {
      iVar7 = 0;
      do {
        iVar1 = *param_1;
        uVar2 = ByteSwap16(*(short *)
                              (param_2 + iVar5 * 0x50 + iVar4 * 10 + iVar7 * 2 + 0x11a2));
        ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar1 + 400),uVar2);
        sVar3 = (short)iVar7 + 1;
        iVar7 = (int)sVar3;
      } while (sVar3 < 5);
      sVar3 = (short)iVar4 + 1;
      iVar4 = (int)sVar3;
    } while (sVar3 < 8);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 8);
  iVar5 = 0;
  do {
    iVar7 = param_2 + iVar5 * 0x2c;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x170),iVar7 + 0x1422,3);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x1426));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x1428));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x142a));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x142c));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x170),iVar7 + 0x142e,0x20);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 8);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x170),param_2 + 0x1582,0x40);
  iVar5 = *param_1;
  uVar2 = ByteSwap16(*(short *)(param_2 + 0x1602));
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar5 + 400),uVar2);
  iVar5 = 0;
  do {
    iVar4 = *param_1;
    iVar7 = param_2 + iVar5 * 0x42;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x1604));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x1606));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x170),iVar7 + 0x1608,0x26);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x162e));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x170),iVar7 + 0x1630,5);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x1636));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x1638));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x163a));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x178),*(unsigned int *)(iVar7 + 0x163c) >> 0x18)
    ;
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x178),
                 *(unsigned int *)(iVar7 + 0x163c) >> 0x10 & 0xff);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x163e));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x1640));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x1642));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    iVar4 = *param_1;
    uVar2 = ByteSwap16(*(short *)(iVar7 + 0x1644));
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(iVar4 + 400),uVar2);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 100);
  return;
}

/* Address: 0x1005cc8c Size: 140 bytes */
void FUN_1005cc8c()

{
  int *piVar1;
  int *piVar2;
  long long uVar3;
  
  piVar2 = gOverviewWindow;
  *puRam10115fa8 = 0;
  piVar1 = (int *)*piVar2;
  uVar3 = ResourceRead_Dispatch((int)piVar1 + (int)*(short *)(*piVar1 + 0x308),0x696e666f);
  if ((int)uVar3 != 0) {
    FUN_100810e8(uVar3);
  }
  piVar2 = (int *)*piVar2;
  ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x4c8));
  return;
}

/* Address: 0x1005f144 Size: 92 bytes */
void FUN_1005f144(short param_1)

{
  unsigned int uVar1;
  int iVar2;
  
  iVar2 = GetTickCount();
  uVar1 = GetTickCount();
  while (uVar1 < (unsigned int)(iVar2 + param_1 * 3)) {
    uVar1 = GetTickCount();
  }
  return;
}

/* Address: 0x1005f1a0 Size: 48 bytes */
void FUN_1005f1a0(short param_1,short param_2,long long param_3)

{
  FUN_10032860(param_1,param_2,param_3,0xfffffffffffffffe);
  return;
}

/* Address: 0x1005f1d0 Size: 48 bytes */
void FUN_1005f1d0(short param_1,short param_2,long long param_3)

{
  FUN_10032860(param_1,param_2,param_3,1);
  return;
}

/* Address: 0x1005f200 Size: 48 bytes */
void FUN_1005f200(short param_1,short param_2,long long param_3)

{
  FUN_10032860(param_1,param_2,param_3,0xffffffffffffffff);
  return;
}

/* Address: 0x1005f678 Size: 56 bytes */
int FUN_1005f678(short param_1,short param_2)

{
  FUN_1005f50c(0,param_1,param_2);
  return 0;
}

/* Address: 0x1005f6b0 Size: 56 bytes */
int FUN_1005f6b0(short param_1,short param_2)

{
  FUN_1005f50c(1,param_1,param_2);
  return 0;
}

/* Address: 0x10061980 Size: 280 bytes */
void FUN_10061980(short param_1,short param_2,short param_3)

{
  int *piVar1;
  int *piVar2;
  int iVar3;
  int *ppuVar4;
  char cVar5;
  short sVar7;
  int iVar6;
  short sVar8;
  int **local_44;
  
  piVar1 = gActiveView;
  iVar3 = gArrowDirTable;
  piVar2 = gScreenGWorld;
  ppuVar4 = 0 /* TVect base */;
  if (*gActiveView != 0) {
    *gScreenGWorld = *gActiveView + 0x80;
  }
  piVar1 = (int *)*piVar1;
  if (piVar1 == (int *)0x0) {
    cVar5 = '\0';
  }
  else {
    cVar5 = ResourceRead_Dispatch((int)piVar1 + (int)*(short *)(*piVar1 + 0x3a8));
    ppuVar4 = local_44;
  }
  if (cVar5 != '\0') {
    sVar7 = param_2 * 2 + -6;
    sVar8 = (short)(((short)(param_1 * 2 + -6) + 4 >> 3) << 3);
    if (sVar8 < 0) {
      sVar8 = 0;
    }
    if (sVar7 < 0) {
      sVar7 = 0;
    }
    EnsureSpriteLoaded(9);
    iVar6 = param_3 * 8 + iVar3;
    BlitSpriteWithMask((*(int*)((char*)ppuVar4 - 0x8e)),*(short *)(param_3 * 8 + iVar3),*(short *)(iVar6 + 2),
                 *(short *)(iVar6 + 4),*(short *)(iVar6 + 6),*piVar2,sVar8,sVar7);
  }
  return;
}

/* Address: 0x10064498 Size: 952 bytes */
void FUN_10064498(short param_1,short param_2,short param_3)

{
  int bVar1;
  unsigned char bVar2;
  char cVar3;
  short sVar4;
  short sVar5;
  int iVar6;
  int *piVar7;
  int *ppuVar8;
  long long uVar9;
  unsigned short uVar10;
  unsigned int uVar11;
  long long lVar12;
  unsigned long long uVar13;
  unsigned long long uVar14;
  unsigned long long uVar15;
  int local_54;
  int local_30;
  int local_2c [11];
  
  piVar7 = piRam101176b8;
  iVar6 = iRam10117684;
  ppuVar8 = 0 /* TVect base */;
  uVar13 = (unsigned long long)param_3;
  uVar14 = (unsigned long long)param_2;
  uVar15 = (unsigned long long)gColorPalette;
  if (param_1 != 0) {
    ppuVar8 = 0 /* TVect base */;
    Render_RefreshFeatureOverlays();
  }
  *piVar7 = *(int *)(*(int*)((char*)ppuVar8 - 0x7b));
  EnsureSpriteLoaded(7);
  EnsureSpriteAndLock(7);
  GetGWorld_Wrapper(local_2c,&local_30);
  SetGWorld_Wrapper(*(int *)(iVar6 + 0x10),0);
  RGBForeColor_Thunk(uVar15);
  SetForeColor_Thunk();
  RGBForeColor_Thunk(uVar15 + 0x5fa);
  SetForeColor_Thunk();
  uVar10 = (unsigned short)(unsigned char)((unsigned int)*(int *)
                                 (**(int **)(local_54 + -0x548) +
                                  (int)(((uVar13 & 0x1fffffff) * 8 - uVar13 & 0xffffffff) << 5) +
                                 (int)((uVar14 & 0xffffffff) << 1)) >> 0x18);
  lVar12 = (unsigned long long)**(unsigned int **)(local_54 + -0x54c) +
           ((uVar13 & 0x1fffffff) * 8 - uVar13 & 0xfffffff) * 0x10;
  uVar15 = lVar12 + uVar14;
  uVar11 = *(unsigned int *)((int)lVar12 + (int)param_2) >> 0x18 & 0x1f;
  bVar1 = uVar11 == 0;
  if (!bVar1) {
    uVar10 = (short)uVar11 - 1;
  }
  if (((short)uVar10 < 0x50) || (0x5f < (short)uVar10)) {
    if (bVar1) {
      uVar11 = **(unsigned int **)(local_54 + -0x1f8);
      bVar2 = *(unsigned char *)(uVar11 + (int)(short)uVar10);
      uVar15 = (unsigned long long)bVar2;
    }
    else {
      bVar2 = *(unsigned char *)(**(int **)(local_54 + -0x208) + (int)(short)uVar10);
    }
    BlitTerrainPixel(iVar6,(short)((uVar14 & 0xffffffff) << 1),(short)((uVar13 & 0xffffffff) << 1),
                 *(short *)(*piVar7 + (char)bVar2 * 2),uVar11,uVar15);
    if (bVar1) {
      cVar3 = *(char *)(**(int **)(local_54 + -0x1fc) + (int)(short)uVar10);
    }
    else {
      cVar3 = *(char *)(**(int **)(local_54 + -0x20c) + (int)(short)uVar10);
    }
    BlitTerrainPixel(iVar6,(short)((uVar14 & 0xffffffff) << 1) + 1,(short)((uVar13 & 0xffffffff) << 1),
                 *(short *)(*piVar7 + cVar3 * 2));
    if (bVar1) {
      cVar3 = *(char *)(**(int **)(local_54 + -0x200) + (int)(short)uVar10);
    }
    else {
      cVar3 = *(char *)(**(int **)(local_54 + -0x210) + (int)(short)uVar10);
    }
    BlitTerrainPixel(iVar6,(short)((uVar14 & 0xffffffff) << 1),(short)((uVar13 & 0xffffffff) << 1) + 1,
                 *(short *)(*piVar7 + cVar3 * 2));
    if (bVar1) {
      cVar3 = *(char *)(**(int **)(local_54 + -0x204) + (int)(short)uVar10);
    }
    else {
      cVar3 = *(char *)(**(int **)(local_54 + -0x214) + (int)(short)uVar10);
    }
    BlitTerrainPixel(iVar6,(short)((uVar14 & 0xffffffff) << 1) + 1,
                 (short)((uVar13 & 0xffffffff) << 1) + 1,*(short *)(*piVar7 + cVar3 * 2));
    UnlockAndReleaseSprite(7);
    SetGWorld_Wrapper(local_2c[0],local_30);
    if (param_1 != 0) {
      FUN_10064104();
    }
  }
  else {
    uVar9 = RandomRange(1,3,1);
    sVar4 = (short)((uVar14 & 0xffffffff) << 1);
    sVar5 = (short)((uVar13 & 0xffffffff) << 1);
    BlitTerrainPixel(iVar6,sVar4,sVar5,uVar9);
    uVar9 = RandomRange(1,3,1);
    BlitTerrainPixel(iVar6,sVar4 + 1,sVar5,uVar9);
    uVar9 = RandomRange(1,3,1);
    BlitTerrainPixel(iVar6,sVar4,sVar5 + 1,uVar9);
    uVar9 = RandomRange(1,3,1);
    BlitTerrainPixel(iVar6,sVar4 + 1,sVar5 + 1,uVar9);
    if (*(int *)(iVar6 + 0x10) != 0) {
      UnlockPixels_Wrapper(*(int *)(iVar6 + 0x10));
    }
    SetGWorld_Wrapper(local_2c[0],local_30);
  }
  return;
}
















































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10066bf4 Size: 140 bytes */
unsigned long long FUN_10066bf4(unsigned long long param_1)

{
  if (((param_1 & 0xffffffff) != 0) || (param_1 = NewPtr_Thunk(0x2a), (param_1 & 0xffffffff) != 0))
  {
    Render_GetPenState(param_1);
    Render_GetFontState(param_1 + 6);
    Render_GetForeColor(param_1 + 0xc);
    Render_SaveClipRegion(param_1 + 0x1e);
  }
  return param_1;
}

/* Address: 0x10066c80 Size: 132 bytes */
void FUN_10066c80(unsigned long long param_1,unsigned long long param_2)

{
  if ((param_1 & 0xffffffff) != 0) {
    IterateListItems(param_1);
    Render_SetFontState(param_1 + 6);
    Render_SetForeColor(param_1 + 0xc);
    Render_RestoreClipRegion(param_1 + 0x1e);
    if (((param_1 & 0xffffffff) != 0) && ((param_2 & 1) != 0)) {
      DisposePtr_Thunk(param_1);
    }
  }
  return;
}

/* Address: 0x10067270 Size: 76 bytes */
void FUN_10067270()

{
  int local_24;
  
  RGBForeColor_Thunk(uRam10116070);
  SetForeColor_Thunk();
  RGBForeColor_Thunk(*(int *)(local_24 + -0xc38));
  Render_PenNormal();
  FUN_10000030();
  return;
}

/* Address: 0x10075ee0 Size: 160 bytes */
void FUN_10075ee0(int param_1,short param_2,short param_3)

{
  int *piVar1;
  
  piVar1 = gInfoWindow;
  *(short *)(param_1 + param_2 * 2 + 0x8c) = param_3;
  if ((*piVar1 != 0) &&
     (piVar1 = (int *)*piVar1,
     piVar1 = (int *)ResourceRead_Dispatch((int)piVar1 + (int)*(short *)(*piVar1 + 0x308),
                                  param_2 + 0x62757431), piVar1 != (int *)0x0)) {
    ResourceRead_Dispatch((int)*(short *)(*piVar1 + 0x7a8) + (int)piVar1,param_3,1);
  }
  return;
}

/* FUN_10079ee8 defined elsewhere */


/* FUN_1007a018 defined elsewhere */


/* Address: 0x1007ec7c Size: 152 bytes */
void FUN_1007ec7c(int param_1,long long param_2)

{
  int *ppuVar1;
  int uVar3;
  long long uVar2;
  
  ppuVar1 = 0 /* TVect base */;
  uVar3 = DisposeObject(*(int *)(param_1 + 0x148));
  *(int *)(param_1 + 0x148) = uVar3;
  uVar2 = NewResourceHandle(0);
  *(int *)(param_1 + 0x148) = (int)uVar2;
  WriteResourceData(uVar2,0x57325343,*(int *)(*(int *)(*(int*)((char*)ppuVar1 - 0x3b7)) + 0x20),0,0,0,1);
  ResourceRead_Dispatch((int)*(int **)(param_1 + 0x148) + (int)*(short *)(**(int **)(param_1 + 0x148) + 0xa8)
               ,param_2);
  return;
}

/* Address: 0x1007ed14 Size: 152 bytes */
void FUN_1007ed14(int param_1,long long param_2)

{
  int *ppuVar1;
  int uVar3;
  long long uVar2;
  
  ppuVar1 = 0 /* TVect base */;
  uVar3 = DisposeObject(*(int *)(param_1 + 0x14c));
  *(int *)(param_1 + 0x14c) = uVar3;
  uVar2 = NewResourceHandle(0);
  *(int *)(param_1 + 0x14c) = (int)uVar2;
  WriteResourceData(uVar2,0x57325445,*(int *)(*(int *)(*(int*)((char*)ppuVar1 - 0x3b7)) + 0x20),0,0,0,1);
  ResourceRead_Dispatch((int)*(int **)(param_1 + 0x14c) + (int)*(short *)(**(int **)(param_1 + 0x14c) + 0xa8)
               ,param_2);
  return;
}

/* Address: 0x1007edac Size: 152 bytes */
void FUN_1007edac(int param_1,long long param_2)

{
  int *ppuVar1;
  int uVar3;
  long long uVar2;
  
  ppuVar1 = 0 /* TVect base */;
  uVar3 = DisposeObject(*(int *)(param_1 + 0x150));
  *(int *)(param_1 + 0x150) = uVar3;
  uVar2 = NewResourceHandle(0);
  *(int *)(param_1 + 0x150) = (int)uVar2;
  WriteResourceData(uVar2,0x57324152,*(int *)(*(int *)(*(int*)((char*)ppuVar1 - 0x3b7)) + 0x20),0,0,0,1);
  ResourceRead_Dispatch((int)*(int **)(param_1 + 0x150) + (int)*(short *)(**(int **)(param_1 + 0x150) + 0xa8)
               ,param_2);
  return;
}

/* Address: 0x1007f0a4 Size: 16 bytes */
void FUN_1007f0a4(int param_1,short param_2)

{
  *(int *)(param_1 + 0x168) = param_2 + 0x709;
  return;
}

/* Address: 0x10082260 Size: 192 bytes */
void FUN_10082260(int *param_1)

{
  int iVar1;
  int local_34;
  
  if ((*(char *)(param_1 + 0x21) != '\0') &&
     (iVar1 = ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x3a8)), iVar1 != 0)) {
    BlitSpriteNormal(param_1 + 0x28,*(short *)((int)param_1 + 0x86) << 5,0,0x1f,0x1e,
                 *(int *)(local_34 + -0x1208),*(short *)((int)param_1 + 0x8a) * 0x28 + 8,
                 *(short *)(param_1 + 0x22) * 0x28 + 7);
  }
  return;
}

/* Address: 0x10082f24 Size: 408 bytes */
void FUN_10082f24(int *param_1,short param_2,short param_3)

{
  int *puVar1;
  int *piVar2;
  unsigned int uVar3;
  char auStack_68 [16];
  int local_58;
  int local_54;
  int local_50;
  int local_4c;
  char auStack_48 [16];
  int local_38;
  int local_34;
  int local_30;
  int local_2c;
  
  puVar1 = puRam10116bc0;
  *(char *)((int)param_1 + 0xbe) = 0;
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x3a8));
  piVar2 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x5f0),1);
  ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x600),auStack_48);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x2c0),auStack_48);
  FUN_100e1a64(&local_50,auStack_48);
  local_38 = *puVar1;
  local_34 = puVar1[1];
  local_30 = local_50;
  local_2c = local_4c;
  uVar3 = 0x28 - local_4c;
  local_54 = param_2 * 0x28 + ((int)uVar3 >> 1) + (unsigned int)((int)uVar3 < 0 && (uVar3 & 1) != 0);
  uVar3 = 0x28 - local_50;
  local_58 = param_3 * 0x28 + ((int)uVar3 >> 1) + (unsigned int)((int)uVar3 < 0 && (uVar3 & 1) != 0);
  FUN_100e15a8(&local_38,auStack_68,&local_58);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x518),auStack_68,&local_50,0);
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x4d0));
  ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x4c8));
  return;
}

/* FUN_100836dc defined elsewhere */


/* Address: 0x10084ef0 Size: 148 bytes */
int * ConstructDialogBehavior(int *param_1)

{
  int *ppuVar1;
  
  ppuVar1 = 0 /* TVect base */;
  if ((param_1 != (int *)0x0) ||
     (ppuVar1 = 0 /* TVect base */, param_1 = (int *)NewPtr_Thunk(0x90),
     param_1 != (int *)0x0)) {
    ConstructViewObject(param_1);
    *param_1 = (*(int*)((char*)ppuVar1 - 0x537));
    *(short *)(param_1 + 0x20) = 0xffff;
    *(short *)((int)param_1 + 0x82) = 0;
    *(short *)(param_1 + 0x21) = 0;
    *(short *)((int)param_1 + 0x86) = 0;
    *(short *)(param_1 + 0x22) = 0;
    *(short *)((int)param_1 + 0x8a) = 0x24;
    *(short *)(param_1 + 0x23) = 0xf;
  }
  return param_1;
}

/* Address: 0x10090e0c Size: 128 bytes */
void FUN_10090e0c(int *param_1,long long param_2,long long param_3,char param_4)

{
  int *piVar1;
  
  if ((param_1 != (int *)0x0) &&
     (piVar1 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308)),
     piVar1 != (int *)0x0)) {
    ResourceRead_Dispatch((int)*(short *)(*piVar1 + 0x7c8) + (int)piVar1,param_3,param_4);
  }
  return;
}

/* Address: 0x10091334 Size: 4 bytes */
void FUN_10091334()

{
  return;
}

/* Address: 0x10091338 Size: 148 bytes */
void FUN_10091338(short param_1,short param_2)

{
  int *piVar1;
  int *piVar2;
  short *psVar3;
  
  psVar3 = psRam101165d8;
  piVar2 = gSidePanelWindow;
  if ((*psRam101165dc != param_1) || (*psRam101165d8 != param_2)) {
    *psRam101165dc = param_1;
    *psVar3 = param_2;
    piVar1 = (int *)*piVar2;
    ResourceRead_Dispatch((int)piVar1 + (int)*(short *)(*piVar1 + 0x4d0));
    piVar2 = (int *)*piVar2;
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x4c8));
  }
  return;
}

/* Address: 0x100913cc Size: 96 bytes */
void FUN_100913cc()

{
  int *ppuVar1;
  int *piVar2;
  
  ppuVar1 = 0 /* TVect base */;
  piVar2 = (int *)Render_GetMinimapView();
  if ((*(short *)(*(int*)((char*)ppuVar1 - 0x4b0)) != 0) && (piVar2 != (int *)0x0)) {
    *(char *)((int)piVar2 + 0x95) = 0;
    ResourceRead_Dispatch((int)*(short *)(*piVar2 + 0x4d0) + (int)piVar2);
  }
  return;
}

/* Address: 0x1009142c Size: 92 bytes */
void FUN_1009142c()

{
  int *ppuVar1;
  int *piVar2;
  int **local_24;
  
  ppuVar1 = 0 /* TVect base */;
  piVar2 = (int *)Render_GetMinimapView();
  if (piVar2 != (int *)0x0) {
    *(char *)((int)piVar2 + 0x95) = 1;
    ResourceRead_Dispatch((int)*(short *)(*piVar2 + 0x4d0) + (int)piVar2);
    ppuVar1 = local_24;
  }
  *(short *)(*(int*)((char*)ppuVar1 - 0x4b0)) = 1;
  return;
}

/* Address: 0x10091488 Size: 132 bytes */
int * FUN_10091488(int *param_1)

{
  int iVar1;
  
  if ((param_1 != (int *)0x0) ||
     (param_1 = (int *)NewPtr_Thunk(8), param_1 != (int *)0x0)) {
    param_1[1] = 0;
    *param_1 = 0;
    iVar1 = SndNewChannel_Thunk(param_1,0,0,0);
    if (iVar1 != 0) {
      *param_1 = 0;
    }
  }
  return param_1;
}
















































































































































































































































































































































































































































































































/* Address: 0x1009150c Size: 380 bytes */
void FUN_1009150c(int *param_1,unsigned long long param_2)

{
  int iVar2;
  long long uVar1;
  char auStack_188 [12];
  char local_17c;
  char auStack_170 [368];
  
  if (param_1 != (int *)0x0) {
    if (*param_1 != 0) {
      do {
        iVar2 = SndChannelStatus_Thunk(*param_1,0x18,auStack_188);
        if (local_17c == '\0') break;
      } while (iVar2 == 0);
      if (*param_1 != 0) {
        SndDisposeChannel_Thunk(*param_1,1);
      }
    }
    *param_1 = 0;
    if (param_1[1] != 0) {
      Sound_InitCollectionIter(auStack_170,param_1[1],1);
      uVar1 = Sound_GetFirstItem(auStack_170);
      iVar2 = Sound_HasMoreItems(auStack_170);
      while (iVar2 != 0) {
        DisposeHandle_Thunk(uVar1);
        uVar1 = Sound_GetNextItem(auStack_170);
        iVar2 = Sound_HasMoreItems(auStack_170);
      }
      iVar2 = DisposeObject(param_1[1]);
      param_1[1] = iVar2;
      Sound_DisposeCollectionIter(auStack_170,2);
    }
    if ((param_1 != (int *)0x0) && ((param_2 & 1) != 0)) {
      DisposePtr_Thunk(param_1);
    }
  }
  return;
}







































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100917d8 Size: 256 bytes */
void FUN_100917d8(int *param_1,short param_2,char param_3)

{
  int iVar2;
  long long uVar1;
  short local_20;
  short local_1e;
  int local_1c;
  
  if ((((param_2 != 0) && (*param_1 != 0)) && (iVar2 = GetSoundVolume(), iVar2 != 0)) &&
     (uVar1 = GetResource_Thunk(0x736e6420,param_2), (int)uVar1 != 0)) {
    local_20 = 3;
    local_1e = 0;
    local_1c = 0;
    SndDoImmediate_Thunk(*param_1,&local_20);
    local_20 = 0x2b;
    local_1e = GetSoundVolume();
    local_1e = local_1e * 0x19;
    local_1c = 0;
    SndDoImmediate_Thunk(*param_1,&local_20);
    SndPlay_Thunk(*param_1,uVar1,param_3);
  }
  return;
}











































































































































/* Address: 0x1009191c Size: 164 bytes */
void FUN_1009191c(int param_1,short param_2)

{
  long long uVar1;
  int uVar2;
  
  uVar1 = GetResource_Thunk(0x736e6420,param_2);
  if ((int)uVar1 != 0) {
    HLock_Thunk_Sound(uVar1);
    DetachResource(uVar1);
    if (*(int *)(param_1 + 4) == 0) {
      uVar2 = Sound_NewCollection(0);
      *(int *)(param_1 + 4) = uVar2;
      Sound_InitCollection();
    }
    ResourceRead_Dispatch((int)*(int **)(param_1 + 4) + (int)*(short *)(**(int **)(param_1 + 4) + 0x158),
                 uVar1);
  }
  return;
}













































































/* Address: 0x100919c0 Size: 344 bytes */
short FUN_100919c0(int param_1)

{
  int *ppuVar1;
  long long uVar2;
  int uVar3;
  short local_218 [2];
  char local_214 [256];
  unsigned char local_114 [256];
  char auStack_14 [20];
  
  ppuVar1 = 0 /* TVect base */;
  BuildPascalString(local_114);
  if (local_114[(short)(local_114[0] - 3)] == 0x2e) {
    local_114[0] = local_114[0] - 4;
  }
  uVar2 = FUN_100b1b90(local_114,ZEXT48((*(int*)((char*)ppuVar1 - 0x4af))) + 0x244,1);
  ConcatPStrings(local_114,1,uVar2);
  uVar2 = FUN_100b1b90(local_114,ZEXT48((*(int*)((char*)ppuVar1 - 0x4af))) + 0x248,1);
  ConcatPStrings(local_114,1,uVar2);
  uVar2 = ConvertToString(local_114);
  uVar2 = Get1NamedResource_Thunk(0x736e6420,uVar2);
  if ((int)uVar2 != 0) {
    DetachResource(uVar2);
  }
  if (*(int *)(param_1 + 4) == 0) {
    uVar3 = Sound_NewCollection(0);
    *(int *)(param_1 + 4) = uVar3;
    Sound_InitCollection();
  }
  ResourceRead_Dispatch((int)*(int **)(param_1 + 4) + (int)*(short *)(**(int **)(param_1 + 4) + 0x158),uVar2)
  ;
  local_214[0] = 0;
  ResolveAlias(uVar2,local_218,auStack_14,local_214);
  return local_218[0];
}




































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10091b18 Size: 104 bytes */
void FUN_10091b18()

{
  int iVar1;
  int *piVar2;
  int *piVar3;
  
  piVar3 = gSndChannel;
  piVar2 = gSndHandle;
  if (*gSndChannel != 0) {
    SndDisposeChannel_Thunk(*gSndChannel,1);
  }
  iVar1 = *piVar2;
  if (iVar1 != 0) {
    DisposeHandle_Thunk(iVar1);
  }
  *piVar2 = 0;
  *piVar3 = 0;
  return;
}



















































































































/* Address: 0x10091b80 Size: 132 bytes */
char FUN_10091b80()

{
  int iVar1;
  char auStack_20 [12];
  char local_14;
  
  if (*gSndChannel == 0) {
    local_14 = '\0';
  }
  else {
    iVar1 = SndChannelStatus_Thunk(*gSndChannel,0x18,auStack_20);
    if (iVar1 == 0) {
      if (local_14 == '\0') {
        StopGlobalSound();
      }
    }
    else {
      StopGlobalSound();
      local_14 = '\0';
    }
  }
  return local_14;
}
























































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10091c04 Size: 160 bytes - stubbed: callers disagree on arg count */
void FUN_10091c04()

{
  return;
}




























































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10091ca4 Size: 288 bytes */
void FUN_10091ca4(short param_1,short param_2)

{
  int *piVar1;
  int *piVar2;
  int iVar3;
  short local_20;
  short local_1e;
  int local_1c;
  
  piVar2 = gSndChannel;
  piVar1 = gSndHandle;
  if (param_1 != 0) {
    iVar3 = IsGlobalSoundPlaying();
    while (iVar3 != 0) {
      iVar3 = IsGlobalSoundPlaying();
    }
    StopGlobalSound();
    iVar3 = GetResource_Thunk(0x736e6420,param_1);
    *piVar1 = iVar3;
    if (iVar3 != 0) {
      iVar3 = SndNewChannel_Thunk(piVar2,0,0,0);
      if ((iVar3 == 0) && (*piVar2 != 0)) {
        HLock_Thunk_Sound(*piVar1);
        if (*(int *)*piVar1 != 0) {
          DetachResource(*piVar1);
          local_20 = 0x2b;
          local_1e = param_2 * 0x19;
          local_1c = 0;
          SndDoImmediate_Thunk(*piVar2,&local_20);
        }
      }
      else {
        StopGlobalSound();
      }
    }
  }
  return;
}







































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10091de0 Size: 272 bytes - stubbed: callers pass 0 args */
short FUN_10091de0()

{
  return 0;
}






























































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10091ef0 Size: 80 bytes */
unsigned long long FUN_10091ef0()

{
  char auStack_28 [12];
  short local_1c;
  
  TuneGetStatus_Thunk(*puRam101165f0,auStack_28);
  return ((-1 - (long long)local_1c) + (unsigned long long)((long long)local_1c == 0) << 0x20) >> 0x3f;
}

















































/* Address: 0x10091f40 Size: 148 bytes */
void FUN_10091f40()

{
  int *piVar1;
  unsigned long long uVar2;
  short uVar3;
  long long lVar4;
  
  piVar1 = gTunePlayer;
  if (*gTunePlayer != 0) {
    uVar2 = GetMusicVolume();
    lVar4 = ((uVar2 & 0x3fffffff) * 4 - uVar2 & 0x1fffff) << 0xb;
    if (-1 < (int)lVar4) {
      do {
        TuneSetVolume_Thunk(*piVar1,lVar4);
        lVar4 = lVar4 + -0x50;
      } while (-1 < lVar4);
    }
    uVar3 = Sound_TuneStopAlt(*piVar1,0);
    EndFocus(uVar3);
  }
  return;
}





































































































































/* Address: 0x10091fd4 Size: 172 bytes */
void FUN_10091fd4()

{
  int *puVar1;
  int *puVar2;
  int iVar3;
  
  puVar2 = gTuneDataH;
  puVar1 = gTuneHeaderH;
  if (*gTunePlayer != 0) {
    TuneUnroll_Thunk();
  }
  iVar3 = IsHandleValid(*puVar2);
  if (iVar3 != 0) {
    DisposeHandle_Thunk(*puVar2);
    ReleaseHandle_Sound(*puVar2);
  }
  iVar3 = IsHandleValid(*puVar1);
  if (iVar3 != 0) {
    DisposeHandle_Thunk(*puVar1);
    ReleaseHandle_Sound(*puVar1);
  }
  *puVar1 = 0;
  *puVar2 = 0;
  return;
}























































































































































/* Address: 0x10092080 Size: 804 bytes */
void FUN_10092080(long long param_1,long long param_2)

{
  int *piVar1;
  int *piVar2;
  int *piVar3;
  int *piVar4;
  int *piVar5;
  int *puVar6;
  int *puVar7;
  int *puVar8;
  int *puVar10;
  int iVar11;
  long long uVar9;
  int uVar12;
  short uVar13;
  unsigned long long uVar14;
  int local_474;
  char auStack_450 [256];
  unsigned char local_350 [256];
  char auStack_250 [256];
  short local_150;
  int local_14c;
  int local_148;
  int local_144;
  int local_140;
  unsigned char local_138 [312];
  
  puVar8 = puRam10117370;
  piVar2 = gProgressView;
  puVar7 = gTuneDataH;
  puVar6 = gTuneHeaderH;
  piVar5 = gTunePlayer;
  piVar4 = gActiveDocView;
  piVar3 = gMainDocView;
  uVar14 = (unsigned long long)gProgressCounterBase;
  puVar10 = (int *)FUN_10001470(0x3f4);
  if (puVar10 != (int *)0x0) {
    piVar1 = (int *)*piVar2;
    if (piVar1 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar1 + (int)*(short *)(*piVar1 + 0xb0),0);
    }
    if (*piVar4 != 0) {
      *(short *)(*piVar4 + 0xbc) = 0xffff;
    }
    if (*piVar3 != 0) {
      *(short *)(*piVar3 + 0xa0) = 0xffff;
    }
    FUN_100014a0(*puVar10);
  }
  local_150 = 0;
  local_14c = 0;
  local_144 = 0;
  local_140 = 0;
  local_148 = *puVar8;
  *puVar8 = auStack_250;
  iVar11 = TrySetjmp(auStack_250);
  if (iVar11 == 0) {
    if (*piVar5 == 0) {
      iVar11 = OpenDefaultComponent_Thunk(0x74756e65,0);
      *piVar5 = iVar11;
      FocusObject();
    }
    BuildPascalString(local_138,param_2);
    if (local_138[(short)(local_138[0] - 3)] == 0x2e) {
      local_138[0] = local_138[0] - 4;
    }
    BuildPascalString(auStack_450,(unsigned long long)*(unsigned int *)(local_474 + -0x12bc) + 0x2b4);
    ConcatBuildName(local_350,auStack_450,local_138);
    Sound_BlockCopy(local_350,local_138,(unsigned long long)local_350[0] + 1);
    uVar9 = ConvertToString(local_138);
    uVar12 = Get1NamedResource_Thunk(0x54756e65,uVar9);
    *puVar7 = uVar12;
    MarkChanged();
    HLock_Thunk_Sound(*puVar7);
    Sound_DetachResourceAlt(*puVar7);
    uVar9 = ConvertToString(local_138);
    uVar12 = Get1NamedResource_Thunk(0x48656164,uVar9);
    *puVar6 = uVar12;
    MarkChanged();
    HLock_Thunk_Sound(*puVar6);
    Sound_DetachResourceAlt(*puVar6);
    uVar13 = TuneSetHeader_Thunk(*piVar5,*(int *)*puVar6);
    EndFocus(uVar13);
    uVar13 = TunePreroll_Thunk(*piVar5);
    EndFocus(uVar13);
    if (puVar10 != (int *)0x0) {
      FUN_100014a0(uVar14 + 0x5e);
      piVar2 = (int *)*piVar2;
      if (piVar2 != (int *)0x0) {
        ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0xb0),1);
      }
      FUN_100014a0(uVar14 + 0x5e);
    }
    *puVar8 = local_148;
  }
  else {
    if (puVar10 != (int *)0x0) {
      FUN_100014a0(uVar14 + 0x5e);
      piVar2 = (int *)*piVar2;
      if (piVar2 != (int *)0x0) {
        ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0xb0),1);
      }
      FUN_100014a0(uVar14 + 0x5e);
    }
    LockHandleRange(local_150,local_14c);
  }
  return;
}













































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100923a4 Size: 224 bytes */
void FUN_100923a4(long long param_1,char param_2)

{
  int *piVar1;
  int *piVar2;
  unsigned long long uVar3;
  short uVar4;
  long long lVar5;
  int local_34;
  
  piVar2 = piRam101165f8;
  piVar1 = gTunePlayer;
  if (((*gTunePlayer != 0) && (*piRam101165f8 != 0)) && (*piRam101165f4 != 0)) {
    uVar3 = GetMusicVolume();
    lVar5 = ((uVar3 & 0x3fffffff) * 4 - uVar3 & 0x1fffff) << 0xb;
    TuneSetVolume_Thunk(*piVar1,lVar5);
    **(char **)(local_34 + -0x12a4) = param_2;
    uVar4 = TuneQueue_Thunk(*piVar1,*(int *)*piVar2,0x10000,0,0xffffffffffffffff,1,0,0);
    EndFocus(uVar4);
    TuneSetVolume_Thunk(*piVar1,lVar5);
  }
  return;
}























































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100929a0 Size: 184 bytes */
void YieldToEventLoop()

{
  int *puVar1;
  int iVar2;
  int local_144;
  char auStack_120 [256];
  short local_20;
  int local_1c;
  int local_18;
  int local_14;
  int local_10;
  
  puVar1 = puRam10117370;
  if (*(char *)(gSoundPrefs + 0x34) != '\0') {
    TuneStop_Thunk(0,0);
    if ((**(char **)(local_144 + -0x12a4) != '\0') && (iVar2 = IsMusicPlaying(), iVar2 == 0)) {
      local_20 = 0;
      local_1c = 0;
      local_14 = 0;
      local_10 = 0;
      local_18 = *puVar1;
      *puVar1 = auStack_120;
      iVar2 = TrySetjmp(auStack_120);
      if (iVar2 == 0) {
        PlayMusic(0,1);
        *puVar1 = local_18;
      }
    }
  }
  return;
}



























































































































































































/* Address: 0x10092a58 Size: 76 bytes */
void FUN_10092a58(short param_1)

{
  if (*gTunePlayer != 0) {
    TuneSetVolume_Thunk(*gTunePlayer,
                 (((long long)param_1 & 0x3fffffffU) * 4 - (long long)param_1 & 0x1fffff) << 0xb);
  }
  return;
}

















































/* Address: 0x10092aa4 Size: 60 bytes */
void FUN_10092aa4()

{
  if (*gCurrentMusicID != -1) {
    SelectMusic(*gCurrentMusicID);
  }
  return;
}

/* Address: 0x10092ae0 Size: 48 bytes */
void FUN_10092ae0()

{
  int iVar1;
  
  iVar1 = IsMusicPlaying();
  if (iVar1 != 0) {
    FadeAndStopMusic();
  }
  DisposeMusicResources();
  return;
}

/* Address: 0x10092b10 Size: 208 bytes */
void FUN_10092b10(short param_1)

{
  int iVar3;
  long long uVar1;
  long long uVar2;
  
  iVar3 = IsMusicEnabled();
  if (iVar3 != 0) {
    if (param_1 == 0) {
      FUN_1005f6b0(0x25,0);
      uVar1 = ResolveSoundName();
      if (-1 < (int)uVar1) {
        uVar2 = GetSoundVolume();
        PlayGlobalSound(uVar1,uVar2);
        StartGlobalSoundPlayback(uVar1,0);
      }
    }
    else {
      FUN_1005f6b0(0x25,1);
      uVar1 = ResolveSoundName();
      if (-1 < (int)uVar1) {
        uVar2 = GetSoundVolume();
        PlayGlobalSound(uVar1,uVar2);
        StartGlobalSoundPlayback(uVar1,0);
      }
    }
  }
  return;
}






























































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10092be0 Size: 124 bytes */
void FUN_10092be0()

{
  int iVar3;
  long long uVar1;
  long long uVar2;
  
  iVar3 = IsMusicEnabled();
  if (iVar3 != 0) {
    FUN_1005f6b0(0x27,0);
    uVar1 = ResolveSoundName();
    if (-1 < (int)uVar1) {
      uVar2 = GetSoundVolume();
      PlayGlobalSound(uVar1,uVar2);
      StartGlobalSoundPlayback(uVar1,0);
    }
  }
  return;
}



















































































































































































































































































































































































































































































































































































































/* Address: 0x10093928 Size: 92 bytes */
void FUN_10093928()

{
  int iVar1;
  int iVar3;
  long long uVar2;
  
  iVar1 = iRam10117738;
  iVar3 = IsMusicEnabled();
  if (iVar3 != 0) {
    uVar2 = GetSoundVolume();
    PlayGlobalSound(*(short *)(iVar1 + 4),uVar2);
    StartGlobalSoundPlayback(*(short *)(iVar1 + 4),0);
  }
  return;
}

































































































































































































































































































































































































































































































/* Address: 0x10093984 Size: 92 bytes */
void FUN_10093984()

{
  int iVar1;
  int iVar3;
  long long uVar2;
  
  iVar1 = iRam10117738;
  iVar3 = IsMusicEnabled();
  if (iVar3 != 0) {
    uVar2 = GetSoundVolume();
    PlayGlobalSound(*(short *)(iVar1 + 2),uVar2);
    StartGlobalSoundPlayback(*(short *)(iVar1 + 2),0);
  }
  return;
}

































































































































































































































































































































































































































































































/* Address: 0x100939e0 Size: 124 bytes */
void FUN_100939e0()

{
  int iVar3;
  long long uVar1;
  long long uVar2;
  
  iVar3 = IsMusicEnabled();
  if (iVar3 != 0) {
    FUN_1005f6b0(0x3f,0);
    uVar1 = ResolveSoundName();
    if (-1 < (int)uVar1) {
      uVar2 = GetSoundVolume();
      PlayGlobalSound(uVar1,uVar2);
      StartGlobalSoundPlayback(uVar1,0);
    }
  }
  return;
}



















































































































































































































































































































































































































































































































































































































/* Address: 0x10093a5c Size: 164 bytes */
void FUN_10093a5c()

{
  short *puVar1;
  int *ppuVar2;
  short uVar3;
  
  puVar1 = gCombatSoundIDs;
  ppuVar2 = 0 /* TVect base */;
  FUN_1005f6b0(0x40,0);
  uVar3 = ResolveSoundName();
  *puVar1 = uVar3;
  FUN_1005f6b0(0x40,1);
  uVar3 = ResolveSoundName();
  puVar1[1] = uVar3;
  FUN_1005f6b0(0x40,2);
  uVar3 = ResolveSoundName();
  puVar1[2] = uVar3;
  FUN_1005f6b0(0x40,5);
  uVar3 = ResolveSoundName();
  puVar1[5] = uVar3;
  *(int *)(*(int*)((char*)ppuVar2 - 0x4ad)) = 0;
  *(int *)(*(int*)((char*)ppuVar2 - 0x4ae)) = 0;
  *(int *)(*(int*)((char*)ppuVar2 - 0x5b)) = 0;
  return;
}


























































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100981f8 Size: 240 bytes */
int OpenResourceStream(int param_1,short param_2)

{
  int bVar1;
  long long lVar2;
  int iVar3;
  
  if ((param_1 != 0) || (param_1 = NewPtr_Thunk(0x4c), param_1 != 0)) {
    lVar2 = FUN_10098188(param_1,0x5072666c,param_2);
    *(int *)(param_1 + 0x48) = (int)lVar2;
    *(char *)(param_1 + 0x46) = '\x01' - (lVar2 == 0);
    iVar3 = FUN_10002b08(0xffffffffffff8000,0x70726566,0,param_1,param_1 + 2);
    if ((*(char *)(param_1 + 0x46) == '\0') || (iVar3 != 0)) {
      bVar1 = false;
    }
    else {
      bVar1 = true;
    }
    *(int *)(param_1 + 0x46) = bVar1;
    if (bVar1) {
      CopyDataToBuffer((unsigned long long)**(unsigned int **)(param_1 + 0x48) + 0xc,param_1 + 6,0x40);
    }
  }
  return param_1;
}

















































































































































/* Address: 0x100982e8 Size: 56 bytes */
void CloseResourceStream(int param_1,unsigned long long param_2)

{
  if ((param_1 != 0) && ((param_2 & 1) != 0)) {
    DisposePtr_Thunk();
  }
  return;
}

/* Address: 0x10098320 Size: 260 bytes */
int FindResourceByType(int param_1,long long param_2,short param_3)

{
  long long uVar1;
  long long uVar2;
  int iVar3;
  int local_20 [8];
  
  local_20[0] = 0;
  uVar1 = FUN_100014b8();
  if (*(char *)(param_1 + 0x46) == '\0') {
    local_20[0] = 0;
  }
  else {
    uVar2 = FUN_100032d0(param_1,1);
    if (((int)uVar2 != 0) && (iVar3 = FUN_10000678(), iVar3 == 0)) {
      local_20[0] = GetResourceByTypeId(param_2,param_3);
      if (local_20[0] != 0) {
        FUN_10002b50(local_20);
      }
      FUN_10003300(uVar2);
    }
    if (local_20[0] == 0) {
      local_20[0] = FUN_10098188(param_1,param_2,param_3);
      if (local_20[0] != 0) {
        FUN_10002b50(local_20);
      }
    }
    DisposeGeneric(uVar1);
  }
  return local_20[0];
}
































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1009eb70 Size: 124 bytes */
short FUN_1009eb70(int param_1,int param_2)

{
  int _sStack00000018;
  int _sStack0000001c;
  short sVar1;
  short sVar2;
  long long lVar3;
  long long lVar4;
  short sStack00000018;
  short sStack0000001a;
  short sStack0000001c;
  short sStack0000001e;
  
  sStack00000018 = (short)((unsigned int)param_1 >> 0x10);
  lVar3 = (long long)sStack00000018;
  sStack0000001c = (short)((unsigned int)param_2 >> 0x10);
  lVar4 = (long long)sStack0000001c;
  _sStack00000018 = param_1;
  _sStack0000001c = param_2;
  sVar1 = AbsShort(lVar3 - lVar4);
  sVar2 = AbsShort((long long)sStack0000001a - (long long)sStack0000001e);
  if (sVar2 < sVar1) {
    sVar2 = sVar1;
  }
  return sVar2;
}

/* Address: 0x1009eef8 Size: 844 bytes */
void ExpandPlayerZone(short param_1)

{
  unsigned int uVar1;
  short sVar2;
  int iVar3;
  int iVar4;
  int iVar5;
  int bVar6;
  int *piVar7;
  unsigned int *puVar8;
  int iVar9;
  short sVar10;
  long long lVar11;
  long long lVar12;
  unsigned long long uVar13;
  short sVar14;
  unsigned long long uVar15;
  short local_40;
  short local_3e;
  short local_38;
  short local_36 [27];
  
  puVar8 = puRam101175f4;
  piVar7 = gMapGenData;
  uVar15 = (unsigned long long)param_1;
  iVar3 = (int)((uVar15 & 0xffffffff) << 2);
  iVar4 = (int)(((uVar15 & 0x3fffffff) * 4 + uVar15 & 0xfffffff) << 4);
  *(int *)(*puRam101175f4 + iVar4 + 0x104) = *(int *)(*puRam101175f4 + iVar3 + 0x244);
  *(short *)(*puVar8 + param_1 * 2 + 0xfc) = 1;
  iVar9 = param_1 * 2 + *puVar8;
  uVar13 = 0;
  bVar6 = false;
  if (*(short *)(iVar9 + 0x2c) < 3) {
    *(short *)(iVar9 + 0xfc) = *(short *)(iVar9 + 0xfc) + 1;
    *(int *)(*puVar8 + iVar4 + 0x108) = *(int *)(*puVar8 + iVar3 + 0x254);
  }
  else {
    iVar9 = *puVar8 + (int)((uVar15 & 0xffffffff) << 2);
    sVar14 = *(short *)(iVar9 + 0x244);
    if ((((sVar14 == 0) || (sVar2 = *(short *)(iVar9 + 0x246), sVar2 == 0)) || (sVar14 == 0x6f)) ||
       (sVar2 == 0x9b)) {
      iVar9 = *puVar8 + (int)((uVar15 & 0xffffffff) << 2);
      sVar14 = *(short *)(iVar9 + 0x254);
      if (((sVar14 == 0) || (sVar2 = *(short *)(iVar9 + 0x256), sVar2 == 0)) ||
         ((sVar14 == 0x6f || (sVar2 == 0x9b)))) {
        iVar9 = *puVar8 + (int)((uVar15 & 0xffffffff) << 1);
        *(short *)(iVar9 + 0xfc) = *(short *)(iVar9 + 0xfc) + 1;
        *(int *)
         (*puVar8 + (int)(((uVar15 & 0x3fffffff) * 4 + uVar15 & 0xffffffff) << 4) + 0x108) =
             *(int *)(*puVar8 + (int)((uVar15 & 0xffffffff) << 2) + 0x254);
        return;
      }
    }
    iVar9 = *puVar8 + (int)((uVar15 & 0xffffffff) << 2);
    iVar9 = ChebyshevDistance(*(int *)(iVar9 + 0x254),*(int *)(iVar9 + 0x244));
    uVar1 = iVar9 / (*(short *)(*puVar8 + (int)((uVar15 & 0xffffffff) << 1) + 0x2c) + -1);
    sVar14 = (short)uVar1;
    do {
      lVar11 = ((uVar15 & 0x3fffffff) * 4 + uVar15 & 0xfffffff) * 0x10;
      iVar4 = (int)lVar11;
      iVar9 = (int)((uVar13 & 0x3fffffff) << 2);
      iVar5 = (int)((uVar15 & 0x3fffffff) << 2);
      CalculateDirectionVector(*(int *)(*puVar8 + iVar4 + iVar9 + 0x104),
                   *(int *)(*puVar8 + iVar5 + 0x254),&local_40);
      iVar9 = iVar4 + *puVar8 + iVar9;
      local_36[0] = *(short *)(iVar9 + 0x104) + local_40 * sVar14;
      local_38 = *(short *)(iVar9 + 0x106) + sVar14 * local_3e;
      RandomScatter(local_36,&local_38,(sVar14 >> 1) + (unsigned short)(sVar14 < 0 && (uVar1 & 1) != 0),0);
      sVar2 = (short)uVar13;
      uVar13 = (unsigned long long)(short)(sVar2 + 1);
      *(char *)(*piVar7 + local_38 * 0x70 + (int)local_36[0] + 0x34a) = 7;
      NotifyTileChanged(local_36[0],local_38);
      lVar12 = (uVar13 & 0x3fffffff) * 4;
      StoreCoords(lVar11 + (unsigned long long)*puVar8 + lVar12 + 0x104,local_36[0],local_38);
      iVar3 = (int)((uVar15 & 0x7fffffff) << 1);
      iVar9 = *puVar8 + iVar3;
      *(short *)(iVar9 + 0xfc) = *(short *)(iVar9 + 0xfc) + 1;
      sVar10 = ChebyshevDistance(*(int *)(iVar5 + *puVar8 + 0x254),
                            *(int *)(iVar4 + *puVar8 + (int)lVar12 + 0x104));
      if ((sVar10 < sVar14) ||
         (iVar3 = iVar3 + *puVar8, *(short *)(iVar3 + 0x2c) + -1 <= (int)*(short *)(iVar3 + 0xfc)))
      {
        bVar6 = true;
      }
    } while (!bVar6);
    *(int *)
     (*puVar8 + (int)(((uVar15 & 0x3fffffff) * 4 + uVar15 & 0xffffffff) << 4) +
      (short)(sVar2 + 2) * 4 + 0x104) =
         *(int *)(*puVar8 + (int)((uVar15 & 0xffffffff) << 2) + 0x254);
    iVar9 = *puVar8 + (int)((uVar15 & 0xffffffff) << 1);
    *(short *)(iVar9 + 0xfc) = *(short *)(iVar9 + 0xfc) + 1;
  }
  return;
}





















































































































































































































































































































































































































































































































































































/* Address: 0x1009f864 Size: 620 bytes */
void ConnectAdjacentZones()

{
  short sVar1;
  int iVar2;
  int *piVar3;
  short sVar4;
  unsigned long long uVar5;
  unsigned long long uVar6;
  unsigned long long uVar7;
  
  piVar3 = gMapGenConfig;
  uVar6 = 0;
  do {
    uVar7 = 0;
    if (0 < (long long)*(short *)(*piVar3 + (int)((uVar6 & 0xffffffff) << 1) + 0xfc) + -1) {
      do {
        iVar2 = *piVar3 + (int)(((uVar6 & 0x3fffffff) * 4 + uVar6 & 0xffffffff) << 4) +
                (int)((uVar7 & 0xffffffff) << 2);
        sVar4 = *(short *)(iVar2 + 0x104);
        if (sVar4 == 0) {
LAB_1009f8e4:
          iVar2 = *piVar3 + (int)(((uVar6 & 0x3fffffff) * 4 + uVar6 & 0xffffffff) << 4) +
                  (int)((uVar7 & 0xffffffff) << 2);
          sVar4 = *(short *)(iVar2 + 0x108);
          if (sVar4 != 0) {
            sVar1 = *(short *)(iVar2 + 0x10a);
            if (((sVar1 != 0) && (sVar4 != 0x6f)) && (sVar1 != 0x9b)) goto LAB_1009f954;
          }
          iVar2 = *piVar3 + (int)(((uVar6 & 0x3fffffff) * 4 + uVar6 & 0xffffffff) << 4) +
                  (int)((uVar7 & 0xffffffff) << 2);
          ConnectWaypointsViaEdge(*(int *)(iVar2 + 0x104),*(int *)(iVar2 + 0x108));
        }
        else {
          sVar1 = *(short *)(iVar2 + 0x106);
          if (((sVar1 == 0) || (sVar4 == 0x6f)) || (sVar1 == 0x9b)) goto LAB_1009f8e4;
LAB_1009f954:
          iVar2 = *piVar3 + (int)(((uVar6 & 0x3fffffff) * 4 + uVar6 & 0xffffffff) << 4) +
                  (int)((uVar7 & 0xffffffff) << 2);
          ConnectWaypoints(*(int *)(iVar2 + 0x104),*(int *)(iVar2 + 0x108));
        }
        sVar4 = (short)uVar7 + 1;
        uVar7 = (unsigned long long)sVar4;
      } while ((int)sVar4 < *(short *)(*piVar3 + (int)((uVar6 & 0xffffffff) << 1) + 0xfc) + -1);
    }
    uVar5 = uVar6 + 1;
    uVar5 = (unsigned long long)
            (short)((short)uVar5 -
                   (short)(((long long)((int)uVar5 >> 2) +
                            (unsigned long long)((int)uVar5 < 0 && (uVar5 & 3) != 0) & 0xffffffff) << 2));
    iVar2 = *piVar3 + (int)(((uVar6 & 0x3fffffff) * 4 + uVar6 & 0xffffffff) << 4) +
            (int)((uVar7 & 0xffffffff) << 2);
    sVar4 = *(short *)(iVar2 + 0x104);
    if (sVar4 == 0) {
LAB_1009fa00:
      iVar2 = *piVar3 + (int)(((uVar5 & 0x3fffffff) * 4 + uVar5 & 0xffffffff) << 4);
      sVar4 = *(short *)(iVar2 + 0x104);
      if (sVar4 != 0) {
        sVar1 = *(short *)(iVar2 + 0x106);
        if (((sVar1 != 0) && (sVar4 != 0x6f)) && (sVar1 != 0x9b)) goto LAB_1009fa78;
      }
      ConnectWaypointsViaEdge(*(int *)
                    (*piVar3 + (int)(((uVar6 & 0x3fffffff) * 4 + uVar6 & 0xffffffff) << 4) +
                     (int)((uVar7 & 0xffffffff) << 2) + 0x104),
                   *(int *)
                    (*piVar3 + (int)(((uVar5 & 0x3fffffff) * 4 + uVar5 & 0xffffffff) << 4) + 0x104))
      ;
    }
    else {
      sVar1 = *(short *)(iVar2 + 0x106);
      if (((sVar1 == 0) || (sVar4 == 0x6f)) || (sVar1 == 0x9b)) goto LAB_1009fa00;
LAB_1009fa78:
      ConnectWaypoints(*(int *)
                    (*piVar3 + (int)(((uVar6 & 0x3fffffff) * 4 + uVar6 & 0xffffffff) << 4) +
                     (int)((uVar7 & 0xffffffff) << 2) + 0x104),
                   *(int *)
                    (*piVar3 + (int)(((uVar5 & 0x3fffffff) * 4 + uVar5 & 0xffffffff) << 4) + 0x104))
      ;
    }
    sVar4 = (short)uVar6 + 1;
    uVar6 = (unsigned long long)sVar4;
    if (3 < sVar4) {
      return;
    }
  } while( true );
}

/* Address: 0x1009fad4 Size: 388 bytes */
long long ShouldBeCoastal(short param_1,short param_2)

{
  unsigned long long uVar1;
  
  uVar1 = (unsigned long long)param_2;
  if (*(char *)(*gMapGenData + (int)(((uVar1 & 0x1fffffff) * 8 - uVar1 & 0xffffffff) << 4) +
                (int)param_1 + 0x34a) == '\x03') {
    return 0;
  }
  if (*(char *)(*gMapGenData + (int)(((uVar1 & 0x1fffffff) * 8 - uVar1 & 0xffffffff) << 4) +
                (int)param_1 + 0x34a) == '\x01') {
    return 0;
  }
  if ((((param_1 == 0) || (param_2 == 0)) || (param_1 == 0x6f)) || (param_2 == 0x9b)) {
    if (*(char *)(*gMapGenData + (int)(((uVar1 & 0x1fffffff) * 8 - uVar1 & 0xffffffff) << 4) +
                  (int)param_1 + 0x34a) == '\0') {
      return 1;
    }
  }
  if (*(char *)(*gMapGenData + (int)(((uVar1 & 0x1fffffff) * 8 - uVar1 & 0xffffffff) << 4) +
                (int)param_1 + 0x34b) == '\x02') {
    return 1;
  }
  if (*(char *)(*gMapGenData + (int)(((uVar1 & 0x1fffffff) * 8 - uVar1 & 0xffffffff) << 4) +
                (int)param_1 + 0x349) == '\x02') {
    return 1;
  }
  if (*(char *)((int)(((uVar1 & 0x1fffffff) * 8 - uVar1 & 0xffffffff) << 4) + *gMapGenData +
                (int)param_1 + 0x3ba) == '\x02') {
    return 1;
  }
  if (*(char *)((int)(((uVar1 & 0x1fffffff) * 8 - uVar1 & 0xffffffff) << 4) + *gMapGenData +
                (int)param_1 + 0x2da) == '\x02') {
    return 1;
  }
  return 0;
}

/* Address: 0x1009fc58 Size: 700 bytes */
void ConvertToTerrainCodes()

{
  int bVar1;
  int *piVar2;
  int iVar3;
  short sVar4;
  short sVar5;
  int iVar6;
  unsigned long long uVar7;
  int iVar8;
  
  piVar2 = gMapGenData;
  iVar6 = 0;
  do {
    uVar7 = 0;
    do {
      iVar8 = *piVar2 + (int)(((uVar7 & 0x1fffffff) * 8 - uVar7 & 0xffffffff) << 4) + iVar6;
      if (*(char *)(iVar8 + 0x34a) == '\a') {
        *(char *)(iVar8 + 0x34a) = 3;
      }
      else {
        *(char *)
         (*piVar2 + (int)(((uVar7 & 0x1fffffff) * 8 - uVar7 & 0xffffffff) << 4) + iVar6 + 0x34a) = 0
        ;
      }
      sVar5 = (short)uVar7 + 1;
      uVar7 = (unsigned long long)sVar5;
    } while (sVar5 < 0x9c);
    sVar5 = (short)iVar6 + 1;
    iVar6 = (int)sVar5;
  } while (sVar5 < 0x70);
  sVar5 = 0;
  do {
    iVar6 = 0;
    do {
      iVar8 = 0;
      do {
        iVar3 = ShouldBeCoastal(iVar6,iVar8);
        if (iVar3 != 0) {
          *(char *)(*piVar2 + iVar8 * 0x70 + iVar6 + 0x34a) = 2;
        }
        sVar4 = (short)iVar8 + 1;
        iVar8 = (int)sVar4;
      } while (sVar4 < 0x9c);
      sVar4 = (short)iVar6 + 1;
      iVar6 = (int)sVar4;
    } while (sVar4 < 0x70);
    iVar6 = 0;
    do {
      iVar8 = 0;
      do {
        iVar3 = ShouldBeCoastal(iVar8,iVar6);
        if (iVar3 != 0) {
          *(char *)(*piVar2 + iVar6 * 0x70 + iVar8 + 0x34a) = 2;
        }
        sVar4 = (short)iVar8 + 1;
        iVar8 = (int)sVar4;
      } while (sVar4 < 0x70);
      sVar4 = (short)iVar6 + 1;
      iVar6 = (int)sVar4;
    } while (sVar4 < 0x9c);
    iVar6 = 0x6f;
    do {
      iVar8 = 0x9b;
      do {
        iVar3 = ShouldBeCoastal(iVar6,iVar8);
        if (iVar3 != 0) {
          *(char *)(*piVar2 + iVar8 * 0x70 + iVar6 + 0x34a) = 2;
        }
        bVar1 = iVar8 != 0;
        iVar8 = (int)(short)((short)iVar8 + -1);
      } while (bVar1);
      bVar1 = iVar6 != 0;
      iVar6 = (int)(short)((short)iVar6 + -1);
    } while (bVar1);
    iVar6 = 0x9b;
    do {
      iVar8 = 0x6f;
      do {
        iVar3 = ShouldBeCoastal(iVar8,iVar6);
        if (iVar3 != 0) {
          *(char *)(*piVar2 + iVar6 * 0x70 + iVar8 + 0x34a) = 2;
        }
        bVar1 = iVar8 != 0;
        iVar8 = (int)(short)((short)iVar8 + -1);
      } while (bVar1);
      bVar1 = iVar6 != 0;
      iVar6 = (int)(short)((short)iVar6 + -1);
    } while (bVar1);
    sVar5 = sVar5 + 1;
  } while (sVar5 < 2);
  iVar6 = 0;
  do {
    uVar7 = 0;
    do {
      iVar8 = *piVar2 + (int)(((uVar7 & 0x1fffffff) * 8 - uVar7 & 0xffffffff) << 4) + iVar6;
      if (*(char *)(iVar8 + 0x34a) == '\x02') {
        *(char *)(iVar8 + 0x34a) = 2;
      }
      else {
        *(char *)
         (*piVar2 + (int)(((uVar7 & 0x1fffffff) * 8 - uVar7 & 0xffffffff) << 4) + iVar6 + 0x34a) = 7
        ;
      }
      sVar5 = (short)uVar7 + 1;
      uVar7 = (unsigned long long)sVar5;
    } while (sVar5 < 0x9c);
    sVar5 = (short)iVar6 + 1;
    iVar6 = (int)sVar5;
  } while (sVar5 < 0x70);
  NotifyAllTilesChanged();
  return;
}

/* Address: 0x1009ff14 Size: 144 bytes */
long long FUN_1009ff14(short param_1,short param_2)

{
  short sVar1;
  int iVar2;
  unsigned long long uVar3;
  long long lVar4;
  int iVar5;
  
  iVar2 = 0;
  while( true ) {
    iVar5 = *gMapGenConfig + iVar2 * 4;
    lVar4 = (long long)param_1 + (long long)*(short *)(iVar5 + 0xbc);
    if ((((-1 < lVar4) &&
         (uVar3 = (long long)param_2 + (long long)*(short *)(iVar5 + 0xbe), -1 < (long long)uVar3)) &&
        (iVar5 = (int)lVar4, iVar5 < 0x70)) &&
       (((int)uVar3 < 0x9c &&
        (*(char *)(*gMapGenData + (int)(((uVar3 & 0x1fffffff) * 8 - uVar3 & 0xffffffff) << 4) +
                   iVar5 + 0x34a) == '\a')))) break;
    sVar1 = (short)iVar2 + 1;
    iVar2 = (int)sVar1;
    if (7 < sVar1) {
      return 0;
    }
  }
  return 1;
}

/* Address: 0x1009ffa4 Size: 148 bytes */
long long FUN_1009ffa4(short param_1,short param_2,short param_3)

{
  short sVar1;
  int iVar2;
  unsigned long long uVar3;
  long long lVar4;
  int iVar5;
  
  iVar2 = 0;
  while( true ) {
    iVar5 = *gMapGenConfig + iVar2 * 4;
    lVar4 = (long long)param_1 + (long long)*(short *)(iVar5 + 0xbc);
    if ((((-1 < lVar4) &&
         (uVar3 = (long long)param_2 + (long long)*(short *)(iVar5 + 0xbe), -1 < (long long)uVar3)) &&
        (iVar5 = (int)lVar4, iVar5 < 0x70)) &&
       (((int)uVar3 < 0x9c &&
        ((int)*(char *)(*gMapGenData + (int)(((uVar3 & 0x1fffffff) * 8 - uVar3 & 0xffffffff) << 4)
                        + iVar5 + 0x34a) == (int)param_3)))) break;
    sVar1 = (short)iVar2 + 1;
    iVar2 = (int)sVar1;
    if (7 < sVar1) {
      return 0;
    }
  }
  return 1;
}

/* Address: 0x100a0038 Size: 336 bytes */
void ExpandCoastalTiles()

{
  int *piVar1;
  int iVar2;
  int iVar3;
  short sVar4;
  int iVar5;
  unsigned long long uVar6;
  
  piVar1 = gMapGenData;
  iVar5 = 0;
  do {
    uVar6 = 0;
    do {
      iVar3 = (int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4);
      if (*(char *)(*piVar1 + iVar3 + iVar5 + 0x34a) == '\x02') {
        iVar2 = HasNeighborOnLand(iVar5,uVar6);
        if (iVar2 != 0) {
          *(char *)(*piVar1 + iVar3 + iVar5 + 0x34a) = 3;
          NotifyTileChanged(iVar5,uVar6);
        }
        iVar3 = HasNeighborWithTerrain(iVar5,uVar6,5);
        if (iVar3 != 0) {
          *(char *)
           (*piVar1 + (int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4) + iVar5 + 0x34a) =
               3;
          NotifyTileChanged(iVar5,uVar6);
        }
        iVar3 = HasNeighborWithTerrain(iVar5,uVar6,6);
        if (iVar3 != 0) {
          *(char *)
           (*piVar1 + (int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4) + iVar5 + 0x34a) =
               3;
          NotifyTileChanged(iVar5,uVar6);
        }
      }
      sVar4 = (short)uVar6 + 1;
      uVar6 = (unsigned long long)sVar4;
    } while (sVar4 < 0x9c);
    sVar4 = (short)iVar5 + 1;
    iVar5 = (int)sVar4;
  } while (sVar4 < 0x70);
  return;
}









































































































































































































































































/* Address: 0x100a018c Size: 92 bytes - stubbed: callers pass 0 args */
void AddExtraLandPatches()

{
  return;
}

/* Address: 0x100a01e8 Size: 172 bytes */
void PlacePlayerStartZones()

{
  int iVar1;
  
  PlacePlayerSeedPoint(0);
  PlacePlayerSeedPoint(1);
  PlacePlayerSeedPoint(2);
  PlacePlayerSeedPoint(3);
  SortStartPointsByAngle();
  ExpandPlayerZone(0);
  ExpandPlayerZone(1);
  ExpandPlayerZone(2);
  ExpandPlayerZone(3);
  ConnectAdjacentZones();
  ConvertToTerrainCodes();
  ExpandCoastalTiles();
  iVar1 = RandomRange(1,100,0);
  if (iVar1 < 0x32) {
    RandomRange(1,3,0xffffffffffffffff);
    AddExtraLandPatches();
  }
  ExpandCoastalTiles();
  return;
}

























































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100a1d8c Size: 156 bytes */
void CalculateLandTarget()

{
  char cVar1;
  int *piVar2;
  short sVar3;
  int iVar4;
  int iVar5;
  int iVar6;
  
  piVar2 = gMapGenConfig;
  iVar5 = 0;
  iVar4 = 0;
  do {
    iVar6 = 0;
    do {
      cVar1 = *(char *)(*gMapGenData + iVar6 * 0x70 + iVar4 + 0x34a);
      if (((cVar1 == '\a') || (cVar1 == '\x05')) || (cVar1 == '\x06')) {
        iVar5 = (int)(short)((short)iVar5 + 1);
      }
      sVar3 = (short)iVar6 + 1;
      iVar6 = (int)sVar3;
    } while (sVar3 < 0x9c);
    sVar3 = (short)iVar4 + 1;
    iVar4 = (int)sVar3;
  } while (sVar3 < 0x70);
  *(short *)(*gMapGenConfig + 0x1f54) = (short)(iVar5 / 100) * *(short *)(*gMapGenConfig + 0x3c);
  *(short *)(*piVar2 + 0x1f52) = 0;
  return;
}

/* Address: 0x100a1e28 Size: 40 bytes */
short CheckLandTarget()

{
  unsigned short uVar1;
  unsigned short uVar2;
  
  uVar1 = *(unsigned short *)(*gMapGenConfig + 0x1f52);
  uVar2 = *(unsigned short *)(*gMapGenConfig + 0x1f54);
  return (((short)uVar1 >> 0xf) - ((short)uVar2 >> 0xf)) + (unsigned short)(uVar2 <= uVar1);
}

/* Address: 0x100a1e50 Size: 1216 bytes */
void GrowLandBlob()

{
  int bVar1;
  int *piVar2;
  int *piVar3;
  int iVar4;
  int iVar5;
  int iVar6;
  short sVar7;
  short sVar8;
  short sVar9;
  unsigned long long uVar10;
  short sVar11;
  unsigned long long uVar12;
  unsigned short uVar13;
  short sVar14;
  unsigned long long uVar15;
  short local_88;
  short local_86;
  short local_84;
  short local_82;
  short local_80;
  short local_7e [3];
  short local_78;
  short local_70;
  int local_6c;
  short local_68;
  int local_64;
  short local_60 [8];
  int local_50;
  int local_4c;
  
  piVar3 = gMapGenConfig;
  piVar2 = gMapGenData;
  bVar1 = false;
  do {
    iVar4 = RandomRange(1,0x70,0xffffffffffffffff);
    iVar5 = RandomRange(1,0x9c,0xffffffffffffffff);
    if (*(char *)(*piVar2 + iVar5 * 0x70 + iVar4 + 0x34a) == '\a') {
      bVar1 = true;
    }
  } while (!bVar1);
  local_70 = (short)iVar5;
  local_78 = (short)iVar4;
  iVar6 = RandomRange(1,100,0);
  if (iVar6 < 0x41) {
    iVar6 = 0;
    local_6c = iVar4;
    local_64 = iVar5;
    do {
      sVar7 = RandomRange(1,8,2);
      local_60[iVar6] = sVar7;
      sVar7 = (short)iVar6 + 1;
      iVar6 = (int)sVar7;
      sVar14 = 2;
    } while (sVar7 < 8);
    local_68 = 4;
  }
  else {
    iVar6 = 0;
    local_50 = iVar4;
    local_4c = iVar5;
    do {
      sVar7 = RandomRange(1,10,5);
      local_60[iVar6] = sVar7;
      sVar7 = (short)iVar6 + 1;
      iVar6 = (int)sVar7;
      sVar14 = 4;
    } while (sVar7 < 8);
    local_68 = 6;
  }
  *(char *)(*piVar2 + local_70 * 0x70 + (int)local_78 + 0x34a) = 4;
  NotifyTileChanged();
  *(short *)(*piVar3 + 0x1f52) = *(short *)(*piVar3 + 0x1f52) + 1;
  uVar15 = 0;
  do {
    local_86 = local_70;
    local_88 = local_78;
    uVar13 = 0;
    sVar7 = 0;
    if (0 < *(short *)((int)local_60 + (int)((uVar15 & 0xffffffff) << 1))) {
      do {
        iVar4 = *piVar3 + (int)((uVar15 & 0xffffffff) << 2);
        local_88 = local_88 + *(short *)(iVar4 + 0xbc);
        local_86 = local_86 + *(short *)(iVar4 + 0xbe);
        ClampToMapBounds(&local_88,&local_86);
        iVar4 = *piVar2 + local_86 * 0x70 + (int)local_88;
        if (*(char *)(iVar4 + 0x34a) == '\a') {
          *(char *)(iVar4 + 0x34a) = 4;
          NotifyTileChanged(local_88,local_86);
          *(short *)(*piVar3 + 0x1f52) = *(short *)(*piVar3 + 0x1f52) + 1;
        }
        uVar10 = uVar15 + 2;
        uVar12 = uVar15 + 6;
        uVar13 = uVar13 + 1;
        sVar9 = local_68 - (((short)uVar13 >> 1) + (unsigned short)((short)uVar13 < 0 && (uVar13 & 1) != 0))
        ;
        sVar8 = RandomRange(1,sVar9,sVar14 - (((short)uVar13 >> 2) +
                                              (unsigned short)((short)uVar13 < 0 && (uVar13 & 3) != 0)));
        sVar9 = RandomRange(1,sVar9,sVar14 - (short)uVar13 / 3);
        local_7e[0] = local_88;
        local_80 = local_86;
        sVar11 = 0;
        if (0 < sVar8) {
          do {
            iVar4 = *piVar3 + (short)((short)uVar10 -
                                     (short)(((long long)((int)uVar10 >> 3) +
                                              (unsigned long long)((int)uVar10 < 0 && (uVar10 & 7) != 0) &
                                             0xffffffff) << 3)) * 4;
            local_7e[0] = local_7e[0] + *(short *)(iVar4 + 0xbc);
            local_80 = local_80 + *(short *)(iVar4 + 0xbe);
            ClampToMapBounds(local_7e,&local_80);
            iVar4 = *piVar2 + local_80 * 0x70 + (int)local_7e[0];
            if (*(char *)(iVar4 + 0x34a) == '\a') {
              *(char *)(iVar4 + 0x34a) = 4;
              NotifyTileChanged(local_7e[0],local_80);
              *(short *)(*piVar3 + 0x1f52) = *(short *)(*piVar3 + 0x1f52) + 1;
            }
            sVar11 = sVar11 + 1;
          } while (sVar11 < sVar8);
        }
        local_82 = local_88;
        local_84 = local_86;
        sVar8 = 0;
        if (0 < sVar9) {
          do {
            iVar4 = *piVar3 + (short)((short)uVar12 -
                                     (short)(((long long)((int)uVar12 >> 3) +
                                              (unsigned long long)((int)uVar12 < 0 && (uVar12 & 7) != 0) &
                                             0xffffffff) << 3)) * 4;
            local_82 = local_82 + *(short *)(iVar4 + 0xbc);
            local_84 = local_84 + *(short *)(iVar4 + 0xbe);
            ClampToMapBounds(&local_82,&local_84);
            iVar4 = *piVar2 + local_84 * 0x70 + (int)local_82;
            if (*(char *)(iVar4 + 0x34a) == '\a') {
              *(char *)(iVar4 + 0x34a) = 4;
              NotifyTileChanged(local_82,local_84);
              *(short *)(*piVar3 + 0x1f52) = *(short *)(*piVar3 + 0x1f52) + 1;
            }
            sVar8 = sVar8 + 1;
          } while (sVar8 < sVar9);
        }
        sVar7 = sVar7 + 1;
      } while (sVar7 < *(short *)((int)local_60 + (int)((uVar15 & 0xffffffff) << 1)));
    }
    sVar7 = (short)uVar15 + 1;
    uVar15 = (unsigned long long)sVar7;
  } while (sVar7 < 8);
  return;
}





































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100a2310 Size: 1036 bytes */
void FillEnclosedWater()

{
  char cVar1;
  short sVar2;
  int *piVar3;
  int *piVar4;
  short sVar6;
  int iVar5;
  unsigned char bVar7;
  int iVar8;
  unsigned long long uVar9;
  
  piVar4 = gMapGenConfig;
  piVar3 = gMapGenData;
  iVar8 = 0;
  do {
    uVar9 = 0;
    do {
      iVar5 = *piVar3 + (int)(((uVar9 & 0x1fffffff) * 8 - uVar9 & 0xffffffff) << 4) + iVar8;
      if (*(char *)(iVar5 + 0x34a) == '\a') {
        bVar7 = *(char *)(iVar5 + 0x3ba) == '\x04';
        if (*(char *)((int)(((uVar9 & 0x1fffffff) * 8 - uVar9 & 0xffffffff) << 4) + *piVar3 + iVar8
                     + 0x2da) == '\x04') {
          bVar7 = bVar7 + 1;
        }
        if (*(char *)(*piVar3 + (int)(((uVar9 & 0x1fffffff) * 8 - uVar9 & 0xffffffff) << 4) + iVar8
                     + 0x34b) == '\x04') {
          bVar7 = bVar7 + 1;
        }
        if (*(char *)(*piVar3 + (int)(((uVar9 & 0x1fffffff) * 8 - uVar9 & 0xffffffff) << 4) + iVar8
                     + 0x349) == '\x04') {
          bVar7 = bVar7 + 1;
        }
        if (2 < bVar7) {
          *(char *)
           (*piVar3 + (int)(((uVar9 & 0x1fffffff) * 8 - uVar9 & 0xffffffff) << 4) + iVar8 + 0x34a) =
               4;
          *(short *)(*piVar4 + 0x1f52) = *(short *)(*piVar4 + 0x1f52) + 1;
          NotifyTileChanged(iVar8,uVar9);
        }
      }
      sVar6 = (short)uVar9 + 1;
      uVar9 = (unsigned long long)sVar6;
    } while (sVar6 < 0x9c);
    sVar6 = (short)iVar8 + 1;
    iVar8 = (int)sVar6;
  } while (sVar6 < 0x70);
  iVar8 = 1;
  do {
    uVar9 = 1;
    do {
      iVar5 = *piVar3 + (int)uVar9 * 0x70 + iVar8;
      sVar2 = (short)iVar8;
      sVar6 = (short)uVar9;
      if (*(char *)(iVar5 + 0x34a) == '\x04') {
        if ((((*(char *)(iVar5 + 0x3bb) == '\x04') && (*(char *)(iVar5 + 0x2d9) == '\x04')) &&
            ((cVar1 = *(char *)(iVar5 + 0x2db), cVar1 == '\x05' ||
             ((cVar1 == '\x03' || (cVar1 == '\a')))))) &&
           ((iVar5 = (int)(((uVar9 & 0x1fffffff) * 8 - uVar9 & 0xffffffff) << 4) + *piVar3,
            *(char *)(iVar5 + iVar8 + 0x3b9) == '\x05' ||
            ((cVar1 = *(char *)(iVar8 + iVar5 + 0x3b9), cVar1 == '\x03' || (cVar1 == '\a')))))) {
          iVar5 = RandomRange(1,10,0);
          if (iVar5 < 5) {
            *(char *)
             (*piVar3 + (int)(((uVar9 & 0x1fffffff) * 8 - uVar9 & 0xffffffff) << 4) + iVar8 + 0x2db)
                 = 4;
            NotifyTileChanged(sVar2 + 1,sVar6 + -1);
          }
          else {
            *(char *)
             (*piVar3 + (int)(((uVar9 & 0x1fffffff) * 8 - uVar9 & 0xffffffff) << 4) + iVar8 + -1 +
             0x3ba) = 4;
            NotifyTileChanged((short)(iVar8 + -1),sVar6 + 1);
          }
        }
        else {
          iVar5 = (int)uVar9 * 0x70 + *piVar3;
          if (((*(char *)(iVar5 + iVar8 + 0x3b9) == '\x04') &&
              (iVar5 = iVar8 + iVar5, *(char *)(iVar5 + 0x2db) == '\x04')) &&
             ((((cVar1 = *(char *)(iVar5 + 0x2d9), cVar1 == '\x05' || (cVar1 == '\x03')) ||
               (cVar1 == '\a')) &&
              (((cVar1 = *(char *)((int)(((uVar9 & 0x1fffffff) * 8 - uVar9 & 0xffffffff) << 4) +
                                   *piVar3 + iVar8 + 0x3bb), cVar1 == '\x05' || (cVar1 == '\x03'))
               || (cVar1 == '\a')))))) {
            iVar5 = RandomRange(1,10,0);
            if (iVar5 < 5) {
              *(char *)
               ((int)(((uVar9 & 0x1fffffff) * 8 - uVar9 & 0xffffffff) << 4) + *piVar3 + iVar8 + -1 +
               0x2da) = 4;
              NotifyTileChanged((short)(iVar8 + -1),sVar6 + -1);
            }
            else {
              *(char *)
               ((int)(((uVar9 & 0x1fffffff) * 8 - uVar9 & 0xffffffff) << 4) + *piVar3 + iVar8 +
               0x3bb) = 4;
              NotifyTileChanged(sVar2 + 1,sVar6 + 1);
            }
          }
        }
      }
      uVar9 = (unsigned long long)(short)(sVar6 + 1);
    } while ((short)(sVar6 + 1) < 0x9b);
    iVar8 = (int)(short)(sVar2 + 1);
  } while ((short)(sVar2 + 1) < 0x6f);
  return;
}





























































































































































































































































































































































































































































































































































































































































/* Address: 0x100a271c Size: 68 bytes */
void GenerateLandMasses()

{
  int iVar1;
  
  CalculateLandTarget();
  iVar1 = CheckLandTarget();
  while (iVar1 == 0) {
    GrowLandBlob();
    FillEnclosedWater();
    iVar1 = CheckLandTarget();
  }
  return;
}

/* Address: 0x100a2760 Size: 380 bytes */
void PlacePlayerCities()

{
  int bVar1;
  int *piVar2;
  int *piVar3;
  int iVar5;
  unsigned long long uVar4;
  short sVar6;
  
  piVar3 = gMapGenConfig;
  piVar2 = gMapGenData;
  if (0 < *(short *)(*gMapGenConfig + 0x34)) {
    sVar6 = 0;
    do {
      bVar1 = false;
      do {
        iVar5 = RandomRange(1,0x70,0xffffffffffffffff);
        uVar4 = RandomRange(1,0x9c,0xffffffffffffffff);
        if (*(char *)(*piVar2 + (int)(((uVar4 & 0x1fffffff) * 8 - uVar4 & 0xffffffff) << 4) + iVar5
                     + 0x34a) == '\a') {
          bVar1 = true;
        }
      } while (!bVar1);
      *(char *)
       (*piVar2 + (int)(((uVar4 & 0x1fffffff) * 8 - uVar4 & 0xffffffff) << 4) + iVar5 + 0x34a) = 6;
      *(short *)(*piVar2 + *(short *)(*piVar2 + 0x348) * 0xe) = (short)iVar5;
      *(short *)(*piVar2 + *(short *)(*piVar2 + 0x348) * 0xe + 2) = (short)uVar4;
      *(short *)(*piVar2 + *(short *)(*piVar2 + 0x348) * 0xe + 4) = 2;
      *(short *)(*piVar2 + *(short *)(*piVar2 + 0x348) * 0xe + 6) = 0;
      *(short *)(*piVar2 + 0x348) = *(short *)(*piVar2 + 0x348) + 1;
      NotifyTileChanged(iVar5,uVar4);
      sVar6 = sVar6 + 1;
    } while (sVar6 < *(short *)(*piVar3 + 0x34));
  }
  return;
}

























































































































































































































/* Address: 0x100a28dc Size: 380 bytes */
void PlaceNeutralCities()

{
  int bVar1;
  int *piVar2;
  int *piVar3;
  int iVar5;
  unsigned long long uVar4;
  short sVar6;
  
  piVar3 = gMapGenConfig;
  piVar2 = gMapGenData;
  if (0 < *(short *)(*gMapGenConfig + 0x36)) {
    sVar6 = 0;
    do {
      bVar1 = false;
      do {
        iVar5 = RandomRange(1,0x70,0xffffffffffffffff);
        uVar4 = RandomRange(1,0x9c,0xffffffffffffffff);
        if (*(char *)(*piVar2 + (int)(((uVar4 & 0x1fffffff) * 8 - uVar4 & 0xffffffff) << 4) + iVar5
                     + 0x34a) == '\a') {
          bVar1 = true;
        }
      } while (!bVar1);
      *(char *)
       (*piVar2 + (int)(((uVar4 & 0x1fffffff) * 8 - uVar4 & 0xffffffff) << 4) + iVar5 + 0x34a) = 5;
      *(short *)(*piVar2 + *(short *)(*piVar2 + 0x348) * 0xe) = (short)iVar5;
      *(short *)(*piVar2 + *(short *)(*piVar2 + 0x348) * 0xe + 2) = (short)uVar4;
      *(short *)(*piVar2 + *(short *)(*piVar2 + 0x348) * 0xe + 4) = 1;
      *(short *)(*piVar2 + *(short *)(*piVar2 + 0x348) * 0xe + 6) = 0;
      *(short *)(*piVar2 + 0x348) = *(short *)(*piVar2 + 0x348) + 1;
      NotifyTileChanged(iVar5,uVar4);
      sVar6 = sVar6 + 1;
    } while (sVar6 < *(short *)(*piVar3 + 0x36));
  }
  return;
}

























































































































































































































/* Address: 0x100a2a58 Size: 616 bytes */
void AssignCityAlliances()

{
  int *piVar1;
  short sVar2;
  short sVar3;
  int iVar4;
  short uVar6;
  int iVar5;
  int iVar7;
  unsigned long long uVar8;
  int iVar9;
  int iVar10;
  
  piVar1 = gMapGenData;
  if ((1 < *(short *)(*gMapGenData + 0x348)) &&
     (uVar8 = 0, 0 < *(short *)(*gMapGenData + 0x348))) {
    do {
      if (*(short *)(*piVar1 + (int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 1) + 4) == 1
         ) {
        sVar2 = RandomRange(1,2,0xffffffffffffffff);
        sVar3 = *(short *)(*piVar1 + 0x348) + -1;
        if (sVar2 < sVar3) {
          sVar3 = sVar2;
        }
        *(short *)(*piVar1 + (int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 1) + 6) =
             sVar3;
      }
      else {
        sVar2 = RandomRange(1,4,0xffffffffffffffff);
        sVar3 = *(short *)(*piVar1 + 0x348) + -1;
        if (sVar2 < sVar3) {
          sVar3 = sVar2;
        }
        *(short *)(*piVar1 + (int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 1) + 6) =
             sVar3;
      }
      iVar9 = 0;
      if (0 < *(short *)(*piVar1 + (int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 1) + 6))
      {
        do {
          iVar7 = 10000;
          iVar5 = -1;
          uVar6 = 0xffff;
          iVar10 = 0;
          if (0 < *(short *)(*piVar1 + 0x348)) {
            do {
              if (iVar10 != (int)uVar8) {
                iVar4 = 0;
                if (0 < iVar9) {
                  do {
                    if (iVar10 == *(short *)(*piVar1 + (int)(((uVar8 & 0x1fffffff) * 8 - uVar8 &
                                                             0xffffffff) << 1) + iVar4 * 2 + 8))
                    break;
                    sVar3 = (short)iVar4 + 1;
                    iVar4 = (int)sVar3;
                  } while (sVar3 < iVar9);
                }
                if (iVar4 == iVar9) {
                  iVar4 = ChebyshevDistance(*(int *)(*piVar1 + iVar10 * 0xe),
                                       *(int *)
                                        (*piVar1 +
                                        (int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 1))
                                      );
                  if (iVar4 < iVar7) {
                    iVar5 = iVar10;
                    iVar7 = iVar4;
                  }
                }
              }
              uVar6 = (short)iVar5;
              sVar3 = (short)iVar10 + 1;
              iVar10 = (int)sVar3;
            } while (sVar3 < *(short *)(*piVar1 + 0x348));
          }
          iVar5 = (int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0x7fffffff) << 1);
          *(short *)(*piVar1 + iVar5 + iVar9 * 2 + 8) = uVar6;
          iVar9 = (int)(short)((short)iVar9 + 1);
        } while (iVar9 < *(short *)(iVar5 + *piVar1 + 6));
      }
      sVar3 = (short)uVar8 + 1;
      uVar8 = (unsigned long long)sVar3;
    } while (sVar3 < *(short *)(*piVar1 + 0x348));
  }
  return;
}

/* Address: 0x100a3280 Size: 56 bytes */
void FUN_100a3280(int param_1,int param_2)

{
  int uStack00000018;
  int uStack0000001c;
  
  uStack00000018 = param_1;
  uStack0000001c = param_2;
  FUN_100a2cc0(param_1,param_2,3,5);
  return;
}

/* Address: 0x100a32b8 Size: 76 bytes */
void FUN_100a32b8(int param_1,int param_2)

{
  long long uVar1;
  int uStack00000018;
  int uStack0000001c;
  
  uStack00000018 = param_1;
  uStack0000001c = param_2;
  uVar1 = RandomRange(1,2,1);
  FUN_100a2cc0(uStack00000018,uStack0000001c,uVar1,6);
  return;
}

/* Address: 0x100a33ac Size: 544 bytes */
void GenerateRoadsBetweenCities()

{
  int iVar1;
  int iVar2;
  int *piVar3;
  short sVar4;
  unsigned long long uVar5;
  unsigned long long uVar6;
  int iVar7;
  
  piVar3 = gMapGenData;
  if ((1 < *(short *)(*gMapGenData + 0x348)) &&
     (uVar5 = 0, 0 < *(short *)(*gMapGenData + 0x348))) {
    do {
      iVar1 = (int)(((uVar5 & 0x1fffffff) * 8 - uVar5 & 0x7fffffff) << 1);
      iVar7 = *piVar3 + iVar1;
      if (*(short *)(iVar7 + 4) == 1) {
        if (*(short *)(iVar7 + 6) == 0) {
          DrawRoadFromCity(*(int *)(*piVar3 + iVar1));
        }
        else {
          uVar6 = 0;
          if (0 < *(short *)(*piVar3 + (int)(((uVar5 & 0x1fffffff) * 8 - uVar5 & 0xffffffff) << 1) +
                            6)) {
            do {
              iVar7 = *piVar3;
              iVar2 = (int)(((uVar5 & 0x1fffffff) * 8 - uVar5 & 0x7fffffff) << 1);
              iVar1 = *(short *)(iVar7 + iVar2 + (int)((uVar6 & 0xffffffff) << 1) + 8) * 0xe;
              if (*(short *)(iVar7 + iVar1 + 4) == 1) {
                DrawRoadBetweenNeutral(*(int *)(iVar2 + iVar7),*(int *)(iVar1 + iVar7));
              }
              else {
                iVar7 = *piVar3;
                iVar1 = (int)(((uVar5 & 0x1fffffff) * 8 - uVar5 & 0x7fffffff) << 1);
                DrawRoadBetweenPlayer(*(int *)(iVar7 + iVar1),
                             *(int *)
                              (iVar7 + *(short *)(iVar7 + iVar1 + (int)((uVar6 & 0xffffffff) << 1) +
                                                 8) * 0xe));
              }
              sVar4 = (short)uVar6 + 1;
              uVar6 = (unsigned long long)sVar4;
            } while (sVar4 < *(short *)(*piVar3 + (int)(((uVar5 & 0x1fffffff) * 8 - uVar5 &
                                                        0xffffffff) << 1) + 6));
          }
        }
      }
      else {
        iVar7 = (int)(((uVar5 & 0x1fffffff) * 8 - uVar5 & 0x7fffffff) << 1);
        if (*(short *)(*piVar3 + iVar7 + 6) == 0) {
          DrawStandaloneRoad(*(int *)(iVar7 + *piVar3));
        }
        else {
          iVar7 = 0;
          if (0 < *(short *)(*piVar3 + (int)(((uVar5 & 0x1fffffff) * 8 - uVar5 & 0xffffffff) << 1) +
                            6)) {
            do {
              iVar1 = *piVar3;
              iVar2 = (int)(((uVar5 & 0x1fffffff) * 8 - uVar5 & 0x7fffffff) << 1);
              DrawRoadBetweenPlayer(*(int *)(iVar1 + iVar2),
                           *(int *)(iVar1 + *(short *)(iVar1 + iVar2 + iVar7 * 2 + 8) * 0xe))
              ;
              sVar4 = (short)iVar7 + 1;
              iVar7 = (int)sVar4;
            } while (sVar4 < *(short *)(iVar2 + *piVar3 + 6));
          }
        }
      }
      sVar4 = (short)uVar5 + 1;
      uVar5 = (unsigned long long)sVar4;
    } while (sVar4 < *(short *)(*piVar3 + 0x348));
  }
  return;
}

/* Address: 0x100a35cc Size: 992 bytes */
void FUN_100a35cc()

{
  short sVar1;
  int *piVar2;
  int iVar3;
  short sVar4;
  int iVar5;
  int iVar7;
  unsigned long long uVar6;
  
  piVar2 = gMapGenData;
  iVar5 = 0;
  do {
    iVar7 = 0;
    do {
      if ((((iVar5 != 0) && (iVar7 != 0)) && (iVar5 != 0x6f)) && (iVar7 != 0x9b)) {
        iVar3 = *piVar2 + iVar7 * 0x70 + iVar5;
        if (*(char *)(iVar3 + 0x34a) == '\a') {
          if (((*(char *)(iVar3 + 0x3ba) == '\x06') && (*(char *)(iVar3 + 0x34b) == '\x06')) &&
             ((*(char *)(iVar3 + 0x349) == '\x06' && (*(char *)(iVar3 + 0x2da) == '\x06')))) {
            *(char *)(iVar3 + 0x34a) = 6;
            NotifyTileChanged(iVar5,iVar7);
          }
          iVar3 = iVar7 * 0x70 + *piVar2 + iVar5;
          if (((*(char *)(iVar3 + 0x3ba) == '\x05') && (*(char *)(iVar3 + 0x34b) == '\x05')) &&
             ((*(char *)(iVar3 + 0x349) == '\x05' && (*(char *)(iVar3 + 0x2da) == '\x05')))) {
            *(char *)(iVar3 + 0x34a) = 5;
            NotifyTileChanged(iVar5,iVar7);
          }
        }
      }
      if ((*(char *)(*piVar2 + iVar7 * 0x70 + iVar5 + 0x34a) == '\x06') &&
         (iVar3 = HasNeighborWithTerrain(iVar5,iVar7,3), iVar3 != 0)) {
        *(char *)(iVar7 * 0x70 + *piVar2 + iVar5 + 0x34a) = 7;
        NotifyTileChanged(iVar5,iVar7);
      }
      sVar4 = (short)iVar7 + 1;
      iVar7 = (int)sVar4;
    } while (sVar4 < 0x9c);
    sVar4 = (short)iVar5 + 1;
    iVar5 = (int)sVar4;
  } while (sVar4 < 0x70);
  iVar5 = 1;
  do {
    uVar6 = 1;
    do {
      iVar3 = (int)uVar6 * 0x70;
      iVar7 = *piVar2 + iVar3 + iVar5;
      sVar4 = (short)uVar6;
      sVar1 = (short)iVar5;
      if (*(char *)(iVar7 + 0x34a) == '\x06') {
        if (((*(char *)(iVar7 + 0x3bb) == '\x06') && (*(char *)(iVar7 + 0x2d9) == '\x06')) &&
           (*(char *)(iVar7 + 0x2db) != '\x06')) {
          if (*(char *)(iVar7 + 0x3b9) == '\x06') {
LAB_100a38a8:
            iVar3 = (int)uVar6 * 0x70;
            iVar7 = iVar3 + *piVar2 + iVar5;
            if (((*(char *)(iVar7 + 0x2db) == '\x06') && (*(char *)(iVar7 + 0x2d9) != '\x06')) &&
               (*(char *)(iVar7 + 0x3bb) != '\x06')) {
              iVar7 = RandomRange(1,10,0);
              if (iVar7 < 6) {
                *(char *)(iVar3 + *piVar2 + iVar5 + -1 + 0x2da) = 6;
                NotifyTileChanged((short)(iVar5 + -1),sVar4 + -1);
              }
              else {
                *(char *)
                 ((int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4) + *piVar2 + iVar5 +
                 0x3bb) = 6;
                NotifyTileChanged(sVar1 + 1,sVar4 + 1);
              }
            }
          }
          else {
            iVar7 = RandomRange(1,10,0);
            if (iVar7 < 6) {
              *(char *)(iVar3 + *piVar2 + iVar5 + 0x2db) = 6;
              NotifyTileChanged(sVar1 + 1,sVar4 + -1);
            }
            else {
              *(char *)
               (*piVar2 + (int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4) + iVar5 + -1 +
               0x3ba) = 6;
              NotifyTileChanged((short)(iVar5 + -1),sVar4 + 1);
            }
          }
        }
        else if (*(char *)((int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4) + *piVar2 +
                           iVar5 + 0x3b9) == '\x06') goto LAB_100a38a8;
      }
      uVar6 = (unsigned long long)(short)(sVar4 + 1);
    } while ((short)(sVar4 + 1) < 0x9b);
    iVar5 = (int)(short)(sVar1 + 1);
    if (0x6e < (short)(sVar1 + 1)) {
      return;
    }
  } while( true );
}







































































































































































































































































































































































































































































































































































































/* Address: 0x100a39ac Size: 212 bytes */
void FUN_100a39ac()

{
  char cVar1;
  int *piVar2;
  int iVar3;
  int iVar4;
  short sVar5;
  int iVar6;
  
  piVar2 = gMapGenData;
  iVar3 = CheckLandConnectivity();
  if (iVar3 != 0) {
    iVar3 = 0;
    do {
      iVar6 = 0;
      do {
        cVar1 = *(char *)(*piVar2 + iVar6 * 0x70 + iVar3 + 0x34a);
        if (((cVar1 != '\x06') && (cVar1 != '\x05')) &&
           (iVar4 = HasNeighborWithTerrain(iVar3,iVar6,6), iVar4 != 0)) {
          *(char *)(*piVar2 + iVar6 * 0x70 + iVar3 + 0x34a) = 5;
          NotifyTileChanged(iVar3,iVar6);
        }
        sVar5 = (short)iVar6 + 1;
        iVar6 = (int)sVar5;
      } while (sVar5 < 0x9c);
      sVar5 = (short)iVar3 + 1;
      iVar3 = (int)sVar5;
    } while (sVar5 < 0x70);
  }
  return;
}



























































































































































































/* Address: 0x100a3a80 Size: 76 bytes */
void PlaceCitiesAndRoads()

{
  *(short *)(*gMapGenData + 0x348) = 0;
  PlacePlayerCities();
  PlaceNeutralCities();
  AssignCityAlliances();
  GenerateRoadsBetweenCities();
  SmoothTerrainAroundCities();
  EnsureCityCoastalAccess();
  return;
}

/* Address: 0x100a49cc Size: 52 bytes */
void FUN_100a49cc()

{
  FUN_100a3acc();
  FUN_100a40bc();
  FUN_100a40bc();
  return;
}

/* Address: 0x100a6204 Size: 148 bytes */
void FinalizeAndSave()

{
  int *puVar1;
  
  puVar1 = gProgressBar;
  AssignTerrainGraphics();
  ProgressUpdate(*puVar1,0x5c);
  RegisterCities();
  AssignPlayerStartPositions();
  ProgressUpdate(*puVar1,0x5e);
  AssignPlayerHomeCities();
  ProgressUpdate(*puVar1,0x60);
  PlaceHeroesOnMap();
  ProgressUpdate(*puVar1,0x62);
  SaveMapToResources();
  SaveScenarioData();
  return;
}

/* Address: 0x100a6364 Size: 348 bytes */
void ScatterSwampTiles(short param_1,short param_2)

{
  int *piVar1;
  int *piVar2;
  short sVar4;
  int iVar3;
  short sVar5;
  short sVar6;
  short local_28;
  short local_26 [19];
  
  piVar2 = gMapGenConfig;
  piVar1 = gMapGenData;
  sVar4 = RandomRange(1,5,3);
  sVar6 = 0;
  if (0 < sVar4) {
    do {
      iVar3 = RandomRange(1,8,0xffffffffffffffff);
      sVar5 = RandomRange(1,3,0);
      local_26[0] = param_1 + sVar5 * *(short *)(*piVar2 + iVar3 * 4 + 0xbc);
      iVar3 = RandomRange(1,8,0xffffffffffffffff);
      sVar5 = RandomRange(1,3,0);
      local_28 = param_2 + sVar5 * *(short *)(*piVar2 + iVar3 * 4 + 0xbe);
      ClampToMapBounds(local_26,&local_28);
      iVar3 = *piVar1 + local_28 * 0x70 + (int)local_26[0];
      if (*(char *)(iVar3 + 0x34a) == '\a') {
        *(char *)(iVar3 + 0x34a) = 8;
        NotifyTileChanged(local_26[0],local_28);
      }
      sVar6 = sVar6 + 1;
    } while (sVar6 < sVar4);
  }
  return;
}













































































































































































































/* Address: 0x100a64c0 Size: 488 bytes */
void PlaceSwampCluster()

{
  short sVar1;
  short sVar2;
  int bVar3;
  int *piVar4;
  int iVar6;
  unsigned long long uVar5;
  int iVar7;
  short sVar8;
  
  piVar4 = gMapGenData;
  sVar8 = 0;
  bVar3 = false;
  do {
    iVar6 = RandomRange(1,0x66,5);
    uVar5 = RandomRange(1,0x92,5);
    sVar2 = (short)uVar5;
    sVar1 = (short)iVar6;
    if (*(char *)(*piVar4 + (int)(((uVar5 & 0x1fffffff) * 8 - uVar5 & 0xffffffff) << 4) + iVar6 +
                 0x34a) == '\a') {
      if (sVar8 < 4) {
        iVar7 = HasNeighborWithTerrain(iVar6,uVar5,3);
        if (iVar7 != 0) {
          bVar3 = true;
        }
        iVar7 = HasNeighborWithTerrain(sVar1 + 1,uVar5,3);
        if (iVar7 != 0) {
          bVar3 = true;
        }
        iVar7 = HasNeighborWithTerrain(iVar6,sVar2 + 1,3);
        if (iVar7 != 0) {
          bVar3 = true;
        }
        iVar7 = HasNeighborWithTerrain(sVar1 + 1,sVar2 + 1,3);
        if (iVar7 != 0) {
          bVar3 = true;
        }
        sVar8 = sVar8 + 1;
      }
      else {
        bVar3 = true;
      }
    }
  } while (!bVar3);
  *(char *)
   (*piVar4 + (int)(((uVar5 & 0x1fffffff) * 8 - uVar5 & 0xffffffff) << 4) + iVar6 + 0x34a) = 8;
  NotifyTileChanged(iVar6,uVar5);
  ScatterSwampTiles(iVar6,uVar5);
  ScatterSwampTiles(sVar1 + 1,sVar2 + 1);
  ScatterSwampTiles(sVar1 + -1,sVar2 + -1);
  ScatterSwampTiles(sVar1 + -1,sVar2 + 1);
  ScatterSwampTiles(sVar1 + 1,sVar2 + -1);
  return;
}











































































































































































































































































































































/* Address: 0x100a66a8 Size: 96 bytes */
void PlaceSwampClusters()

{
  short sVar1;
  short sVar2;
  
  sVar1 = RandomRange(1,3,0);
  sVar2 = 0;
  if (0 < sVar1) {
    do {
      PlaceSwampCluster();
      sVar2 = sVar2 + 1;
    } while (sVar2 < sVar1);
  }
  return;
}

/* Address: 0x100a6f38 Size: 784 bytes */
void PlaceRuins()

{
  int *puVar1;
  int *puVar2;
  unsigned int *puVar3;
  int *piVar4;
  unsigned int *puVar5;
  int *ppuVar6;
  unsigned long long uVar7;
  unsigned long long uVar8;
  unsigned long long uVar9;
  int iVar10;
  short sVar11;
  short sVar12;
  long long lVar13;
  int iVar15;
  unsigned long long uVar14;
  int **local_16c;
  char auStack_148 [16];
  short local_138;
  short local_136;
  char auStack_134 [308];
  
  puVar5 = puRam101175f4;
  piVar4 = gMapGenData;
  puVar3 = puRam1011735c;
  puVar2 = gProgressBar;
  *(short *)(*puRam1011735c + 0x810) = 0x28;
  sVar12 = 0;
  iVar15 = 0;
  do {
    *(short *)(*puVar3 + iVar15 * 0x20 + 0x814) = 0xffff;
    iVar10 = iVar15 * 0x20 + *puVar3;
    *(short *)(iVar10 + 0x812) = *(short *)(iVar10 + 0x814);
    sVar11 = (short)iVar15 + 1;
    iVar15 = (int)sVar11;
  } while (sVar11 < 0x28);
  uVar14 = 0;
  ppuVar6 = 0 /* TVect base */;
  if (0 < *(short *)(*puVar3 + 0x810)) {
    do {
      sVar11 = (short)((int)(((uVar14 & 0x3fffffff) * 4 + uVar14 & 0xffffffff) << 1) /
                      (int)*(short *)(*puVar3 + 0x810));
      if (sVar12 != sVar11) {
        ProgressUpdate(*puVar2,sVar11 + 0x46);
        sVar12 = sVar11;
      }
      FindRuinsLocation(7,&local_136,&local_138);
      lVar13 = (uVar14 & 0x7ffffff) * 0x20;
      iVar15 = (int)lVar13;
      *(short *)(*puVar3 + iVar15 + 0x812) = local_136;
      *(short *)(iVar15 + *puVar3 + 0x814) = local_138;
      *(char *)(*piVar4 + local_138 * 0x70 + (int)local_136 + 0x34a) = 0xb;
      if (*(char *)(iVar15 + *puVar3 + 0x82a) == '\x01') {
        uVar7 = RandomRange(1,10,0xffffffffffffffff);
        puVar1 = (*(int*)((char*)ppuVar6 - 0x422));
        DrawNumber(auStack_134,ZEXT48(puVar1) + 0x44,
                     (unsigned long long)*puVar5 + ((uVar7 & 0x3fffffff) * 4 + uVar7 & 0x7fffffff) * 2 +
                     0x1000);
        lVar13 = (uVar14 & 0x7ffffff) * 0x20;
        BlockMoveData_Thunk((unsigned long long)*puVar3 + lVar13 + 0x816,auStack_134);
        DrawNumber(auStack_134,ZEXT48(puVar1) + 0x50,uVar14,lVar13 + (unsigned long long)*puVar3 + 0x816);
      }
      else {
        uVar7 = RandomRange(1,*(short *)(*puVar5 + 0xed2),0xffffffffffffffff);
        uVar8 = RandomRange(1,10,0xffffffffffffffff);
        uVar9 = RandomRange(1,10,0xffffffffffffffff);
        DrawNumber(auStack_148,(*(int*)((char*)ppuVar6 - 0x422)),
                     (unsigned long long)*puVar5 + ((uVar9 & 0x3fffffff) * 4 + uVar9 & 0x7fffffff) * 2 +
                     0xf38,(unsigned long long)*puVar5 + ((uVar8 & 0x3fffffff) * 4 + uVar8 & 0x7fffffff) * 2
                           + 0xf9c);
        DrawNumber(auStack_134,
                     (unsigned long long)*puVar5 + ((uVar7 & 0x3fffffff) * 4 + uVar7 & 0x3fffffff) * 4 +
                     0xed4,auStack_148);
        BlockMoveData_Thunk(lVar13 + (unsigned long long)*puVar3 + 0x816,auStack_134);
        DrawNumber(auStack_134,ZEXT48(local_16c[-0x422]) + 8,uVar14,
                     lVar13 + (unsigned long long)*puVar3 + 0x816);
      }
      BlockMoveData_Thunk(*puVar5 + (int)*(short *)(*puVar5 + 0x1066) + 0x3e96,auStack_134);
      sVar11 = StringLength(auStack_134);
      *(short *)(*puVar5 + 0x1066) = *(short *)(*puVar5 + 0x1066) + sVar11;
      sVar11 = (short)uVar14 + 1;
      uVar14 = (unsigned long long)sVar11;
      ppuVar6 = local_16c;
    } while (sVar11 < *(short *)(*puVar3 + 0x810));
  }
  return;
}

















































































































































































































































































































































































































































































































































/* Address: 0x100a9628 Size: 1504 bytes */
void FUN_100a9628()

{
  char cVar1;
  short sVar2;
  int *piVar3;
  short sVar5;
  int iVar4;
  unsigned short uVar6;
  int iVar7;
  unsigned long long uVar8;
  int iVar9;
  
  piVar3 = gMapGenData;
  iVar7 = 0;
  do {
    uVar8 = 0;
    do {
      cVar1 = *(char *)(*piVar3 + (int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) +
                        iVar7 + 0x34a);
      uVar6 = 0;
      if (((cVar1 == '\a') || (cVar1 == '\x05')) || (cVar1 == '\x06')) {
        cVar1 = *(char *)((int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) + *piVar3 +
                          iVar7 + 0x3ba);
        if ((cVar1 == '\x03') || (cVar1 == '\x02')) {
          uVar6 = 1;
        }
        cVar1 = *(char *)((int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) + *piVar3 +
                          iVar7 + 0x2da);
        if ((cVar1 == '\x03') || (cVar1 == '\x02')) {
          uVar6 = uVar6 + 1;
        }
        cVar1 = *(char *)(*piVar3 + (int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) +
                          iVar7 + 0x34b);
        if ((cVar1 == '\x03') || (cVar1 == '\x02')) {
          uVar6 = uVar6 + 1;
        }
        iVar9 = *piVar3 + (int)uVar8 * 0x70;
        if (*(char *)(iVar9 + iVar7 + 0x349) == '\x03') {
LAB_100a9768:
          uVar6 = uVar6 + 1;
        }
        else if (*(char *)(iVar9 + iVar7 + 0x349) == '\x02') goto LAB_100a9768;
        if (2 < uVar6) {
          *(char *)
           (*piVar3 + (int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) + iVar7 + 0x34a) =
               2;
          NotifyTileChanged(iVar7,uVar8);
        }
      }
      sVar5 = (short)uVar8 + 1;
      uVar8 = (unsigned long long)sVar5;
    } while (sVar5 < 0x9c);
    sVar5 = (short)iVar7 + 1;
    iVar7 = (int)sVar5;
  } while (sVar5 < 0x70);
  iVar7 = 0;
  do {
    iVar9 = 0;
    do {
      if ((((*(char *)(*piVar3 + iVar9 * 0x70 + iVar7 + 0x34a) == '\x03') &&
           (iVar4 = HasNeighborWithTerrain(iVar7,iVar9,7), iVar4 == 0)) &&
          (iVar4 = HasNeighborWithTerrain(iVar7,iVar9,5), iVar4 == 0)) &&
         (iVar4 = HasNeighborWithTerrain(iVar7,iVar9,6), iVar4 == 0)) {
        *(char *)(iVar9 * 0x70 + *piVar3 + iVar7 + 0x34a) = 2;
        NotifyTileChanged(iVar7,iVar9);
      }
      sVar5 = (short)iVar9 + 1;
      iVar9 = (int)sVar5;
    } while (sVar5 < 0x9c);
    sVar5 = (short)iVar7 + 1;
    iVar7 = (int)sVar5;
  } while (sVar5 < 0x70);
  iVar7 = 1;
  do {
    uVar8 = 1;
    do {
      cVar1 = *(char *)(*piVar3 + (int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) +
                        iVar7 + 0x34a);
      sVar2 = (short)iVar7;
      sVar5 = (short)uVar8;
      if ((cVar1 == '\x02') || (cVar1 == '\x03')) {
        cVar1 = *(char *)((int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) + *piVar3 +
                          iVar7 + 0x3bb);
        if (((cVar1 == '\x02') || (cVar1 == '\x03')) &&
           ((iVar9 = (int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) + *piVar3,
            *(char *)(iVar9 + iVar7 + 0x2d9) == '\x02' ||
            (*(char *)(iVar9 + iVar7 + 0x2d9) == '\x03')))) {
          cVar1 = *(char *)((int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) + *piVar3 +
                            iVar7 + 0x2db);
          if ((cVar1 == '\x05') || ((cVar1 == '\x04' || (cVar1 == '\a')))) {
            iVar9 = (int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) + *piVar3;
            if (*(char *)(iVar9 + iVar7 + 0x3b9) != '\x05') {
              cVar1 = *(char *)(iVar7 + iVar9 + 0x3b9);
              if ((cVar1 != '\x04') && (cVar1 != '\a')) goto LAB_100a9a50;
            }
            iVar9 = RandomRange(1,10,0);
            if (iVar9 < 5) {
              *(char *)
               (*piVar3 + (int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) + iVar7 +
               0x2db) = 3;
              NotifyTileChanged(sVar2 + 1,sVar5 + -1);
            }
            else {
              *(char *)
               (*piVar3 + (int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) + iVar7 + -1 +
               0x3ba) = 3;
              NotifyTileChanged((short)(iVar7 + -1),sVar5 + 1);
            }
            goto LAB_100a9bd4;
          }
        }
LAB_100a9a50:
        iVar9 = (int)uVar8 * 0x70 + *piVar3;
        if (*(char *)(iVar9 + iVar7 + 0x3b9) != '\x02') {
          if (*(char *)(iVar9 + iVar7 + 0x3b9) != '\x03') goto LAB_100a9bd4;
        }
        cVar1 = *(char *)((int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) + *piVar3 +
                          iVar7 + 0x2db);
        if ((cVar1 == '\x02') || (cVar1 == '\x03')) {
          iVar9 = (int)uVar8 * 0x70 + *piVar3;
          if (*(char *)(iVar9 + iVar7 + 0x2d9) != '\x05') {
            cVar1 = *(char *)(iVar7 + iVar9 + 0x2d9);
            if ((cVar1 != '\x04') && (cVar1 != '\a')) goto LAB_100a9bd4;
          }
          cVar1 = *(char *)((int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) + *piVar3 +
                            iVar7 + 0x3bb);
          if ((cVar1 == '\x05') || ((cVar1 == '\x04' || (cVar1 == '\a')))) {
            iVar9 = RandomRange(1,10,0);
            if (iVar9 < 5) {
              *(char *)
               ((int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) + *piVar3 + iVar7 + -1 +
               0x2da) = 3;
              NotifyTileChanged((short)(iVar7 + -1),sVar5 + -1);
            }
            else {
              *(char *)
               ((int)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 4) + *piVar3 + iVar7 +
               0x3bb) = 3;
              NotifyTileChanged(sVar2 + 1,sVar5 + 1);
            }
          }
        }
      }
LAB_100a9bd4:
      uVar8 = (unsigned long long)(short)(sVar5 + 1);
    } while ((short)(sVar5 + 1) < 0x9b);
    iVar7 = (int)(short)(sVar2 + 1);
    if (0x6e < (short)(sVar2 + 1)) {
      return;
    }
  } while( true );
}

















































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100a9c08 Size: 216 bytes */
void PlaceRiverWaterChannels()

{
  int *piVar1;
  short sVar2;
  
  piVar1 = gMapGenConfig;
  sVar2 = 0;
  if (0 < *(short *)(*gMapGenConfig + 0x3a)) {
    do {
      CutWaterChannel(0,1,0);
      sVar2 = sVar2 + 1;
    } while (sVar2 < *(short *)(*piVar1 + 0x3a));
  }
  sVar2 = 0;
  if (0 < *(short *)(*piVar1 + 0x38)) {
    do {
      CutWaterChannel(1,1,0);
      sVar2 = sVar2 + 1;
    } while (sVar2 < *(short *)(*piVar1 + 0x38));
  }
  ErodeLandTiles();
  ExpandCoastalTiles();
  SmoothTerrainAroundCities();
  EnsureCityCoastalAccess();
  ErodeLandTiles();
  ExpandCoastalTiles();
  return;
}

/* Address: 0x100ab22c Size: 148 bytes */
void FUN_100ab22c()

{
  unsigned int *puVar1;
  unsigned int *puVar2;
  unsigned long long uVar3;
  short sVar4;
  unsigned long long uVar5;
  
  puVar2 = puRam101175f4;
  puVar1 = puRam1011735c;
  uVar5 = 0;
  do {
    uVar3 = RandomRange(1,5,0xffffffffffffffff);
    BlockMoveData_Thunk((unsigned long long)*puVar1 + ((uVar5 & 0x3fffffff) * 4 + uVar5 & 0x3fffffff) * 4,
                 (unsigned long long)*puVar2 + uVar5 * 100 +
                 ((uVar3 & 0x3fffffff) * 4 + uVar3 & 0x3fffffff) * 4 + 0x764);
    sVar4 = (short)uVar5 + 1;
    uVar5 = (unsigned long long)sVar4;
  } while (sVar4 < 8);
  return;
}































































































































/* Address: 0x100ab334 Size: 52 bytes */
void FUN_100ab334()

{
  FUN_100a7544();
  AssignPlayerColors();
  RollPlayerGold();
  return;
}

/* Address: 0x100ab9e4 Size: 384 bytes */
void FUN_100ab9e4()

{
  int iVar1;
  int *piVar2;
  short sVar3;
  unsigned char bVar4;
  int iVar5;
  unsigned long long uVar6;
  
  piVar2 = gMapGenData;
  iVar5 = 0;
  do {
    uVar6 = 0;
    do {
      if ((((iVar5 != 0) && ((int)uVar6 != 0)) && (iVar5 != 0x6f)) &&
         (((int)uVar6 != 0x9b &&
          (iVar1 = *piVar2 + (int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4) + iVar5,
          *(char *)(iVar1 + 0x34a) == '\x05')))) {
        bVar4 = *(char *)(iVar1 + 0x3ba) == '\x06';
        if (*(char *)(*piVar2 + (int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4) + iVar5
                     + 0x2da) == '\x06') {
          bVar4 = bVar4 + 1;
        }
        if (*(char *)(*piVar2 + (int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4) + iVar5
                     + 0x34b) == '\x06') {
          bVar4 = bVar4 + 1;
        }
        if (*(char *)(*piVar2 + (int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4) + iVar5
                     + 0x349) == '\x06') {
          bVar4 = bVar4 + 1;
        }
        if (2 < bVar4) {
          *(char *)
           (*piVar2 + (int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4) + iVar5 + 0x34a) =
               6;
          NotifyTileChanged(iVar5,uVar6);
        }
      }
      sVar3 = (short)uVar6 + 1;
      uVar6 = (unsigned long long)sVar3;
    } while (sVar3 < 0x9c);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 0x70);
  return;
}





















































































































































































































































































/* Address: 0x100abb68 Size: 384 bytes */
void FUN_100abb68()

{
  int iVar1;
  int *piVar2;
  short sVar3;
  unsigned char bVar4;
  int iVar5;
  unsigned long long uVar6;
  
  piVar2 = gMapGenData;
  iVar5 = 0;
  do {
    uVar6 = 0;
    do {
      if ((((iVar5 != 0) && ((int)uVar6 != 0)) && (iVar5 != 0x6f)) &&
         (((int)uVar6 != 0x9b &&
          (iVar1 = *piVar2 + (int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4) + iVar5,
          *(char *)(iVar1 + 0x34a) == '\a')))) {
        bVar4 = *(char *)(iVar1 + 0x3ba) == '\x05';
        if (*(char *)(*piVar2 + (int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4) + iVar5
                     + 0x2da) == '\x05') {
          bVar4 = bVar4 + 1;
        }
        if (*(char *)(*piVar2 + (int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4) + iVar5
                     + 0x34b) == '\x05') {
          bVar4 = bVar4 + 1;
        }
        if (*(char *)(*piVar2 + (int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4) + iVar5
                     + 0x349) == '\x05') {
          bVar4 = bVar4 + 1;
        }
        if (2 < bVar4) {
          *(char *)
           (*piVar2 + (int)(((uVar6 & 0x1fffffff) * 8 - uVar6 & 0xffffffff) << 4) + iVar5 + 0x34a) =
               5;
          NotifyTileChanged(iVar5,uVar6);
        }
      }
      sVar3 = (short)uVar6 + 1;
      uVar6 = (unsigned long long)sVar3;
    } while (sVar3 < 0x9c);
    sVar3 = (short)iVar5 + 1;
    iVar5 = (int)sVar3;
  } while (sVar3 < 0x70);
  return;
}





















































































































































































































































































/* Address: 0x100abcec Size: 160 bytes */
void PlaceMountainForestChains()

{
  int *piVar1;
  short sVar2;
  
  piVar1 = gMapGenConfig;
  sVar2 = 0;
  if (0 < *(short *)(*gMapGenConfig + 0x40)) {
    do {
      DrawTerrainChain(0);
      sVar2 = sVar2 + 1;
    } while (sVar2 < *(short *)(*piVar1 + 0x40));
  }
  sVar2 = 0;
  if (0 < *(short *)(*piVar1 + 0x3e)) {
    do {
      DrawTerrainChain(1);
      sVar2 = sVar2 + 1;
    } while (sVar2 < *(short *)(*piVar1 + 0x3e));
  }
  SmoothMountainRegions();
  SmoothForestRegions();
  EnsureCityCoastalAccess();
  return;
}

/* Address: 0x100afb5c Size: 148 bytes */
void FUN_100afb5c(int param_1,unsigned long long param_2,unsigned long long param_3)

{
  unsigned long long uVar1;
  
  if ((unsigned long long)*(unsigned int *)(param_1 + 0x1c) < (param_2 & 0xffffffff)) {
    *(int *)(param_1 + 0x1c) = (int)param_2;
  }
  if ((unsigned long long)*(unsigned int *)(param_1 + 0x18) < (param_3 & 0xffffffff)) {
    *(int *)(param_1 + 0x18) = (int)param_3;
  }
  uVar1 = ResourceRead_Dispatch(param_1 + *(short *)(*(int *)(param_1 + 0x10) + 0x30));
  if ((uVar1 & 0xffffffff) < (param_2 & 0xffffffff)) {
    FUN_100afb10(param_1,param_2);
  }
  else {
    FUN_100afb10(param_1,param_3);
  }
  return;
}

/* Address: 0x100b02d0 Size: 84 bytes */
unsigned long long FUN_100b02d0(unsigned long long param_1)

{
  if ((param_1 & 0xffffffff) == 0) {
    param_1 = NewPtr_Thunk(4);
  }
  return param_1;
}

/* Address: 0x100b0578 Size: 4 bytes */
int FUN_100b0578()

{
  return 0;
}

/* Address: 0x100b057c Size: 20 bytes */
long long FUN_100b057c(long long param_1,char param_2)

{
  if (param_2 == '\0') {
    return param_1;
  }
  return param_1 + 4;
}

/* Address: 0x100b06d4 Size: 68 bytes */
void FUN_100b06d4(short *param_1,short *param_2)

{
  *param_1 = *param_1 + *param_2;
  param_1[1] = param_1[1] + param_2[1];
  param_1[2] = param_1[2] - *param_2;
  param_1[3] = param_1[3] - param_2[1];
  return;
}

/* Address: 0x100b08d4 Size: 64 bytes */
void GetScreenBounds(int *param_1,short *param_2)

{
  int local_8;
  
  local_8 = CONCAT22(param_2[2] - *param_2,param_2[3] - param_2[1]);
  *param_1 = local_8;
  return;
}

/* Address: 0x100b0960 Size: 176 bytes */
char * FUN_100b0960(int param_1,long long param_2)

{
  char *puVar1;
  unsigned int uVar2;
  char *puVar3;
  unsigned int local_10 [4];
  
  puVar1 = (char *)
           ResourceRead_Dispatch(param_1 + *(short *)(*(int *)(param_1 + 0x10) + 0x60),param_2,local_10);
  if (puVar1 != (char *)0x0) {
    if ((*(char *)(param_1 + 4) != '\0') && (uVar2 = 0, puVar3 = puVar1, local_10[0] != 0)) {
      do {
        *puVar3 = 0;
        puVar3 = puVar3 + 1;
        uVar2 = uVar2 + 1;
      } while (uVar2 < local_10[0]);
    }
    *(unsigned int *)(param_1 + 8) = *(int *)(param_1 + 8) + local_10[0];
    *(int *)(param_1 + 0xc) = *(int *)(param_1 + 0xc) + 1;
  }
  return puVar1;
}

/* Address: 0x100b0a18 Size: 192 bytes */
void FUN_100b0a18(int param_1,char *param_2)

{
  unsigned int uVar1;
  unsigned int uVar2;
  char *puVar3;
  
  if (param_2 != (char *)0x0) {
    uVar1 = ResourceRead_Dispatch(param_1 + *(short *)(*(int *)(param_1 + 0x10) + 0x68),param_2);
    if (*(char *)(param_1 + 5) != '\0') {
      uVar2 = 0;
      puVar3 = param_2;
      if (uVar1 != 0) {
        do {
          *puVar3 = 0;
          puVar3 = puVar3 + 1;
          uVar2 = uVar2 + 1;
        } while (uVar2 < uVar1);
      }
    }
    ResourceRead_Dispatch(param_1 + *(short *)(*(int *)(param_1 + 0x10) + 0x70),param_2);
    *(unsigned int *)(param_1 + 8) = *(int *)(param_1 + 8) - uVar1;
    *(int *)(param_1 + 0xc) = *(int *)(param_1 + 0xc) + -1;
  }
  return;
}

/* Address: 0x100b1058 Size: 220 bytes */
int FUN_100b1058(int param_1,long long param_2,long long param_3,short param_4)

{
  int iVar1;
  int *ppuVar2;
  short sVar3;
  
  ppuVar2 = 0 /* TVect base */;
  if ((param_1 != 0) ||
     (ppuVar2 = 0 /* TVect base */, param_1 = FUN_100b0c24(0xbc), param_1 != 0)) {
    FUN_100af728(param_1,param_2,param_3);
    *(int **)(param_1 + 0x10) = (*(int*)((char*)ppuVar2 - 0x40a));
    *(int **)(param_1 + 0x34) = (*(int*)((char*)ppuVar2 - 0x409));
    FUN_101175c4(param_1 + 0x38,0x10,8,(*(int*)((char*)ppuVar2 - 0x408)));
    *(short *)(param_1 + 0xb8) = param_4;
    *(short *)(param_1 + 0x30) = 0;
    iVar1 = **(int **)(param_1 + 0x34);
    while (iVar1 != 0) {
      sVar3 = *(short *)(param_1 + 0x30) + 1;
      *(short *)(param_1 + 0x30) = sVar3;
      iVar1 = *(int *)(*(int *)(param_1 + 0x34) + sVar3 * 4);
    }
  }
  return param_1;
}

/* Address: 0x100b1134 Size: 220 bytes */
int FUN_100b1134(int param_1,int param_2,long long param_3,long long param_4,
                short param_5)

{
  int iVar1;
  int *ppuVar2;
  short sVar3;
  
  ppuVar2 = 0 /* TVect base */;
  if ((param_1 != 0) ||
     (ppuVar2 = 0 /* TVect base */, param_1 = FUN_100b0c24(0xbc), param_1 != 0)) {
    FUN_100af728(param_1,param_3,param_4);
    *(int **)(param_1 + 0x10) = (*(int*)((char*)ppuVar2 - 0x40a));
    *(int *)(param_1 + 0x34) = param_2;
    FUN_101175c4(param_1 + 0x38,0x10,8,(*(int*)((char*)ppuVar2 - 0x408)));
    *(short *)(param_1 + 0xb8) = param_5;
    *(short *)(param_1 + 0x30) = 0;
    iVar1 = **(int **)(param_1 + 0x34);
    while (iVar1 != 0) {
      sVar3 = *(short *)(param_1 + 0x30) + 1;
      *(short *)(param_1 + 0x30) = sVar3;
      iVar1 = *(int *)(*(int *)(param_1 + 0x34) + sVar3 * 4);
    }
  }
  return param_1;
}

/* Address: 0x100b1210 Size: 36 bytes */
void FUN_100b1210()

{
  FUN_100afb34();
  return;
}

/* Address: 0x100b1b90 Size: 136 bytes */
unsigned int FUN_100b1b90(unsigned char *param_1,long long param_2,unsigned int param_3)

{
  char *puVar1;
  int iVar2;
  unsigned int uVar3;
  char auStack_111 [273];
  
  puVar1 = auStack_111 + 1;
  Sound_BlockCopy(param_1 + 1,puVar1,*param_1);
  puVar1[*param_1] = 0;
  iVar2 = FUN_10002958(auStack_111 + (param_3 & 0xff),param_2);
  if (iVar2 == 0) {
    uVar3 = 0;
  }
  else {
    uVar3 = (iVar2 - (int)puVar1) + 1;
  }
  return uVar3 & 0xff;
}

/* Address: 0x100b1c84 Size: 4 bytes */
int ConvertToString()

{
  return 0;
}

/* Address: 0x100b1c88 Size: 148 bytes */
void FUN_100b1c88(long long param_1,short param_2,short param_3)

{
  unsigned char *pbVar1;
  long long lVar2;
  long long lVar3;
  
  lVar2 = (long long)param_2;
  if ((0 < lVar2) && (0 < (long long)param_3)) {
    pbVar1 = (unsigned char *)param_1;
    if ((int)param_2 <= (int)(unsigned int)*pbVar1) {
      if ((int)(unsigned int)*pbVar1 < (int)param_3 + (int)param_2) {
        *pbVar1 = (char)param_2 - 1;
      }
      else {
        lVar3 = lVar2 + param_3;
        Sound_BlockCopy(lVar3 + param_1,lVar2 + param_1,((unsigned long long)*pbVar1 - lVar3) + 1);
        *pbVar1 = *pbVar1 - (char)param_3;
      }
    }
  }
  return;
}

/* Address: 0x100b1d90 Size: 128 bytes */
char * FUN_100b1d90(char *param_1,long long param_2)

{
  unsigned int uVar1;
  
  if ((int)param_2 == 0) {
    *param_1 = 0;
  }
  else {
    uVar1 = StringLength(param_2);
    if (uVar1 < 0x100) {
      *param_1 = (char)uVar1;
    }
    else {
      *param_1 = 0xff;
    }
    Sound_BlockCopy(param_2,param_1 + 1,*param_1);
  }
  return param_1;
}

/* FUN_100b1e10 defined elsewhere */


/* Address: 0x100b1ed0 Size: 156 bytes */
void FUN_100b1ed0(unsigned char *param_1,unsigned char *param_2,unsigned char *param_3)

{
  *param_1 = 0;
  if ((unsigned int)*param_2 + (unsigned int)*param_3 < 0x100) {
    *param_1 = *param_2 + *param_3;
  }
  else {
    *param_1 = 0xff;
  }
  Sound_BlockCopy(param_2 + 1,param_1 + 1,*param_2);
  Sound_BlockCopy(param_3 + 1,param_1 + *param_2 + 1,(unsigned int)*param_1 - (unsigned int)*param_2);
  return;
}

/* Address: 0x100b226c Size: 68 bytes */
long long FUN_100b226c(short *param_1,short *param_2)

{
  long long uVar1;
  
  if (((*param_1 == *param_2) && (param_1[1] == param_2[1])) && (param_1[2] == param_2[2])) {
    uVar1 = 1;
  }
  else {
    uVar1 = 0;
  }
  return uVar1;
}

/* Address: 0x100b2e74 Size: 888 bytes */
void FUN_100b2e74()

{
  unsigned long long uVar1;
  int iVar2;
  int uVar3;
  unsigned int uVar4;
  int local_198 [4];
  int uStack_188;
  int uStack_184;
  int uStack_180;
  int uStack_17c;
  int local_178;
  int uStack_174;
  int uStack_170;
  int uStack_16c;
  int uStack_168;
  int uStack_164;
  int uStack_160;
  int local_158 [4];
  int uStack_148;
  int uStack_144;
  int uStack_140;
  int uStack_13c;
  int local_138;
  int uStack_134;
  int uStack_130;
  int uStack_12c;
  int uStack_128;
  int uStack_124;
  int uStack_120;
  char auStack_118 [280];
  
  uVar1 = ZEXT48(0 /* TVect base */);
  local_158[0] = *puRam10116910;
  local_158[1] = puRam10116910[1];
  local_158[2] = puRam10116910[2];
  local_158[3] = puRam10116910[3];
  uStack_148 = puRam10116910[4];
  uStack_144 = puRam10116910[5];
  uStack_140 = puRam10116910[6];
  uStack_13c = puRam10116910[7];
  local_138 = puRam10116910[8];
  uStack_134 = puRam10116910[9];
  uStack_130 = puRam10116910[10];
  uStack_12c = puRam10116910[0xb];
  uStack_128 = puRam10116910[0xc];
  uStack_124 = puRam10116910[0xd];
  uStack_120 = puRam10116910[0xe];
  local_198[0] = *puRam10116914;
  local_198[1] = puRam10116914[1];
  local_198[2] = puRam10116914[2];
  local_198[3] = puRam10116914[3];
  uStack_188 = puRam10116914[4];
  uStack_184 = puRam10116914[5];
  uStack_180 = puRam10116914[6];
  uStack_17c = puRam10116914[7];
  local_178 = puRam10116914[8];
  uStack_174 = puRam10116914[9];
  uStack_170 = puRam10116914[10];
  uStack_16c = puRam10116914[0xb];
  uStack_168 = puRam10116914[0xc];
  uStack_164 = puRam10116914[0xd];
  uStack_160 = puRam10116914[0xe];
  if (*pcRam10117378 != '\0') {
    FUN_100b2570();
    FUN_100b25f4(0);
  }
  if (**(char **)((int)uVar1 + -0x528) != '\0') {
    FUN_100b2678();
    FUN_100b2680(0);
  }
  if (**(char **)((int)uVar1 + -0x528) != '\0') {
    FUN_100b26fc();
    FUN_100b2704(0);
  }
  if (**(char **)((int)uVar1 + -0x528) != '\0') {
    FUN_100b2780();
    FUN_100b2788(0);
  }
  if (**(char **)((int)uVar1 + -0x528) != '\0') {
    FUN_100b2804();
    FUN_100b280c(0);
  }
  if (**(char **)((int)uVar1 + -0x528) != '\0') {
    FUN_100b2874();
    FUN_100b287c(0);
  }
  if (**(char **)((int)uVar1 + -0x528) != '\0') {
    FUN_100b28f8();
    FUN_100b2900(0);
  }
  if (**(char **)((int)uVar1 + -0x528) != '\0') {
    FUN_100b2968();
    FUN_100b2970(0);
  }
  if (**(char **)((int)uVar1 + -0x528) != '\0') {
    FUN_100b29d8();
    FUN_100b29e0(0);
  }
  if (**(char **)((int)uVar1 + -0x528) != '\0') {
    FUN_100b2a48();
    FUN_100b2a50(0);
  }
  if (**(char **)((int)uVar1 + -0x528) != '\0') {
    FUN_100b2ab8();
    FUN_100b2ac0(0);
  }
  if (**(char **)((int)uVar1 + -0x528) != '\0') {
    FUN_100b2b28();
    FUN_100b2b30(0);
  }
  if (**(char **)((int)uVar1 + -0x528) != '\0') {
    FUN_100b2bac();
    FUN_100b2bb4(0);
  }
  if (**(char **)((int)uVar1 + -0x528) != '\0') {
    FUN_100b2c1c();
    FUN_100b2c24(0);
  }
  if (**(char **)((int)uVar1 + -0x528) != '\0') {
    FUN_100b2ca0();
    FUN_100b2ca8(0);
  }
  uVar4 = 0;
  do {
    BuildPascalString(auStack_118,local_158[uVar4]);
    FUN_100ed954(auStack_118,local_198[uVar4]);
    iVar2 = (int)uVar1;
    uVar4 = (unsigned int)(short)((short)uVar4 + 1);
  } while (uVar4 < 0xf);
  uVar3 = FUN_100b2680(0);
  **(int **)(iVar2 + -0xec) = uVar3;
  FUN_100b2d54();
  uVar3 = FUN_100b2704(0);
  **(int **)(iVar2 + -0xe8) = uVar3;
  FUN_100b2d84();
  uVar3 = FUN_100b2788(0);
  **(int **)(iVar2 + -0xe4) = uVar3;
  FUN_100b2db4();
  uVar3 = FUN_100b287c(0);
  **(int **)(iVar2 + -0xe0) = uVar3;
  FUN_100b2de4();
  uVar3 = FUN_100b2b30(0);
  **(int **)(iVar2 + -0xdc) = uVar3;
  FUN_100b2e14();
  uVar3 = FUN_100b2c24(0);
  **(int **)(iVar2 + -0xd8) = uVar3;
  FUN_100b2e44();
  return;
}

/* Address: 0x100b4c68 Size: 524 bytes */

void FUN_100b4c68()

{
  int *puVar1;
  int uVar4;
  short sVar7;
  int iVar5;
  int *puVar6;
  char uVar10;
  unsigned long long uVar2;
  long long uVar3;
  short sVar8;
  short sVar9;
  short sVar11;
  int *puVar12;
  int local_174;
  char auStack_14c [256];
  short local_4c;
  int local_48;
  int local_44;
  int local_40;
  int local_3c;
  
  puVar1 = puRam10117370;
  uVar4 = FUN_10000a98(uRam101169c8,0xfd0,1);
  **(int **)(local_174 + -0xed4) = uVar4;
  FocusObject();
  sVar7 = FUN_100023d0(0x61656474);
  MarkChanged();
  local_4c = 0;
  local_48 = 0;
  local_40 = 0;
  local_3c = 0;
  local_44 = *puVar1;
  *puVar1 = auStack_14c;
  iVar5 = TrySetjmp(auStack_14c);
  if (iVar5 == 0) {
    sVar9 = 1;
    if (0 < sVar7) {
      do {
        puVar6 = (int *)FUN_100023e8(0x61656474,sVar9);
        FUN_100db2a4();
        uVar10 = Sound_DetachResourceAlt(puVar6);
        uVar2 = GetHandleByteCount(puVar6);
        iVar5 = local_174;
        CheckMemError();
        sVar11 = (short)((uVar2 & 0xffffffff) / 0xc);
        puVar12 = (int *)*puVar6;
        uVar3 = FUN_100eecf8(*(int *)(iVar5 + -0xed0));
        uVar3 = FUN_10000a98(uVar3,0xfe0,1);
        FocusObject(uVar3);
        sVar8 = 1;
        if (0 < sVar11) {
          do {
            FUN_10002418(*puVar12,puVar12[1],uVar3,puVar12[2],0);
            EndFocus();
            puVar12 = puVar12 + 3;
            sVar8 = sVar8 + 1;
          } while (sVar8 <= sVar11);
        }
        RestoreHandleFlags(puVar6,uVar10);
        ReleaseHandle_Sound(puVar6);
        sVar9 = sVar9 + 1;
      } while (sVar9 <= sVar7);
    }
    *puVar1 = local_44;
  }
  else {
    LockHandleRange(local_4c,local_48);
  }
  return;
}

/* Address: 0x100beb7c Size: 104 bytes */
int * FUN_100beb7c(int *param_1)

{
  int *ppuVar1;
  
  ppuVar1 = 0 /* TVect base */;
  if ((param_1 != (int *)0x0) ||
     (ppuVar1 = 0 /* TVect base */, param_1 = (int *)NewPtr_Thunk(0x24),
     param_1 != (int *)0x0)) {
    FUN_100e904c(param_1);
    *param_1 = (*(int*)((char*)ppuVar1 - 0x36d));
  }
  return param_1;
}

/* Address: 0x100becb8 Size: 36 bytes */
void FUN_100becb8()

{
  FUN_100e9124();
  return;
}

/* Address: 0x100c0f10 Size: 96 bytes */
void FUN_100c0f10()

{
  int *ppuVar1;
  int *piVar2;
  
  ppuVar1 = 0 /* TVect base */;
  piVar2 = (int *)FUN_100c0b74(0);
  FUN_100c0c18();
  *(int **)(*(int*)((char*)ppuVar1 - 0x396)) = piVar2;
  ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 200));
  return;
}

/* Address: 0x100c2120 Size: 176 bytes */
int FUN_100c2120(long long param_1)

{
  int *piVar1;
  int *piVar2;
  int *ppuVar3;
  int iVar4;
  int local_120;
  char auStack_11c [264];
  int local_14 [5];
  
  piVar2 = piRam10117804;
  ppuVar3 = 0 /* TVect base */;
  local_14[0] = 0;
  FUN_100c19e4(auStack_11c,param_1,0,*piRam10117804);
  piVar1 = (int *)*piVar2;
  iVar4 = ResourceRead_Dispatch((int)piVar1 + (int)*(short *)(*piVar1 + 0x110),(*(int*)((char*)ppuVar3 - 0x346)),auStack_11c,
                       &local_120);
  if (iVar4 != 0) {
    piVar2 = (int *)*piVar2;
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0xd8),local_120,local_14,1);
  }
  return local_14[0];
}

/* Address: 0x100c43ec Size: 8 bytes */
int FUN_100c43ec()

{
  return uRam10116bbc;
}

/* Address: 0x100c43fc Size: 292 bytes */
int * FUN_100c43fc(int *param_1)

{
  int uVar1;
  int *puVar2;
  int *ppuVar3;
  
  puVar2 = puRam10116bc0;
  ppuVar3 = 0 /* TVect base */;
  if ((param_1 != (int *)0x0) ||
     (ppuVar3 = 0 /* TVect base */, param_1 = (int *)NewPtr_Thunk(0x84),
     param_1 != (int *)0x0)) {
    ConstructCommand(param_1);
    *param_1 = (*(int*)((char*)ppuVar3 - 0x337));
    FUN_100b02d0(param_1 + 0x1f);
    *(char *)(param_1 + 0xf) = 0;
    uVar1 = puVar2[1];
    param_1[0xd] = *puVar2;
    param_1[0xe] = uVar1;
    param_1[0x10] = 0;
    *(char *)((int)param_1 + 0x3e) = 0;
    param_1[0xc] = 0;
    *(char *)((int)param_1 + 0x3d) = 1;
    *(char *)(param_1 + 0x11) = 0;
    uVar1 = puVar2[1];
    param_1[0x12] = *puVar2;
    param_1[0x13] = uVar1;
    uVar1 = puVar2[1];
    param_1[0x14] = *puVar2;
    param_1[0x15] = uVar1;
    uVar1 = puVar2[1];
    param_1[0x16] = *puVar2;
    param_1[0x17] = uVar1;
    uVar1 = puVar2[1];
    param_1[0x18] = *puVar2;
    param_1[0x19] = uVar1;
    uVar1 = puVar2[1];
    param_1[0x1a] = *puVar2;
    param_1[0x1b] = uVar1;
    uVar1 = puVar2[1];
    param_1[0x1c] = *puVar2;
    param_1[0x1d] = uVar1;
    param_1[0x1e] = 0;
    param_1[0x1f] = *(int *)(*(int*)((char*)ppuVar3 - 0x398));
    *(char *)(param_1 + 0x20) = 0;
  }
  return param_1;
}

/* FUN_100c5dcc defined elsewhere */


/* Address: 0x100c604c Size: 112 bytes */
int * FUN_100c604c(int *param_1)

{
  int *ppuVar1;
  
  ppuVar1 = 0 /* TVect base */;
  if ((param_1 != (int *)0x0) ||
     (ppuVar1 = 0 /* TVect base */, param_1 = (int *)NewPtr_Thunk(0x20),
     param_1 != (int *)0x0)) {
    FUN_100d94c8(param_1);
    *param_1 = (*(int*)((char*)ppuVar1 - 0x323));
    param_1[7] = 0;
  }
  return param_1;
}

/* Address: 0x100c7d84 Size: 136 bytes */
int * FUN_100c7d84(int *param_1)

{
  int *ppuVar1;
  
  ppuVar1 = 0 /* TVect base */;
  if ((param_1 != (int *)0x0) ||
     (ppuVar1 = 0 /* TVect base */, param_1 = (int *)NewPtr_Thunk(0xc0),
     param_1 != (int *)0x0)) {
    ConstructExtendedObject(param_1);
    *param_1 = (*(int*)((char*)ppuVar1 - 0x317));
    param_1[0x2a] = 0;
    *(short *)(param_1 + 0x2b) = 0;
    param_1[0x2c] = 0;
    param_1[0x2d] = 0;
    param_1[0x2e] = 0;
    *(short *)(param_1 + 0x2f) = 0;
  }
  return param_1;
}

/* Address: 0x100c9950 Size: 8 bytes */
int FUN_100c9950()

{
  return uRam10116c54;
}

/* Address: 0x100c9960 Size: 120 bytes */
int * FUN_100c9960(int *param_1)

{
  int *ppuVar1;
  
  ppuVar1 = 0 /* TVect base */;
  if ((param_1 != (int *)0x0) ||
     (ppuVar1 = 0 /* TVect base */, param_1 = (int *)NewPtr_Thunk(0xc4),
     param_1 != (int *)0x0)) {
    FUN_100c7d84(param_1);
    *param_1 = (*(int*)((char*)ppuVar1 - 0x312));
    *(char *)(param_1 + 0x30) = 1;
    param_1[0x20] = 0xe;
  }
  return param_1;
}

/* Address: 0x100ca068 Size: 8 bytes */
int FUN_100ca068()

{
  return uRam10116c60;
}

/* Address: 0x100ca078 Size: 112 bytes */
int * FUN_100ca078(int *param_1)

{
  int *ppuVar1;
  
  ppuVar1 = 0 /* TVect base */;
  if ((param_1 != (int *)0x0) ||
     (ppuVar1 = 0 /* TVect base */, param_1 = (int *)NewPtr_Thunk(200),
     param_1 != (int *)0x0)) {
    FUN_100c9960(param_1);
    *param_1 = (*(int*)((char*)ppuVar1 - 0x30f));
    param_1[0x31] = 0;
  }
  return param_1;
}

/* Address: 0x100d7adc Size: 156 bytes */
int * FUN_100d7adc(int *param_1)

{
  int *ppuVar1;
  
  ppuVar1 = 0 /* TVect base */;
  if ((param_1 != (int *)0x0) ||
     (ppuVar1 = 0 /* TVect base */, param_1 = (int *)NewPtr_Thunk(0x20),
     param_1 != (int *)0x0)) {
    FUN_100f4dc8(param_1);
    *param_1 = (*(int*)((char*)ppuVar1 - 0x280));
    param_1[1] = 0;
    param_1[5] = 0;
    param_1[4] = 6;
    *(short *)(param_1 + 3) = 1;
    *(short *)((int)param_1 + 0xe) = 0;
    *(char *)(param_1 + 6) = 0;
    param_1[2] = 0;
    param_1[7] = 0;
  }
  return param_1;
}

/* Address: 0x100d7ba0 Size: 276 bytes */
void FUN_100d7ba0(int *param_1,long long param_2,short param_3)

{
  int *puVar1;
  int iVar2;
  short sVar3;
  char auStack_138 [256];
  short local_38;
  int local_34;
  int local_30;
  int local_2c;
  int local_28;
  
  puVar1 = puRam10117370;
  FUN_100f5274(param_1);
  param_1[2] = 0;
  *(short *)(param_1 + 3) = param_3;
  param_1[5] = 0;
  *(short *)((int)param_1 + 0xe) = 0;
  if (0 < (long long)param_3 + -1) {
    do {
      sVar3 = *(short *)((int)param_1 + 0xe) + 1;
      *(short *)((int)param_1 + 0xe) = sVar3;
    } while (0 < param_3 + -1 >> ((int)sVar3 & 0x3fU));
  }
  local_38 = 0;
  local_34 = 0;
  local_2c = 0;
  local_28 = 0;
  local_30 = *puVar1;
  *puVar1 = auStack_138;
  iVar2 = TrySetjmp(auStack_138);
  if (iVar2 == 0) {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xb0),param_2);
    *puVar1 = local_30;
  }
  else {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x90));
    LockHandleRange(local_38,local_34);
  }
  return;
}

/* Address: 0x100da260 Size: 96 bytes */
void FUN_100da260(int *param_1,long long param_2)

{
  long long uVar1;
  
  uVar1 = ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0xb0));
  if ((int)uVar1 != 0) {
    FUN_100da1d0(uVar1,param_2);
  }
  return;
}

/* FUN_100da3f0 defined elsewhere */


/* Address: 0x100db1ec Size: 64 bytes */
void CheckMemError()

{
  long long uVar1;
  
  uVar1 = FUN_10002b20();
  if ((int)uVar1 != 0) {
    LockHandleRange(uVar1,0);
  }
  return;
}









































/* Address: 0x100e1674 Size: 68 bytes */
void FUN_100e1674(int *param_1,int *param_2)

{
  *param_1 = *param_1 - *param_2;
  param_1[1] = param_1[1] - param_2[1];
  param_1[2] = param_1[2] - *param_2;
  param_1[3] = param_1[3] - param_2[1];
  return;
}

/* Address: 0x100e8cdc Size: 8 bytes */
int InitEventSystem()

{
  return uRam10116ff0;
}

/* Address: 0x100e8cec Size: 200 bytes */
int * ConfigureEventSystem(int *param_1)

{
  int *ppuVar1;
  int *piVar2;
  int local_18;
  int local_14 [5];
  
  ppuVar1 = 0 /* TVect base */;
  piVar2 = &local_18;
  if ((param_1 != (int *)0x0) ||
     (ppuVar1 = 0 /* TVect base */, param_1 = (int *)NewPtr_Thunk(0x128),
     param_1 != (int *)0x0)) {
    ConstructSimpleObject(param_1);
    *param_1 = (*(int*)((char*)ppuVar1 - 0x22b));
    *(char *)(param_1 + 8) = 0;
    param_1[0x49] = 0;
    FUN_100b1d90(param_1 + 8,ZEXT48((*(int*)((char*)ppuVar1 - 0x22d))) + 0x18);
    local_14[0] = FUN_10000eb8();
    local_18 = 0xff;
    if (local_14[0] < 0xff) {
      piVar2 = local_14;
    }
    param_1[0x48] = *piVar2;
  }
  return param_1;
}

/* Address: 0x100e90bc Size: 104 bytes */
int * FUN_100e90bc(int *param_1)

{
  int *ppuVar1;
  
  ppuVar1 = 0 /* TVect base */;
  if ((param_1 != (int *)0x0) ||
     (ppuVar1 = 0 /* TVect base */, param_1 = (int *)NewPtr_Thunk(0x24),
     param_1 != (int *)0x0)) {
    FUN_100e904c(param_1);
    *param_1 = (*(int*)((char*)ppuVar1 - 0x226));
  }
  return param_1;
}

/* Address: 0x100e9e78 Size: 8 bytes */
int FUN_100e9e78()

{
  return uRam10117018;
}

/* Address: 0x100ebbe0 Size: 136 bytes */
void FUN_100ebbe0(int param_1)

{
  int *piVar1;
  int uVar2;
  
  if (*(int *)(param_1 + 0x10) != 0) {
    uVar2 = FUN_100eb8d0(param_1);
    *(int *)(*(int *)(param_1 + 0x10) + 4) = uVar2;
    piVar1 = *(int **)(param_1 + 0x10);
    if ((*(char *)(piVar1 + 6) != '\0') && (piVar1[1] == 0)) {
      ResourceRead_Dispatch((int)*(short *)(*piVar1 + 0x90) + (int)piVar1);
      *(int *)(param_1 + 0x10) = 0;
    }
  }
  return;
}

/* Address: 0x100ec5c8 Size: 100 bytes */
void FUN_100ec5c8(int param_1,long long param_2)

{
  long long uVar1;
  int uVar2;
  
  uVar1 = FUN_100eecf8(param_2);
  uVar2 = FUN_10000a98(uVar1,0,1);
  *(int *)(param_1 + 8) = uVar2;
  FocusObject();
  FUN_100f53ec(param_1,0xffffffffffffa9f4,*(int *)(param_1 + 8));
  return;
}

/* Address: 0x100ecf88 Size: 1436 bytes */
void FUN_100ecf88(short *param_1)

{
  char uVar3;
  int iVar2;
  long long lVar1;
  unsigned short uVar4;
  unsigned long long uVar5;
  int local_60;
  int local_5c;
  int local_58;
  int local_54;
  unsigned int local_40;
  int local_20;
  int local_18 [6];
  
  FUN_10000648(0x76657273,local_18);
  *param_1 = (short)local_18[0];
  FUN_10000648(0x6d616368,local_18);
  uVar4 = (unsigned short)local_18[0];
  param_1[1] = uVar4;
  uVar5 = (long long)((int)(short)uVar4 >> 0x1f) + (unsigned long long)(2 < uVar4);
  *(char *)(param_1 + 10) = (char)uVar5;
  if ((uVar5 & 0xff) == 0) {
    lVar1 = FUN_10001020();
    *(unsigned char *)((int)param_1 + 0x15) = (unsigned char)((-1 - lVar1) + (unsigned long long)(lVar1 == 0) >> 0x18) >> 7;
  }
  else {
    *(char *)((int)param_1 + 0x15) = 1;
  }
  FUN_10000648(0x73797376,local_18);
  param_1[2] = (short)local_18[0];
  FUN_10000648(0x70726f63,local_18);
  param_1[3] = (short)local_18[0];
  FUN_10000648(0x66707520,local_18);
  *(int *)(param_1 + 4) = local_18[0] != 0;
  FUN_10000648(0x71642020,local_18);
  *(int *)((int)param_1 + 9) = local_18[0] != 0;
  FUN_10000648(0x71642020,local_18);
  if ((*(char *)((int)param_1 + 9) == '\0') || (local_18[0] == 0x100)) {
    uVar3 = 0;
  }
  else {
    uVar3 = 1;
  }
  *(char *)((int)param_1 + 0x1f) = uVar3;
  FUN_10000648(0x6b626420,local_18);
  param_1[5] = (short)local_18[0];
  FUN_10000648(0x61746c6b,local_18);
  param_1[6] = (short)local_18[0];
  uVar3 = FUN_100ecf28(0x68647772,7);
  *(char *)((int)param_1 + 0x1b) = uVar3;
  FUN_10000648(0x612f7578,local_18);
  *(int *)((int)param_1 + 0x1d) = local_18[0] != 0;
  FUN_10000648(0x73637269,local_18);
  if ((*(char *)(param_1 + 10) == '\0') || (local_18[0] == 0)) {
    uVar3 = 0;
  }
  else {
    uVar3 = 1;
  }
  *(char *)((int)param_1 + 0x17) = uVar3;
  uVar3 = FUN_100ecf28(0x6f732020,4);
  *(char *)(param_1 + 0xf) = uVar3;
  FUN_10000648(0x74652020,local_18);
  *(int *)(param_1 + 8) = local_18[0];
  uVar5 = FUN_10001ba8();
  *(unsigned char *)(param_1 + 0xe) =
       (unsigned char)((-1 - (uVar5 & 0x400)) + (unsigned long long)((uVar5 & 0x400) == 0) >> 0x18) >> 7;
  if ((*(char *)(param_1 + 10) == '\0') || (iVar2 = FUN_100eeff4(0xffffffffffffa80b), iVar2 == 0)) {
    uVar3 = 0;
  }
  else {
    uVar3 = 1;
  }
  *(char *)(param_1 + 0xb) = uVar3;
  *(char *)(param_1 + 0xc) = (char)((short)param_1[2] >> 7) + (0x5ff < (unsigned short)param_1[2]);
  if ((*(char *)(param_1 + 10) == '\0') || (iVar2 = FUN_100eeff4(0xffffffffffffa803), iVar2 == 0)) {
    uVar3 = 0;
  }
  else {
    uVar3 = 1;
  }
  *(char *)((int)param_1 + 0x19) = uVar3;
  if ((*(char *)(param_1 + 10) == '\0') || (iVar2 = FUN_100eeff4(0xffffffffffffa860), iVar2 == 0)) {
    uVar3 = 0;
  }
  else {
    uVar3 = 1;
  }
  *(char *)(param_1 + 0xd) = uVar3;
  uVar3 = FUN_100ecf28(0x65766e74,0);
  *(char *)(param_1 + 0x10) = uVar3;
  uVar3 = FUN_100ecf28(0x6564746e,0);
  *(char *)((int)param_1 + 0x21) = uVar3;
  uVar3 = FUN_100ecf28(0x68656c70,0);
  *(char *)(param_1 + 0x11) = uVar3;
  uVar3 = FUN_100ecf28(0x616c6973,0);
  *(char *)((int)param_1 + 0x23) = uVar3;
  uVar3 = FUN_100ecf28(0x666f6c64,0);
  *(char *)(param_1 + 0x12) = uVar3;
  uVar3 = FUN_100ecf28(0x706f7021,0);
  *(char *)(param_1 + 0x13) = uVar3;
  uVar3 = FUN_100ecf28(0x666f6e74,0);
  *(char *)((int)param_1 + 0x27) = uVar3;
  *(char *)((int)param_1 + 0x25) = 0;
  *(char *)(param_1 + 0x14) = 0;
  *(char *)((int)param_1 + 0x29) = 0;
  iVar2 = FUN_100ecf28(0x6f732020,3);
  if (iVar2 != 0) {
    *(char *)((int)param_1 + 0x25) = 1;
    local_60 = 0;
    local_5c = 2;
    local_58 = 0x3c;
    local_54 = 0;
    local_20 = 0;
    iVar2 = Game_GetString(&local_60,&local_58);
    if (iVar2 == 0) {
      *(int *)(param_1 + 0x14) = (local_40 & 0x400) != 0;
      *(int *)((int)param_1 + 0x29) = (local_40 & 0x40) != 0;
    }
  }
  uVar3 = FUN_100ecf28(0x64726167,0);
  *(char *)(param_1 + 0x15) = uVar3;
  uVar3 = FUN_100ecf28(0x74686473,0);
  *(char *)((int)param_1 + 0x2b) = uVar3;
  uVar3 = FUN_100ecf28(0x6f636575,0);
  *(char *)(param_1 + 0x16) = uVar3;
  uVar3 = FUN_100ecf28(0x67726678,0);
  *(char *)((int)param_1 + 0x2d) = uVar3;
  uVar3 = FUN_100ecf28(0x706d6772,0);
  *(char *)(param_1 + 0x17) = uVar3;
  uVar3 = FUN_100ecf28(0x61736c6d,0);
  *(char *)((int)param_1 + 0x2f) = uVar3;
  uVar3 = FUN_100ecf28(0x63667267,0);
  *(char *)(param_1 + 0x18) = uVar3;
  uVar3 = FUN_100ecf28(0x786c6174,0);
  *(char *)((int)param_1 + 0x31) = uVar3;
  uVar3 = FUN_100ecf28(0x74747363,0);
  *(char *)(param_1 + 0x19) = uVar3;
  uVar3 = FUN_100ecf28(0x73746466,0);
  *(char *)((int)param_1 + 0x33) = uVar3;
  lVar1 = FUN_10000648(0x7174696d,local_18);
  *(int *)(param_1 + 0x1a) = lVar1 == 0;
  lVar1 = FUN_10000648(0x74736d76,local_18);
  *(int *)((int)param_1 + 0x35) = lVar1 == 0;
  return;
}




















































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* FUN_100eecf8 defined elsewhere */


/* FUN_100ef284 defined elsewhere */






















/* Address: 0x100ef2f8 Size: 56 bytes */
unsigned long long FUN_100ef2f8()

{
  long long lVar1;
  long long lVar2;
  
  lVar1 = FUN_10000468();
  lVar2 = -lVar1 + -1;
  return lVar2 + (-(unsigned long long)(lVar2 == 0) - (-lVar1 + -2)) & 0xff;
}

/* FUN_100f0334 defined elsewhere */


/* Address: 0x100f06dc Size: 172 bytes */
void FUN_100f06dc(short param_1,short *param_2)

{
  unsigned int uVar1;
  unsigned int *puVar2;
  short uVar3;
  char local_110 [272];
  
  local_110[0] = 0;
  puVar2 = (unsigned int *)GetResource_Thunk(0x54785374,param_1);
  FUN_100db2a4();
  *(char *)(param_2 + 1) = *(char *)*puVar2;
  param_2[2] = *(short *)(*puVar2 + 2);
  uVar1 = *(unsigned int *)(*puVar2 + 8);
  *(int *)(param_2 + 3) = *(int *)(*puVar2 + 4);
  *(unsigned int *)(param_2 + 5) = uVar1 & 0xffff0000;
  FUN_100ef284((unsigned long long)*puVar2 + 10,local_110);
  uVar3 = FUN_100ef6e4(local_110);
  *param_2 = uVar3;
  return;
}

/* Address: 0x100f0d3c Size: 312 bytes */

void FUN_100f0d3c(int param_1,char param_2)

{
  int *puVar1;
  int *puVar2;
  char *puVar3;
  char *pcVar4;
  int *ppuVar5;
  int iVar6;
  int iVar7;
  int *piVar8;
  int **local_54;
  int local_30;
  int local_2c [11];
  
  pcVar4 = pcRam1011785c;
  puVar3 = puRam10117858;
  puVar2 = puRam10117854;
  puVar1 = puRam10117850;
  ppuVar5 = 0 /* TVect base */;
  piVar8 = &local_30;
  local_2c[1] = 0;
  *puRam10117858 = 1;
  if (*pcVar4 == '\0') {
    iVar6 = FUN_100f0b50(*puVar2);
    if (iVar6 != 0) {
      FUN_100011a0(*puVar2);
      *pcVar4 = '\0';
      ppuVar5 = local_54;
    }
    iVar6 = FUN_100f0cc8(0,local_2c + 1);
    local_2c[0] = (*(int *)(*(int*)((char*)ppuVar5 - 0x10)) - iVar6) + -8;
    if (local_2c[0] < param_1) {
      piVar8 = local_2c;
    }
    iVar6 = *piVar8;
    if (0 < iVar6) {
      local_30 = param_1;
      iVar7 = FUN_100f0b50(*puVar1);
      if (iVar7 != 0) {
        FUN_100011a0(*puVar1);
      }
      if (param_2 == '\0') {
        FUN_100002d0(iVar6);
        param_1 = FUN_100002e8(iVar6);
      }
      if (param_1 < iVar6) {
        *puVar3 = 0;
        *pcVar4 = '\0';
      }
    }
  }
  return;
}

/* Address: 0x100f1264 Size: 68 bytes */
int FUN_100f1264()

{
  int *puVar1;
  
  puVar1 = puRam10117850;
  FUN_100f0e74();
  return *(int *)*puVar1 == 0;
}

/* Address: 0x100f2214 Size: 92 bytes */
long long FUN_100f2214(short param_1)

{
  long long uVar1;
  int local_2c;
  
  uVar1 = FUN_100003f0(param_1);
  if ((int)uVar1 == 0) {
    uVar1 = FUN_100f2184(**(int **)(local_2c + -0x794),param_1);
  }
  return uVar1;
}

/* Address: 0x100f25e0 Size: 76 bytes */
void FUN_100f25e0(long long param_1,long long param_2,long long param_3)

{
  ResourceRead_Dispatch(*piRam10117114 + (int)*(short *)(*(int *)*piRam10117114 + 0x118),param_1,param_2,
               param_3);
  return;
}

/* Address: 0x100f2a54 Size: 716 bytes */
void FUN_100f2a54()

{
  int *ppuVar1;
  int iVar2;
  int *puVar3;
  short sVar9;
  int *piVar4;
  int uVar5;
  int iVar6;
  int iVar7;
  int *puVar8;
  short sVar10;
  int local_16c;
  int local_148;
  int uStack_144;
  int uStack_140;
  int uStack_13c;
  int uStack_138;
  int uStack_134;
  int uStack_130;
  int uStack_12c;
  char auStack_128 [256];
  char auStack_28 [4];
  short local_24 [18];
  
  ppuVar1 = 0 /* TVect base */;
  puVar3 = (int *)NewPtr_Thunk(0x20);
  if (puVar3 != (int *)0x0) {
    FUN_100d8824(puVar3);
    *puVar3 = (*(int*)((char*)ppuVar1 - 0x1ee));
  }
  *(int **)ppuVar1[-10] = puVar3;
  FUN_100f1940(puVar3);
  puVar3 = (int *)NewPtr_Thunk(0x20);
  if (puVar3 != (int *)0x0) {
    FUN_100d8824(puVar3);
    *puVar3 = (*(int*)((char*)ppuVar1 - 0x1eb));
  }
  *(int **)(*(int*)((char*)ppuVar1 - 0x1e3)) = puVar3;
  FUN_100f1adc(puVar3);
  puVar3 = (int *)NewPtr_Thunk(0x20);
  if (puVar3 != (int *)0x0) {
    FUN_100ea428(puVar3);
    *puVar3 = (*(int*)((char*)ppuVar1 - 0x1e8));
  }
  *(int **)(*(int*)((char*)ppuVar1 - 0x1e5)) = puVar3;
  FUN_100f2160(puVar3);
  sVar9 = FUN_10001c68(0x434d4e55);
  MarkChanged();
  sVar10 = 1;
  if (0 < sVar9) {
    do {
      puVar3 = (int *)FUN_10001c80(0x434d4e55,sVar10);
      FUN_100db2a4(puVar3);
      ResolveAlias(puVar3,local_24,auStack_28,auStack_128);
      if (0 < *(short *)*puVar3) {
        piVar4 = (int *)FUN_100f2844(puVar3);
        uVar5 = GetResource_Thunk(0x4d444546,*(short *)(*piVar4 + 6));
        *(int *)(*piVar4 + 6) = uVar5;
        FUN_100f29e8(piVar4,local_24[0]);
      }
      sVar10 = sVar10 + 1;
    } while (sVar10 <= sVar9);
  }
  sVar9 = FUN_10001c68(0x4d454e55);
  iVar2 = local_16c;
  MarkChanged();
  sVar10 = 1;
  if (0 < sVar9) {
    do {
      piVar4 = (int *)FUN_10001c80(0x4d454e55,sVar10);
      iVar2 = local_16c;
      if (piVar4 != (int *)0x0) {
        ResolveAlias(piVar4,local_24,auStack_28,auStack_128);
        iVar6 = FUN_100022f8(piVar4);
        iVar7 = FUN_10002310();
        if (iVar6 != iVar7) {
          if (0 < *(short *)*piVar4) {
            uVar5 = GetResource_Thunk(0x4d444546,((short *)*piVar4)[3]);
            *(int *)(*piVar4 + 6) = uVar5;
            FUN_100f29e8(piVar4,local_24[0]);
          }
        }
      }
      sVar10 = sVar10 + 1;
    } while (sVar10 <= sVar9);
  }
  puVar8 = (int *)AllocateHandle(0x20);
  **(int **)(iVar2 + -0x788) = puVar8;
  puVar3 = *(int **)(iVar2 + -0x784);
  local_148 = *puVar3;
  uStack_144 = puVar3[1];
  uStack_140 = puVar3[2];
  uStack_13c = puVar3[3];
  uStack_138 = puVar3[4];
  uStack_134 = puVar3[5];
  uStack_130 = puVar3[6];
  uStack_12c = puVar3[7];
  CopyDataToBuffer(&local_148,*puVar8,0x20);
  return;
}































































































































































































































































































































































































































































































































































































































/* FUN_100f5108 defined elsewhere */


/* Address: 0x100f56c0 Size: 36 bytes */
void FUN_100f56c0()

{
  FUN_100f5640();
  return;
}

/* FUN_100f5754 defined elsewhere */


/* Address: 0x100f5814 Size: 36 bytes */
void FUN_100f5814()

{
  FUN_100c2250();
  return;
}

/* Address: 0x100fb260 Size: 120 bytes */
int * FUN_100fb260(int *param_1)

{
  int *ppuVar1;
  
  ppuVar1 = 0 /* TVect base */;
  if ((param_1 != (int *)0x0) ||
     (ppuVar1 = 0 /* TVect base */, param_1 = (int *)NewPtr_Thunk(0x1c),
     param_1 != (int *)0x0)) {
    FUN_100f913c(param_1);
    *param_1 = (*(int*)((char*)ppuVar1 - 0x1b8));
    param_1[4] = 0;
    param_1[5] = 0;
    param_1[6] = 0;
  }
  return param_1;
}

/* Address: 0x100fb300 Size: 276 bytes */
void FUN_100fb300(int *param_1,long long param_2,short param_3)

{
  int *puVar1;
  int iVar3;
  long long uVar2;
  char auStack_130 [256];
  short local_30;
  int local_2c;
  int local_28;
  int local_24;
  int local_20;
  
  puVar1 = puRam10117370;
  FUN_100f91e0(param_1);
  local_30 = 0;
  local_2c = 0;
  local_24 = 0;
  local_20 = 0;
  local_28 = *puVar1;
  *puVar1 = auStack_130;
  iVar3 = TrySetjmp(auStack_130);
  if (iVar3 == 0) {
    uVar2 = FUN_10000420();
    FUN_10000438(0);
    iVar3 = GetResource_Thunk(param_2,param_3);
    param_1[4] = iVar3;
    FUN_10000438(uVar2);
    FUN_100db2a4(param_1[4]);
    iVar3 = FUN_10003888(param_1[4]);
    param_1[6] = iVar3;
    MarkChanged();
    *puVar1 = local_28;
  }
  else {
    ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x90));
    LockHandleRange(local_30,local_2c);
  }
  return;
}

/* Address: 0x1010597c Size: 8 bytes */
int FUN_1010597c()

{
  return uRam10116c7c;
}

/* Address: 0x1010d6a8 Size: 8 bytes */
int FUN_1010d6a8()

{
  return uRam101172f8;
}

/* Address: 0x1010d6b8 Size: 180 bytes */
int * FUN_1010d6b8(int *param_1)

{
  int uVar1;
  int *ppuVar2;
  
  ppuVar2 = 0 /* TVect base */;
  if ((param_1 != (int *)0x0) ||
     (ppuVar2 = 0 /* TVect base */, param_1 = (int *)NewPtr_Thunk(400),
     param_1 != (int *)0x0)) {
    ConstructViewObject(param_1);
    *param_1 = (*(int*)((char*)ppuVar2 - 0x169));
    *(char *)(param_1 + 0x23) = 0;
    *(short *)(param_1 + 0x20) = 0xffff;
    uVar1 = *(int *)((int)(*(int*)((char*)ppuVar2 - 0x338)) + 4);
    param_1[0x21] = *(int *)(*(int*)((char*)ppuVar2 - 0x338));
    param_1[0x22] = uVar1;
    FUN_100b1d90(param_1 + 0x23,ZEXT48((*(int*)((char*)ppuVar2 - 0x16d))) + 0x10c);
    *(short *)(param_1 + 99) = 0x20;
    *(char *)(param_1 + 5) = 0;
  }
  return param_1;
}

/* Address: 0x1010f044 Size: 8 bytes */
int FUN_1010f044()

{
  return uRam10116f40;
}

/* Address: 0x1010f054 Size: 344 bytes */
int * FUN_1010f054(int *param_1)

{
  int uVar1;
  int *puVar2;
  int *ppuVar3;
  
  puVar2 = puRam10116a40;
  ppuVar3 = 0 /* TVect base */;
  if ((param_1 != (int *)0x0) ||
     (ppuVar3 = 0 /* TVect base */, param_1 = (int *)NewPtr_Thunk(200),
     param_1 != (int *)0x0)) {
    ConstructViewObject(param_1);
    *param_1 = (*(int*)((char*)ppuVar3 - 0x15c));
    FUN_100b02d0((int)param_1 + 0xba);
    FUN_100b02d0((int)param_1 + 0xbe);
    *(char *)((int)param_1 + 0xb1) = 0;
    *(char *)((int)param_1 + 0xa9) = 1;
    *(short *)((int)param_1 + 0xa2) = 0;
    *(int *)((int)param_1 + 0xbe) = *puVar2;
    *(int *)((int)param_1 + 0xba) = *puVar2;
    *(char *)(param_1 + 0x2a) = 1;
    *(char *)((int)param_1 + 0xab) = 0;
    *(char *)((int)param_1 + 0xb6) = 0;
    *(char *)((int)param_1 + 0xb5) = 0;
    *(char *)((int)param_1 + 0xa7) = 0;
    *(char *)((int)param_1 + 0xb7) = 0;
    *(char *)((int)param_1 + 0xb2) = 0;
    *(char *)(param_1 + 0x29) = 0;
    *(char *)((int)param_1 + 0xa6) = 1;
    *(char *)((int)param_1 + 0xa5) = 1;
    uVar1 = *(int *)((int)(*(int*)((char*)ppuVar3 - 0x1fe)) + 4);
    param_1[0x22] = *(int *)(*(int*)((char*)ppuVar3 - 0x1fe));
    param_1[0x23] = uVar1;
    *(char *)(param_1 + 0x2b) = 0;
    *(char *)(param_1 + 0x2c) = 0;
    *(char *)((int)param_1 + 0xad) = 0;
    *(char *)((int)param_1 + 0xaf) = 0;
    *(char *)((int)param_1 + 0xae) = 0;
    *(char *)((int)param_1 + 0xaa) = 1;
    *(short *)(param_1 + 0x28) = 0;
    *(short *)(param_1 + 0x21) = 0;
    uVar1 = *(int *)((int)(*(int*)((char*)ppuVar3 - 0x25c)) + 4);
    param_1[0x24] = *(int *)(*(int*)((char*)ppuVar3 - 0x25c));
    param_1[0x25] = uVar1;
    *(char *)(param_1 + 0x2d) = 0;
    param_1[0x26] = param_1;
    param_1[0x27] = param_1[6];
    *(char *)((int)param_1 + 0x86) = 0;
    *(char *)((int)param_1 + 0xb3) = 0;
    *(char *)(param_1 + 0x2e) = 0;
    param_1[0x20] = 0;
    *(char *)((int)param_1 + 0xb9) = 1;
    *(short *)((int)param_1 + 0xc2) = 0xffff;
    *(short *)(param_1 + 0x31) = 0xffff;
  }
  return param_1;
}

/* FUN_10115a00 defined elsewhere */


































































/* Address: 0x10115d3c Size: 48 bytes */
long long FUN_10115d3c(int param_1)

{
  if (*(int *)(param_1 + 0xc) != 0) {
    return 2;
  }
  if (*(int *)(param_1 + 0x14) != 0) {
    return 1;
  }
  return 0;
}

/* Address: 0x10115e4c Size: 448 bytes */
unsigned long long FUN_10115e4c(int param_1,unsigned int param_2)

{
  char *puVar1;
  unsigned long long uVar2;
  int iVar3;
  int bVar4;
  long long lVar5;
  char local_10 [16];
  
  if (*(short *)(param_1 + 2) == 0) {
    if ((*(int *)(param_1 + 4) == 0) && (*(short *)(param_1 + 2) == 0)) {
      bVar4 = true;
    }
    else {
      bVar4 = false;
    }
    if (bVar4) {
      iVar3 = ResourceRead_Dispatch(param_1 + *(short *)(*(int *)(param_1 + 0x24) + 8));
    }
    else {
      iVar3 = 0;
    }
    if (iVar3 == -1) {
      uVar2 = 0xffffffffffffffff;
    }
    else {
      if (*(int *)(param_1 + 0x14) != 0) {
        ResourceRead_Dispatch(*(short *)(*(int *)(param_1 + 0x24) + 0x28) + param_1);
      }
      if (*(int *)(param_1 + 0xc) == 0) {
        iVar3 = *(int *)(param_1 + 4);
        *(int *)(param_1 + 0xc) = iVar3;
        *(int *)(param_1 + 8) = iVar3;
        *(int *)(param_1 + 0x10) = iVar3 + *(int *)(param_1 + 0x20);
      }
      else {
        lVar5 = (unsigned long long)*(unsigned int *)(param_1 + 0xc) - (unsigned long long)*(unsigned int *)(param_1 + 8);
        if ((lVar5 != 0) &&
           (iVar3 = FUN_10001668(*(int *)(param_1 + 0x28),*(int *)(param_1 + 4),lVar5)
           , iVar3 != (int)lVar5)) {
          return 0xffffffffffffffff;
        }
        iVar3 = *(int *)(param_1 + 4);
        *(int *)(param_1 + 0xc) = iVar3;
        *(int *)(param_1 + 8) = iVar3;
        *(int *)(param_1 + 0x10) = iVar3 + *(int *)(param_1 + 0x20);
      }
      if (param_2 != 0xffffffff) {
        if (*(unsigned int *)(param_1 + 0xc) < *(unsigned int *)(param_1 + 0x10)) {
          puVar1 = *(char **)(param_1 + 0xc);
          *(char **)(param_1 + 0xc) = puVar1 + 1;
          *puVar1 = (char)param_2;
        }
        else {
          ResourceRead_Dispatch(param_1 + *(short *)(*(int *)(param_1 + 0x24) + 0x10),param_2 & 0xff);
        }
      }
      uVar2 = (unsigned long long)param_2 & 0xff;
    }
  }
  else if (param_2 == 0xffffffff) {
    uVar2 = 0xff;
  }
  else {
    local_10[0] = (char)param_2;
    iVar3 = FUN_10001668(*(int *)(param_1 + 0x28),local_10,1);
    if (iVar3 == 1) {
      uVar2 = 1;
    }
    else {
      uVar2 = 0xffffffffffffffff;
    }
  }
  return uVar2;
}

/* Address: stubbed - had compile errors on 64-bit */
unsigned long long FUN_10116338(unsigned long long param_1) { return 0; }





































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: stubbed - had compile errors on 64-bit */
long long FUN_10116e14(short *param_1) { return 0; }





















































































































































































































































































































































/* Address: stubbed - had compile errors on 64-bit */
int * FUN_10117024(int *param_1,int *param_2) { return 0; }





























































































































































































































































































































































































































































































































































/* Address: stubbed - had compile errors on 64-bit */
int * FUN_101171bc(int *param_1,int *param_2) { return 0; }
































































































































































































































































































































































































































































































































































































































































































































































































































































/* === Recovered from stubs.c === */





/* Address: 0x10004c54 Size: 136 bytes */
void FUN_10004c54()

{
  int iVar1;
  short *puVar2;
  
  puVar2 = puRam10117424;
  iVar1 = *piRam1011735c;
  *puRam10117424 = *(short *)(iVar1 + 0x17e);
  puVar2[1] = *(short *)(iVar1 + 0x180);
  puVar2[2] = *(short *)(iVar1 + 0x17c) - *(short *)(iVar1 + 0x178);
  puVar2[3] = *(short *)(iVar1 + 0x17a) - *(short *)(iVar1 + 0x176);
  FUN_10003b7c(puVar2);
  FUN_1000419c(puVar2);
  FUN_10004400(puVar2);
  FUN_100047d0(puVar2);
  FUN_100049bc(puVar2);
  return;
}

/* Address: 0x10005f90 Size: 7312 bytes */
void FUN_10005f90()

{
  short sVar1;
  int uVar2;
  int *piVar3;
  int uVar4;
  int iVar5;
  int iVar6;
  unsigned int uVar7;
  unsigned int *puVar8;
  int iVar9;
  unsigned int uVar10;
  char *pcVar11;
  int *piVar12;
  int iVar13;
  unsigned long long uVar14;
  int *ppuVar16;
  unsigned long long uVar15;
  int iVar23;
  int iVar24;
  int uVar25;
  int *piVar26;
  char uVar31;
  long long uVar17;
  long long uVar18;
  long long uVar19;
  short sVar30;
  long long uVar20;
  long long uVar21;
  int uVar27;
  int uVar28;
  int uVar29;
  long long uVar22;
  short sVar33;
  unsigned int uVar32;
  unsigned short uVar34;
  short uVar35;
  short uVar36;
  unsigned long long uVar37;
  long long lVar38;
  unsigned long long uVar39;
  long long lVar40;
  unsigned long long uVar41;
  unsigned long long uVar42;
  unsigned long long uVar43;
  short *puVar44;
  int iVar45;
  int iVar46;
  unsigned int local_4f4;
  short local_49c;
  short local_49a;
  int local_394;
  int local_378;
  int local_374;
  
  iVar46 = iRam10117700;
  iVar13 = iRam10117424;
  piVar12 = piRam1011735c;
  piVar26 = gActiveDocView;
  pcVar11 = pcRam10116398;
  uVar14 = ZEXT48(((char*)0));
  ppuVar16 = 0 /* TVect base */;
  uVar37 = uVar14 - 0x4d0;
  uVar39 = uVar14 - 0x4c8;
  uVar41 = uVar14 - 0x4c0;
  uVar42 = uVar14 - 0x4b8;
  uVar43 = uVar14 - 0x4b0;
  iVar23 = AllocateBlock((long long)((int)*(short *)(iRam10117424 + 4) << 2) *
                        (long long)(int)*(short *)(iRam10117424 + 6));
  uVar2 = *(int *)(*(int*)((char*)ppuVar16 - 0x14c));
  *(int *)(*(int*)((char*)ppuVar16 - 0x14c)) = (int)(uVar14 - 0x498);
  iVar24 = TrySetjmp(uVar14 - 0x498);
  iVar45 = 0;
  if (iVar24 == 0) {
    if ((*pcVar11 == '\0') && (piVar3 = (int *)*piVar26, piVar3 != (int *)0x0)) {
      ResourceRead_Dispatch((int)*(short *)(*piVar3 + 0x3a8) + (int)piVar3);
      piVar26 = (int *)*piVar26;
      uVar25 = ResourceRead_Dispatch((int)piVar26 + (int)*(short *)(*piVar26 + 0x5e8));
      *(int *)(*(int *)(local_4f4 - 0x1208) + 0x10) = uVar25;
    }
    else {
      iVar24 = Render_ValidatePixMap(*(int *)(*(int *)(iVar46 + 0x10) + 2));
      if (iVar24 == 0) {
        SetMacRect(uVar14 - 0x380,0,0,*(short *)(iVar46 + 4),*(short *)(iVar46 + 6));
        piVar26 = (int *)Render_GetMainDevice();
        sVar30 = *(short *)(**(int **)(*piVar26 + 0x16) + 0x20);
        if (sVar30 == 8) {
          uVar25 = 0;
        }
        else {
          uVar25 = **(int **)(local_4f4 - 0x1cc);
        }
        uVar31 = FUN_100f1574(1);
        if (sVar30 == 8) {
          uVar35 = 0;
        }
        else {
          uVar35 = 8;
        }
        NewGWorld_Wrapper(iVar46 + 0x10,uVar35,uVar14 - 0x380,uVar25,0,0x10000000);
        **(char **)(local_4f4 - 0x3c) = uVar31;
      }
      else {
        HUnlock_Thunk(*(int *)(*(int *)(iVar46 + 0x10) + 2));
      }
    }
    if (*pcVar11 == '\0') {
      iVar46 = *(int *)(local_4f4 - 0x1208);
    }
    GetGWorld_Wrapper(uVar14 - 0x378,uVar14 - 0x374);
    uVar25 = *(int *)(iVar46 + 0x10);
    SetGWorld_Wrapper(uVar25,0);
    uVar32 = local_4f4;
    uVar17 = LockPixels_Wrapper(uVar25);
    SaveDrawingState(uVar14 - 0x36c);
    RGBForeColor_Thunk(*(int *)(uVar32 - 0x1d4));
    Render_PenNormal();
    RGBForeColor_Thunk((unsigned long long)*(unsigned int *)(local_4f4 - 0x1d4) + 0x5fa);
    SetForeColor_Thunk();
    sVar30 = *(short *)(iVar13 + 4);
    sVar1 = *(short *)(iVar13 + 6);
    local_49c = 0;
    iVar24 = 0;
    if (0 < sVar30) {
      do {
        iVar45 = iVar24;
        piVar26 = (int *)(*(int *)(iVar13 + 0x24) +
                          ((int)local_49c - (int)*(short *)(iVar13 + 0x1e)) *
                          (((int)*(short *)(iVar13 + 0x20) - (int)*(short *)(iVar13 + 0x1c)) + 1) *
                          10 + (*(short *)(iVar13 + 0x1c) * 5 & 0x7fffffffU) * -2);
        local_49a = 0;
        if (0 < sVar1) {
          do {
            if (*piVar26 < 0) {
              if (((unsigned int)piVar26[1] >> 0x1a & 0x1f) == 0xe) {
                sVar33 = local_49a;
                if (*pcVar11 == '\0') {
                  sVar33 = local_49a + *(short *)(*piVar12 + 0x176);
                }
                *(short *)(uVar14 - 0x4a4) = sVar33 * 0x28;
                GetBitMapPtr(uVar14 - 0x4a4);
                PaintRect_Thunk();
              }
              else {
                *(short *)(iVar23 + iVar45 * 4) = local_49c;
                *(short *)(iVar45 * 4 + iVar23 + 2) = local_49a;
                iVar45 = (int)(short)((short)iVar45 + 1);
              }
            }
            local_49a = local_49a + 1;
            piVar26 = (int *)((int)piVar26 + 10);
          } while (local_49a < sVar1);
        }
        local_49c = local_49c + 1;
        iVar24 = iVar45;
      } while (local_49c < sVar30);
    }
    uVar15 = (unsigned long long)local_4f4;
    if (iVar45 != 0) {
      EnsureSpriteLoaded(0x1a);
      iVar24 = *(int *)((int)uVar15 + -0x464);
      uVar4 = *(int *)(iVar24 + 0x10);
      LockPixels_Wrapper(uVar4);
      EnsureSpriteLoaded(0x1b);
      uVar28 = *(int *)(iVar24 + 0x24);
      LockPixels_Wrapper(uVar28);
      iVar24 = 0;
      if (0 < iVar45) {
        lVar38 = uVar14 - 0x228;
        lVar40 = uVar14 - 0x21c;
        do {
          local_49c = *(short *)(iVar23 + iVar24 * 4);
          local_49a = *(short *)(iVar23 + iVar24 * 4 + 2);
          uVar34 = (unsigned short)((unsigned int)*(int *)
                                   (*(int *)(iVar13 + 0x24) +
                                    ((int)local_49c - (int)*(short *)(iVar13 + 0x1e)) *
                                    (((int)*(short *)(iVar13 + 0x20) -
                                     (int)*(short *)(iVar13 + 0x1c)) + 1) * 10 +
                                    *(short *)(iVar13 + 0x1c) * -10 + local_49a * 10) >> 0x16) &
                   0xff;
          if (*pcVar11 == '\0') {
            local_49c = local_49c + *(short *)(*piVar12 + 0x178);
            local_49a = local_49a + *(short *)(*piVar12 + 0x176);
          }
          *(short *)lVar38 = 0;
          uVar32 = (int)(short)uVar34 + ((unsigned int)(int)(short)uVar34 / 0x60) * -0x60;
          uVar15 = (long long)((int)uVar32 >> 4) +
                   (unsigned long long)((int)uVar32 < 0 && (uVar32 & 0xf) != 0);
          *(short *)(uVar14 - 0x210) =
               (short)(((uVar15 & 0x3fffffff) * 4 + uVar15 & 0xffffffff) << 3);
          FUN_100b065c(lVar38,uVar14 - 0x210);
          *(short *)lVar40 = 0;
          *(short *)(uVar14 - 0x204) = local_49a * 0x28;
          FUN_100b065c(lVar40,uVar14 - 0x204);
          iVar5 = -0x238;
          if ((*(unsigned int *)(*(int *)(iVar13 + 0x24) +
                         ((int)local_49c - (int)*(short *)(iVar13 + 0x1e)) *
                         (((int)*(short *)(iVar13 + 0x20) - (int)*(short *)(iVar13 + 0x1c)) + 1) *
                         10 + *(short *)(iVar13 + 0x1c) * -10 + local_49a * 10) >> 0x1e & 1) != 0) {
            iVar5 = -0x230;
          }
          uVar18 = GetBitMapPtr(lVar40);
          uVar19 = GetBitMapPtr(lVar38);
          CopyBits_Thunk(*(int *)(((char*)0) + iVar5),uVar17,uVar19,uVar18,0,0);
          uVar15 = (unsigned long long)local_4f4;
          sVar30 = (short)iVar24 + 1;
          iVar24 = (int)sVar30;
        } while (sVar30 < iVar45);
        uVar43 = uVar43 & 0xffffffff;
        uVar42 = uVar42 & 0xffffffff;
        uVar41 = uVar41 & 0xffffffff;
        uVar39 = uVar39 & 0xffffffff;
        uVar37 = uVar37 & 0xffffffff;
      }
      UnlockPixels_Wrapper(uVar4);
      UnlockPixels_Wrapper(uVar28);
      if ((*(unsigned int *)(iVar13 + 0x28) >> 0x1c & 1) != 0) {
        EnsureSpriteLoaded(0x29);
        uVar4 = *(int *)(*(int *)((int)uVar15 + -0x460) + 0x10);
        uVar28 = *(int *)(*(int *)((int)uVar15 + -0x45c) + 0x10);
        uVar18 = LockPixels_Wrapper(uVar4);
        uVar19 = LockPixels_Wrapper(uVar28);
        iVar24 = 0;
        if (0 < iVar45) {
          lVar38 = uVar14 - 0x1e8;
          do {
            local_49a = *(short *)(iVar23 + iVar24 * 4 + 2);
            uVar32 = *(unsigned int *)(*(int *)(iVar13 + 0x24) +
                               ((int)*(short *)(iVar23 + iVar24 * 4) -
                               (int)*(short *)(iVar13 + 0x1e)) *
                               (((int)*(short *)(iVar13 + 0x20) - (int)*(short *)(iVar13 + 0x1c)) +
                               1) * 10 + *(short *)(iVar13 + 0x1c) * -10 + local_49a * 10 + 4);
            if ((uVar32 >> 0x19 & 1) != 0) {
              if (*pcVar11 == '\0') {
                local_49a = local_49a + *(short *)(*piVar12 + 0x176);
              }
              puVar44 = (short *)uVar43;
              *puVar44 = 0;
              puVar44[1] = 0;
              puVar44[2] = 0x28;
              puVar44[3] = 0x28;
              *(short *)(uVar14 - 0x1dc) =
                   (short)((unsigned int)(int)(short)((unsigned short)(uVar32 >> 0x14) & 0x1f) / 0xd) * 0x28;
              FUN_100b065c(uVar43,uVar14 - 0x1dc);
              *(short *)lVar38 = 0;
              *(short *)(uVar14 - 0x1d0) = local_49a * 0x28;
              FUN_100b065c(lVar38,uVar14 - 0x1d0);
              uVar20 = GetBitMapPtr(lVar38);
              uVar21 = GetBitMapPtr(uVar43);
              uVar27 = GetBitMapPtr(uVar43);
              CopyMask_Thunk(uVar18,uVar19,uVar17,uVar27,uVar21,uVar20);
              uVar15 = (unsigned long long)local_4f4;
            }
            sVar30 = (short)iVar24 + 1;
            iVar24 = (int)sVar30;
          } while (sVar30 < iVar45);
          uVar41 = uVar41 & 0xffffffff;
          uVar39 = uVar39 & 0xffffffff;
          uVar37 = uVar37 & 0xffffffff;
        }
        UnlockPixels_Wrapper(uVar4);
        UnlockPixels_Wrapper(uVar28);
      }
      if ((*(unsigned int *)(iVar13 + 0x28) >> 0x1b & 1) != 0) {
        EnsureSpriteLoaded(9);
        uVar4 = *(int *)(*(int *)((int)uVar15 + -0x238) + 0x10);
        uVar32 = *(unsigned int *)(*(int *)((int)uVar15 + -0x23c) + 0x10);
        uVar43 = (unsigned long long)uVar32;
        uVar18 = LockPixels_Wrapper(uVar4);
        uVar19 = LockPixels_Wrapper(uVar43);
        iVar24 = 0;
        if (0 < iVar45) {
          lVar38 = uVar14 - 0x1c0;
          lVar40 = uVar14 - 0x1b4;
          do {
            local_49a = *(short *)(iVar23 + iVar24 * 4 + 2);
            if ((*(unsigned int *)(*(int *)(iVar13 + 0x24) +
                           ((int)*(short *)(iVar23 + iVar24 * 4) - (int)*(short *)(iVar13 + 0x1e)) *
                           (((int)*(short *)(iVar13 + 0x20) - (int)*(short *)(iVar13 + 0x1c)) + 1) *
                           10 + *(short *)(iVar13 + 0x1c) * -10 + local_49a * 10) >> 1 & 1) != 0) {
              if (*pcVar11 == '\0') {
                local_49a = local_49a + *(short *)(*piVar12 + 0x176);
              }
              *(short *)lVar38 = 0;
              *(short *)lVar40 = 0;
              *(short *)(uVar14 - 0x1a8) = local_49a * 0x28;
              FUN_100b065c(lVar40,uVar14 - 0x1a8);
              uVar20 = GetBitMapPtr(lVar40);
              uVar21 = GetBitMapPtr(lVar38);
              uVar28 = GetBitMapPtr(lVar38);
              CopyMask_Thunk(uVar18,uVar19,uVar17,uVar28,uVar21,uVar20);
              uVar15 = (unsigned long long)local_4f4;
            }
            sVar30 = (short)iVar24 + 1;
            iVar24 = (int)sVar30;
          } while (sVar30 < iVar45);
          uVar41 = uVar41 & 0xffffffff;
          uVar39 = uVar39 & 0xffffffff;
          uVar37 = uVar37 & 0xffffffff;
        }
        UnlockPixels_Wrapper(uVar4);
        UnlockPixels_Wrapper(uVar32);
      }
      if ((*(unsigned int *)(iVar13 + 0x28) >> 0x1d & 1) != 0) {
        EnsureSpriteLoaded(0x29);
        uVar4 = *(int *)(*(int *)((int)uVar15 + -0x460) + 0x10);
        uVar32 = *(unsigned int *)(*(int *)((int)uVar15 + -0x45c) + 0x10);
        uVar43 = (unsigned long long)uVar32;
        uVar18 = LockPixels_Wrapper(uVar4);
        uVar19 = LockPixels_Wrapper(uVar43);
        iVar24 = 0;
        if (0 < iVar45) {
          lVar38 = uVar14 - 0x198;
          do {
            local_49a = *(short *)(iVar23 + iVar24 * 4 + 2);
            if ((*(unsigned int *)(*(int *)(iVar13 + 0x24) +
                           ((int)*(short *)(iVar23 + iVar24 * 4) - (int)*(short *)(iVar13 + 0x1e)) *
                           (((int)*(short *)(iVar13 + 0x20) - (int)*(short *)(iVar13 + 0x1c)) + 1) *
                           10 + *(short *)(iVar13 + 0x1c) * -10 + local_49a * 10 + 4) >> 0x13 & 1)
                != 0) {
              if (*pcVar11 == '\0') {
                local_49a = local_49a + *(short *)(*piVar12 + 0x176);
              }
              uVar35 = 0;
              puVar44 = (short *)uVar42;
              *puVar44 = 0;
              puVar44[1] = 0;
              puVar44[2] = 0x28;
              puVar44[3] = 0x28;
              *(short *)(uVar14 - 0x18c) = 0x28;
              FUN_100b065c(uVar42,uVar14 - 0x18c);
              *(short *)lVar38 = uVar35;
              *(short *)(uVar14 - 0x180) = local_49a * 0x28;
              FUN_100b065c(lVar38,uVar14 - 0x180);
              uVar20 = GetBitMapPtr(lVar38);
              uVar21 = GetBitMapPtr(uVar42);
              uVar28 = GetBitMapPtr(uVar42);
              CopyMask_Thunk(uVar18,uVar19,uVar17,uVar28,uVar21,uVar20);
              uVar15 = (unsigned long long)local_4f4;
            }
            sVar30 = (short)iVar24 + 1;
            iVar24 = (int)sVar30;
          } while (sVar30 < iVar45);
          uVar41 = uVar41 & 0xffffffff;
          uVar39 = uVar39 & 0xffffffff;
          uVar37 = uVar37 & 0xffffffff;
        }
        UnlockPixels_Wrapper(uVar4);
        UnlockPixels_Wrapper(uVar32);
      }
      puVar44 = (short *)uVar41;
      if ((*(unsigned int *)(iVar13 + 0x28) >> 0x1a & 1) != 0) {
        EnsureSpriteLoaded(9);
        uVar4 = *(int *)(*(int *)((int)uVar15 + -0x238) + 0x10);
        uVar28 = *(int *)(*(int *)((int)uVar15 + -0x23c) + 0x10);
        uVar18 = LockPixels_Wrapper(uVar4);
        uVar19 = LockPixels_Wrapper(uVar28);
        iVar24 = 0;
        if (0 < iVar45) {
          lVar38 = uVar14 - 0x174;
          do {
            local_49a = *(short *)(iVar23 + iVar24 * 4 + 2);
            if ((*(unsigned int *)(*(int *)(iVar13 + 0x24) +
                           ((int)*(short *)(iVar23 + iVar24 * 4) - (int)*(short *)(iVar13 + 0x1e)) *
                           (((int)*(short *)(iVar13 + 0x20) - (int)*(short *)(iVar13 + 0x1c)) + 1) *
                           10 + *(short *)(iVar13 + 0x1c) * -10 + local_49a * 10) & 1) != 0) {
              if (*pcVar11 == '\0') {
                local_49a = local_49a + *(short *)(*piVar12 + 0x176);
              }
              uVar35 = 0;
              *puVar44 = 0;
              puVar44[1] = 0;
              puVar44[2] = 0x1d;
              puVar44[3] = 0x20;
              *(short *)(uVar14 - 0x168) = 0;
              FUN_100b065c(uVar41,uVar14 - 0x168);
              *(short *)lVar38 = uVar35;
              *(short *)(uVar14 - 0x15c) = local_49a * 0x28;
              FUN_100b065c(lVar38,uVar14 - 0x15c);
              uVar20 = GetBitMapPtr(lVar38);
              uVar21 = GetBitMapPtr(uVar41);
              uVar27 = GetBitMapPtr(uVar41);
              CopyMask_Thunk(uVar18,uVar19,uVar17,uVar27,uVar21,uVar20);
              uVar15 = (unsigned long long)local_4f4;
            }
            sVar30 = (short)iVar24 + 1;
            iVar24 = (int)sVar30;
          } while (sVar30 < iVar45);
          uVar39 = uVar39 & 0xffffffff;
          uVar37 = uVar37 & 0xffffffff;
        }
        UnlockPixels_Wrapper(uVar4);
        UnlockPixels_Wrapper(uVar28);
      }
      puVar44 = (short *)uVar39;
      if (((*(unsigned int *)(iVar13 + 0x28) >> 0x19 & 1) != 0) && (iVar24 = 0, 0 < iVar45)) {
        lVar38 = uVar14 - 0x150;
        iVar5 = *(int *)((int)uVar15 + -0x46c);
        iVar6 = *(int *)((int)uVar15 + -0x468);
        do {
          local_49c = *(short *)(iVar23 + iVar24 * 4);
          local_49a = *(short *)(iVar23 + iVar24 * 4 + 2);
          if ((*(unsigned int *)(*(int *)(iVar13 + 0x24) +
                         ((int)local_49c - (int)*(short *)(iVar13 + 0x1e)) *
                         (((int)*(short *)(iVar13 + 0x20) - (int)*(short *)(iVar13 + 0x1c)) + 1) *
                         10 + *(short *)(iVar13 + 0x1c) * -10 + local_49a * 10) >> 6 & 1) != 0) {
            if (*pcVar11 == '\0') {
              local_49c = local_49c + *(short *)(*piVar12 + 0x178);
              local_49a = local_49a + *(short *)(*piVar12 + 0x176);
            }
            uVar34 = (unsigned short)(*(unsigned int *)(*(int *)(iVar13 + 0x24) +
                                        ((int)local_49c - (int)*(short *)(iVar13 + 0x1e)) *
                                        (((int)*(short *)(iVar13 + 0x20) -
                                         (int)*(short *)(iVar13 + 0x1c)) + 1) * 10 +
                                        *(short *)(iVar13 + 0x1c) * -10 + local_49a * 10) >> 2) &
                     0xf;
            uVar41 = (unsigned long long)(short)uVar34;
            EnsureSpriteLoaded(uVar34 + 0x10);
            iVar9 = (int)(((uVar41 & 0x3fffffff) * 4 + uVar41 & 0x3fffffff) << 2);
            uVar41 = (unsigned long long)*(unsigned int *)(iVar9 + iVar6 + 0x10);
            uVar18 = LockPixels_Wrapper(uVar41);
            uVar4 = *(int *)(iVar9 + iVar5 + 0x10);
            uVar28 = LockPixels_Wrapper(uVar4);
            *puVar44 = 0;
            puVar44[1] = 0;
            puVar44[2] = 0x1d;
            puVar44[3] = 0x20;
            *(short *)(uVar14 - 0x144) = 0x1d;
            FUN_100b065c(uVar39,uVar14 - 0x144);
            *(short *)lVar38 = 0;
            *(short *)(uVar14 - 0x138) = local_49a * 0x28;
            FUN_100b065c(lVar38,uVar14 - 0x138);
            uVar27 = GetBitMapPtr(lVar38);
            uVar29 = GetBitMapPtr(uVar39);
            uVar19 = GetBitMapPtr(uVar39);
            CopyMask_Thunk(uVar18,uVar28,uVar17,uVar19,uVar29,uVar27);
            uVar15 = (unsigned long long)local_4f4;
            UnlockPixels_Wrapper(uVar41);
            UnlockPixels_Wrapper(uVar4);
          }
          sVar30 = (short)iVar24 + 1;
          iVar24 = (int)sVar30;
        } while (sVar30 < iVar45);
      }
      uVar32 = 0;
      do {
        if ((*(short *)(iVar13 + uVar32 * 2 + 8) != 0) && (iVar24 = 0, 0 < iVar45)) {
          do {
            local_49c = *(short *)(iVar23 + iVar24 * 4);
            local_49a = *(short *)(iVar23 + iVar24 * 4 + 2);
            puVar8 = (unsigned int *)(*(int *)(iVar13 + 0x24) +
                              ((int)local_49c - (int)*(short *)(iVar13 + 0x1e)) *
                              (((int)*(short *)(iVar13 + 0x20) - (int)*(short *)(iVar13 + 0x1c)) + 1
                              ) * 10 + *(short *)(iVar13 + 0x1c) * -10 + local_49a * 10);
            uVar7 = puVar8[1];
            if (((uVar7 >> 0x13 & 1) == 0) || ((uVar7 >> 10 & 1) != 0)) {
              uVar7 = *puVar8;
              uVar10 = uVar7 >> 0xb & 0x1f;
              if ((uVar10 != 0) && ((uVar7 >> 7 & 0xf) == uVar32)) {
                if (*pcVar11 == '\0') {
                  local_49c = local_49c + *(short *)(*piVar12 + 0x178);
                  local_49a = local_49a + *(short *)(*piVar12 + 0x176);
                }
                DrawArmyIconFullMap(iVar46,uVar32,(unsigned short)(uVar7 >> 0x11) & 0x1f,(short)uVar10,local_49c,
                             local_49a);
              }
            }
            sVar30 = (short)iVar24 + 1;
            iVar24 = (int)sVar30;
          } while (sVar30 < iVar45);
        }
        sVar30 = (short)uVar32 + 1;
        uVar32 = (unsigned int)sVar30;
      } while (sVar30 < 9);
      uVar17 = LockPixels_Wrapper(uVar25);
      uVar32 = (unsigned int)uVar15;
      if ((*(unsigned int *)(iVar13 + 0x28) >> 0x18 & 1) != 0) {
        EnsureSpriteLoaded(0x2a);
        uVar4 = *(int *)(*(int *)((int)uVar15 + -0x458) + 0x10);
        uVar28 = *(int *)(*(int *)((int)uVar15 + -0x46c) + 0xc4);
        uVar18 = LockPixels_Wrapper(uVar4);
        uVar19 = LockPixels_Wrapper(uVar28);
        uVar32 = (unsigned int)uVar15;
        iVar46 = 0;
        if (0 < iVar45) {
          lVar38 = uVar14 - 0x124;
          do {
            local_49a = *(short *)(iVar23 + iVar46 * 4 + 2);
            uVar32 = *(unsigned int *)(*(int *)(iVar13 + 0x24) +
                               ((int)*(short *)(iVar23 + iVar46 * 4) -
                               (int)*(short *)(iVar13 + 0x1e)) *
                               (((int)*(short *)(iVar13 + 0x20) - (int)*(short *)(iVar13 + 0x1c)) +
                               1) * 10 + *(short *)(iVar13 + 0x1c) * -10 + local_49a * 10 + 4) >>
                     0x11 & 3;
            if (uVar32 != 0) {
              if (uVar32 == 1) {
                uVar35 = 0x1f;
              }
              else {
                uVar35 = 0x30;
              }
              if (*pcVar11 == '\0') {
                local_49a = local_49a + *(short *)(*piVar12 + 0x176);
              }
              uVar36 = 0;
              puVar44 = (short *)uVar37;
              *puVar44 = 0;
              puVar44[1] = 0;
              puVar44[2] = 0xe;
              puVar44[3] = 0x10;
              *(short *)(uVar14 - 0x118) = uVar35;
              FUN_100b065c(uVar37,uVar14 - 0x118);
              *(short *)lVar38 = uVar36;
              *(short *)(uVar14 - 0x10c) = local_49a * 0x28 + 0xd;
              FUN_100b065c(lVar38,uVar14 - 0x10c);
              uVar20 = GetBitMapPtr(lVar38);
              uVar21 = GetBitMapPtr(uVar37);
              uVar27 = GetBitMapPtr(uVar37);
              CopyMask_Thunk(uVar18,uVar19,uVar17,uVar27,uVar21,uVar20);
              uVar15 = (unsigned long long)local_4f4;
            }
            uVar32 = (unsigned int)uVar15;
            sVar30 = (short)iVar46 + 1;
            iVar46 = (int)sVar30;
          } while (sVar30 < iVar45);
        }
        UnlockPixels_Wrapper(uVar4);
        UnlockPixels_Wrapper(uVar28);
      }
      if ((*(unsigned int *)(iVar13 + 0x28) >> 0x17 & 1) != 0) {
        EnsureSpriteLoaded(0x2a);
        uVar4 = *(int *)(*(int *)(uVar32 - 0x458) + 0x10);
        uVar28 = *(int *)(*(int *)(uVar32 - 0x46c) + 0xc4);
        uVar18 = LockPixels_Wrapper(uVar4);
        uVar19 = LockPixels_Wrapper(uVar28);
        lVar38 = uVar14 - 0x104;
        FUN_100b065c(lVar38,uVar14 - 0xfc);
        FUN_100b065c(uVar14 - 0xf4,uVar14 - 0xec);
        uVar20 = GetBitMapPtr(uVar14 - 0xf4);
        uVar21 = GetBitMapPtr(lVar38);
        uVar22 = GetBitMapPtr(lVar38);
        CopyMask_Thunk(uVar18,uVar19,uVar17,uVar22,uVar21,uVar20);
        uVar32 = local_4f4;
        UnlockPixels_Wrapper(uVar4);
        UnlockPixels_Wrapper(uVar28);
      }
      if (*(short *)(*piVar12 + 0x124) != 0) {
        if (**(char **)(uVar32 - 0x16ac) == '\0') {
          uVar4 = *(int *)(*(int *)(uVar32 - 0x328) + 0x10);
          uVar18 = LockPixels_Wrapper(uVar4);
          RGBForeColor_Thunk((unsigned long long)*(unsigned int *)(uVar32 - 0x1d4) + 0x5fa);
          Render_PenNormal();
          iVar46 = 0;
          if (0 < iVar45) {
            lVar38 = uVar14 - 0xe0;
            lVar40 = uVar14 - 0xd4;
            do {
              local_49a = *(short *)(iVar23 + iVar46 * 4 + 2);
              iVar24 = *(int *)(*(int *)(iVar13 + 0x24) +
                                ((int)*(short *)(iVar23 + iVar46 * 4) -
                                (int)*(short *)(iVar13 + 0x1e)) *
                                (((int)*(short *)(iVar13 + 0x20) - (int)*(short *)(iVar13 + 0x1c)) +
                                1) * 10 + *(short *)(iVar13 + 0x1c) * -10 + local_49a * 10 + 4);
              if (iVar24 < 0) {
                if (*pcVar11 == '\0') {
                  local_49a = local_49a + *(short *)(*piVar12 + 0x176);
                }
                uVar35 = 0;
                *(short *)lVar40 = 0;
                *(short *)(uVar14 - 200) =
                     (short)((unsigned int)(int)(short)((unsigned short)((unsigned int)iVar24 >> 0x1a) & 0x1f) / 7) * 0x29;
                FUN_100b065c(lVar40,uVar14 - 200);
                *(short *)lVar38 = uVar35;
                *(short *)(uVar14 - 0xbc) = local_49a * 0x28;
                FUN_100b065c(lVar38,uVar14 - 0xbc);
                uVar19 = GetBitMapPtr(lVar38);
                uVar20 = GetBitMapPtr(lVar40);
                uVar21 = GetBitMapPtr(lVar38);
                FUN_10000258(uVar17,uVar18,uVar17,uVar21,uVar20,uVar19,0x40,0);
              }
              sVar30 = (short)iVar46 + 1;
              iVar46 = (int)sVar30;
            } while (sVar30 < iVar45);
          }
          UnlockPixels_Wrapper(uVar4);
        }
        else {
          uVar4 = *(int *)(*(int *)(uVar32 - 0x1a24) + 0x10);
          uVar28 = *(int *)(*(int *)(uVar32 - 0x32c) + 0x10);
          uVar18 = LockPixels_Wrapper(uVar4);
          uVar19 = LockPixels_Wrapper(uVar28);
          iVar46 = 0;
          if (0 < iVar45) {
            lVar38 = uVar14 - 0xb0;
            lVar40 = uVar14 - 0xa4;
            do {
              local_49a = *(short *)(iVar23 + iVar46 * 4 + 2);
              iVar24 = *(int *)(*(int *)(iVar13 + 0x24) +
                                ((int)*(short *)(iVar23 + iVar46 * 4) -
                                (int)*(short *)(iVar13 + 0x1e)) *
                                (((int)*(short *)(iVar13 + 0x20) - (int)*(short *)(iVar13 + 0x1c)) +
                                1) * 10 + *(short *)(iVar13 + 0x1c) * -10 + local_49a * 10 + 4);
              if (iVar24 < 0) {
                if (*pcVar11 == '\0') {
                  local_49a = local_49a + *(short *)(*piVar12 + 0x176);
                }
                uVar35 = 0;
                *(short *)(uVar14 - 0x98) = 0;
                *(short *)lVar38 = 0;
                *(short *)(uVar14 - 0x8c) =
                     (short)((unsigned int)(int)(short)((unsigned short)((unsigned int)iVar24 >> 0x1a) & 0x1f) / 7) * 0x29;
                FUN_100b065c(lVar38,uVar14 - 0x8c);
                *(short *)lVar40 = uVar35;
                *(short *)(uVar14 - 0x80) = local_49a * 0x28;
                FUN_100b065c(lVar40,uVar14 - 0x80);
                uVar20 = GetBitMapPtr(lVar40);
                uVar21 = GetBitMapPtr(lVar38);
                uVar27 = GetBitMapPtr(uVar14 - 0x98);
                CopyMask_Thunk(uVar18,uVar19,uVar17,uVar27,uVar21,uVar20);
              }
              sVar30 = (short)iVar46 + 1;
              iVar46 = (int)sVar30;
            } while (sVar30 < iVar45);
          }
          UnlockPixels_Wrapper(uVar4);
          UnlockPixels_Wrapper(uVar28);
        }
      }
    }
    RestoreDrawingState(uVar14 - 0x36c,2);
    UnlockPixels_Wrapper(uVar25);
    SetGWorld_Wrapper(local_378,local_374);
    FreeBlock(iVar23);
    **(int **)(local_4f4 - 0x530) = uVar2;
  }
  else {
    FreeBlock(iVar23);
    LockHandleRange(0,local_394);
  }
  return;
}
























































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100090f0 Size: 556 bytes */
void FUN_100090f0()

{
  short sVar1;
  int *piVar2;
  int iVar3;
  int *piVar4;
  int *piVar5;
  int *piVar6;
  int *piVar7;
  int *ppuVar8;
  int iVar9;
  char uVar11;
  unsigned int uVar10;
  char *puVar12;
  int iVar13;
  short sVar14;
  short sVar15;
  short sVar16;
  int **local_3c;
  
  piVar7 = piRam1011735c;
  piVar6 = piRam10117354;
  piVar5 = piRam10115cf4;
  piVar4 = gCityOwnership;
  ppuVar8 = 0 /* TVect base */;
  if (*piRam10117354 == 0) {
    ppuVar8 = 0 /* TVect base */;
    iVar9 = AllocateBlock(0x4440);
    *piVar6 = iVar9;
  }
  piVar2 = (int *)*piVar4;
  if (piVar2 != (int *)0x0) {
    DetachResource(piVar2);
    ppuVar8 = local_3c;
  }
  puVar12 = (char *)*piVar2;
  *piVar5 = (int)puVar12;
  if (*(short *)(*piVar7 + 0x124) == 0) {
    uVar11 = 0xff;
  }
  else {
    uVar11 = 0;
  }
  iVar9 = 0;
  do {
    iVar13 = 0;
    do {
      *puVar12 = uVar11;
      puVar12 = puVar12 + 1;
      iVar3 = *piVar6 + iVar9 * 0x70;
      *(unsigned int *)(iVar3 + iVar13) = *(unsigned int *)(iVar3 + iVar13) | 0x20000000;
      iVar3 = iVar9 * 0x70 + *piVar6;
      *(unsigned int *)(iVar3 + iVar13) = *(unsigned int *)(iVar3 + iVar13) & 0x7fffffff;
      sVar16 = (short)iVar13 + 1;
      iVar13 = (int)sVar16;
    } while (sVar16 < 0x70);
    sVar16 = (short)iVar9 + 1;
    iVar9 = (int)sVar16;
  } while (sVar16 < 0x9c);
  FUN_10000ff0(*(int *)(*(int*)((char*)ppuVar8 - 0x11d)),0xffffffffffffffff,0x4440);
  uVar10 = 0;
  do {
    iVar9 = 0;
    if (0 < *(short *)(*piVar7 + 0x1602)) {
      do {
        iVar13 = *piVar7 + iVar9 * 0x42;
        if ((int)*(char *)(iVar13 + 0x1619) == uVar10) {
          sVar16 = *(short *)(iVar13 + 0x1604);
          sVar1 = *(short *)(iVar13 + 0x1606);
          sVar14 = sVar16 + -2;
          if ((int)sVar14 <= sVar16 + 3) {
            do {
              if (((-1 < sVar14) && (sVar14 < 0x70)) &&
                 (sVar15 = sVar1 + -2, (int)sVar15 <= sVar1 + 3)) {
                do {
                  if ((-1 < sVar15) && (sVar15 < 0x9c)) {
                    iVar13 = *piVar5 + sVar15 * 0x70;
                    *(unsigned char *)(iVar13 + sVar14) =
                         *(unsigned char *)(iVar13 + sVar14) | (unsigned char)(1 << (uVar10 & 0x3f));
                  }
                  sVar15 = sVar15 + 1;
                } while ((int)sVar15 <= sVar1 + 3);
              }
              sVar14 = sVar14 + 1;
            } while ((int)sVar14 <= sVar16 + 3);
          }
        }
        sVar16 = (short)iVar9 + 1;
        iVar9 = (int)sVar16;
      } while (sVar16 < *(short *)(*piVar7 + 0x1602));
    }
    sVar16 = (short)uVar10 + 1;
    uVar10 = (unsigned int)sVar16;
  } while (sVar16 < 8);
  iVar9 = *piVar4;
  if (iVar9 != 0) {
    DisposeHandle_Thunk(iVar9);
  }
  return;
}














































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1000cf78 Size: 328 bytes */
/* Address: 0x1000cf78 Size: 328 bytes */
/* AI_FindArmyForHero - clean version in ai/ai.c */

/* Address: 0x1000d0c0 Size: 228 bytes */
/* Address: 0x1000d0c0 Size: 228 bytes */
/* AI_PlaceNewUnit - clean version in ai/ai.c */

/* Address: 0x1000f7bc Size: 776 bytes */
void FUN_1000f7bc(short param_1)

{
  int bVar1;
  int *piVar2;
  int *piVar3;
  int iVar4;
  int iVar5;
  short sVar6;
  int iVar7;
  int iVar8;
  short sVar9;
  int iVar10;
  unsigned long long uVar11;
  int iVar12;
  short auStack_40 [8];
  short local_30 [24];
  
  piVar3 = piRam10117468;
  piVar2 = piRam1011735c;
  iVar12 = *piRam10117468 + param_1 * 0x5c;
  iVar10 = iVar12 + 0x252;
  if ((*(unsigned short *)(iVar12 + 0x2a6) & 0x10) == 0) {
    if ((*(unsigned short *)(*piRam10117468 + param_1 * 0x5c + 0x2a6) & 8) == 0) {
      iVar12 = (int)*(short *)(*piRam10117468 + param_1 * 0x5c + 0x2a4);
    }
    else {
      iVar12 = 0x10;
    }
  }
  else {
    iVar12 = 0xc;
  }
  iVar5 = 3;
  do {
    local_30[iVar5 + -4] = -1;
    auStack_40[iVar5] = 0xffff;
    local_30[iVar5 + 2] = -1;
    *(short *)(iVar5 * 2 + iVar10) = 0xffff;
    bVar1 = iVar5 != 0;
    iVar5 = (int)(short)((short)iVar5 + -1);
  } while (bVar1);
  sVar9 = *(short *)(*piVar2 + 0x1602);
  do {
    do {
      if (sVar9 == 0) {
        iVar12 = 0;
        do {
          sVar9 = *(short *)(iVar10 + iVar12 * 2);
          if (sVar9 != -1) {
            *(char *)((int)sVar9 + *piVar3 + 0x56) = 6;
          }
          sVar9 = (short)iVar12 + 1;
          iVar12 = (int)sVar9;
        } while (sVar9 < 4);
        return;
      }
      sVar9 = sVar9 + -1;
      uVar11 = (unsigned long long)sVar9;
    } while ((((((int)*(char *)(*piVar2 + (int)(((uVar11 & 0x7ffffff) * 0x20 + uVar11 & 0xffffffff)
                                               << 1) + 0x1619) != (int)*(short *)(*piVar2 + 0x110))
               || (iVar5 = CanArmyEnterCity(uVar11,0), iVar5 == 0)) ||
              (((*(unsigned short *)(*piVar3 + param_1 * 0x5c + 0x2a6) & 0x10) != 0 &&
               ((*(unsigned char *)(*piVar3 + (int)sVar9 + 0x11e) & 8) == 0)))) ||
             ((((*(unsigned short *)(*piVar3 + param_1 * 0x5c + 0x2a6) & 8) != 0 &&
               ((*(unsigned char *)(*piVar3 + (int)sVar9 + 0x11e) & 0x80) == 0)) ||
              (((*(unsigned short *)(*piVar3 + param_1 * 0x5c + 0x2a6) & 0x20) != 0 &&
               ((*(unsigned char *)(*piVar3 + (int)sVar9 + 0x11e) & 0x40) == 0)))))) ||
            (((((*(unsigned short *)(*piVar3 + param_1 * 0x5c + 0x2a6) & 0x38) == 0 &&
               ((*(unsigned char *)(*piVar3 + (int)sVar9 + 0x11e) & 200) != 0)) ||
              (iVar5 = GetArmyInfo(uVar11,local_30), iVar5 == 0)) ||
             (*(char *)(*piVar2 + (int)(((uVar11 & 0x7ffffff) * 0x20 + uVar11 & 0xffffffff) << 1) +
                        (int)local_30[0] + 0x1626) < iVar12))));
    iVar7 = -1;
    iVar5 = 100;
    iVar4 = 0;
    do {
      iVar8 = iVar4;
      if (*(short *)(iVar10 + iVar8 * 2) == -1) break;
      if (local_30[iVar8 + 2] < iVar5) {
        iVar5 = (int)local_30[iVar8 + 2];
        iVar7 = iVar8;
      }
      sVar6 = (short)iVar8 + 1;
      iVar8 = iVar7;
      iVar4 = (int)sVar6;
    } while (sVar6 < 4);
    if ((iVar8 != -1) &&
       ((*(short *)(iVar10 + iVar8 * 2) == -1 ||
        (iVar5 <= *(char *)(*piVar2 + (int)(((uVar11 & 0x7ffffff) * 0x20 + uVar11 & 0xffffffff) << 1
                                           ) + (int)local_30[0] + 0x1622))))) {
      *(short *)(iVar10 + iVar8 * 2) = sVar9;
      local_30[iVar8 + 2] =
           (short)*(char *)(*piVar2 + (int)(((uVar11 & 0x7ffffff) * 0x20 + uVar11 & 0xffffffff) << 1
                                           ) + (int)local_30[0] + 0x1622);
    }
  } while( true );
}

/* Address: 0x1000fac4 Size: 228 bytes */
void FUN_1000fac4()

{
  short sVar1;
  int iVar2;
  int *piVar3;
  int *piVar4;
  long long lVar5;
  
  piVar4 = piRam10117468;
  piVar3 = piRam1011735c;
  sVar1 = *(short *)(*piRam1011735c + 0x1602);
  while (sVar1 != 0) {
    sVar1 = sVar1 + -1;
    iVar2 = *piVar3;
    if (((int)*(char *)(iVar2 + sVar1 * 0x42 + 0x1619) == (int)*(short *)(iVar2 + 0x110)) &&
       (iVar2 = *piVar4 + (int)sVar1, *(char *)(iVar2 + 0x56) == '\x06')) {
      *(char *)(iVar2 + 0x56) = 8;
      *(short *)(sVar1 * 0x42 + *piVar3 + 0x1636) = 0;
    }
  }
  lVar5 = 3;
  do {
    iVar2 = (int)lVar5;
    if (*(short *)(*piVar4 + iVar2 * 0x5c + 0x24c) != 0) {
      FUN_1000f7bc(lVar5);
    }
    lVar5 = (long long)(short)((short)lVar5 + -1);
  } while (iVar2 != 0);
  return;
}

/* Address: 0x1000fba8 Size: 144 bytes */
void FUN_1000fba8(short param_1,short param_2)

{
  int iVar1;
  
  iVar1 = *piRam1011735c + *(short *)(*piRam1011735c + 0x110) * 2;
  if ((((*(short *)(iVar1 + 0x148) != 0) || (*(short *)(iVar1 + 0xd0) != 1)) &&
      (iVar1 = *piRam1011735c, *(short *)(iVar1 + 0x178) <= param_1)) &&
     (((param_1 <= *(short *)(iVar1 + 0x17c) && (*(short *)(iVar1 + 0x176) <= param_2)) &&
      (param_2 <= *(short *)(iVar1 + 0x17a))))) {
    MapRefreshAndCombat();
  }
  return;
}

/* Address: 0x1000fccc Size: 280 bytes */
void FUN_1000fccc(short param_1)

{
  short *psVar1;
  int *piVar2;
  unsigned int *puVar3;
  int iVar4;
  unsigned long long uVar5;
  long long lVar6;
  short sVar7;
  int iVar8;
  short sVar9;
  long long lVar10;
  unsigned long long uVar11;
  
  iVar4 = gCombatDisplayPieces;
  puVar3 = puRam10117360;
  piVar2 = piRam1011735c;
  uVar5 = ZEXT48(0 /* TVect base */);
  lVar10 = -1;
  sVar9 = -1;
  uVar11 = 0;
  do {
    if (*(int *)((int)((uVar11 & 0xffffffff) << 2) + iVar4) != 0) {
      lVar10 = (long long)(short)(*(short *)(*piVar2 + 0x182) + -1);
      lVar6 = lVar10;
      if (*(short *)(*piVar2 + 0x182) != 0) {
        do {
          lVar6 = lVar10;
          if (((unsigned long long)*puVar3 + lVar10 * 0x16 & 0xffffffff) ==
              (unsigned long long)*(unsigned int *)((int)((uVar11 & 0xffffffff) << 2) + iVar4)) break;
          lVar6 = (long long)(short)((short)lVar10 + -1);
          iVar8 = (int)lVar10;
          lVar10 = lVar6;
        } while (iVar8 != 0);
      }
      psVar1 = *(short **)((int)((uVar11 & 0xffffffff) << 2) + iVar4);
      lVar10 = (long long)*psVar1;
      sVar9 = psVar1[1];
      if ((int)lVar6 != -1) {
        DisbandUnit(lVar6,0);
      }
      *(int *)((int)((uVar11 & 0xffffffff) << 2) + iVar4) = 0;
    }
    sVar7 = (short)uVar11 + 1;
    uVar11 = (unsigned long long)sVar7;
    if (7 < sVar7) {
      **(int **)((int)uVar5 + -0x3f0) = 0;
      **(int **)((int)uVar5 + -0x1c0) = 0;
      if ((param_1 != 0) && ((int)lVar10 != -1)) {
        RefreshMapAfterCombat(lVar10,sVar9);
      }
      UpdateDisplayState();
      return;
    }
  } while( true );
}









































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1000fe90 Size: 336 bytes */
long long FUN_1000fe90(short param_1)

{
  int bVar1;
  char cVar2;
  unsigned char bVar3;
  int iVar4;
  long long lVar5;
  int *piVar6;
  int *piVar7;
  long long uVar8;
  int iVar9;
  int iVar10;
  short sVar11;
  long long lVar12;
  short sVar13;
  unsigned char abStack_20 [8];
  char auStack_18 [24];
  
  piVar7 = piRam1011735c;
  piVar6 = piRam10117358;
  GetAdjacentArmies(param_1,0,abStack_20,auStack_18);
  sVar11 = 0;
  sVar13 = 0;
  uVar8 = 2;
  iVar10 = 5;
  do {
    bVar3 = abStack_20[iVar10];
    if (bVar3 != 0xff) {
      iVar4 = *piVar7;
      iVar9 = iVar4 + (unsigned int)bVar3 * 0x42;
      cVar2 = *(char *)(iVar9 + 0x1619);
      if ((cVar2 != 0xf) && ((int)cVar2 != (int)*(short *)(iVar4 + 0x110))) {
        lVar5 = -(long long)
                 *(char *)((*(unsigned int *)(*piVar6 + *(short *)(iVar9 + 0x1606) * 0xe0 +
                                     *(short *)(iVar9 + 0x1604) * 2) >> 0x18) + iVar4 + 0x711);
        lVar12 = lVar5 + 10;
        if ((lVar12 + (-(unsigned long long)(lVar12 == 0) - (lVar5 + 9)) & 0xff) == 0) {
          sVar11 = sVar11 + 1;
          if ((*(unsigned int *)(*(short *)(iVar4 + 0x110) * 0x10 + iVar4 + cVar2 * 2 + 0x1582) >> 0x1a & 3)
              == 2) {
            sVar13 = sVar13 + 1;
          }
        }
      }
    }
    bVar1 = iVar10 != 0;
    iVar10 = (int)(short)((short)iVar10 + -1);
  } while (bVar1);
  if (sVar13 == 0) {
    if (sVar11 < 2) {
      if (sVar11 != 0) {
        uVar8 = 3;
      }
    }
    else {
      uVar8 = 4;
    }
  }
  else {
    uVar8 = 8;
  }
  return uVar8;
}

/* Address: 0x1000ffe0 Size: 1868 bytes */
long long FUN_1000ffe0(short param_1,short param_2,int param_3,int param_4)

{
  int bVar1;
  char cVar2;
  char cVar3;
  char cVar4;
  short sVar5;
  int iVar6;
  int iVar7;
  int bVar8;
  int *piVar9;
  int *piVar10;
  int iVar11;
  short sVar13;
  int iVar12;
  int iVar14;
  int iVar15;
  unsigned short uVar16;
  short sVar18;
  short sVar19;
  short sVar20;
  int iVar17;
  int iVar21;
  int iVar22;
  long long lVar23;
  long long lVar24;
  unsigned long long uVar25;
  int iVar26;
  
  piVar10 = piRam10117364;
  piVar9 = piRam10117360;
  if (param_2 < 0xd) {
    sVar13 = (short)*(char *)(param_2 + gFrontLineLookup);
  }
  else {
    sVar13 = 8;
  }
  lVar24 = (long long)sVar13;
  uVar25 = 0;
  bVar8 = false;
  lVar23 = 0;
  iVar26 = 0;
  uVar16 = 0;
  iVar11 = (int)param_2;
  if (*(char *)(*piRam10117468 + (int)param_1 + 0x56) == '\a') {
    sVar20 = 0;
    sVar19 = 0;
    sVar18 = param_2;
    if (param_2 != 0) {
      do {
        sVar18 = sVar18 + -1;
        sVar5 = *(short *)(param_3 + sVar18 * 2);
        if (sVar5 != -1) {
          if (*(char *)(*piRam10117360 + sVar5 * 0x16 + 4) == '\x1c') {
            sVar19 = sVar19 + 1;
          }
          else {
            iVar26 = *piRam10117360 + sVar5 * 0x16;
            if ((*(char *)(*piRam10117364 + *(char *)(iVar26 + 4) * 6) != '\0') &&
               (('\x03' < *(char *)(iVar26 + 8) || (sVar20 == 0)))) {
              sVar20 = sVar20 + 1;
            }
          }
        }
      } while (sVar18 != 0);
    }
    if ((sVar20 != 0) && (sVar19 != 0)) {
      sVar20 = sVar20 + sVar19;
    }
    if (((3 < sVar20) && (iVar11 < 0x11)) || ((bVar8 = false, 2 < sVar20 && (iVar11 < 9)))) {
      bVar8 = true;
    }
    iVar14 = 3;
    iVar26 = 0;
    do {
      iVar21 = *piRam10117468 + iVar14 * 0x5c;
      if ((*(short *)(iVar21 + 0x24c) != 0) && (*(short *)(iVar21 + 0x250) == param_1)) {
        if ((*(unsigned short *)(iVar21 + 0x2a6) & 0x10) == 0) {
          if ((*(unsigned short *)(*piRam10117468 + iVar14 * 0x5c + 0x2a6) & 8) == 0) {
            iVar26 = (int)*(short *)(*piRam10117468 + iVar14 * 0x5c + 0x2a4);
          }
          else {
            iVar26 = 0x10;
          }
        }
        else {
          iVar26 = 0xc;
        }
        if ((*(unsigned short *)(*piRam10117468 + iVar14 * 0x5c + 0x2a6) & 0x20) != 0) {
          bVar8 = true;
        }
      }
      bVar1 = iVar14 != 0;
      iVar14 = (int)(short)((short)iVar14 + -1);
    } while (bVar1);
  }
  sVar18 = 3 - *(unsigned char *)(*piRam10117468 + (int)param_1 + 0xba) / 3;
  if (((0 < sVar18) && (sVar18 < iVar11)) && ((int)param_2 - (int)sVar13 < (int)sVar18)) {
    lVar24 = (long long)(short)(param_2 - sVar18);
  }
  while( true ) {
    sVar13 = param_2;
    iVar14 = -1;
    iVar21 = -1;
    iVar6 = -1;
    iVar7 = -1;
    iVar15 = iVar11;
    while (iVar15 != 0) {
      sVar13 = sVar13 + -1;
      iVar15 = (int)sVar13;
      sVar18 = *(short *)(param_3 + iVar15 * 2);
      iVar17 = (int)sVar18;
      if (sVar18 != -1) {
        iVar12 = *piVar9 + iVar17 * 0x16;
        cVar2 = *(char *)(iVar12 + 8);
        iVar22 = (int)cVar2;
        if ((int)uVar25 < (int)lVar24) {
          cVar3 = *(char *)(iVar12 + 4);
          cVar4 = *(char *)(*piVar10 + cVar3 * 6 + 5);
          if (((uVar16 & 1) == 0) && (cVar3 == '\x1c')) {
            iVar22 = (int)(short)(cVar2 + 1000);
          }
          else {
            iVar12 = *piVar9 + iVar17 * 0x16;
            if (iVar26 <= *(char *)(iVar12 + 6)) {
              if (((uVar16 & 0x10) == 0) &&
                 (*(char *)(*piVar10 + *(char *)(iVar12 + 4) * 6) != '\0')) {
                iVar22 = (int)(short)(cVar2 + 900);
              }
              else if (!bVar8) {
                if (((uVar16 & 2) == 0) && (cVar4 == '\x02')) {
                  iVar22 = (int)(short)(cVar2 + 800);
                }
                else if (((uVar16 & 4) == 0) && (cVar4 == '\x03')) {
                  iVar22 = (int)(short)(cVar2 + 700);
                }
                else if (((uVar16 & 0x80) == 0) &&
                        (*(char *)(*piVar10 + *(char *)(*piVar9 + sVar18 * 0x16 + 4) * 6 + 4) !=
                         '\0')) {
                  iVar22 = (int)(short)(cVar2 + 600);
                }
                else if (((uVar16 & 8) == 0) && (cVar4 == '\x01')) {
                  iVar22 = (int)(short)(cVar2 + 500);
                }
                else if (((uVar16 & 0x20) == 0) &&
                        (*(char *)(*piVar10 + *(char *)(*piVar9 + sVar18 * 0x16 + 4) * 6 + 1) !=
                         '\0')) {
                  iVar22 = (int)(short)(cVar2 + 400);
                }
                else if (((uVar16 & 0x40) == 0) &&
                        (*(char *)(*piVar10 + *(char *)(*piVar9 + sVar18 * 0x16 + 4) * 6 + 2) !=
                         '\0')) {
                  iVar22 = (int)(short)(cVar2 + 300);
                }
              }
            }
          }
        }
        else {
          uVar16 = 0xfff;
        }
        if (((uVar16 & 1) != 0) && (*(char *)(*piVar9 + sVar18 * 0x16 + 4) == '\x1c')) {
          iVar22 = 1;
        }
        if (((uVar16 & 0x80) != 0) &&
           (*(char *)(*piVar10 + *(char *)(*piVar9 + sVar18 * 0x16 + 4) * 6 + 4) != '\0')) {
          iVar22 = 2;
        }
        iVar17 = *piVar9 + iVar17 * 0x16;
        if ((*(char *)(iVar17 + 6) < iVar26) ||
           (((bVar8 && (cVar2 = *(char *)(iVar17 + 4), *(char *)(*piVar10 + cVar2 * 6) == '\0')) &&
            (cVar2 != '\x1c')))) {
          if (iVar14 < iVar22) {
            iVar14 = iVar22;
            iVar7 = iVar15;
          }
        }
        else if (iVar21 < iVar22) {
          iVar21 = iVar22;
          iVar6 = iVar15;
        }
      }
    }
    if ((iVar7 == -1) && (iVar6 == -1)) break;
    if (uVar16 != 0xfff) {
      if (iVar6 == -1) {
        sVar13 = *(short *)(param_3 + iVar7 * 2);
      }
      else {
        sVar13 = *(short *)(param_3 + iVar6 * 2);
      }
      cVar2 = *(char *)(*piVar9 + sVar13 * 0x16 + 4);
      cVar3 = *(char *)(*piVar10 + cVar2 * 6 + 5);
      if (cVar2 == '\x1c') {
        uVar16 = uVar16 | 1;
      }
      else {
        if (cVar3 == '\x02') {
          uVar16 = uVar16 | 2;
        }
        if (cVar3 == '\x03') {
          uVar16 = uVar16 | 4;
        }
        if (*(char *)(*piVar10 + *(char *)(*piVar9 + sVar13 * 0x16 + 4) * 6) != '\0') {
          uVar16 = uVar16 | 0x10;
        }
        if (*(char *)(*piVar10 + *(char *)(*piVar9 + sVar13 * 0x16 + 4) * 6 + 4) != '\0') {
          uVar16 = uVar16 | 0x80;
        }
        if (cVar3 == '\x01') {
          uVar16 = uVar16 | 8;
        }
        if (*(char *)(*piVar10 + *(char *)(*piVar9 + sVar13 * 0x16 + 4) * 6 + 1) != '\0') {
          uVar16 = uVar16 | 0x20;
        }
        if (*(char *)(*piVar10 + *(char *)(*piVar9 + sVar13 * 0x16 + 4) * 6 + 2) != '\0') {
          uVar16 = uVar16 | 0x40;
        }
      }
    }
    if (iVar6 == -1) {
      *(short *)(param_4 + (int)((uVar25 & 0xffffffff) << 1)) =
           *(short *)(param_3 + iVar7 * 2);
      *(short *)(param_3 + iVar7 * 2) = 0xffff;
      uVar25 = (long long)(short)((short)uVar25 + 1);
    }
    else {
      *(short *)(param_4 + (int)((uVar25 & 0xffffffff) << 1)) =
           *(short *)(param_3 + iVar6 * 2);
      *(short *)(param_3 + iVar6 * 2) = 0xffff;
      lVar23 = (long long)(short)((short)lVar23 + 1);
      uVar25 = (long long)(short)((short)uVar25 + 1);
    }
  }
  if ((int)lVar24 < (int)lVar23) {
    lVar23 = lVar24;
  }
  return lVar23;
}


























































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1001072c Size: 1028 bytes */
/* Address: 0x1001072c Size: 1028 bytes */
/* AI_CullWeakUnits - clean version in ai/ai.c */

/* Address: 0x10010b30 Size: 2468 bytes */
/* Address: 0x10010b30 Size: 2468 bytes */
/* AI_ArmyManagement - clean version in ai/ai.c */




























































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100121f8 Size: 300 bytes */
/* Address: 0x100121f8 Size: 300 bytes */
/* AI_CheckArmyOwnership - clean version in ai/ai.c */

/* Address: 0x10012324 Size: 248 bytes */
/* Address: 0x10012324 Size: 248 bytes */
/* AI_EarlyGameAttack - clean version in ai/ai.c */

/* Address: 0x1001241c Size: 648 bytes */
/* Address: 0x1001241c Size: 648 bytes */
/* AI_SearchForRuins - clean version in ai/ai.c */
































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100126a4 Size: 1000 bytes */
/* Address: 0x100126a4 Size: 1000 bytes */
/* AI_AttackNeutralArmies - clean version in ai/ai.c */

long long ProcessQuestEvent();  /* forward declaration */

/* Address: 0x10012a8c Size: 572 bytes */
long long FUN_10012a8c(long long param_1,short param_2)

{
  int iVar1;
  short sVar2;
  int *piVar3;
  int *piVar4;
  int bVar5;
  
  piVar4 = piRam10117468;
  piVar3 = piRam1011735c;
  if (*(short *)(*piRam1011735c + 0x11e) != 0) {
    param_1 = (long long)(short)param_1;
    bVar5 = false;
    if (*(short *)(*piRam1011735c + *(short *)(*piRam1011735c + 0x110) * 0xc + 0x1142) != 0) {
      sVar2 = (short)*gSelectionCount;
      if (*gSelectionCount != '\0') {
        do {
          sVar2 = sVar2 + -1;
          if (*(int *)(sVar2 * 4 + gSelectionArray) ==
              *piRam10117360 +
              *(short *)(*piRam1011735c + *(short *)(*piRam1011735c + 0x110) * 0xc + 0x1146) * 0x16)
          {
            bVar5 = true;
          }
        } while (sVar2 != 0);
      }
      if ((bVar5) &&
         (((sVar2 = *(short *)(*piRam1011735c + *(short *)(*piRam1011735c + 0x110) * 0xc + 0x1144),
           sVar2 == 4 || (sVar2 == 5)) &&
          (param_1 = LookupCityAtPos(param_1,param_2),
          (int)param_1 == (int)*(short *)(*piVar3 + *(short *)(*piVar3 + 0x110) * 0xc + 0x1148)))))
      {
        iVar1 = *piVar4;
        *(short *)(iVar1 + 0x4a) = *(short *)(iVar1 + 0x4a) + 1;
        *(short *)(*piVar4 + 0x46) = 0xffff;
        if (*(short *)(*piVar3 + *(short *)(*piVar3 + 0x110) * 0xc + 0x1144) == 5) {
          LaunchAllianceAttack(param_1,1);
          if (*(short *)(*piVar3 + *(short *)(*piVar3 + 0x110) * 0xc + 0x1142) == 0) {
            param_1 = 0;
          }
          else {
            param_1 = ProcessQuestEvent(2,0,0,0);
          }
        }
        else if (*(short *)(*piVar3 + *(short *)(*piVar3 + 0x110) * 0xc + 0x1142) == 0) {
          param_1 = 0;
        }
        else {
          param_1 = ProcessQuestEvent(4,0,0,0);
        }
      }
    }
  }
  return param_1;
}


















































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10013a10 Size: 728 bytes */
/* Address: 0x10013a10 Size: 728 bytes */
/* AI_AttackCity - clean version in ai/ai.c */



















































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100143b8 Size: 528 bytes */
/* Address: 0x100143b8 Size: 528 bytes */
/* AI_FindBestMovableArmy - clean version in ai/ai.c */

/* Address: 0x10015c48 Size: 384 bytes */
/* Address: 0x10015c48 Size: 384 bytes */
/* AI_CheckRuinAccessible - clean version in ai/ai.c */

/* Address: 0x10016344 Size: 416 bytes */
/* Address: 0x10016344 Size: 416 bytes */
/* AI_AttackEnemyArmy - clean version in ai/ai.c */

/* Address: 0x10016cc4 Size: 300 bytes */
void FUN_10016cc4(int param_1,int param_2,int param_3,short param_4)

{
  char cVar1;
  int iVar2;
  int bVar3;
  int *piVar4;
  int *piVar5;
  int iVar6;
  int iVar7;
  short sVar8;
  int iVar9;
  short sVar10;
  int iVar11;
  
  piVar5 = piRam10117468;
  piVar4 = piRam1011735c;
  do {
    bVar3 = true;
    iVar9 = 0;
    do {
      if ((*(short *)(param_1 + iVar9 * 2) != -1) && ('\x01' < *(char *)(param_2 + iVar9))) {
        iVar6 = 0;
        do {
          if (((iVar9 != iVar6) && (sVar10 = *(short *)(param_1 + iVar6 * 2), sVar10 != -1)) &&
             (*(char *)(iVar6 + param_2) == '\0')) {
            iVar11 = 0;
            do {
              iVar7 = param_3 + iVar9 * 0x16;
              cVar1 = *(char *)(iVar7 + iVar11);
              if ((cVar1 != 0xff) &&
                 (iVar2 = cVar1 * 0x1e, (int)*(char *)(*piVar4 + iVar2 + 0xd26) == (int)param_4)) {
                *(char *)(iVar7 + iVar11) = 0xff;
                *(short *)(*piVar5 + 0x4c) = *(short *)(*piVar5 + 0x4c) + 1;
                *(short *)(*piVar4 + iVar2 + 0xd2a) = sVar10;
                *(char *)(param_2 + iVar9) = *(char *)(param_2 + iVar9) + -1;
                *(char *)(param_2 + iVar6) = *(char *)(param_2 + iVar6) + '\x01';
                bVar3 = false;
                break;
              }
              sVar8 = (short)iVar11 + 1;
              iVar11 = (int)sVar8;
            } while (sVar8 < 0x16);
          }
          sVar10 = (short)iVar6 + 1;
          iVar6 = (int)sVar10;
        } while (sVar10 < 8);
      }
      sVar10 = (short)iVar9 + 1;
      iVar9 = (int)sVar10;
    } while (sVar10 < 8);
    if (bVar3) {
      return;
    }
  } while( true );
}
























































































/* Address: 0x10018574 Size: 344 bytes */
long long FUN_10018574(short param_1,int param_2,int param_3)

{
  unsigned char bVar1;
  long long lVar2;
  int iVar3;
  int *piVar4;
  int *piVar5;
  int *piVar6;
  int *piVar7;
  unsigned long long uVar8;
  int iVar9;
  short sVar10;
  long long lVar11;
  int iVar12;
  long long lVar13;
  
  piVar7 = piRam10117470;
  piVar6 = piRam10117468;
  piVar5 = piRam1011735c;
  piVar4 = piRam10117358;
  uVar8 = (unsigned long long)param_1;
  lVar11 = 0;
  iVar12 = 0;
  do {
    if (param_2 != 0) {
      *(char *)(iVar12 + param_2) = 0xff;
    }
    if (param_3 != 0) {
      *(char *)(iVar12 + param_3) = 0;
    }
    iVar3 = *piVar7 + (int)(((uVar8 & 0x3fffffff) * 4 - uVar8 & 0xffffffff) << 1);
    bVar1 = *(unsigned char *)(iVar3 + iVar12);
    if ((bVar1 != 0xff) && ((unsigned short)bVar1 != *(unsigned short *)(*piVar6 + 0x46))) {
      iVar9 = *piVar5 + (unsigned int)bVar1 * 0x42;
      lVar2 = -(long long)
               *(char *)(*piVar5 + (*(unsigned int *)(*piVar4 + *(short *)(iVar9 + 0x1606) * 0xe0 +
                                             *(short *)(iVar9 + 0x1604) * 2) >> 0x18) + 0x711);
      lVar13 = lVar2 + 10;
      if ((((lVar13 + (-(unsigned long long)(lVar13 == 0) - (lVar2 + 9)) & 0xff) == 0) &&
          (*(char *)(iVar9 + 0x1619) == '\x0f')) &&
         ((((int)lVar11 == 0 || (iVar12 < 2)) || (*(unsigned char *)(iVar3 + iVar12 + 600) < 0x14)))) {
        if (param_2 != 0) {
          *(unsigned char *)(iVar12 + param_2) = bVar1;
        }
        if (param_3 != 0) {
          *(char *)(param_3 + iVar12) =
               *(char *)
                (*piVar7 + (int)(((uVar8 & 0x3fffffff) * 4 - uVar8 & 0xffffffff) << 1) + iVar12 +
                600);
        }
        lVar11 = (long long)(short)((short)lVar11 + 1);
      }
    }
    sVar10 = (short)iVar12 + 1;
    iVar12 = (int)sVar10;
  } while (sVar10 < 6);
  return lVar11;
}























































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1001a12c Size: 540 bytes */
void FUN_1001a12c(short param_1,short param_2)

{
  int bVar1;
  int *piVar2;
  int *ppuVar3;
  int iVar4;
  int iVar5;
  short local_98;
  short local_96;
  short local_94 [2];
  char auStack_90 [104];
  short local_28;
  short local_26 [19];
  
  piVar2 = piRam10117360;
  ppuVar3 = 0 /* TVect base */;
  iVar5 = (int)param_1;
  iVar4 = FUN_10019174(iVar5,local_26,&local_28,local_94,&local_96,&local_98);
  if (iVar4 == 0) {
    if ((*(char *)(*(int *)(*(int*)((char*)ppuVar3 - 0x14f)) + *(char *)(*piVar2 + param_1 * 0x16 + 4) * 6) == '\0')
       || (iVar4 = FUN_10019f14(iVar5,local_26,&local_28), iVar4 == 0)) {
      iVar4 = 99;
      do {
        auStack_90[iVar4] = 0;
        bVar1 = iVar4 != 0;
        iVar4 = (int)(short)((short)iVar4 + -1);
      } while (bVar1);
      if ((param_2 == -1) || (iVar4 = FUN_1001a0a0(param_2,auStack_90), iVar4 == 0xff)) {
        iVar5 = iVar5 * 0x16;
        *(unsigned int *)(*piVar2 + iVar5 + 0xc) = *(unsigned int *)(*piVar2 + iVar5 + 0xc) & 0xffff0fff;
        *(unsigned int *)(iVar5 + *piVar2 + 0xc) = *(unsigned int *)(iVar5 + *piVar2 + 0xc) & 0xffffff80;
        *(unsigned short *)(iVar5 + *piVar2 + 0xc) = *(unsigned short *)(iVar5 + *piVar2 + 0xc) & 0xffdf;
        *(short *)(iVar5 + *piVar2 + 0x14) = 0xffff;
        *(short *)(iVar5 + *piVar2 + 0x12) = *(short *)(iVar5 + *piVar2 + 0x14);
        *(char *)(iVar5 + *piVar2 + 0x11) = 0;
      }
      else {
        iVar4 = *(int *)(*(int*)((char*)ppuVar3 - 0x151)) + iVar4 * 0x42;
        FUN_10019e00(iVar5,*(short *)(iVar4 + 0x1604),*(short *)(iVar4 + 0x1606));
      }
    }
    else {
      FUN_10019e00(iVar5,local_26[0],local_28);
    }
  }
  else {
    if ((local_94[0] != 0) &&
       ((local_94[0] != 1 || (iVar4 = FUN_10019718(iVar5,local_96,local_98), iVar4 != 0)))) {
      if (local_94[0] != 2) {
        return;
      }
      iVar4 = FUN_10019a40(iVar5,local_96,local_98);
      if (iVar4 != 0) {
        return;
      }
    }
    FUN_10019e00(iVar5,local_26[0],local_28);
  }
  return;
}

/* Address: 0x1001a864 Size: 568 bytes */
short FUN_1001a864(short param_1,long long param_2)

{
  char cVar1;
  short sVar2;
  int *piVar3;
  int *piVar4;
  int iVar5;
  short sVar6;
  short sVar7;
  int iVar8;
  int iVar9;
  char auStack_2c [44];
  
  piVar4 = piRam10117468;
  piVar3 = piRam1011735c;
  if (*(short *)(*piRam1011735c + 0x1602) == 0) {
    sVar7 = 0;
  }
  else {
    sVar2 = *(short *)(*piRam1011735c + 0x1602);
    sVar7 = 0;
    do {
      sVar2 = sVar2 + -1;
      iVar8 = (int)sVar2;
      iVar9 = (int)sVar2;
      if ((int)*(char *)(*piVar3 + iVar8 * 0x42 + 0x1619) == (int)*(short *)(*piVar3 + 0x110)) {
        if (param_1 == 0) {
          if (*(char *)(*piVar4 + (int)sVar2 + 0x56) == '\x01') goto LAB_1001a920;
        }
        else {
          if (*(char *)(*piVar4 + iVar9 + 0x56) != '\x01') goto LAB_1001aa70;
LAB_1001a920:
/*           AI_ArmyManagement(iVar8,1); -- clean version in ai/ai.c */
          *(char *)(iVar9 + *piVar4 + 0x56) = 3;
        }
        iVar5 = GetNeighborArmies(iVar8,auStack_2c,0);
        if (iVar5 == 0) {
          cVar1 = *(char *)(*piVar4 + iVar9 + 0x56);
          if ((cVar1 == '\x03') || (cVar1 == '\x02')) {
            *(char *)(*piVar4 + iVar9 + 0x56) = 4;
          }
          if ((*(short *)(*piVar3 + 0x128) == 0) && (*(char *)(*piVar4 + iVar9 + 0x56) == '\x05')) {
            sVar6 = *(short *)(*piVar3 + 0x136);
            if (sVar6 < 2) {
              sVar6 = 1;
            }
            if (sVar6 < 6) goto LAB_1001a9cc;
          }
        }
        else {
LAB_1001a9cc:
          sVar7 = sVar7 + 1;
          iVar5 = FUN_10018b14(iVar8,param_2);
          if (iVar5 == 0) {
            iVar8 = FUN_1001a0a0(iVar8,param_2);
            if (iVar8 == 0xff) {
              *(char *)(iVar9 + *piVar4 + 0x56) = 4;
            }
            else {
              *(char *)(*piVar4 + iVar9 + 0x56) = 3;
            }
          }
          else {
            sVar6 = *(short *)(*piVar3 + 0x136);
            if (sVar6 < 2) {
              sVar6 = 1;
            }
            FUN_1001a470(iVar8,10 < sVar6);
            *(char *)(iVar9 + *piVar4 + 0x56) = 2;
          }
        }
      }
LAB_1001aa70:
    } while (iVar9 != 0);
  }
  return sVar7;
}

/* Address: 0x1001ab94 Size: 328 bytes */
/* Address: 0x1001ab94 Size: 328 bytes */
/* AI_CheckArmyPathsBlocked - clean version in ai/ai.c */

/* Address: 0x1001b584 Size: 860 bytes */
short FUN_1001b584(long long param_1,short *param_2,unsigned short param_3,short param_4,int *param_5)

{
  char cVar1;
  int *piVar2;
  unsigned int uVar3;
  int *piVar4;
  int *piVar5;
  short *psVar6;
  unsigned long long uVar7;
  int *ppuVar8;
  int iVar9;
  int iVar10;
  int iVar11;
  short sVar12;
  unsigned short uVar13;
  short sVar14;
  short sVar15;
  unsigned long long uVar16;
  int iVar17;
  unsigned long long uVar18;
  unsigned int local_7c;
  short local_58;
  short local_56;
  short local_54;
  short local_52;
  short local_50;
  short local_4e;
  
  psVar6 = psRam101176fc;
  piVar5 = piRam1011735c;
  piVar4 = piRam10117358;
  piVar2 = piRam10117354;
  uVar7 = ZEXT48(0 /* TVect base */);
  ppuVar8 = 0 /* TVect base */;
  local_52 = *param_2;
  local_50 = *(short *)(*piRam10117360 + local_52 * 0x16);
  iVar17 = *piRam10117360 + local_52 * 0x16;
  sVar14 = *(short *)(iVar17 + 2);
  iVar11 = -1;
  uVar16 = 0xffffffffffffffff;
  iVar10 = 0;
  local_4e = 0;
  if ((*(unsigned short *)(iVar17 + 0xc) & 0x1000) == 0) {
    sVar15 = local_50 - param_4;
    if ((int)sVar15 < (int)local_50 + (int)param_4) {
      do {
        iVar17 = (int)sVar15;
        sVar12 = sVar14 - param_4;
        if ((int)sVar12 < (int)sVar14 + (int)param_4) {
          do {
            uVar18 = (unsigned long long)sVar12;
            if (((((-1 < sVar15) && (-1 < sVar12)) && (sVar15 < 0x70)) &&
                (((sVar12 < 0x9c &&
                  (uVar3 = *(unsigned int *)(*piVar4 + (int)(((uVar18 & 0x1fffffff) * 8 - uVar18 &
                                                     0xffffffff) << 5) + iVar17 * 2),
                  (uVar3 >> 0x14 & 1) != 0)) &&
                 ((((unsigned short)(uVar3 >> 0x10) & 0xf) == param_3 &&
                  ((cVar1 = *(char *)(*piVar5 + (uVar3 >> 0x18) + 0x711), cVar1 != '\n' &&
                   (cVar1 != '\x03')))))))) && (cVar1 != '\x02')) {
              iVar9 = AbsShort(*(short *)
                                    (*param_5 + iVar17 * 0x138 + (int)((uVar18 & 0xffffffff) << 1)))
              ;
              uVar7 = (unsigned long long)local_7c;
              if ((iVar9 <= *psVar6 + -1) &&
                 ((*(unsigned int *)(*piVar2 + (int)(((uVar18 & 0x1fffffff) * 8 - uVar18 & 0xffffffff) << 4)
                            + (int)sVar15) >> 0x1d & 1) != 0)) {
                FUN_1001f0ac(iVar17,uVar18,&local_54,&local_56,&local_58);
                iVar9 = EvaluatePosition(iVar17,uVar18);
                if (((0x4b < iVar9) && (((2 < local_54 && (10 < local_56)) || (local_58 != 0)))) &&
                   (iVar10 < iVar9)) {
                  uVar16 = uVar18;
                  iVar11 = iVar17;
                  iVar10 = iVar9;
                }
              }
            }
            sVar12 = sVar12 + 1;
          } while ((int)sVar12 < (int)sVar14 + (int)param_4);
        }
        ppuVar8 = (int **)uVar7;
        sVar15 = sVar15 + 1;
      } while ((int)sVar15 < (int)local_50 + (int)param_4);
    }
    if (iVar11 != -1) {
      uVar3 = *(unsigned int *)(*(int *)(*(int*)((char*)ppuVar8 - 0x150)) + local_52 * 0x16 + 0xc);
      if ((uVar3 >> 0xc & 0xf) == 1) {
        uVar13 = (unsigned short)uVar3 & 0x7f;
      }
      else {
        uVar13 = 0xffff;
      }
      MoveArmyToTarget(param_2,4,0,0x20);
      piVar2 = (int *)(*(int*)((char*)ppuVar8 - 0x70));
      *(short *)(*piVar2 + 0x12) = (short)iVar11;
      *(short *)(*piVar2 + 0x14) = (short)uVar16;
      ExecuteArmyMove(iVar11,uVar16,0);
      uVar16 = 0;
      piVar2 = (int *)(*(int*)((char*)ppuVar8 - 0x150));
      do {
        iVar11 = (int)((uVar16 & 0xffffffff) << 1);
        sVar14 = *(short *)((int)param_2 + iVar11);
        if (sVar14 != -1) {
          if ((int)*(char *)(*piVar2 + sVar14 * 0x16 + 5) == (int)*(short *)(*piVar5 + 0x110)) {
            if (uVar13 != 0xffff) {
              iVar11 = (int)((uVar16 & 0xffffffff) << 1);
              iVar10 = *piVar2 + *(short *)((int)param_2 + iVar11) * 0x16;
              *(unsigned int *)(iVar10 + 0xc) = *(unsigned int *)(iVar10 + 0xc) & 0xffff0fff | 0x1000;
              iVar11 = *piVar2 + *(short *)((int)param_2 + iVar11) * 0x16;
              *(unsigned int *)(iVar11 + 0xc) =
                   (int)(short)uVar13 & 0x7fU | *(unsigned int *)(iVar11 + 0xc) & 0xffffff80;
            }
          }
          else {
            *(short *)((int)param_2 + iVar11) = 0xffff;
          }
        }
        sVar14 = (short)uVar16 + 1;
        uVar16 = (unsigned long long)sVar14;
      } while (sVar14 < 8);
      local_4e = 2;
    }
  }
  else {
    local_4e = 0;
  }
  return local_4e;
}

/* Address: 0x1001b8e0 Size: 384 bytes */
void FUN_1001b8e0(short param_1)

{
  int bVar1;
  unsigned char bVar2;
  int *piVar3;
  int uVar4;
  int *ppuVar5;
  int iVar6;
  int iVar7;
  short sVar8;
  int iVar9;
  
  uVar4 = uRam1011772c;
  piVar3 = piRam1011735c;
  ppuVar5 = 0 /* TVect base */;
  iVar9 = (int)param_1;
  iVar6 = FUN_1004645c(iVar9);
  bVar1 = *(short *)(*piVar3 + *(short *)(*piVar3 + 0x110) * 2 + 0xc0) != 2;
  bVar2 = *(unsigned char *)(*piVar3 + iVar9 * 0x42 + 0x1633);
  if ((bVar2 != 0xf) && (*(short *)((unsigned int)bVar2 * 2 + *piVar3 + 0xd0) == 0)) {
    bVar1 = true;
  }
  iVar7 = RandomRange(1,2,0xffffffffffffffff);
  if ((iVar7 != 0) && (299 < iVar6)) {
    bVar1 = true;
  }
  if ((bVar1) && (iVar6 != 0)) {
    DrawNumber(uVar4,(*(int*)((char*)ppuVar5 - 0x6d4)),*piVar3 + iVar9 * 0x42 + 0x1608);
    FUN_10030454(uVar4);
    iVar7 = *piVar3 + *(short *)(*piVar3 + 0x110) * 0x14;
    *(short *)(iVar7 + 0x186) = *(short *)(iVar7 + 0x186) + (short)iVar6;
    FUN_100465a8(iVar9);
    sVar8 = RandomRange(1,5,0);
    *(short *)(*piVar3 + *(short *)(*piVar3 + 0x110) * 2 + 0x1122) =
         *(short *)(*piVar3 + *(short *)(*piVar3 + 0x110) * 2 + 0x1122) + sVar8;
  }
  return;
}

/* Address: 0x1001c2dc Size: 1056 bytes */
long long FUN_1001c2dc(short param_1,short *param_2,short param_3)

{
  char cVar1;
  long long lVar2;
  int iVar3;
  int iVar4;
  int iVar5;
  int bVar6;
  int *piVar7;
  int *piVar8;
  int *piVar9;
  int *piVar10;
  int *piVar11;
  int iVar12;
  long long lVar13;
  short sVar14;
  short sVar15;
  unsigned long long uVar16;
  unsigned long long uVar17;
  unsigned long long uVar18;
  int iVar19;
  int iVar20;
  
  piVar11 = piRam101176e0;
  piVar10 = piRam10117468;
  piVar9 = piRam10117360;
  piVar8 = piRam1011735c;
  piVar7 = piRam10117358;
  uVar17 = (unsigned long long)param_3;
  sVar15 = *param_2;
  if ((sVar15 != -1) &&
     (iVar20 = *piRam1011735c + (int)(((uVar17 & 0x7ffffff) * 0x20 + uVar17 & 0xffffffff) << 1),
     lVar2 = -(long long)
              *(char *)(*piRam1011735c +
                        (*(unsigned int *)(*piRam10117358 + *(short *)(iVar20 + 0x1606) * 0xe0 +
                                  *(short *)(iVar20 + 0x1604) * 2) >> 0x18) + 0x711),
     lVar13 = lVar2 + 10, (lVar13 + (-(unsigned long long)(lVar13 == 0) - (lVar2 + 9)) & 0xff) != 0)) {
    iVar20 = 1000;
    uVar16 = 0xffffffffffffffff;
    iVar19 = 0;
    do {
      sVar14 = *(short *)(*piVar10 + param_1 * 0x5c + iVar19 * 2 + 0x25a);
      uVar18 = (unsigned long long)sVar14;
      if (sVar14 != -1) {
        iVar12 = *piVar9;
        iVar3 = sVar15 * 0x16;
        iVar5 = (int)(((uVar18 & 0x7ffffff) * 0x20 + uVar18 & 0x7fffffff) << 1);
        iVar4 = *piVar8 + iVar5;
        iVar12 = CalcDistance(*(short *)(iVar12 + iVar3),*(short *)(iVar12 + iVar3 + 2),
                              *(short *)(iVar4 + 0x1604),*(short *)(iVar4 + 0x1606));
        if ((iVar12 < iVar20) &&
           (iVar5 = *piVar8 + iVar5,
           lVar2 = -(long long)
                    *(char *)(*piVar8 + (*(unsigned int *)(*piVar7 + *(short *)(iVar5 + 0x1606) * 0xe0 +
                                                  *(short *)(iVar5 + 0x1604) * 2) >> 0x18) + 0x711),
           lVar13 = lVar2 + 10, (lVar13 + (-(unsigned long long)(lVar13 == 0) - (lVar2 + 9)) & 0xff) == 0)) {
          uVar16 = uVar18;
          iVar20 = iVar12;
        }
      }
      sVar14 = (short)iVar19 + 1;
      iVar19 = (int)sVar14;
    } while (sVar14 < 6);
    if ((int)uVar16 != -1) {
      uVar17 = uVar16;
    }
  }
  while( true ) {
    sVar15 = 0;
    iVar20 = 0;
    do {
      if (*(short *)(*piVar10 + param_1 * 0x5c + iVar20 * 2 + 0x25a) != -1) {
        sVar15 = sVar15 + 1;
      }
      sVar14 = (short)iVar20 + 1;
      iVar20 = (int)sVar14;
    } while (sVar14 < 6);
    iVar20 = 0;
    do {
      if ((param_2[iVar20] != -1) &&
         ((int)*(char *)(*piVar9 + param_2[iVar20] * 0x16 + 5) != (int)*(short *)(*piVar8 + 0x110)))
      {
        param_2[iVar20] = -1;
      }
      sVar14 = (short)iVar20 + 1;
      iVar20 = (int)sVar14;
    } while (sVar14 < 8);
    cVar1 = *(char *)(*piVar8 + (int)(((uVar17 & 0x7ffffff) * 0x20 + uVar17 & 0xffffffff) << 1) +
                     0x1619);
    MoveArmyToTarget(param_2,1,uVar17,0);
    iVar20 = ExecuteArmyMove(*(short *)(*piVar11 + 0x12),*(short *)(*piVar11 + 0x14),1);
    if (iVar20 == 1) {
      FUN_1000fccc(1);
      FUN_1001b4ac(param_1);
      FUN_100219a8(1,1,1);
      return 3;
    }
    FUN_1001b4ac(param_1);
    iVar20 = *piVar8;
    iVar19 = iVar20 + (int)uVar17 * 0x42;
    sVar14 = *(short *)(iVar20 + 0x110);
    if (((int)*(char *)(iVar19 + 0x1619) != (int)sVar14) || ((int)cVar1 == (int)sVar14)) {
      FUN_100219a8(1,1,1);
      return 3;
    }
    iVar20 = sVar14 * 0x14 + iVar20;
    if ((*(short *)(iVar19 + 0x1604) == *(short *)(iVar20 + 0x194)) &&
       (*(short *)(iVar19 + 0x1606) == *(short *)(iVar20 + 0x196))) {
      bVar6 = true;
    }
    else {
      bVar6 = false;
    }
    if (bVar6) break;
    if ((sVar15 < 2) || ((*(unsigned short *)(*piVar10 + param_1 * 0x5c + 0x2a6) & 1) == 0)) {
      if ((*(unsigned short *)(*piVar10 + param_1 * 0x5c + 0x2a6) & 2) == 0) {
        if ((*(unsigned short *)(*piVar10 + param_1 * 0x5c + 0x2a6) & 4) != 0) {
          FUN_1001b8e0(uVar17);
        }
      }
      else {
        LaunchAllianceDefense(uVar17);
      }
      break;
    }
    uVar17 = LaunchAllianceAttack(uVar17,0);
    if ((int)uVar17 == -1) {
      FUN_100219a8(1,1,1);
      return 1;
    }
  }
  FUN_1001bfa0(param_1,uVar17);
  FUN_100219a8(1,1,1);
  return 1;
}





























































































































































































































































































































































































































































































































































/* Address: 0x1001c6fc Size: 344 bytes */
long long FUN_1001c6fc(short param_1,long long param_2,short param_3,short param_4)

{
  short sVar1;
  short sVar2;
  int iVar3;
  int *piVar4;
  short *psVar5;
  long long uVar6;
  short sVar8;
  int iVar7;
  int iVar9;
  char auStack_40 [2];
  char auStack_3e [62];
  
  psVar5 = psRam101176fc;
  piVar4 = piRam1011735c;
  uVar6 = AllocInfluenceMap();
  sVar1 = *(short *)(*piVar4 + 0x1602);
  iVar3 = -1;
  sVar2 = 1000;
  while (sVar1 != 0) {
    sVar1 = sVar1 + -1;
    iVar9 = (int)sVar1;
    if ((int)*(char *)(*piVar4 + iVar9 * 0x42 + 0x1619) == (int)param_3) {
      sVar8 = FindBestTarget(iVar9,auStack_3e,auStack_40,uVar6,0);
      if ((((sVar8 < 0x32) && (sVar8 < *psVar5)) &&
          (iVar7 = iVar9 * 0x42 + *piVar4,
          iVar7 = EvaluatePosition(*(short *)(iVar7 + 0x1604),*(short *)(iVar7 + 0x1606)),
          0x4b < iVar7)) && (sVar8 < sVar2)) {
        iVar3 = iVar9;
        sVar2 = sVar8;
      }
    }
  }
  uVar6 = FUN_1001b584(param_1,param_2,param_3,param_4,uVar6);
  if (((int)uVar6 != 0) && (iVar3 != -1)) {
    uVar6 = FUN_1001c2dc(param_1,param_2,iVar3);
  }
  FreeInfluenceMap();
  return uVar6;
}

/* Address: 0x1001cd68 Size: 684 bytes */
long long FUN_1001cd68(short param_1)

{
  short sVar1;
  int *piVar2;
  int *piVar3;
  unsigned long long uVar4;
  int iVar6;
  long long uVar5;
  int bVar7;
  int iVar8;
  int iVar9;
  
  piVar3 = piRam10117468;
  piVar2 = piRam1011735c;
  iVar9 = (int)param_1;
  iVar6 = *piRam1011735c + *(short *)(*piRam1011735c + 0x110) * 0x14;
  uVar4 = LookupCityAtPos(*(short *)(iVar6 + 0x194),*(short *)(iVar6 + 0x196));
  iVar6 = (int)*(char *)(*piVar2 + (int)(((uVar4 & 0x7ffffff) * 0x20 + uVar4 & 0xffffffff) << 1) +
                        0x1619);
  if ((iVar6 != 0xf) && (iVar6 != *(short *)(*piVar2 + 0x110))) {
    sVar1 = *(short *)(*piVar3 + 0x24a);
    bVar7 = false;
    if (sVar1 != 0) {
      do {
        sVar1 = sVar1 + -1;
        iVar6 = *piVar3 + sVar1 * 0x5c;
        if ((*(short *)(iVar6 + 0x24c) != 0) &&
           ((int)*(short *)(iVar6 + 0x24e) ==
            (int)*(char *)(*piVar2 + (int)(((uVar4 & 0x7ffffff) * 0x20 + uVar4 & 0xffffffff) << 1) +
                          0x1619))) {
          bVar7 = true;
        }
      } while (sVar1 != 0);
    }
    if (!bVar7) {
/*       AI_ActivateBestGroup(); -- clean version in ai/ai.c */
      return 0;
    }
  }
  iVar6 = FUN_1001af38(iVar9);
  if (iVar6 == 0) {
    AI_ActivateAttackGroup(iVar9,0);
    return 0;
  }
  FUN_1001b198(iVar9);
  sVar1 = *(short *)(*piVar3 + 0x24a);
  bVar7 = false;
  if (sVar1 != 0) {
    do {
      sVar1 = sVar1 + -1;
      iVar8 = (int)sVar1;
      iVar6 = *piVar3 + iVar8 * 0x5c;
      if (((*(short *)(iVar6 + 0x24c) != 0) && (iVar8 != param_1)) &&
         (*(short *)(iVar6 + 0x250) == *(short *)(*piVar3 + param_1 * 0x5c + 0x250))) {
        bVar7 = true;
      }
    } while (iVar8 != 0);
  }
  if (((bVar7) ||
      ((int)*(char *)(*piVar2 + *(short *)(*piVar3 + param_1 * 0x5c + 0x250) * 0x42 + 0x1619) !=
       (int)*(short *)(*piVar2 + 0x110))) && (iVar6 = FUN_1001b35c(iVar9), iVar6 == 0)) {
    return 0;
  }
  FUN_1001c854(iVar9);
  if (*(short *)(*piVar3 + param_1 * 0x5c + 0x24c) == 0) {
    uVar5 = 0;
  }
  else {
/*     AI_ArmyManagement(*(short *)(*piVar3 + iVar9 * 0x5c + 0x250),1); -- clean version in ai/ai.c */
    iVar6 = FUN_1001cb24(iVar9);
    if ((iVar6 != 0) && (sVar1 = *(short *)(*piVar3 + iVar9 * 0x5c + 0x250), -1 < sVar1)) {
/*       AI_ArmyManagement(sVar1,1); -- clean version in ai/ai.c */
    }
    if (*(short *)(*piVar3 + iVar9 * 0x5c + 0x24c) == 0) {
      uVar5 = 0;
    }
    else {
      FUN_1001bdc8(iVar9);
      uVar5 = 1;
    }
  }
  return uVar5;
}

/* Address: 0x1001d15c Size: 288 bytes */
void FUN_1001d15c()

{
  int bVar1;
  unsigned int uVar2;
  int *piVar3;
  int iVar4;
  int iVar5;
  
  piVar3 = piRam1011735c;
  iVar5 = 99;
  do {
    iVar4 = iVar5 * 0x42;
    *(char *)(*piVar3 + iVar4 + 0x1631) = 0;
    *(char *)(iVar4 + *piVar3 + 0x1632) = *(char *)(iVar4 + *piVar3 + 0x1631);
    *(char *)(iVar4 + *piVar3 + 0x1630) = *(char *)(iVar4 + *piVar3 + 0x1632);
    *(short *)(iVar4 + *piVar3 + 0x163a) = 0;
    *(short *)(iVar4 + *piVar3 + 0x1638) = *(short *)(iVar4 + *piVar3 + 0x163a);
    *(short *)(iVar4 + *piVar3 + 0x1636) = *(short *)(iVar4 + *piVar3 + 0x1638);
    *(unsigned int *)(iVar4 + *piVar3 + 0x163c) = *(unsigned int *)(iVar4 + *piVar3 + 0x163c) & 0xff00ffff;
    uVar2 = *(unsigned int *)(iVar4 + *piVar3 + 0x163c);
    *(unsigned int *)(iVar4 + *piVar3 + 0x163c) = (uVar2 >> 0x10) << 0x18 | uVar2 & 0xffffff;
    *(short *)(iVar4 + *piVar3 + 0x1644) = 0;
    *(short *)(iVar4 + *piVar3 + 0x1642) = *(short *)(iVar4 + *piVar3 + 0x1644);
    *(short *)(iVar4 + *piVar3 + 0x1640) = *(short *)(iVar4 + *piVar3 + 0x1642);
    *(short *)(iVar4 + *piVar3 + 0x163e) = *(short *)(iVar4 + *piVar3 + 0x1640);
    *(char *)(iVar4 + *piVar3 + 0x1633) = *(char *)(iVar4 + *piVar3 + 0x1619);
    *(char *)(iVar4 + *piVar3 + 0x1634) = 0;
    bVar1 = iVar5 != 0;
    iVar5 = (int)(short)((short)iVar5 + -1);
  } while (bVar1);
  return;
}

/* Address: 0x1001e014 Size: 332 bytes */
void MoveUnitToTarget(short *param_1,short param_2,short param_3)

{
  int iVar1;
  unsigned long long uVar2;
  short local_10;
  short local_e [7];
  
  uVar2 = (unsigned long long)param_3;
  local_e[0] = -1;
  local_10 = -1;
  if (param_2 != 4) {
    if (param_2 == 1) {
      iVar1 = *piRam1011735c + (param_3 * 0x20 + (int)param_3) * 2;
      local_e[0] = *(short *)(iVar1 + 0x1604);
      local_10 = *(short *)(iVar1 + 0x1606);
      if ((int)*(char *)(iVar1 + 0x1619) == (int)*(short *)(*piRam1011735c + 0x110)) {
        local_e[0] = local_e[0] + 1;
        local_10 = local_10 + 1;
      }
      else {
        FUN_1001dfe0(*param_1,param_1[1],local_e,&local_10);
      }
    }
    else if (param_2 == 2) {
      iVar1 = *piRam1011735c + (int)(((uVar2 & 0xfffffff) * 0x10 - uVar2 & 0xffffffff) << 1);
      local_e[0] = *(short *)(iVar1 + 0xd2c);
      local_10 = *(short *)(iVar1 + 0xd2e);
    }
    else if (param_2 == 3) {
      iVar1 = *piRam1011735c + (int)((uVar2 & 0xffffffff) << 5);
      local_e[0] = *(short *)(iVar1 + 0x812);
      local_10 = *(short *)(iVar1 + 0x814);
    }
    if (local_e[0] == -1) {
      local_e[0] = *param_1;
    }
    param_1[9] = local_e[0];
    if (local_10 == -1) {
      local_10 = param_1[1];
    }
    param_1[10] = local_10;
  }
  return;
}

/* Address: 0x1001e160 Size: 464 bytes */
long long FUN_1001e160(int param_1,short param_2,short param_3,unsigned short param_4)

{
  char cVar1;
  int iVar2;
  int *piVar3;
  int *piVar4;
  int iVar5;
  long long uVar6;
  char uVar7;
  short sVar8;
  short sVar9;
  int iVar10;
  int iVar11;
  char cVar12;
  int iVar14;
  unsigned long long uVar13;
  
  iVar5 = gCombatDisplayPieces;
  piVar4 = piRam101176e0;
  piVar3 = piRam10117360;
  iVar10 = (int)*(short *)(*piRam1011735c + 0x110);
  cVar12 = -1;
  if (*(short *)(*piRam1011735c + 0x110) == 0xf) {
    iVar10 = 8;
  }
  iVar2 = *piRam1011735c;
  uVar7 = 0;
  iVar14 = 0;
  sVar8 = 0;
  do {
    if (*(short *)(param_1 + iVar14 * 2) != -1) {
      sVar8 = sVar8 + 1;
    }
    sVar9 = (short)iVar14 + 1;
    iVar14 = (int)sVar9;
  } while (sVar9 < 8);
  if (sVar8 == 0) {
    uVar6 = 0;
  }
  else {
    if (1 < sVar8) {
      uVar7 = FUN_10021d50();
    }
    uVar13 = 0;
    do {
      sVar8 = *(short *)(param_1 + (int)((uVar13 & 0xffffffff) << 1));
      if (sVar8 == -1) {
        *(int *)((int)((uVar13 & 0xffffffff) << 2) + iVar5) = 0;
      }
      else {
        iVar14 = *piVar3 + sVar8 * 0x16;
        *(int *)((int)((uVar13 & 0xffffffff) << 2) + iVar5) = iVar14;
        cVar1 = *(char *)(iVar2 + iVar10 * 0x1d + 0x60c + (int)*(char *)(iVar14 + 4));
        if (cVar12 < cVar1) {
          *piVar4 = iVar14;
          cVar12 = cVar1;
        }
        if (param_2 != -1) {
          iVar14 = sVar8 * 0x16;
          *(unsigned int *)(*piVar3 + iVar14 + 0xc) =
               (unsigned int)(((long long)param_2 & 0xffffffffU) << 0xc) & 0xf000 |
               *(unsigned int *)(*piVar3 + iVar14 + 0xc) & 0xffff0fff;
          *(unsigned int *)(iVar14 + *piVar3 + 0xc) =
               (int)param_3 & 0x7fU | *(unsigned int *)(iVar14 + *piVar3 + 0xc) & 0xffffff80;
          *(char *)(iVar14 + *piVar3 + 0x11) = uVar7;
          MoveUnitToTarget(iVar14 + *piVar3,(long long)param_2,param_3);
        }
        iVar11 = sVar8 * 0x16;
        iVar14 = *piVar3 + iVar11;
        *(unsigned short *)(iVar14 + 0xc) = *(unsigned short *)(iVar14 + 0xc) & 0xff5f;
        if (param_4 != 0) {
          iVar11 = *piVar3 + iVar11;
          *(unsigned short *)(iVar11 + 0xc) = param_4 | *(unsigned short *)(iVar11 + 0xc);
        }
      }
      sVar8 = (short)uVar13 + 1;
      uVar13 = (unsigned long long)sVar8;
    } while (sVar8 < 8);
    PrepareUnitDisplay();
    uVar6 = 1;
  }
  return uVar6;
}






















































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1001e4b0 Size: 180 bytes */
long long FUN_1001e4b0(short param_1)

{
  int iVar1;
  int *piVar2;
  long long uVar3;
  int iVar4;
  unsigned long long uVar5;
  
  piVar2 = piRam1011735c;
  uVar5 = (unsigned long long)param_1;
  if (*(char *)((int)(((uVar5 & 0x7ffffff) * 0x20 + uVar5 & 0xffffffff) << 1) + *piRam1011735c +
               0x1631) == '\0') {
    uVar3 = 0;
  }
  else {
    iVar1 = (int)(((uVar5 & 0x7ffffff) * 0x20 + uVar5 & 0x7fffffff) << 1);
    iVar4 = FUN_10047d80(uVar5,*(char *)(*piRam1011735c + iVar1 + 0x1630));
    iVar1 = iVar1 + *piVar2;
    if (*(char *)(iVar1 + 0x1631) == *(char *)(iVar1 + iVar4 + 0x161e)) {
      uVar3 = 0;
    }
    else {
      uVar3 = 1;
    }
  }
  return uVar3;
}

/* Address: 0x1001e564 Size: 272 bytes */
void FUN_1001e564(short param_1,short param_2)

{
  int *piVar1;
  int iVar2;
  unsigned long long uVar3;
  
  piVar1 = piRam1011735c;
  uVar3 = (unsigned long long)param_1;
  iVar2 = CheckArmyEscort(uVar3);
  if (iVar2 == 0) {
    iVar2 = (int)(((uVar3 & 0x7ffffff) * 0x20 + uVar3 & 0x7fffffff) << 1);
    *(short *)(*piVar1 + iVar2 + 0x1636) = 0;
    *(short *)(iVar2 + *piVar1 + 0x1638) = 0xffff;
    *(short *)(iVar2 + *piVar1 + 0x163a) = 0xffff;
  }
  else {
    if (param_1 < 100) {
      iVar2 = (int)(((uVar3 & 0x7ffffff) * 0x20 + uVar3 & 0xffffffff) << 1);
      *(short *)(*piVar1 + iVar2 + 0x1638) = *(short *)(*piVar1 + param_2 * 0x42 + 0x1604)
      ;
      *(short *)(iVar2 + *piVar1 + 0x163a) = *(short *)(param_2 * 0x42 + *piVar1 + 0x1606)
      ;
    }
    else {
      iVar2 = (int)(((uVar3 & 0x7ffffff) * 0x20 + uVar3 & 0xffffffff) << 1);
      *(short *)(*piVar1 + iVar2 + 0x1638) = 0xfffe;
      *(short *)(iVar2 + *piVar1 + 0x163a) = 0xfffe;
    }
    *(short *)
     (*piVar1 + (int)(((uVar3 & 0x7ffffff) * 0x20 + uVar3 & 0xffffffff) << 1) + 0x1636) = 1;
  }
  return;
}


















































































































/* Address: 0x1001f174 Size: 172 bytes */
long long IsPositionReachable(short param_1,short param_2)

{
  short sVar1;
  short sVar2;
  
  sVar2 = param_1 + -1;
  if ((int)sVar2 <= param_1 + 1) {
    do {
      sVar1 = param_2 + -1;
      if ((int)sVar1 <= param_2 + 1) {
        do {
          if ((-1 < sVar2) && (-1 < sVar1)) {
            if ((sVar2 < 0x70) && (sVar1 < 0x9c)) {
              if ((*(unsigned int *)(*piRam10117354 + sVar1 * 0x70 + (int)sVar2) >> 0x1d & 1) != 0) {
                return 1;
              }
            }
          }
          sVar1 = sVar1 + 1;
        } while ((int)sVar1 <= param_2 + 1);
      }
      sVar2 = sVar2 + 1;
    } while ((int)sVar2 <= param_1 + 1);
  }
  return 0;
}

/* Address: 0x1001f758 Size: 316 bytes */
/* Address: 0x1001f758 Size: 316 bytes */
/* AI_ResetProductionState - clean version in ai/ai.c */

/* Address: 0x1001f894 Size: 196 bytes */
/* Address: 0x1001f894 Size: 196 bytes */
/* AI_CheckNearbyThreats - clean version in ai/ai.c */

/* Address: 0x1001f958 Size: 140 bytes */
/* Address: 0x1001f958 Size: 140 bytes */
/* AI_PrecomputeThreats - clean version in ai/ai.c */














































































/* Address: 0x1002122c Size: 312 bytes */
int FUN_1002122c(short param_1,short param_2,short *param_3)

{
  int bVar1;
  short sVar2;
  int *piVar3;
  int *piVar4;
  int iVar5;
  char cVar6;
  int iVar7;
  int iVar8;
  int iVar9;
  int iVar10;
  
  iVar5 = iRam101174ac;
  piVar4 = piRam10117360;
  piVar3 = piRam1011735c;
  *param_3 = 0;
  iVar7 = 0;
  cVar6 = '\0';
  iVar9 = 7;
  do {
    *(int *)(iVar9 * 4 + iVar5) = 0;
    bVar1 = iVar9 != 0;
    iVar9 = (int)(short)((short)iVar9 + -1);
  } while (bVar1);
  sVar2 = *(short *)(*piVar3 + 0x182);
  if (sVar2 != 0) {
    do {
      sVar2 = sVar2 + -1;
      iVar8 = (int)sVar2;
      iVar9 = *piVar4;
      iVar10 = iVar9 + iVar8 * 0x16;
      if (((*(short *)(iVar9 + iVar8 * 0x16) == param_1) && (*(short *)(iVar10 + 2) == param_2)) &&
         (*param_3 < 8)) {
        *(int *)(*param_3 * 4 + iVar5) = iVar10;
        iVar9 = (int)*(char *)(iVar10 + 5);
        if (*(char *)(iVar10 + 5) == '\x0f') {
          iVar9 = 8;
        }
        if ((iVar7 == 0) ||
           (cVar6 < *(char *)(*piVar3 + iVar9 * 0x1d + (int)*(char *)(iVar7 + 4) + 0x60c))) {
          iVar7 = *piVar4 + iVar8 * 0x16;
          cVar6 = *(char *)(*piVar3 + iVar9 * 0x1d + (int)*(char *)(iVar7 + 4) + 0x60c);
        }
        *param_3 = *param_3 + 1;
      }
    } while (iVar8 != 0);
  }
  return iVar7;
}
































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1002279c Size: 1480 bytes */
void FUN_1002279c()

{
  short sVar1;
  short *psVar2;
  short *psVar3;
  short *psVar4;
  int *piVar5;
  int *piVar6;
  int *piVar7;
  int *piVar8;
  long long lVar9;
  int local_54;
  short local_30 [2];
  char auStack_2c [44];
  
  piVar7 = piRam1011735c;
  psVar4 = psRam1011677c;
  piVar8 = piRam10115d88;
  psVar3 = psRam10115d84;
  psVar2 = psRam10115d80;
  if (*psRam10115d80 == 2) {
    piVar5 = (int *)ResourceRead_Dispatch(*piRam10115d88 + (int)*(short *)(*(int *)*piRam10115d88 + 0x308),
                                 0x73746f70);
    if (piVar5 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar5 + (int)*(short *)(*piVar5 + 0x738),
                   *(char *)(*piVar7 + *psVar4 * 0x42 + 0x1631) == '\0',1);
      lVar9 = (long long)*(char *)(*piVar7 + *psVar4 * 0x42 + 0x1631);
      ResourceRead_Dispatch((int)piVar5 + (int)*(short *)(*piVar5 + 0xd8),
                   ((-1 - lVar9) + (unsigned long long)(lVar9 == 0) << 0x20) >> 0x3f);
    }
  }
  if (*psVar2 == 3) {
    piVar5 = (int *)ResourceRead_Dispatch(*piVar8 + (int)*(short *)(*(int *)*piVar8 + 0x308),0x73656520);
    if (piVar5 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar5 + (int)*(short *)(*piVar5 + 0x748),
                   **(unsigned short **)(local_54 + -0x1b14) & 0xff,1);
    }
    FUN_10047de8(*psVar4,auStack_2c,local_30);
    lVar9 = (long long)*(char *)(*piVar7 + *psVar4 * 0x42 + 0x1631);
    sVar1 = (short)((-1 - lVar9) + (unsigned long long)(lVar9 == 0) >> 0x10);
    piVar5 = (int *)ResourceRead_Dispatch(*piVar8 + (int)*(short *)(*(int *)*piVar8 + 0x308),0x746f2020);
    piVar6 = (int *)ResourceRead_Dispatch(*piVar8 + (int)*(short *)(*(int *)*piVar8 + 0x308),0x64657374);
    if ((piVar5 != (int *)0x0) && (piVar6 != (int *)0x0)) {
      if (*psVar3 == 0) {
        ResourceRead_Dispatch((int)piVar5 + (int)*(short *)(*piVar5 + 0x748),0,0);
        ResourceRead_Dispatch((int)*(short *)(*piVar6 + 0x748) + (int)piVar6,0,0);
        ResourceRead_Dispatch((int)piVar5 + (int)*(short *)(*piVar5 + 0x738),-1 < sVar1,0);
        ResourceRead_Dispatch((int)*(short *)(*piVar6 + 0x738) + (int)piVar6,
                     ((long long)local_30[0] + -1 + (unsigned long long)((long long)local_30[0] != 0) << 0x20)
                     >> 0x3f,0);
      }
      else if (*psVar3 == 1) {
        ResourceRead_Dispatch((int)piVar5 + (int)*(short *)(*piVar5 + 0x748),1,0);
        ResourceRead_Dispatch((int)piVar6 + (int)*(short *)(*piVar6 + 0x748),0,0);
        ResourceRead_Dispatch((int)piVar5 + (int)*(short *)(*piVar5 + 0x738),0,0);
        ResourceRead_Dispatch((int)piVar6 + (int)*(short *)(*piVar6 + 0x738),
                     ((long long)local_30[0] + -1 + (unsigned long long)((long long)local_30[0] != 0) << 0x20)
                     >> 0x3f,0);
      }
      else if (*psVar3 == 2) {
        ResourceRead_Dispatch((int)piVar5 + (int)*(short *)(*piVar5 + 0x748),0,0);
        ResourceRead_Dispatch((int)piVar6 + (int)*(short *)(*piVar6 + 0x748),1,0);
        ResourceRead_Dispatch((int)piVar5 + (int)*(short *)(*piVar5 + 0x738),sVar1 < 0,0);
        ResourceRead_Dispatch((int)piVar6 + (int)*(short *)(*piVar6 + 0x738),0,0);
      }
      ResourceRead_Dispatch((int)piVar5 + (int)*(short *)(*piVar5 + 0x4d0));
      ResourceRead_Dispatch((int)piVar6 + (int)*(short *)(*piVar6 + 0x4d0));
    }
  }
  piVar5 = (int *)ResourceRead_Dispatch(*piVar8 + (int)*(short *)(*(int *)*piVar8 + 0x308),0x72617a65);
  if (piVar5 != (int *)0x0) {
    lVar9 = -(long long)*(short *)(*piVar7 + 0x114) + 0xf;
    ResourceRead_Dispatch((int)piVar5 + (int)*(short *)(*piVar5 + 0x738),
                 lVar9 + (-(unsigned long long)(lVar9 == 0) - (-(long long)*(short *)(*piVar7 + 0x114) + 0xe))
                 & 0xff,1);
  }
  if (*(short *)(*(short *)(*piVar7 + 0x110) * 2 + *(int *)(local_54 + -0x324)) < 1) {
    piVar7 = (int *)ResourceRead_Dispatch(*piVar8 + (int)*(short *)(*(int *)*piVar8 + 0x308),0x6275696c);
    if (piVar7 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar7 + (int)*(short *)(*piVar7 + 0x738),1,1);
    }
    piVar7 = (int *)ResourceRead_Dispatch(*piVar8 + (int)*(short *)(*(int *)*piVar8 + 0x308),0x70726f64);
    if (piVar7 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar7 + (int)*(short *)(*piVar7 + 0x738),1,1);
    }
    piVar8 = (int *)ResourceRead_Dispatch(*piVar8 + (int)*(short *)(*(int *)*piVar8 + 0x308),0x76656374);
    if (piVar8 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x738),1,1);
    }
  }
  return;
}





































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10025f2c Size: 104 bytes */
void FUN_10025f2c()

{
  int *piVar1;
  short sVar2;
  int iVar3;
  
  piVar1 = piRam1011735c;
  FUN_1001db60();
  iVar3 = 0;
  do {
    *(short *)(*piVar1 + iVar3 * 2 + 0x1122) = 0;
    *(short *)(iVar3 * 2 + *piVar1 + 0x1132) = 0xffff;
    sVar2 = (short)iVar3 + 1;
    iVar3 = (int)sVar2;
  } while (sVar2 < 8);
  return;
}

/* Address: 0x100275ec Size: 256 bytes */
void FUN_100275ec()

{
  unsigned int uVar1;
  int iVar2;
  int iVar3;
  int iVar4;
  int *piVar5;
  short sVar6;
  unsigned long long uVar7;
  unsigned long long uVar8;
  
  piVar5 = piRam1011735c;
  uVar7 = 0;
  do {
    uVar8 = 0;
    do {
      if (*(short *)(*piVar5 + 0x11c) == 0) {
        iVar4 = (int)((uVar7 & 0xfffffff) << 4);
        iVar3 = (int)((uVar8 & 0x7fffffff) << 1);
        iVar2 = *piVar5 + iVar4 + iVar3;
        *(unsigned int *)(iVar2 + 0x1582) = *(unsigned int *)(iVar2 + 0x1582) & 0xcfffffff | 0x20000000;
        iVar3 = iVar4 + *piVar5 + iVar3;
        uVar1 = *(unsigned int *)(iVar3 + 0x1582);
        *(unsigned int *)(iVar3 + 0x1582) = (uVar1 >> 0x1c & 3) << 0x1a | uVar1 & 0xf3ffffff;
      }
      else {
        iVar4 = (int)((uVar7 & 0xfffffff) << 4);
        iVar3 = (int)((uVar8 & 0x7fffffff) << 1);
        iVar2 = iVar4 + *piVar5 + iVar3;
        *(unsigned int *)(iVar2 + 0x1582) = *(unsigned int *)(iVar2 + 0x1582) & 0xcfffffff;
        iVar3 = iVar4 + *piVar5 + iVar3;
        uVar1 = *(unsigned int *)(iVar3 + 0x1582);
        *(unsigned int *)(iVar3 + 0x1582) = (uVar1 >> 0x1c & 3) << 0x1a | uVar1 & 0xf3ffffff;
      }
      iVar4 = (int)((uVar7 & 0xfffffff) << 4);
      iVar3 = (int)((uVar8 & 0x7fffffff) << 1);
      iVar2 = *piVar5 + iVar4 + iVar3;
      *(unsigned int *)(iVar2 + 0x1582) = *(unsigned int *)(iVar2 + 0x1582) & 0xbfffffff;
      iVar3 = iVar4 + *piVar5 + iVar3;
      *(unsigned int *)(iVar3 + 0x1582) = *(unsigned int *)(iVar3 + 0x1582) & 0x7fffffff;
      sVar6 = (short)uVar8 + 1;
      uVar8 = (unsigned long long)sVar6;
    } while (sVar6 < 8);
    sVar6 = (short)uVar7 + 1;
    uVar7 = (unsigned long long)sVar6;
  } while (sVar6 < 8);
  return;
}

/* Address: 0x100276ec Size: 16 bytes */
char FUN_100276ec()

{
  return *(char *)(*piRam1011734c + 0x1a1);
}

/* Address: 0x1002772c Size: 16 bytes */
char FUN_1002772c()

{
  return *(char *)(*piRam1011734c + 0x1a8);
}

/* Address: 0x1002869c Size: 1172 bytes */
long long FUN_1002869c()

{
  int *piVar1;
  int iVar2;
  int uVar3;
  int bVar4;
  int *piVar5;
  int iVar6;
  long long uVar7;
  int iVar12;
  int iVar13;
  int iVar14;
  long long uVar8;
  long long uVar9;
  long long uVar10;
  long long uVar11;
  unsigned short uVar15;
  long long uVar16;
  unsigned long long uVar17;
  int local_d4;
  int local_b0;
  int local_ac;
  int local_a8;
  int local_a4;
  char auStack_a0 [8];
  char auStack_98 [72];
  short local_50;
  short local_4e;
  short local_4c;
  short local_4a;
  short local_48;
  short local_46;
  short local_44;
  short local_42;
  
  iVar6 = iRam10117684;
  piVar5 = piRam1011734c;
  iVar2 = gSpriteTable;
  uVar17 = (unsigned long long)gColorPalette;
  uVar16 = 0;
  if (*(short *)(*piRam1011735c + 0x124) != 0) {
    CenterMapOnArmy(*(short *)(*piRam1011735c + 0x110));
  }
  uVar7 = FUN_1005f6b0(3,7);
  piVar1 = *(int **)(*(int *)(*piVar5 + 0x88) + 0x148);
  ResourceRead_Dispatch((int)piVar1 + (int)*(short *)(*piVar1 + 0x2b8));
  iVar12 = GetResourceByTypeId(0x50494354,0x2711);
  bVar4 = false;
  iVar14 = local_d4;
  if ((iVar12 == 0) || (iVar13 = FUN_10044c60(iVar6,auStack_a0,uVar7), iVar13 == 0)) {
    if (iVar12 == 0) {
      FUN_100efc78(**(short **)(iVar14 + -0x60));
    }
    iVar14 = FUN_10044c60(iVar6,auStack_a0,uVar7);
    if (iVar14 == 0) {
      FocusObject(0);
    }
    bVar4 = true;
  }
  *(unsigned int *)(iVar2 + 0x8c) = *(unsigned int *)(iVar2 + 0x8c) | 0x20000000;
  if (bVar4) {
    uVar7 = HLock_Thunk(*(int *)(iVar6 + 0x10));
    FUN_10002c10(uVar7);
    FUN_10000d98(uVar7);
    iVar2 = *(int *)(*piVar5 + 0x88);
    ResourceRead_Dispatch((int)*(int **)(iVar2 + 0x148) + (int)*(short *)(**(int **)(iVar2 + 0x148) + 0x2b8))
    ;
    uVar8 = GetResourceByTypeId(0x50494354,0x2711);
    if ((int)uVar8 == 0) {
      FUN_100641d0();
      GetGWorld_Wrapper(&local_a4,&local_a8);
      uVar3 = *(int *)(iVar6 + 0x10);
      local_48 = 0;
      local_46 = 0;
      local_44 = 0x138;
      local_42 = 0xe0;
      SetGWorld_Wrapper(uVar3,0);
      GetBitMapPtr(&local_48);
      uVar8 = FUN_100001b0();
      uVar16 = 1;
      if ((int)uVar8 != 0) {
        uVar9 = LockPixels_Wrapper(uVar3);
        RGBForeColor_Thunk(uVar17);
        Render_PenNormal();
        RGBForeColor_Thunk(uVar17 + 0x5fa);
        SetForeColor_Thunk();
        uVar10 = GetBitMapPtr(&local_48);
        uVar11 = GetBitMapPtr(&local_48);
        CopyBits_Thunk(uVar9,uVar9,uVar11,uVar10,0,0);
        UnlockPixels_Wrapper(uVar3);
        FUN_100001c8();
      }
      SetGWorld_Wrapper(local_a4,local_a8);
      if ((int)uVar8 != 0) {
        ResourceRead_Dispatch((int)*(int **)(iVar2 + 0x148) +
                     (int)*(short *)(**(int **)(iVar2 + 0x148) + 0x178),auStack_98);
        FUN_1007ec7c(iVar2,auStack_98);
        ResourceRead_Dispatch((int)*(int **)(iVar2 + 0x148) +
                     (int)*(short *)(**(int **)(iVar2 + 0x148) + 0x2a0),3,3);
        ResourceRead_Dispatch((int)*(int **)(iVar2 + 0x148) +
                     (int)*(short *)(**(int **)(iVar2 + 0x148) + 0x118));
        AddResource(uVar8,0x50494354,0x2711,(unsigned long long)*(unsigned int *)(local_d4 + -0x1ad8) + 0x40);
        uVar15 = FUN_10002718(uVar8);
        FUN_10000dc8(uVar8,uVar15 | 0x20);
        FUN_10000de0(uVar8);
        ReleaseResource_Thunk(uVar8);
        FUN_1007ec7c(iVar2,auStack_98);
        ResourceRead_Dispatch((int)*(int **)(iVar2 + 0x148) +
                     (int)*(short *)(**(int **)(iVar2 + 0x148) + 0x118));
      }
    }
    else {
      GetGWorld_Wrapper(&local_ac,&local_b0);
      SetGWorld_Wrapper(*(int *)(iVar6 + 0x10),0);
      RGBForeColor_Thunk(uVar17);
      Render_PenNormal();
      RGBForeColor_Thunk(uVar17 + 0x5fa);
      SetForeColor_Thunk();
      if (*(int *)(iVar6 + 0x10) != 0) {
        LockPixels_Wrapper();
      }
      local_50 = 0;
      local_4e = 0;
      local_4c = 0x138;
      local_4a = 0xe0;
      uVar9 = GetBitMapPtr(&local_50);
      FUN_10000750(uVar8,uVar9);
      if (*(int *)(iVar6 + 0x10) != 0) {
        UnlockPixels_Wrapper(*(int *)(iVar6 + 0x10));
      }
      SetGWorld_Wrapper(local_ac,local_b0);
    }
    FUN_10000d98(uVar7);
  }
  return uVar16;
}

























































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10028b30 Size: 736 bytes */
long long FUN_10028b30(int *param_1,short param_2,short param_3)

{
  int uVar1;
  int *piVar2;
  int *piVar3;
  int iVar4;
  int *ppuVar5;
  int iVar8;
  long long uVar6;
  long long uVar7;
  int iVar9;
  int **local_74;
  char auStack_50 [24];
  char local_38 [16];
  int local_28;
  int local_24;
  
  iVar4 = gCombatDisplayPieces;
  piVar3 = piRam10117360;
  piVar2 = piRam10115f7c;
  if ((*psRam10116304 == 0) && (*psRam10115ce0 != 0)) {
    BlockMoveData_Thunk(local_38,(unsigned long long)uRam10117604 + 0x44);
    ppuVar5 = local_74;
  }
  else {
    local_38[0] = *(char *)(iRam10115dc8 + 0x48);
    ppuVar5 = 0 /* TVect base */;
  }
  InitCityNames();
  FUN_100099b4();
  FUN_100099cc();
  if (*piVar3 == 0) {
    iVar8 = AllocateBlock(22000);
    *piVar3 = iVar8;
  }
  iVar8 = 0;
  do {
    iVar9 = iVar8 * 0x16;
    *(char *)(iVar9 + *piVar3 + 5) = 0xf;
    *(short *)(iVar9 + *piVar3 + 2) = 0xffff;
    *(short *)(iVar9 + *piVar3) = *(short *)(iVar9 + *piVar3 + 2);
    iVar8 = iVar8 + 1;
  } while (iVar8 < 10);
  *(int *)(*(int*)((char*)ppuVar5 - 0xfc)) = 0;
  *(int *)(*(int*)((char*)ppuVar5 - 0x70)) = 0;
  iVar8 = 0;
  do {
    *(int *)(iVar8 * 4 + iVar4) = 0;
    iVar8 = iVar8 + 1;
  } while (iVar8 < 8);
  uVar1 = *(int *)(*(int *)(*(int*)((char*)ppuVar5 - 0x155)) + 0x88);
  FUN_10027774(uVar1,param_3);
  local_28 = 0xca;
  local_24 = 0x3c;
  ProgressCreate(*piVar2,&local_28,0x3f3);
  ProgressUpdate(*piVar2,0);
  uVar6 = ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x2b8));
  FUN_10028040(uVar1,param_1);
  ProgressUpdate(*piVar2,10);
  if (param_3 == 0) {
    uVar7 = FUN_1005f6b0(0x1a,param_2);
    DrawNumber(auStack_50,uVar7);
  }
  else {
    uVar7 = FUN_1005f6b0(0x1c,param_2);
    DrawNumber(auStack_50,uVar7);
  }
  uVar7 = FUN_100522dc(param_1,1,1);
  if ((int)uVar7 != 0) {
    FUN_100283f8(uVar1,param_3,uVar7);
    CloseStream(uVar7,0);
  }
  ProgressUpdate(*piVar2,0x50);
  ProgressUpdate(*piVar2,0x5a);
  ProgressUpdate(*piVar2,100);
  uVar7 = PostLoadInitialization();
  if (*(short *)(*(int *)local_74[-0x151] + 0x120) != 0) {
    FUN_10025e04();
  }
  InitArmyData();
  ResourceRead_Dispatch(*piVar2 + (int)*(short *)(*(int *)*piVar2 + 0x738));
  *piVar2 = 0;
  DisposeGeneric(uVar6);
  return uVar7;
}




































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10028e10 Size: 3248 bytes */
short FUN_10028e10(long long param_1,short param_2,short param_3)

{
  int iVar1;
  int *puVar2;
  unsigned int uVar3;
  unsigned int uVar4;
  unsigned int uVar5;
  int *piVar6;
  int *piVar7;
  int *ppuVar8;
  int iVar12;
  long long uVar9;
  int *piVar13;
  short uVar18;
  unsigned short uVar19;
  unsigned long long uVar10;
  char *puVar14;
  int uVar15;
  int *piVar16;
  char uVar20;
  long long uVar11;
  short *puVar17;
  int iVar21;
  short sVar22;
  short sVar23;
  short sVar24;
  unsigned int uVar25;
  char *puVar26;
  char *puVar27;
  char *puVar28;
  int *puVar29;
  char *puVar30;
  int iVar31;
  short *puVar32;
  int *puVar33;
  int **local_3ec;
  char auStack_3c8 [56];
  char auStack_390 [24];
  char local_378 [16];
  int local_368 [2];
  char auStack_360 [104];
  int local_2f8 [3];
  char auStack_2ec [16];
  char auStack_2dc [256];
  char auStack_1dc [72];
  char auStack_194 [256];
  char auStack_94 [148];
  
  iVar1 = gCombatDisplayPieces;
  iVar31 = gSCENInfo;
  piVar16 = piRam10117360;
  piVar7 = piRam1011735c;
  piVar13 = piRam1011734c;
  piVar6 = piRam10115f7c;
  puVar14 = ((char*)0) + -0x3c8;
  puVar27 = ((char*)0) + -0x390;
  puVar26 = ((char*)0) + -0x378;
  puVar33 = (int *)(((char*)0) + -0x368);
  puVar28 = ((char*)0) + -0x360;
  puVar29 = (int *)(((char*)0) + -0x2f8);
  puVar30 = ((char*)0) + -0x2ec;
  if ((*psRam10116304 == 0) && (*psRam10115ce0 != 0)) {
    BlockMoveData_Thunk(puVar26,gSCENInfo + 0x44);
    ppuVar8 = local_3ec;
  }
  else {
    *puVar26 = *(char *)(iRam10115dc8 + 0x4c);
    ppuVar8 = 0 /* TVect base */;
  }
  InitCityNames();
  FUN_100099b4();
  FUN_100099cc();
  if (*piVar16 == 0) {
    iVar12 = AllocateBlock(22000);
    *piVar16 = iVar12;
  }
  iVar12 = 0;
  do {
    iVar21 = iVar12 * 0x16;
    *(char *)(iVar21 + *piVar16 + 5) = 0xf;
    *(short *)(iVar21 + *piVar16 + 2) = 0xffff;
    *(short *)(iVar21 + *piVar16) = *(short *)(iVar21 + *piVar16 + 2);
    iVar12 = iVar12 + 1;
  } while (iVar12 < 10);
  *(int *)(*(int*)((char*)ppuVar8 - 0xfc)) = 0;
  *(int *)(*(int*)((char*)ppuVar8 - 0x70)) = 0;
  iVar12 = 0;
  do {
    *(int *)(iVar12 * 4 + iVar1) = 0;
    iVar12 = iVar12 + 1;
  } while (iVar12 < 8);
  iVar1 = *(int *)(*piVar13 + 0x88);
  FUN_10027774(iVar1,param_3);
  *puVar33 = 0xca;
  ProgressCreate(*piVar6,puVar33,0x3f3);
  ProgressUpdate(*piVar6,0);
  ProgressUpdate(*piVar6,10);
  if (param_3 == 0) {
    uVar9 = FUN_1005f6b0(0x1a,param_2);
    DrawNumber(puVar27,uVar9);
  }
  else {
    uVar9 = FUN_1005f6b0(0x1c,param_2);
    DrawNumber(puVar27,uVar9);
  }
  piVar13 = (int *)FUN_100522dc(param_1,1,0);
  if (piVar13 != (int *)0x0) {
    ResourceRead_Dispatch((int)*(short *)(*piVar13 + 0xe0) + (int)piVar13,iVar31,0x3a);
    ResourceRead_Dispatch((int)*(short *)(*piVar13 + 0x100) + (int)piVar13);
    uVar18 = ByteSwap16();
    *(short *)(iVar31 + 0x3a) = uVar18;
    ResourceRead_Dispatch((int)*(short *)(*piVar13 + 0xe0) + (int)piVar13,iVar31 + 0x3c,0x10);
    puVar32 = (short *)(iVar31 + 0x4c);
    sVar22 = 0;
    do {
      ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0x100));
      ppuVar8 = local_3ec;
      uVar18 = ByteSwap16();
      *puVar32 = uVar18;
      puVar32 = puVar32 + 1;
      sVar22 = sVar22 + 1;
    } while (sVar22 < 4);
    puVar2 = (*(int*)((char*)ppuVar8 - 0x6b6));
    DrawNumber(puVar14,ZEXT48(puVar2) + 0x50,iVar31 + 0x3c);
    puVar26 = ((char*)0) + -0x2dc;
    BuildPascalString(puVar26,puVar14);
    FUN_10027a58(0,puVar26,0);
    FUN_1007ec7c(iVar1,((char*)0) + -0x1dc);
    ResourceRead_Dispatch((int)*(int **)(iVar1 + 0x148) + (int)*(short *)(**(int **)(iVar1 + 0x148) + 0x118))
    ;
    ppuVar8 = local_3ec;
    ReadSCNResource(piVar13,*piVar7);
    iVar31 = *piVar7;
    *(short *)(*(int*)((char*)ppuVar8 - 0x6b5)) = *(short *)(iVar31 + 0x17e);
    *(short *)(*(int*)((char*)ppuVar8 - 0x6b4)) = *(short *)(iVar31 + 0x180);
    ProgressUpdate(*piVar6,0x14);
    DisposeGeneric(*(short *)(*(int*)((char*)ppuVar8 - 0x2d)));
    DrawNumber(local_3ec[-0x5d],ZEXT48(puVar2) + 0x54,*(short *)(*piVar7 + 0x162));
    puVar26 = ((char*)0) + -0x194;
    BuildPascalString(puVar26,local_3ec[-0x5d]);
    puVar14 = ((char*)0) + -0x94;
    FUN_10027a58(1,puVar26,0);
    FUN_1007ed14(iVar1,puVar14);
    ResourceRead_Dispatch((int)*(int **)(iVar1 + 0x14c) + (int)*(short *)(**(int **)(iVar1 + 0x14c) + 0x118))
    ;
    StreamRead(piVar13,*(int *)local_3ec[-0x152],0x8880);
    iVar31 = 0;
    do {
      iVar12 = *piVar16;
      iVar21 = iVar12 + iVar31 * 0x16;
      ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0x100));
      uVar18 = ByteSwap16();
      *(short *)(iVar12 + iVar31 * 0x16) = uVar18;
      ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0x100));
      uVar18 = ByteSwap16();
      *(short *)(iVar21 + 2) = uVar18;
      ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0xe0),iVar21 + 4,8);
      ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0x100));
      uVar18 = ByteSwap16();
      *(short *)(iVar21 + 0xc) = uVar18;
      ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0x100));
      uVar19 = ByteSwap16();
      uVar25 = *(unsigned int *)(iVar21 + 0xc);
      uVar3 = (unsigned int)(uVar19 >> 9);
      *(unsigned int *)(iVar21 + 0xc) = uVar3 | uVar25 & 0xffffff80;
      uVar4 = (uVar19 >> 7 & 3) << 7;
      *(unsigned int *)(iVar21 + 0xc) = uVar4 | uVar3 | uVar25 & 0xfffffe00;
      uVar5 = (uVar19 >> 4 & 7) << 9;
      *(unsigned int *)(iVar21 + 0xc) = uVar5 | uVar4 | uVar3 | uVar25 & 0xfffff000;
      *(unsigned int *)(iVar21 + 0xc) = (uVar19 & 0xf) << 0xc | uVar5 | uVar4 | uVar3 | uVar25 & 0xffff0000;
      ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0xe0),iVar21 + 0x10,2);
      ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0x100));
      uVar18 = ByteSwap16();
      *(short *)(iVar21 + 0x12) = uVar18;
      ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0x100));
      ppuVar8 = local_3ec;
      uVar18 = ByteSwap16();
      *(short *)(iVar21 + 0x14) = uVar18;
      sVar22 = (short)iVar31 + 1;
      iVar31 = (int)sVar22;
    } while (sVar22 < 1000);
    StreamRead(piVar13,*(int *)(*(int*)((char*)ppuVar8 - 0x153)),0x4440);
    ProgressUpdate(*piVar6,0x1e);
    puVar33 = *(int **)(*(int*)((char*)ppuVar8 - 0x6ec));
    if (puVar33 != (int *)0x0) {
      DetachResource(puVar33);
      ppuVar8 = local_3ec;
    }
    uVar15 = *puVar33;
    *(int *)(*(int*)((char*)ppuVar8 - 0x6eb)) = uVar15;
    StreamRead(piVar13,uVar15,0x4440);
    if (*(int *)(*(int*)((char*)ppuVar8 - 0x6ec)) != 0) {
      DisposeHandle_Thunk(*(int *)(*(int*)((char*)ppuVar8 - 0x6ec)));
    }
    ProgressUpdate(*piVar6,0x28);
    StreamRead(piVar13,puVar29,4);
    uVar10 = ByteSwap32(*puVar29);
    *puVar29 = (int)uVar10;
    if ((uVar10 & 0xffffffff) != 0xffffffff) {
      puVar14 = (char *)AllocateBlock(uVar10);
      CheckMemError();
      StreamRead(piVar13,puVar14,*puVar29);
      uVar9 = FUN_100522dc(*(int *)(iVar1 + 0x15c),6,0);
      StreamWrite(uVar9,puVar14,*puVar29);
      CloseStream(uVar9,1);
      FreeBlock(puVar14);
    }
    ProgressUpdate(*piVar6,0x32);
    StreamRead(piVar13,puVar29,4);
    uVar15 = ByteSwap32(*puVar29);
    *puVar29 = uVar15;
    piVar16 = (int *)FUN_100522dc(*(int *)(iVar1 + 0x160),6,0);
    StreamRead(piVar13,puVar29,4);
    uVar9 = ByteSwap32(*puVar29);
    *puVar29 = (int)uVar9;
    ResourceRead_Dispatch((int)piVar16 + (int)*(short *)(*piVar16 + 0x198),uVar9);
    sVar22 = *(short *)(*piVar7 + 0x136);
    if (sVar22 < 2) {
      sVar22 = 1;
    }
    if (0xc9 < sVar22) {
      sVar22 = 0xc9;
    }
    sVar24 = 1;
    if (1 < sVar22) {
      do {
        iVar31 = *piVar16;
        ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0x100));
        uVar9 = ByteSwap16();
        ResourceRead_Dispatch((int)piVar16 + (int)*(short *)(iVar31 + 400),uVar9);
        sVar23 = 0;
        do {
          iVar31 = *piVar16;
          iVar1 = *piVar13;
          ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(iVar1 + 0x100),iVar1,
                       *(int *)(iVar1 + 0x104));
          uVar9 = ByteSwap16();
          ResourceRead_Dispatch((int)piVar16 + (int)*(short *)(iVar31 + 400),uVar9);
          sVar23 = sVar23 + 1;
        } while (sVar23 < 0x18);
        sVar23 = *(short *)(*piVar7 + 0x1602);
        if ((sVar23 != 0) &&
           (uVar25 = (int)sVar23 - 1,
           ((int)sVar23 >> 1) + (unsigned int)(sVar23 < 0 && ((int)sVar23 & 1U) != 0) ==
           ((int)uVar25 >> 1) + (unsigned int)((int)uVar25 < 0 && (uVar25 & 1) != 0))) {
          sVar23 = sVar23 + 1;
        }
        ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0xe0),puVar28,sVar23);
        ResourceRead_Dispatch((int)piVar16 + (int)*(short *)(*piVar16 + 0x170),puVar28,sVar23);
        ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0x100));
        uVar9 = ByteSwap16();
        ResourceRead_Dispatch((int)piVar16 + (int)*(short *)(*piVar16 + 400),uVar9);
        sVar23 = 0;
        if (0 < (int)uVar9) {
          do {
            iVar31 = *piVar16;
            iVar1 = *piVar13;
            ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(iVar1 + 0xe8),iVar1,
                         *(int *)(iVar1 + 0xec));
            uVar20 = ByteSwap16();
            ResourceRead_Dispatch((int)piVar16 + (int)*(short *)(iVar31 + 0x178),uVar20);
            iVar31 = *piVar16;
            iVar1 = *piVar13;
            ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(iVar1 + 0xe8),iVar1,
                         *(int *)(iVar1 + 0xec));
            uVar20 = ByteSwap16();
            ResourceRead_Dispatch((int)piVar16 + (int)*(short *)(iVar31 + 0x178),uVar20);
            iVar31 = *piVar16;
            iVar1 = *piVar13;
            ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(iVar1 + 0x100),iVar1,
                         *(int *)(iVar1 + 0x104));
            uVar11 = ByteSwap16();
            ResourceRead_Dispatch((int)piVar16 + (int)*(short *)(iVar31 + 400),uVar11);
            iVar31 = *piVar16;
            iVar1 = *piVar13;
            ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(iVar1 + 0x100),iVar1,
                         *(int *)(iVar1 + 0x104));
            uVar11 = ByteSwap16();
            ResourceRead_Dispatch((int)piVar16 + (int)*(short *)(iVar31 + 400),uVar11);
            ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0xe0),puVar30,0x10);
            ResourceRead_Dispatch((int)piVar16 + (int)*(short *)(*piVar16 + 0x170),puVar30,0x10);
            sVar23 = sVar23 + 1;
          } while ((int)sVar23 < (int)uVar9);
        }
        sVar24 = sVar24 + 1;
      } while (sVar24 < sVar22);
    }
    CloseStream(piVar16,1);
    FreeBlock(puVar14);
    *(short *)(*piVar7 + 0x130) = 1;
    if (param_3 != 0) {
      sVar22 = 0;
      do {
        puVar17 = (short *)FUN_1000d910(sVar22);
        sVar24 = 0;
        puVar32 = puVar17;
        do {
          ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0x100));
          uVar18 = ByteSwap16();
          *puVar32 = uVar18;
          puVar32 = puVar32 + 1;
          sVar24 = sVar24 + 1;
        } while (sVar24 < 0x2b);
        ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0xe0),puVar17 + 0x2b,500);
        ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0x100));
        uVar18 = ByteSwap16();
        puVar17[0x125] = uVar18;
        puVar32 = puVar17 + 0x126;
        uVar19 = 0;
        do {
          ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0x100));
          uVar18 = ByteSwap16();
          *puVar32 = uVar18;
          puVar32 = puVar32 + 1;
          uVar19 = uVar19 + 1;
        } while (uVar19 < 0xb8);
        puVar17 = puVar17 + 0x1de;
        uVar19 = 0;
        do {
          ResourceRead_Dispatch((int)piVar13 + (int)*(short *)(*piVar13 + 0x100));
          ppuVar8 = local_3ec;
          uVar18 = ByteSwap16();
          *puVar17 = uVar18;
          puVar17 = puVar17 + 1;
          uVar19 = uVar19 + 1;
        } while (uVar19 < 0x38);
        ReleaseCityData(sVar22);
        sVar22 = sVar22 + 1;
      } while (sVar22 < 8);
      FlushCityCache();
      StreamRead(piVar13,*(int *)(*(int*)((char*)ppuVar8 - 0x10c)),0x4b0);
      if ((*(short *)(*(int*)((char*)ppuVar8 - 0x69d)) == 0) && (*(int *)(*(int*)((char*)ppuVar8 - 0xff)) != 0)) {
        DisposeHandle_Thunk(*(int *)(*(int*)((char*)ppuVar8 - 0xff)));
      }
      PostLoadCityInit();
    }
    CloseStream(piVar13,1);
  }
  ProgressUpdate(*piVar6,0x50);
  uVar18 = PostLoadInitialization();
  if (*(short *)(*piVar7 + 0x120) != 0) {
    FUN_10025e04();
  }
  ProgressUpdate(*piVar6,100);
  InitArmyData();
  ResourceRead_Dispatch(*piVar6 + (int)*(short *)(*(int *)*piVar6 + 0x738));
  *piVar6 = 0;
  return uVar18;
}




























































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1002b230 Size: 688 bytes */
void FUN_1002b230(short param_1)

{
  int iVar1;
  int *piVar2;
  int *piVar3;
  int *piVar4;
  int iVar5;
  unsigned long long uVar6;
  long long uVar7;
  long long uVar8;
  short sVar9;
  int iVar10;
  int iVar11;
  int iVar12;
  
  iVar5 = gCombatDisplayPieces;
  piVar4 = piRam10117360;
  piVar3 = piRam1011735c;
  piVar2 = piRam10117354;
  uVar6 = ZEXT48(0 /* TVect base */);
  if (param_1 == 0) {
    FUN_1005f678(0xa6,6);
    FUN_1003c938();
    FUN_1005f678(0xa6,7);
    FUN_1003c938();
    FUN_1005f678(0xa6,8);
    FUN_1003c938();
    uVar7 = FUN_1005f678(0xa6,10);
    uVar8 = FUN_1005f678(0xa6,9);
    FUN_1003ced4(uVar8,uVar7);
    MapRefreshAndCombat();
  }
  iVar12 = 0;
  if (0 < *(short *)(*piVar3 + 0x1602)) {
    do {
      iVar11 = *piVar3 + iVar12 * 0x42;
      if ((int)*(char *)(iVar11 + 0x1619) == (int)*(short *)(*piVar3 + 0x110)) {
        FUN_1004f438(*(short *)(iVar11 + 0x1604),*(short *)(iVar11 + 0x1606),0);
      }
      sVar9 = (short)iVar12 + 1;
      iVar12 = (int)sVar9;
    } while (sVar9 < *(short *)(*piVar3 + 0x1602));
  }
  iVar12 = (int)uVar6;
  sVar9 = *(short *)(*piVar3 + 0x182);
  while (sVar9 != 0) {
    sVar9 = sVar9 + -1;
    iVar12 = *piVar4;
    iVar11 = sVar9 * 0x16;
    iVar10 = iVar12 + iVar11;
    if ((int)*(char *)(iVar10 + 5) == (int)*(short *)(*piVar3 + 0x110)) {
      if (*(char *)(iVar10 + 4) == '\x1c') {
        HandleHeroDefeat(iVar10,*(short *)(iVar11 + iVar12),*(short *)(iVar10 + 2));
      }
      RemoveBattlePiece(*piVar4 + sVar9 * 0x16);
    }
    iVar12 = (int)uVar6;
  }
  if (*(short *)(*piVar3 + 0x124) != 0) {
    iVar11 = 0;
    do {
      iVar10 = 0;
      do {
        iVar1 = *piVar2 + iVar10 * 0x70;
        *(unsigned int *)(iVar1 + iVar11) = *(unsigned int *)(iVar1 + iVar11) | 0x20000000;
        sVar9 = (short)iVar10 + 1;
        iVar10 = (int)sVar9;
      } while (sVar9 < 0x9c);
      sVar9 = (short)iVar11 + 1;
      iVar11 = (int)sVar9;
    } while (sVar9 < 0x70);
    CenterMapOnArmy(*(short *)(*piVar3 + 0x110));
    RefreshDisplay(1,1,0);
    MapRefreshAndCombat();
  }
  **(int **)(iVar12 + -0x3f0) = 0;
  **(int **)(iVar12 + -0x1c0) = 0;
  iVar12 = 0;
  do {
    *(int *)(iVar12 * 4 + iVar5) = 0;
    sVar9 = (short)iVar12 + 1;
    iVar12 = (int)sVar9;
  } while (sVar9 < 8);
  UpdateDisplayState();
  FUN_1002c85c();
  if (param_1 == 1) {
    uVar7 = FUN_1005f678(0xa6,5);
    uVar8 = FUN_1005f678(0xa6,4);
    FUN_1003ced4(uVar8,uVar7);
  }
  return;
}





































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1002bbd4 Size: 260 bytes */
void FUN_1002bbd4()

{
  char cVar1;
  int *piVar2;
  int iVar3;
  short sVar4;
  short sVar5;
  int iVar6;
  int *piVar7;
  int iVar8;
  int iVar9;
  int local_8;
  int local_4;
  
  iVar3 = iRam10117630;
  piVar2 = piRam10117360;
  iVar8 = 0;
  do {
    *(short *)(iVar8 * 2 + iVar3) = 0;
    sVar4 = (short)iVar8 + 1;
    iVar8 = (int)sVar4;
  } while (sVar4 < 8);
  sVar4 = *(short *)(*piRam1011735c + 0x182);
  if (sVar4 != 0) {
    do {
      sVar4 = sVar4 + -1;
      iVar9 = (int)sVar4;
      iVar8 = *piVar2;
      iVar6 = iVar8 + iVar9 * 0x16;
      if (((*(char *)(iVar6 + 5) < '\b') && (-1 < *(short *)(iVar9 * 0x16 + iVar8))) &&
         (-1 < *(short *)(iVar6 + 2))) {
        cVar1 = *(char *)(iVar6 + 0xb);
        sVar5 = (short)cVar1;
        if ((*(unsigned short *)(iVar6 + 0xc) & 0x1000) != 0) {
          local_4 = (int)cVar1;
          local_8 = 4;
          piVar7 = &local_8;
          if (4 < cVar1) {
            piVar7 = &local_4;
          }
          sVar5 = (short)*piVar7;
        }
        iVar8 = *(char *)(*piVar2 + iVar9 * 0x16 + 5) * 2;
        *(short *)(iVar8 + iVar3) = *(short *)(iVar8 + iVar3) + sVar5;
      }
    } while (iVar9 != 0);
  }
  return;
}

/* Address: 0x1002bdc4 Size: 140 bytes */
void FUN_1002bdc4()

{
  char cVar1;
  int iVar2;
  int *piVar3;
  int iVar4;
  short sVar5;
  int iVar6;
  
  iVar4 = gPerPlayerArmyCount;
  piVar3 = piRam1011735c;
  iVar6 = 0;
  do {
    *(short *)(iVar6 * 2 + iVar4) = 0;
    sVar5 = (short)iVar6 + 1;
    iVar6 = (int)sVar5;
  } while (sVar5 < 8);
  iVar6 = 0;
  if (0 < *(short *)(*piVar3 + 0x1602)) {
    do {
      cVar1 = *(char *)(*piVar3 + iVar6 * 0x42 + 0x1619);
      if (cVar1 < '\b') {
        iVar2 = cVar1 * 2;
        *(short *)(iVar2 + iVar4) = *(short *)(iVar2 + iVar4) + 1;
      }
      sVar5 = (short)iVar6 + 1;
      iVar6 = (int)sVar5;
    } while (sVar5 < *(short *)(*piVar3 + 0x1602));
  }
  return;
}










































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1002bef8 Size: 108 bytes */
int FUN_1002bef8(short param_1,short param_2)

{
  int iVar1;
  short sVar2;
  int iVar3;
  
  iVar3 = 0;
  if (0 < *(short *)(*piRam1011735c + 0x810)) {
    do {
      iVar1 = *piRam1011735c + iVar3 * 0x20;
      if ((*(short *)(iVar1 + 0x812) == param_1) && (*(short *)(iVar1 + 0x814) == param_2)) {
        return iVar3;
      }
      sVar2 = (short)iVar3 + 1;
      iVar3 = (int)sVar2;
    } while (sVar2 < *(short *)(*piRam1011735c + 0x810));
  }
  return -1;
}

/* Address: 0x1002bf64 Size: 444 bytes */
unsigned long long FUN_1002bf64(short param_1,short param_2,short param_3,short param_4)

{
  int iVar1;
  int *piVar2;
  unsigned long long uVar3;
  int iVar4;
  short sVar5;
  int iVar6;
  unsigned long long uVar7;
  
  piVar2 = piRam1011735c;
  if (*(short *)(*piRam1011735c + 0x1602) < 1) {
    uVar3 = 0xffffffffffffffff;
  }
  else {
    uVar3 = 0xffffffffffffffff;
    iVar6 = 1000;
    uVar7 = 0;
    do {
      if ((*(short *)(*piVar2 + 0x124) == 0) || (param_4 == 0)) {
LAB_1002c08c:
        iVar1 = (int)(((uVar7 & 0x7ffffff) * 0x20 + uVar7 & 0x7fffffff) << 1);
        iVar4 = *piVar2 + iVar1;
        iVar4 = CalcDistance(param_1,param_2,*(short *)(iVar4 + 0x1604),
                             *(short *)(iVar4 + 0x1606));
        if ((iVar4 < iVar6) &&
           ((param_3 == 0xe || ((int)*(char *)(iVar1 + *piVar2 + 0x1619) == (int)param_3)))) {
          uVar3 = uVar7;
          iVar6 = iVar4;
        }
      }
      else {
        iVar1 = (int)(((uVar7 & 0x7ffffff) * 0x20 + uVar7 & 0x7fffffff) << 1);
        iVar4 = iVar1 + *piVar2;
        iVar4 = FUN_10009630(1,*(short *)(iVar4 + 0x1604),*(short *)(iVar4 + 0x1606));
        if ((iVar4 != 0) ||
           (((iVar4 = FUN_10009630(1,*(short *)(*piVar2 + iVar1 + 0x1604) + 1,
                                   *(short *)(*piVar2 + iVar1 + 0x1606)), iVar4 != 0 ||
             (iVar4 = FUN_10009630(1,*(short *)(iVar1 + *piVar2 + 0x1604),
                                   *(short *)(iVar1 + *piVar2 + 0x1606) + 1), iVar4 != 0)) ||
            (iVar4 = FUN_10009630(1,*(short *)(*piVar2 + iVar1 + 0x1604) + 1,
                                  *(short *)(*piVar2 + iVar1 + 0x1606) + 1), iVar4 != 0))))
        goto LAB_1002c08c;
      }
      sVar5 = (short)uVar7 + 1;
      uVar7 = (unsigned long long)sVar5;
    } while (sVar5 < *(short *)(*piVar2 + 0x1602));
  }
  return uVar3;
}

/* Address: 0x1002c200 Size: 272 bytes */
int FUN_1002c200(short param_1,short param_2,short param_3)

{
  short sVar1;
  int *piVar2;
  int iVar3;
  short sVar4;
  int iVar5;
  int iVar6;
  int iVar7;
  
  piVar2 = piRam1011735c;
  if (*(short *)(*piRam1011735c + 0x810) < 1) {
    iVar5 = -1;
  }
  else {
    sVar1 = *(short *)(*piRam1011735c + 0x110);
    iVar5 = -1;
    iVar6 = 1000;
    iVar7 = 0;
    do {
      iVar3 = *piVar2 + iVar7 * 0x20;
      if (((*(unsigned short *)(iVar3 + 0x830) & (unsigned short)(1 << ((int)sVar1 & 0x3fU))) != 0) &&
         (((*(short *)(*piVar2 + 0x124) == 0 || (param_3 == 0)) ||
          (iVar3 = FUN_10009630(1,*(short *)(iVar3 + 0x812),*(short *)(iVar3 + 0x814)),
          iVar3 != 0)))) {
        iVar3 = *piVar2 + iVar7 * 0x20;
        iVar3 = CalcDistance(param_1,param_2,*(short *)(iVar3 + 0x812),
                             *(short *)(iVar3 + 0x814));
        if (iVar3 < iVar6) {
          iVar5 = iVar7;
          iVar6 = iVar3;
        }
      }
      sVar4 = (short)iVar7 + 1;
      iVar7 = (int)sVar4;
    } while (sVar4 < *(short *)(*piVar2 + 0x810));
  }
  return iVar5;
}


/* Address: 0x1002cbbc Size: 636 bytes */
void FUN_1002cbbc()

{
  short sVar1;
  long long lVar2;
  int bVar3;
  int iVar4;
  int *piVar5;
  int *piVar6;
  short sVar10;
  int iVar7;
  int iVar8;
  short *puVar9;
  long long lVar11;
  int unaff_r29;
  unsigned long long uVar12;
  
  piVar6 = piRam1011735c;
  piVar5 = piRam10117358;
  iVar4 = iRam10115dd4;
  if (*(short *)(*piRam1011735c + 0x1602) != 0) {
    sVar1 = *(short *)(*piRam1011735c + 0x1602);
    do {
      sVar1 = sVar1 + -1;
      uVar12 = (unsigned long long)sVar1;
      iVar7 = *piVar6 + (sVar1 * 0x20 + (int)sVar1) * 2;
      lVar2 = -(long long)
               *(char *)(*piVar6 + (*(unsigned int *)(*piVar5 + *(short *)(iVar7 + 0x1606) * 0xe0 +
                                             *(short *)(iVar7 + 0x1604) * 2) >> 0x18) + 0x711);
      lVar11 = lVar2 + 10;
      if ((lVar11 + (-(unsigned long long)(lVar11 == 0) - (lVar2 + 9)) & 0xff) == 0) {
        bVar3 = false;
        if (*(char *)(iVar7 + 0x1619) == '\x0f') {
          if (*(short *)(*piVar6 + 0x11a) == 0) {
            bVar3 = true;
          }
          else if (0 < *(short *)(*piVar6 + 0x11a)) {
            unaff_r29 = RandomRange(1,4,*(short *)(*piVar6 + 0x11a) + -2);
            if (3 < unaff_r29) {
              unaff_r29 = 3;
            }
          }
        }
        else {
          unaff_r29 = 3;
        }
        if (!bVar3) {
          if (*(short *)(*piVar6 + 0x128) == 0) {
            sVar10 = 1;
          }
          else {
            sVar10 = RandomRange(1,4,0);
          }
          while (sVar10 != 0) {
            sVar10 = sVar10 + -1;
            iVar7 = FUN_1001e794(uVar12,*(short *)(unaff_r29 * 2 + iVar4),0xffffffffffffffff,0)
            ;
            if (iVar7 == 0) {
              bVar3 = true;
              break;
            }
            iVar7 = (int)(((uVar12 & 0x7ffffff) * 0x20 + uVar12 & 0x7fffffff) << 1);
            iVar8 = FUN_10047d80(uVar12,*(char *)(*piVar6 + iVar7 + 0x1630));
            iVar7 = iVar7 + *piVar6;
            *(char *)(iVar7 + 0x1630) = *(char *)(iVar7 + iVar8 + 0x161a);
            FUN_1002cae8(uVar12);
          }
          if (!bVar3) goto LAB_1002ce10;
        }
        puVar9 = (short *)FUN_10021434();
        if (puVar9 != (short *)0x0) {
          *(char *)((int)puVar9 + 5) = 0xf;
          *(char *)(puVar9 + 2) = 0xb;
          iVar7 = (int)(((uVar12 & 0x7ffffff) * 0x20 + uVar12 & 0xffffffff) << 1);
          *puVar9 = *(short *)(*piVar6 + iVar7 + 0x1604);
          puVar9[1] = *(short *)(iVar7 + *piVar6 + 0x1606);
          *(char *)(puVar9 + 4) = 1;
        }
      }
LAB_1002ce10:
    } while (sVar1 != 0);
  }
  return;
}






















































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1002ce38 Size: 388 bytes */
void FUN_1002ce38()

{
  short sVar1;
  long long lVar2;
  int iVar3;
  int *piVar4;
  int *piVar5;
  int iVar6;
  long long lVar7;
  int iVar8;
  short local_18 [12];
  
  piVar5 = piRam1011735c;
  piVar4 = piRam10117358;
  if (1 < *(short *)(*piRam1011735c + 0x11a)) {
    sVar1 = *(short *)(*piRam1011735c + 0x1602);
    while (sVar1 != 0) {
      sVar1 = sVar1 + -1;
      iVar8 = (int)sVar1;
      iVar3 = (sVar1 * 0x20 + (int)sVar1) * 2;
      iVar6 = *piVar5 + iVar3;
      if (((*(char *)(iVar6 + 0x1619) == '\x0f') &&
          (lVar2 = -(long long)
                    *(char *)((*(unsigned int *)(*piVar4 + *(short *)(iVar6 + 0x1606) * 0xe0 +
                                        *(short *)(iVar6 + 0x1604) * 2) >> 0x18) + *piVar5 + 0x711),
          lVar7 = lVar2 + 10, (lVar7 + (-(unsigned long long)(lVar7 == 0) - (lVar2 + 9)) & 0xff) == 0)) &&
         (*(char *)(iVar6 + 0x1634) != '\0')) {
        if ((*(char *)(iVar6 + 0x1631) != '\0') &&
           (*(char *)(iVar6 + 0x1631) = *(char *)(iVar6 + 0x1631) + -1,
           *(char *)(*piVar5 + iVar3 + 0x1631) == '\0')) {
          FUN_1002cae8(iVar8);
        }
        iVar6 = CheckArmyEscort(iVar8);
        if ((iVar6 == 0) &&
           (iVar6 = *piVar5 + iVar8 * 0x42,
           FUN_1002122c(*(short *)(iVar6 + 0x1604),*(short *)(iVar6 + 0x1606),local_18),
           local_18[0] < 4)) {
          FUN_1001e794(iVar8,2,0xffffffffffffffff,0);
        }
      }
    }
  }
  return;
}

/* Address: 0x1003357c Size: 132 bytes */
void FUN_1003357c(int param_1,short param_2)

{
  unsigned int uVar1;
  int iVar2;
  
  if (*(char *)(param_1 + 4) != '\x1c') {
    return;
  }
  iVar2 = *piRam1011735c + (int)*(char *)(param_1 + 10);
  uVar1 = *(unsigned int *)(iVar2 + 0x5e4);
  if (0x3b < (uVar1 >> 0x1a) + (int)param_2) {
    *(unsigned int *)(iVar2 + 0x5e4) = uVar1 & 0x3ffffff | 0xf0000000;
    return;
  }
  iVar2 = *piRam1011735c + (int)*(char *)(param_1 + 10);
  uVar1 = *(unsigned int *)(iVar2 + 0x5e4);
  *(unsigned int *)(iVar2 + 0x5e4) = ((uVar1 >> 0x1a) + (int)param_2) * 0x4000000 | uVar1 & 0x3ffffff;
  return;
}

/* Address: 0x10035d7c Size: 288 bytes */
void FUN_10035d7c()

{
  int *piVar1;
  int *piVar2;
  int *puVar3;
  int iVar4;
  char auStack_128 [256];
  short local_28;
  int local_24;
  int local_20;
  int local_1c;
  int local_18;
  
  puVar3 = puRam10117370;
  piVar2 = piRam10115e34;
  *puRam10117544 = *(short *)(*piRam1011735c + 0x110);
  local_28 = 0;
  local_24 = 0;
  local_1c = 0;
  local_18 = 0;
  local_20 = *puVar3;
  *puVar3 = auStack_128;
  iVar4 = TrySetjmp(auStack_128);
  if (iVar4 == 0) {
    FUN_100358d4(1);
    FUN_10035ccc();
    piVar1 = (int *)*piVar2;
    if (piVar1 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar1 + (int)*(short *)(*piVar1 + 0x810));
      ResourceRead_Dispatch(*piVar2 + (int)*(short *)(*(int *)*piVar2 + 0x738));
      *piVar2 = 0;
    }
    *puVar3 = local_20;
  }
  else {
    piVar1 = (int *)*piVar2;
    if (piVar1 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar1 + (int)*(short *)(*piVar1 + 0x738));
    }
    *piVar2 = 0;
    LockHandleRange(local_28,local_24);
  }
  return;
}





















































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10038860 Size: 48 bytes */
void FUN_10038860()

{
  int *piVar1;
  short sVar2;
  int iVar3;
  
  piVar1 = piRam1011735c;
  iVar3 = 0;
  do {
    *(char *)(*piVar1 + iVar3 * 0x2c + 0x1422) = 0;
    sVar2 = (short)iVar3 + 1;
    iVar3 = (int)sVar2;
  } while (sVar2 < 8);
  return;
}

/* Address: 0x10038890 Size: 976 bytes */
void FUN_10038890()

{
  int uVar1;
  int iVar2;
  long long lVar3;
  int *piVar4;
  int *piVar5;
  long long uVar6;
  int *piVar7;
  long long lVar8;
  char uVar10;
  unsigned short uVar9;
  unsigned long long uVar11;
  short sVar12;
  int local_1dc;
  short local_1b8 [2];
  char auStack_1b4 [224];
  char auStack_d4 [16];
  short local_c4;
  short local_c2;
  short local_c0 [8];
  char local_b0 [104];
  int local_48;
  short local_44;
  short local_42;
  
  piVar5 = piRam1011735c;
  piVar4 = piRam10117358;
  uVar1 = *(int *)(*(int *)(*piRam1011734c + 0x88) + 0x160);
  local_44 = *(short *)(*piRam1011735c + 0x136);
  sVar12 = local_44;
  if (local_44 < 2) {
    sVar12 = 1;
  }
  if (sVar12 == 2) {
    uVar6 = FUN_100522dc(uVar1,6,0);
    if ((int)uVar6 == 0) {
      return;
    }
    local_48 = 4;
    StreamWrite(uVar6,&local_48);
    CloseStream(uVar6,1);
  }
  else {
    local_42 = *(short *)(*piRam1011735c + 0x136);
    sVar12 = local_42;
    if (local_42 < 2) {
      sVar12 = 1;
    }
    if (0xc9 < sVar12) {
      return;
    }
  }
  uVar6 = FUN_100522dc(uVar1,1,0);
  StreamRead(uVar6,&local_48,4);
  CloseStream(uVar6,1);
  piVar7 = (int *)FUN_100522dc(uVar1,2,0);
  iVar2 = *piVar7;
  uVar6 = ResourceRead_Dispatch((int)piVar7 + (int)*(short *)(iVar2 + 0xb8),iVar2,
                       *(int *)(iVar2 + 0xbc));
  ResourceRead_Dispatch((int)piVar7 + (int)*(short *)(iVar2 + 0xb0),uVar6);
  uVar11 = 0;
  if (0 < *(short *)(*piVar5 + 0x1602)) {
    do {
      iVar2 = *piVar5 + (int)(((uVar11 & 0x7ffffff) * 0x20 + uVar11 & 0xffffffff) << 1);
      lVar3 = -(long long)
               *(char *)(*piVar5 + (*(unsigned int *)(*piVar4 + *(short *)(iVar2 + 0x1606) * 0xe0 +
                                             *(short *)(iVar2 + 0x1604) * 2) >> 0x18) + 0x711);
      lVar8 = lVar3 + 10;
      if ((lVar8 + (-(unsigned long long)(lVar8 == 0) - (lVar3 + 9)) & 0xff) == 0) {
        uVar10 = *(char *)
                  (*piVar5 + (int)(((uVar11 & 0x7ffffff) * 0x20 + uVar11 & 0xffffffff) << 1) +
                  0x1619);
      }
      else {
        uVar10 = 0xff;
      }
      local_b0[(int)uVar11] = uVar10;
      sVar12 = (short)uVar11 + 1;
      uVar11 = (unsigned long long)sVar12;
    } while (sVar12 < *(short *)(*piVar5 + 0x1602));
  }
  uVar11 = 0;
  do {
    *(short *)((int)local_c0 + (int)((uVar11 & 0xffffffff) << 1)) =
         *(short *)
          (*piVar5 + (int)(((uVar11 & 0x3fffffff) * 4 + uVar11 & 0xffffffff) << 2) + 0x186);
    sVar12 = (short)uVar11 + 1;
    uVar11 = (unsigned long long)sVar12;
  } while (sVar12 < 8);
  GetScoreSnapshot(auStack_d4);
  GetAIStateSnapshot();
  local_1b8[0] = SerializeMoveData(auStack_1b4,&local_c2);
  sVar12 = *(short *)(*piVar5 + 0x1602) + local_c2;
  local_c4 = sVar12 + 0x34;
  uVar9 = *(unsigned short *)(*piVar5 + 0x1602);
  if ((int)(short)uVar9 +
      (((int)(short)uVar9 >> 1) + (unsigned int)((short)uVar9 < 0 && (uVar9 & 1) != 0) & 0x7fffffff) * -2 ==
      1) {
    uVar9 = uVar9 + 1;
    local_c4 = sVar12 + 0x35;
  }
  StreamWrite(piVar7,&local_c4,2);
  StreamWrite(piVar7,local_c0,0x10);
  StreamWrite(piVar7,auStack_d4,0x10);
  StreamWrite(piVar7,*(int *)(local_1dc + -0x324),0x10);
  StreamWrite(piVar7,local_b0,uVar9);
  StreamWrite(piVar7,local_1b8,2);
  StreamWrite(piVar7,auStack_1b4,local_c2);
  local_48 = ResourceRead_Dispatch((int)piVar7 + (int)*(short *)(*piVar7 + 0xa8));
  StreamSetPosition(piVar7,0,0);
  StreamWrite(piVar7,&local_48,4);
  CloseStream(piVar7,1);
  ClearCommandRecords();
  return;
}













































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1003956c Size: 1772 bytes */
void FUN_1003956c()

{
  int bVar1;
  int iVar2;
  int iVar3;
  int iVar4;
  int *piVar5;
  int *piVar6;
  int iVar8;
  unsigned long long uVar7;
  int iVar9;
  int iVar10;
  char uVar11;
  short sVar12;
  short sVar13;
  int iVar14;
  unsigned long long uVar15;
  int iVar17;
  unsigned long long uVar16;
  short local_a0 [40];
  short local_50;
  
  piVar6 = piRam1011735c;
  piVar5 = piRam10117358;
  iVar4 = iRam10115e64;
  iVar3 = iRam10115e60;
  iVar2 = iRam10115e5c;
  iVar10 = iRam10115e58;
  uVar16 = ZEXT48(0 /* TVect base */);
  sVar13 = (short)(((int)*(short *)(*piRam1011735c + 0x810) << 1) / 10);
  FUN_10038d8c(iRam10115e64,iRam10115e60,iRam10115e5c,4,3,2);
  FUN_10038fb8();
  FUN_10039180(sVar13);
  iVar8 = RandomRange(2,3,1);
  local_50 = (short)iVar8;
  if (sVar13 <= iVar8) {
    iVar8 = (int)sVar13;
  }
  uVar15 = 0;
  if (0 < *(short *)(*piVar6 + 0x810)) {
    do {
      iVar9 = (int)((uVar15 & 0x7ffffff) << 5);
      iVar14 = *piVar6 + iVar9;
      iVar17 = *piVar5 + *(short *)(iVar14 + 0x814) * 0xe0;
      iVar14 = *(short *)(iVar14 + 0x812) * 2;
      *(unsigned int *)(iVar17 + iVar14) = *(unsigned int *)(iVar17 + iVar14) & 0xffbfffff;
      iVar14 = (int)((uVar15 & 0xffffffff) << 1);
      *(short *)((int)local_a0 + iVar14) = 0;
      if (*(char *)(iVar9 + *piVar6 + 0x82a) == '\x01') {
        *(short *)((int)local_a0 + iVar14) = 1;
      }
      sVar12 = (short)uVar15 + 1;
      uVar15 = (unsigned long long)sVar12;
    } while (sVar12 < *(short *)(*piVar6 + 0x810));
  }
  for (sVar12 = *(short *)(*piVar6 + 0x810); sVar12 < 0x28; sVar12 = sVar12 + 1) {
    local_a0[sVar12] = 1;
  }
  sVar12 = RandomRange(1,5,0xfffffffffffffffd);
  sVar12 = *(short *)(*piVar6 + 0x810) / 3 + sVar12 + 8;
  iVar14 = (int)sVar12;
  if (0x16 < sVar12) {
    iVar14 = 0x16;
  }
  uVar15 = 8;
  if (8 < iVar14) {
    do {
      iVar17 = (int)uVar15;
      if ((iVar17 < iVar8 + 8) || (sVar13 + 8 <= iVar17)) {
        sVar12 = 0;
        bVar1 = false;
        do {
          sVar12 = sVar12 + 1;
          uVar7 = RandomRange(1,*(short *)(*piVar6 + 0x810),0xffffffffffffffff);
          iVar9 = *piVar6 + iVar17 * 0x1e;
          iVar9 = FUN_100390e4(*(int *)(iVar9 + 0xd12),*(int *)(iVar9 + 0xd16),
                               *(int *)(iVar9 + 0xd1a),*(int *)(iVar9 + 0xd1e),
                               *(int *)(iVar9 + 0xd22),*(int *)(iVar9 + 0xd26),
                               *(int *)(iVar9 + 0xd2a),*(int *)(iVar9 + 0xd2e));
          if ((iVar9 == 0) || (*(short *)(*piVar6 + (int)((uVar7 & 0xffffffff) << 5) + 0x82e) != 0))
          {
            iVar9 = *piVar6 + iVar17 * 0x1e;
            iVar9 = FUN_100390e4(*(int *)(iVar9 + 0xd12),*(int *)(iVar9 + 0xd16),
                                 *(int *)(iVar9 + 0xd1a),*(int *)(iVar9 + 0xd1e),
                                 *(int *)(iVar9 + 0xd22),*(int *)(iVar9 + 0xd26),
                                 *(int *)(iVar9 + 0xd2a),*(int *)(iVar9 + 0xd2e));
            if ((iVar9 != 0) ||
               (*(short *)(*piVar6 + (int)((uVar7 & 0xffffffff) << 5) + 0x82e) == 0)) {
              iVar9 = (int)((uVar7 & 0xffffffff) << 1);
              if (*(short *)((int)local_a0 + iVar9) == 0) {
                *(short *)((int)local_a0 + iVar9) = 1;
                bVar1 = true;
              }
              if (100 < sVar12) {
                iVar17 = (int)(((uVar15 & 0xfffffff) * 0x10 - uVar15 & 0xffffffff) << 1);
                *(char *)(*piVar6 + iVar17 + 0xd28) = 0;
                *(short *)(iVar17 + *piVar6 + 0xd2a) = (short)uVar7;
                break;
              }
            }
          }
        } while (!bVar1);
        if (sVar12 < 0x65) {
          iVar17 = (int)(((uVar15 & 0xfffffff) * 0x10 - uVar15 & 0xffffffff) << 1);
          *(char *)(*piVar6 + iVar17 + 0xd28) = 2;
          *(short *)(iVar17 + *piVar6 + 0xd2a) = (short)uVar7;
          iVar17 = (int)((uVar7 & 0xffffffff) << 5);
          *(char *)(*piVar6 + iVar17 + 0x82a) = 2;
          *(char *)(iVar17 + *piVar6 + 0x82b) = (char)uVar15;
        }
      }
      else {
        *(char *)
         (*piVar6 + (int)(((uVar15 & 0xfffffff) * 0x10 - uVar15 & 0xffffffff) << 1) + 0xd28) = 0;
      }
      sVar12 = (short)uVar15 + 1;
      uVar15 = (unsigned long long)sVar12;
    } while (sVar12 < iVar14);
  }
  if (iVar14 < 0x16) {
    do {
      *(char *)(*piVar6 + iVar14 * 0x1e + 0xd28) = 0;
      sVar13 = (short)iVar14 + 1;
      iVar14 = (int)sVar13;
    } while (sVar13 < 0x16);
  }
  iVar8 = 0;
  if (0 < *(short *)(*piVar6 + 0x810)) {
    do {
      if (local_a0[iVar8] == 0) {
        if (*(short *)(*piVar6 + iVar8 * 0x20 + 0x82e) == 0) {
          bVar1 = true;
          uVar15 = 0;
          do {
            iVar14 = *piVar6;
            if ((*(short *)(iVar14 + (int)((uVar15 & 0xffffffff) << 1) + 0x138) != 0) &&
               (iVar17 = iVar14 + (int)(((uVar15 & 0x3fffffff) * 4 + uVar15 & 0xffffffff) << 2),
               iVar14 = iVar14 + iVar8 * 0x20,
               iVar14 = CalcDistance(*(short *)(iVar17 + 0x18a),*(short *)(iVar17 + 0x18c)
                                     ,*(short *)(iVar14 + 0x812),
                                     *(short *)(iVar14 + 0x814)), iVar14 < 0xf)) {
              bVar1 = false;
              break;
            }
            sVar13 = (short)uVar15 + 1;
            uVar15 = (unsigned long long)sVar13;
          } while (sVar13 < 8);
          if (bVar1) {
            iVar14 = RandomRange(1,5,0xffffffffffffffff);
            *(char *)(*piVar6 + iVar8 * 0x20 + 0x82a) =
                 *(char *)(iVar14 + *(int *)((int)uVar16 + -0x1a34));
          }
          else {
            iVar14 = RandomRange(1,3,0xffffffffffffffff);
            *(char *)(*piVar6 + iVar8 * 0x20 + 0x82a) = *(char *)(iVar14 + iVar10);
          }
        }
        else {
          iVar14 = RandomRange(1,3,0xffffffffffffffff);
          *(char *)(iVar8 * 0x20 + *piVar6 + 0x82a) =
               *(char *)(iVar14 + *(int *)((int)uVar16 + -0x1a38));
        }
        *(char *)(*piVar6 + iVar8 * 0x20 + 0x82b) = 0;
        local_a0[iVar8] = 1;
      }
      sVar13 = (short)iVar8 + 1;
      iVar8 = (int)sVar13;
    } while (sVar13 < *(short *)(*piVar6 + 0x810));
  }
  uVar16 = 0;
  if (0 < *(short *)(*piVar6 + 0x810)) {
    do {
      iVar10 = *piVar6 + (int)((uVar16 & 0xffffffff) << 5);
      if (*(char *)(iVar10 + 0x82a) == '\x01') {
        *(char *)(iVar10 + 0x82c) = 0;
      }
      else {
        iVar8 = (int)((uVar16 & 0x7ffffff) << 5);
        iVar10 = *piVar6 + iVar8;
        if (*(char *)(iVar10 + 0x82a) == '\x05') {
          if (*(short *)(iVar10 + 0x82e) == 0) {
            bVar1 = true;
            uVar15 = 0;
            do {
              iVar10 = *piVar6;
              if ((*(short *)(iVar10 + (int)((uVar15 & 0xffffffff) << 1) + 0x138) != 0) &&
                 (iVar8 = iVar10 + (int)(((uVar15 & 0x3fffffff) * 4 + uVar15 & 0xffffffff) << 2),
                 iVar10 = iVar10 + (int)((uVar16 & 0xffffffff) << 5),
                 iVar10 = CalcDistance(*(short *)(iVar8 + 0x18a),*(short *)(iVar8 + 0x18c)
                                       ,*(short *)(iVar10 + 0x812),
                                       *(short *)(iVar10 + 0x814)), iVar10 < 0xf)) {
                bVar1 = false;
                break;
              }
              sVar13 = (short)uVar15 + 1;
              uVar15 = (unsigned long long)sVar13;
            } while (sVar13 < 8);
            if (bVar1) {
              iVar10 = RandomRange(1,3,0xffffffffffffffff);
              *(char *)(*piVar6 + (int)((uVar16 & 0xffffffff) << 5) + 0x82c) =
                   *(char *)(iVar10 + iVar3);
            }
            else {
              iVar10 = RandomRange(1,2,0xffffffffffffffff);
              *(char *)(*piVar6 + (int)((uVar16 & 0xffffffff) << 5) + 0x82c) =
                   *(char *)(iVar10 + iVar2);
            }
          }
          else {
            iVar10 = RandomRange(1,4,0xffffffffffffffff);
            *(char *)(iVar8 + *piVar6 + 0x82c) = *(char *)(iVar10 + iVar4);
          }
        }
        else {
          uVar11 = RandomRange(1,9,0);
          *(char *)(*piVar6 + (int)((uVar16 & 0xffffffff) << 5) + 0x82c) = uVar11;
        }
      }
      sVar13 = (short)uVar16 + 1;
      uVar16 = (unsigned long long)sVar13;
    } while (sVar13 < *(short *)(*piVar6 + 0x810));
  }
  return;
}

































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10039c58 Size: 296 bytes */
short FUN_10039c58(short param_1)

{
  short sVar1;
  int iVar2;
  int *piVar3;
  int *piVar4;
  short *psVar5;
  int iVar6;
  short sVar7;
  int iVar8;
  short sVar9;
  
  iVar6 = iRam1011773c;
  psVar5 = psRam1011760c;
  piVar4 = piRam10117360;
  piVar3 = piRam1011735c;
  if (*(short *)(*piRam1011735c + 0x182) == 0) {
    sVar9 = 0;
  }
  else {
    sVar1 = *(short *)(*piRam1011735c + 0x182);
    sVar9 = 0;
    do {
      sVar1 = sVar1 + -1;
      iVar8 = *piVar4 + sVar1 * 0x16;
      if (((int)*(char *)(iVar8 + 5) == (int)param_1) && (*(char *)(iVar8 + 4) == '\x1c')) {
        FUN_1003aeb0(2,0,0,iVar8);
        iVar8 = 0;
        if (0 < *psVar5) {
          do {
            iVar2 = *piVar3 + *(short *)(iVar8 * 2 + iVar6) * 0x1e;
            if (*(char *)(iVar2 + 0xd26) == '\a') {
              sVar9 = sVar9 + *(char *)(iVar2 + 0xd27);
            }
            sVar7 = (short)iVar8 + 1;
            iVar8 = (int)sVar7;
          } while (sVar7 < *psVar5);
        }
      }
    } while (sVar1 != 0);
  }
  return sVar9;
}



















































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10039ec8 Size: 556 bytes */
void UpdateArmyDisplay(short param_1)

{
  int iVar1;
  int iVar2;
  int *piVar3;
  int *piVar4;
  short sVar5;
  unsigned long long uVar6;
  
  piVar4 = piRam1011735c;
  piVar3 = piRam10117358;
  if (*(short *)(*piRam1011735c + 0x11e) != 0) {
    Render_RefreshFeatureOverlays();
  }
  uVar6 = 0;
  if (0 < *(short *)(*piVar4 + 0x810)) {
    do {
      iVar1 = *piVar4 + (int)uVar6 * 0x20;
      if (*(char *)(iVar1 + 0x82a) == '\x01') {
        iVar2 = *piVar3 + *(short *)(iVar1 + 0x814) * 0xe0;
        iVar1 = *(short *)(iVar1 + 0x812) * 2;
        *(unsigned int *)(iVar2 + iVar1) = *(unsigned int *)(iVar2 + iVar1) & 0xffffff | 0xb000000;
      }
      else {
        iVar1 = *piVar4 + (int)uVar6 * 0x20;
        if (*(short *)(iVar1 + 0x82e) == 0) {
          iVar1 = *piVar4 + (int)((uVar6 & 0xffffffff) << 5);
          iVar2 = *piVar3 + *(short *)(iVar1 + 0x814) * 0xe0;
          iVar1 = *(short *)(iVar1 + 0x812) * 2;
          *(unsigned int *)(iVar2 + iVar1) = *(unsigned int *)(iVar2 + iVar1) & 0xffffff | 0xc000000;
        }
        else if (((int)*(short *)(iVar1 + 0x830) & 1 << ((int)param_1 & 0x3fU)) == 0) {
          if (*(short *)(*piVar4 + 0x11e) == 0) {
            iVar1 = *piVar4 + (int)((uVar6 & 0xffffffff) << 5);
            iVar2 = *piVar3 + *(short *)(iVar1 + 0x814) * 0xe0;
            iVar1 = *(short *)(iVar1 + 0x812) * 2;
            *(unsigned int *)(iVar2 + iVar1) = *(unsigned int *)(iVar2 + iVar1) & 0xffffff | 0xa000000;
          }
          else {
            iVar1 = *piVar4 + (int)((uVar6 & 0xffffffff) << 5);
            iVar2 = *piVar3 + *(short *)(iVar1 + 0x814) * 0xe0;
            iVar1 = *(short *)(iVar1 + 0x812) * 2;
            *(unsigned int *)(iVar2 + iVar1) = *(unsigned int *)(iVar2 + iVar1) & 0xffffff | 0x9000000;
          }
        }
        else {
          iVar2 = *piVar3 + *(short *)(iVar1 + 0x814) * 0xe0;
          iVar1 = *(short *)(iVar1 + 0x812) * 2;
          *(unsigned int *)(iVar2 + iVar1) = *(unsigned int *)(iVar2 + iVar1) & 0xffffff | 0xa000000;
        }
      }
      if (*(short *)(*piVar4 + 0x11e) != 0) {
        iVar1 = *piVar4 + (int)((uVar6 & 0xffffffff) << 5);
        DrawSingleMapTile(0,*(short *)(iVar1 + 0x812),*(short *)(iVar1 + 0x814));
      }
      sVar5 = (short)uVar6 + 1;
      uVar6 = (unsigned long long)sVar5;
    } while (sVar5 < *(short *)(*piVar4 + 0x810));
  }
  if (*(short *)(*piVar4 + 0x11e) != 0) {
    FUN_10064104();
  }
  return;
}

/* Address: 0x1003c068 Size: 768 bytes */
void FUN_1003c068()

{
  short uVar1;
  int *piVar2;
  int iVar3;
  int iVar4;
  char uVar5;
  short sVar6;
  unsigned long long uVar7;
  unsigned long long uVar8;
  short local_40 [8];
  short local_30 [24];
  
  piVar2 = piRam1011735c;
  uVar8 = 0xffffffffffffffff;
  uVar7 = 7;
  do {
    if (*(short *)(*piVar2 + (int)((uVar7 & 0xffffffff) << 1) + 0x138) == 0) {
      iVar3 = (int)(((uVar7 & 0x3fffffff) * 4 + uVar7 & 0xffffffff) << 2);
      *(short *)(*piVar2 + iVar3 + 0x194) = 0xffff;
      *(short *)(iVar3 + *piVar2 + 0x196) = 0xffff;
    }
    else {
      iVar4 = (int)(((uVar7 & 0x3fffffff) * 4 + uVar7 & 0x3fffffff) << 2);
      iVar3 = iVar4 + *piVar2;
      *(short *)(iVar3 + 0x194) = *(short *)(iVar3 + 0x18a);
      iVar4 = iVar4 + *piVar2;
      *(short *)(iVar4 + 0x196) = *(short *)(iVar4 + 0x18c);
    }
    iVar3 = *piVar2 + (int)(((uVar7 & 0x3fffffff) * 4 + uVar7 & 0xffffffff) << 2);
    uVar1 = *(short *)(iVar3 + 0x18a);
    iVar4 = (int)((uVar7 & 0x7fffffff) << 1);
    *(short *)((int)local_30 + iVar4) = uVar1;
    *(short *)((int)local_40 + iVar4) = *(short *)(iVar3 + 0x18c);
    iVar3 = LookupCityAtPos(uVar1);
    if (-1 < iVar3) {
      if (*(short *)(*piVar2 + iVar4 + 0x138) == 0) {
        FUN_1002ca18(*piVar2 + iVar3 * 0x42 + 0x1604,0xf);
        *(char *)(iVar3 * 0x42 + *piVar2 + 0x1619) = 0xf;
      }
      else {
        FUN_1002ca18(*piVar2 + iVar3 * 0x42 + 0x1604,uVar7);
        *(char *)(iVar3 * 0x42 + *piVar2 + 0x1619) = (char)uVar7;
        uVar8 = uVar7;
      }
    }
    iVar3 = (int)uVar7;
    uVar7 = (long long)(short)((short)uVar7 + -1);
  } while (iVar3 != 0);
  if (((int)uVar8 != -1) && (*(short *)(*piVar2 + 0x128) != 0)) {
    iVar3 = (int)((uVar8 & 0xffffffff) << 1);
    for (iVar3 = FUN_1002bf64(*(short *)((int)local_30 + iVar3),
                              *(short *)((int)local_40 + iVar3),0xf,0); -1 < iVar3;
        iVar3 = FUN_1002bf64(*(short *)((int)local_30 + iVar3),
                             *(short *)((int)local_40 + iVar3),0xf,0)) {
      FUN_1002ca18(*piVar2 + iVar3 * 0x42 + 0x1604,uVar8);
      *(char *)(iVar3 * 0x42 + *piVar2 + 0x1619) = (char)uVar8;
      iVar4 = RandomRange(1,10,0xffffffffffffffff);
      if (iVar4 < 5) {
        iVar3 = *piVar2 + (int)(((uVar8 & 0x3fffffff) * 4 + uVar8 & 0xffffffff) << 2);
        iVar4 = (int)((uVar8 & 0xffffffff) << 1);
        *(short *)((int)local_30 + iVar4) = *(short *)(iVar3 + 0x18a);
        *(short *)((int)local_40 + iVar4) = *(short *)(iVar3 + 0x18c);
      }
      else {
        iVar3 = *piVar2 + iVar3 * 0x42;
        iVar4 = (int)((uVar8 & 0xffffffff) << 1);
        *(short *)((int)local_30 + iVar4) = *(short *)(iVar3 + 0x1604);
        *(short *)((int)local_40 + iVar4) = *(short *)(iVar3 + 0x1606);
      }
      do {
        uVar8 = uVar8 + 1;
        uVar8 = (unsigned long long)
                (short)((short)uVar8 -
                       (short)(((long long)((int)uVar8 >> 3) +
                                (unsigned long long)((int)uVar8 < 0 && (uVar8 & 7) != 0) & 0xffffffff) << 3))
        ;
        iVar3 = (int)((uVar8 & 0x7fffffff) << 1);
      } while (*(short *)(*piVar2 + iVar3 + 0x138) == 0);
    }
  }
  FUN_1003b9f8();
  iVar3 = 0;
  if (0 < *(short *)(*piVar2 + 0x1602)) {
    do {
      uVar5 = FUN_10048c90(iVar3);
      *(char *)(*piVar2 + iVar3 * 0x42 + 0x1618) = uVar5;
      sVar6 = (short)iVar3 + 1;
      iVar3 = (int)sVar6;
    } while (sVar6 < *(short *)(*piVar2 + 0x1602));
  }
  return;
}

/* Address: 0x1003cb84 Size: 848 bytes */
void FUN_1003cb84()

{
  char cVar1;
  short uVar2;
  unsigned int uVar3;
  int bVar4;
  int *piVar5;
  int *piVar6;
  unsigned int *puVar7;
  int *piVar8;
  int iVar9;
  long long uVar10;
  int *piVar11;
  short sVar12;
  int iVar13;
  unsigned long long uVar14;
  int iVar16;
  unsigned long long uVar15;
  int iVar17;
  unsigned int local_a4;
  char auStack_80 [128];
  
  iVar9 = gPerPlayerArmyCount;
  piVar8 = piRam10117360;
  puVar7 = puRam1011735c;
  piVar6 = piRam1011734c;
  piVar5 = gMainGameWindow;
  uVar14 = ZEXT48(0 /* TVect base */);
  GetAIStateSnapshot();
  bVar4 = false;
  iVar16 = 0;
  while ((iVar13 = *puVar7 + iVar16 * 2, *(short *)(iVar13 + 0x138) == 0 ||
         (*(short *)(iVar13 + 0xd0) != 0))) {
    sVar12 = (short)iVar16 + 1;
    iVar16 = (int)sVar12;
    if (7 < sVar12) {
LAB_1003cbf8:
      uVar15 = 0;
      do {
        iVar13 = (int)uVar14;
        iVar16 = (int)((uVar15 & 0xffffffff) << 1);
        if ((*(short *)(*puVar7 + iVar16 + 0x138) != 0) && (*(short *)(iVar16 + iVar9) == 0)) {
          sVar12 = *(short *)(*puVar7 + 0x182);
          if (sVar12 != 0) {
            do {
              sVar12 = sVar12 + -1;
              iVar17 = (int)sVar12;
              iVar16 = iVar17 * 0x16;
              cVar1 = *(char *)(*piVar8 + iVar16 + 5);
              if (cVar1 == (char)uVar15) {
                if (*(char *)(*piVar8 + iVar16 + 4) == '\x1c') {
                  uVar2 = *(short *)(*puVar7 + 0x110);
                  *(short *)(*puVar7 + 0x110) = (short)cVar1;
                  iVar13 = *piVar8 + iVar16;
                  HandleHeroDefeat(iVar13,*(short *)(*piVar8 + iVar16),*(short *)(iVar13 + 2))
                  ;
                  *(short *)(*puVar7 + 0x110) = uVar2;
                }
                RemoveBattlePiece(*piVar8 + iVar17 * 0x16);
              }
              iVar13 = (int)uVar14;
            } while (iVar17 != 0);
          }
          FUN_10038c60(uVar15,4,uVar15,0,*(int *)(iVar13 + -0x1a14));
          uVar10 = FUN_1005f678(0xc,0xffffffffffffffff);
          DrawNumber(auStack_80,uVar10,
                       (unsigned long long)*puVar7 + ((uVar15 & 0x3fffffff) * 4 + uVar15 & 0x3fffffff) * 4);
          if (*(short *)(*puVar7 + (int)((uVar15 & 0xffffffff) << 1) + 0xd0) == 0) {
            if (*(char *)(*(int *)(*piVar6 + 0x88) + 0x16e) == '\0') {
              FUN_1003c938(auStack_80);
            }
            else {
              FUN_1007fd6c(*(int *)(*piVar6 + 0x88),uVar15,8,auStack_80);
            }
          }
          else if (bVar4) {
            FUN_1003c938(auStack_80);
          }
          else {
            FUN_1003cac4(auStack_80,100);
          }
          *(short *)(*puVar7 + (int)((uVar15 & 0xffffffff) << 1) + 0x138) = 0;
          *(short *)
           (*puVar7 + (int)(((uVar15 & 0x3fffffff) * 4 + uVar15 & 0xffffffff) << 2) + 0x186) = 0;
          uVar14 = 0;
          do {
            iVar17 = (int)((uVar15 & 0xfffffff) << 4);
            iVar13 = (int)((uVar14 & 0x7fffffff) << 1);
            iVar16 = *puVar7 + iVar17 + iVar13;
            *(unsigned int *)(iVar16 + 0x1582) = *(unsigned int *)(iVar16 + 0x1582) & 0xcfffffff | 0x10000000;
            iVar13 = iVar17 + *puVar7 + iVar13;
            uVar3 = *(unsigned int *)(iVar13 + 0x1582);
            *(unsigned int *)(iVar13 + 0x1582) = (uVar3 >> 0x1c & 3) << 0x1a | uVar3 & 0xf3ffffff;
            iVar17 = (int)((uVar14 & 0xfffffff) << 4);
            iVar13 = (int)((uVar15 & 0x7fffffff) << 1);
            iVar16 = *puVar7 + iVar17 + iVar13;
            *(unsigned int *)(iVar16 + 0x1582) = *(unsigned int *)(iVar16 + 0x1582) & 0xcfffffff | 0x10000000;
            iVar13 = iVar17 + *puVar7 + iVar13;
            uVar3 = *(unsigned int *)(iVar13 + 0x1582);
            *(unsigned int *)(iVar13 + 0x1582) = (uVar3 >> 0x1c & 3) << 0x1a | uVar3 & 0xf3ffffff;
            sVar12 = (short)uVar14 + 1;
            uVar14 = (unsigned long long)sVar12;
          } while (sVar12 < 8);
          piVar11 = (int *)ResourceRead_Dispatch(*piVar5 + (int)*(short *)(*(int *)*piVar5 + 0x308),
                                        0x7475726e);
          uVar14 = (unsigned long long)local_a4;
          if (piVar11 != (int *)0x0) {
            ResourceRead_Dispatch((int)piVar11 + (int)*(short *)(*piVar11 + 0x4d0));
            ResourceRead_Dispatch((int)piVar11 + (int)*(short *)(*piVar11 + 0x4c8));
            uVar14 = (unsigned long long)local_a4;
          }
        }
        sVar12 = (short)uVar15 + 1;
        uVar15 = (unsigned long long)sVar12;
      } while (sVar12 < 8);
      return;
    }
  }
  bVar4 = true;
  goto LAB_1003cbf8;
}
































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1003d094 Size: 1096 bytes */
void FUN_1003d094()

{
  unsigned short uVar1;
  long long lVar2;
  int bVar3;
  int *piVar4;
  unsigned int *puVar5;
  int *ppuVar6;
  long long uVar7;
  long long uVar8;
  int iVar9;
  short sVar12;
  int bVar13;
  int iVar10;
  unsigned int uVar11;
  short sVar14;
  unsigned long long uVar15;
  long long lVar16;
  short sVar17;
  unsigned long long unaff_r26;
  unsigned int uVar18;
  int iVar19;
  int iVar20;
  int local_9c;
  char auStack_78 [96];
  int uStack_18;
  
  iVar9 = gPerPlayerArmyCount;
  puVar5 = puRam1011735c;
  piVar4 = piRam10117358;
  ppuVar6 = 0 /* TVect base */;
  uStack_18 = (int)unaff_r26;
  GetAIStateSnapshot();
  uVar18 = 0;
  iVar19 = 0;
  uVar11 = 0;
  if (0 < *(short *)(*puVar5 + 0x1602)) {
    do {
      uVar18 = uVar11;
      iVar20 = *puVar5 + iVar19 * 0x42;
      lVar2 = -(long long)
               *(char *)(*puVar5 + (*(unsigned int *)(*piVar4 + *(short *)(iVar20 + 0x1606) * 0xe0 +
                                             *(short *)(iVar20 + 0x1604) * 2) >> 0x18) + 0x711);
      lVar16 = lVar2 + 10;
      if ((lVar16 + (-(unsigned long long)(lVar16 == 0) - (lVar2 + 9)) & 0xff) == 0) {
        uVar18 = (unsigned int)(short)((short)uVar18 + 1);
      }
      sVar12 = (short)iVar19 + 1;
      iVar19 = (int)sVar12;
      uVar11 = uVar18;
    } while (sVar12 < *(short *)(*puVar5 + 0x1602));
  }
  sVar17 = 0;
  sVar12 = 0;
  bVar13 = true;
  bVar3 = false;
  uVar15 = 7;
  do {
    if (*(short *)(*puVar5 + (int)((uVar15 & 0xffffffff) << 1) + 0x138) != 0) {
      bVar13 = false;
    }
    iVar20 = (int)uVar15;
    iVar19 = *puVar5 + iVar20 * 2;
    if (*(short *)(iVar19 + 0xd0) == 0) {
      if (*(short *)(iVar19 + 0x138) == 0) {
        *(short *)(iVar19 + 0xd0) = 1;
        bVar3 = true;
        goto LAB_1003d1b4;
      }
LAB_1003d1cc:
      if (*(short *)(*puVar5 + (int)((uVar15 & 0xffffffff) << 1) + 0xd0) == 0) {
        sVar12 = sVar12 + 1;
        bVar3 = true;
        unaff_r26 = uVar15;
      }
      iVar19 = *puVar5 + (int)((uVar15 & 0xffffffff) << 1);
      if ((*(short *)(iVar19 + 0x138) != 0) && (*(short *)(iVar19 + 0xd0) == 1)) {
        sVar17 = sVar17 + 1;
      }
    }
    else {
LAB_1003d1b4:
      if (*(short *)(*puVar5 + (int)((uVar15 & 0xffffffff) << 1) + 0x138) != 0) goto LAB_1003d1cc;
    }
    uVar15 = (unsigned long long)(short)((short)uVar15 + -1);
  } while (iVar20 != 0);
  if (bVar13) {
    uVar7 = FUN_1005f678(0xd,1);
    uVar8 = FUN_1005f678(0xd,0);
    FUN_1003ced4(uVar8,uVar7);
    uVar7 = FUN_1005f678(0xd,3);
    uVar8 = FUN_1005f678(0xd,2);
    FUN_1003ced4(uVar8,uVar7);
  }
  if (*(short *)(*puVar5 + 0x15c) == 0) {
    if ((sVar12 == 0) && (bVar3)) {
      uVar7 = FUN_1005f678(0xe,1);
      uVar8 = FUN_1005f678(0xe,0);
      FUN_1003ced4(uVar8,uVar7);
    }
    if (*(short *)(*puVar5 + 0x15c) == 0) {
      if (((sVar12 == 1) && (sVar17 == 0)) &&
         ((int)(((int)uVar18 >> 1) + (unsigned int)((int)uVar18 < 0 && (uVar18 & 1) != 0)) <
          (int)*(short *)((int)((unaff_r26 & 0xffffffff) << 1) + iVar9))) {
        *(short *)(*puVar5 + 0x15c) = 1;
        FUN_10038c60(unaff_r26,7,unaff_r26,0,ZEXT48((*(int*)((char*)ppuVar6 - 0x685))) + 4);
      }
      if (*(short *)(*puVar5 + 0x15c) == 0) {
        if ((sVar12 == 0) && (sVar17 == 1)) {
          *(short *)(*puVar5 + 0x15c) = 1;
          uVar15 = 0;
          do {
            if (*(short *)(*puVar5 + (int)((uVar15 & 0xffffffff) << 1) + 0x138) != 0) break;
            sVar14 = (short)uVar15 + 1;
            uVar15 = (unsigned long long)sVar14;
          } while (sVar14 < 8);
          uVar7 = FUN_1005f678(0x10,0);
          DrawNumber(auStack_78,uVar7,
                       (unsigned long long)*puVar5 + ((uVar15 & 0x3fffffff) * 4 + uVar15 & 0x3fffffff) * 4);
          FUN_1003c938(auStack_78);
          *(short *)(*puVar5 + (int)((uVar15 & 0xffffffff) << 1) + 0xd0) = 0;
          FUN_10038c60(uVar15,7,uVar15,0,(unsigned long long)*(unsigned int *)(local_9c + -0x1a14) + 8);
        }
        if (((*(short *)(*puVar5 + 0x15c) == 0) && (sVar12 == 1)) &&
           ((0 < sVar17 && (*(short *)(*puVar5 + 0x15e) == 0)))) {
          iVar19 = 0;
          iVar20 = 0;
          do {
            iVar10 = *puVar5 + iVar20 * 2;
            if ((*(short *)(iVar10 + 0x138) != 0) && (*(short *)(iVar10 + 0xd0) == 1)) {
              sVar12 = *(short *)(iVar20 * 2 + iVar9);
              if (iVar19 < sVar12) {
                iVar19 = (int)sVar12;
              }
            }
            sVar12 = (short)iVar20 + 1;
            iVar20 = (int)sVar12;
          } while (sVar12 < 8);
          uVar1 = *(unsigned short *)(*puVar5 + 0x1602);
          uVar11 = (unsigned int)(short)uVar1;
          iVar9 = (int)*(short *)((int)((unaff_r26 & 0xffffffff) << 1) + iVar9);
          if (((int)(((int)uVar11 >> 1) + (unsigned int)((short)uVar1 < 0 && (uVar1 & 1) != 0)) < iVar9) &&
             ((int)(iVar19 + ((int)uVar11 >> 3) + (unsigned int)((int)uVar11 < 0 && (uVar11 & 7) != 0)) <
              iVar9)) {
            *(short *)(*puVar5 + 0x15e) = 1;
          }
        }
      }
    }
  }
  return;
}









































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1003d734 Size: 1268 bytes */
void FUN_1003d734(int *param_1)

{
  int iVar1;
  unsigned int uVar2;
  int *piVar3;
  char cVar6;
  int *piVar4;
  int *piVar5;
  short sVar7;
  long long uVar8;
  long long lVar9;
  int local_3c;
  
  piVar5 = piRam101176e0;
  piVar3 = piRam1011735c;
  if (param_1 != (int *)0x0) {
    cVar6 = FUN_10056534();
    piVar4 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308),0x68656c70);
    if (piVar4 != (int *)0x0) {
      ResourceRead_Dispatch((int)*(short *)(*piVar4 + 0x738) + (int)piVar4,0,1);
    }
    piVar4 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308),0x6d6f7665);
    if (piVar4 != (int *)0x0) {
      if ((*piVar5 == 0) || (**(short **)(local_3c + -0x1a10) <= **(short **)(local_3c + -0x1a04)))
      {
        uVar8 = 1;
      }
      else {
        uVar8 = 0;
      }
      ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x738),uVar8,1);
    }
    piVar4 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308),0x6e657874);
    if (piVar4 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x738),cVar6 == '\0',1);
    }
    piVar4 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308),0x6c656176);
    if (piVar4 != (int *)0x0) {
      if ((cVar6 == '\0') || (*piVar5 == 0)) {
        uVar8 = 1;
      }
      else {
        uVar8 = 0;
      }
      ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x738),uVar8,1);
    }
    piVar4 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308),0x67756172);
    if (piVar4 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x738),*piVar5 == 0,1);
    }
    piVar4 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308),0x64657365);
    if (piVar4 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x738),*piVar5 == 0,1);
    }
    piVar4 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308),0x70617468);
    if (piVar4 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x738),*piVar5 == 0,1);
    }
    piVar4 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308),0x64656c65);
    if (piVar4 != (int *)0x0) {
      iVar1 = *piVar5;
      if (((iVar1 == 0) || (*(short *)(iVar1 + 0x12) < 0)) || (*(short *)(iVar1 + 0x14) < 0)) {
        uVar8 = 1;
      }
      else {
        uVar8 = 0;
      }
      ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x738),uVar8,1);
    }
    piVar4 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308),0x63656e74);
    if (piVar4 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x738),*piVar5 == 0,1);
    }
    lVar9 = 0;
    do {
      piVar5 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308),
                                   lVar9 + 0x73637230);
      if (piVar5 != (int *)0x0) {
        ResourceRead_Dispatch((int)piVar5 + (int)*(short *)(*piVar5 + 0x738),0,1);
      }
      sVar7 = (short)lVar9 + 1;
      lVar9 = (long long)sVar7;
    } while (sVar7 < 8);
    piVar5 = (int *)ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x308),0x6469706c);
    if (piVar5 != (int *)0x0) {
      iVar1 = *piVar3;
      sVar7 = 0x3ef;
      if ((*(short *)(iVar1 + 0x11c) != 0) &&
         (uVar2 = *(unsigned int *)(iVar1 + *(short *)(iVar1 + 0x110) * 0x12 + 0x1582),
         (uVar2 >> 0x1e & 1) != 0)) {
        if ((int)uVar2 < 0) {
          sVar7 = 2;
        }
        else {
          sVar7 = 1;
        }
        sVar7 = sVar7 + 0x3ef;
      }
      if (*(short *)((int)piVar5 + 0xaa) != sVar7) {
        ResourceRead_Dispatch((int)piVar5 + (int)*(short *)(*piVar5 + 0x7a8),sVar7,1);
      }
      ResourceRead_Dispatch((int)piVar5 + (int)*(short *)(*piVar5 + 0x738),*(short *)(*piVar3 + 0x11c) == 0,1
                  );
    }
  }
  return;
}














































































































































































































































































































































/* Address: 0x10042a24 Size: 400 bytes */
int FUN_10042a24(short param_1,short param_2)

{
  char cVar1;
  int iVar2;
  int *piVar3;
  int *piVar4;
  int iVar5;
  int iVar6;
  int iVar7;
  short sStack0000001a;
  short sStack0000001e;
  
  piVar4 = piRam1011735c;
  piVar3 = piRam10117358;
  iVar2 = gTileWalkOffsets;
  iVar6 = 0;
  iVar7 = 0;
  sStack0000001a = param_1;
  if (*(char *)(*piRam1011735c +
                (*(unsigned int *)(*piRam10117358 + param_2 * 0xe0 + param_1 * 2 + -2) >> 0x18) + 0x711) ==
      '\n') {
    sStack0000001a = param_1 + -1;
  }
  sStack0000001e = param_2;
  if (*(char *)(*piRam1011735c +
                (*(unsigned int *)(*piRam10117358 + param_2 * 0xe0 + sStack0000001a * 2 + -0xe0) >> 0x18) +
               0x711) == '\n') {
    sStack0000001e = param_2 + -1;
  }
  while( true ) {
    if (*(short *)(iVar7 * 2 + iVar2) == -1) {
      return iVar6;
    }
    if (iVar6 != 0) break;
    iVar5 = ApplyDirectionOffset(((char*)0 + 0x0000001a),((char*)0 + 0x0000001e),*(short *)(iVar7 * 2 + iVar2));
    if (iVar5 == 0) {
      return 0;
    }
    cVar1 = *(char *)(*piVar4 + (*(unsigned int *)(*piVar3 + sStack0000001e * 0xe0 + sStack0000001a * 2) >>
                                0x18) + 0x711);
    if (((cVar1 == '\x01') || (cVar1 == '\x02')) || (cVar1 == '\x03')) {
      iVar6 = 1;
    }
    iVar7 = (int)(short)((short)iVar7 + 1);
  }
  return iVar6;
}


























































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10042bb4 Size: 372 bytes */
void FUN_10042bb4(short param_1,short param_2)

{
  int bVar1;
  int iVar2;
  int iVar3;
  int iVar4;
  int *piVar5;
  int *piVar6;
  int *piVar7;
  short sVar8;
  short sVar9;
  int iVar10;
  int iVar11;
  int iVar12;
  
  piVar7 = gCostMapPtr;
  piVar6 = piRam1011735c;
  piVar5 = piRam10117358;
  iVar4 = gZoCNeighborDXOffsets;
  iVar3 = gZoCNeighborDYOffsets;
  iVar2 = gStdMoveCostTable;
  iVar10 = 3;
  do {
    iVar12 = *piVar6 + param_1 * 0x42;
    sVar8 = *(short *)(iVar12 + 0x1604) + *(short *)(iVar10 * 2 + iVar4);
    iVar11 = (int)sVar8;
    sVar9 = *(short *)(iVar12 + 0x1606) + *(short *)(iVar10 * 2 + iVar3);
    iVar12 = (int)sVar9;
    *(char *)(*piVar7 + sVar8 * 0x9c + (int)sVar9) = (char)*(short *)(iVar2 + 0x14);
    if (param_2 == 0) {
      if ((*(unsigned int *)(*piVar5 + iVar12 * 0xe0 + iVar11 * 2) >> 0x10 & 0xf) ==
          (int)*(short *)(*piVar6 + 0x110)) {
        iVar12 = FUN_10042a24(iVar11,iVar12);
        if (iVar12 != 0) {
          iVar12 = *piVar7 + sVar8 * 0x9c;
          *(unsigned char *)(iVar12 + sVar9) = *(unsigned char *)(iVar12 + sVar9) | 0x18;
        }
      }
      else {
        *(char *)(*piVar7 + sVar8 * 0x9c + (int)sVar9) = 0;
      }
    }
    else {
      iVar12 = FUN_10042a24(iVar11,iVar12);
      if (iVar12 != 0) {
        iVar12 = *piVar7 + sVar8 * 0x9c;
        *(unsigned char *)(iVar12 + sVar9) = *(unsigned char *)(iVar12 + sVar9) | 0x18;
      }
    }
    iVar12 = *piVar7 + sVar8 * 0x9c;
    *(unsigned char *)(iVar12 + sVar9) = *(unsigned char *)(iVar12 + sVar9) | 0x80;
    bVar1 = iVar10 != 0;
    iVar10 = (int)(short)((short)iVar10 + -1);
  } while (bVar1);
  return;
}









































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10044110 Size: 888 bytes */
void FUN_10044110(short param_1,short param_2)

{
  short sVar1;
  short sVar2;
  int *puVar3;
  int *piVar4;
  int iVar5;
  long long lVar6;
  int *piVar7;
  int *piVar8;
  int *piVar9;
  int *piVar10;
  int *piVar11;
  int *puVar12;
  int *piVar13;
  short *psVar14;
  int *ppuVar15;
  char uVar17;
  int iVar16;
  unsigned long long uVar18;
  int iVar19;
  int iVar20;
  long long lVar21;
  unsigned long long uVar22;
  unsigned long long uVar23;
  int iVar24;
  int **local_5c;
  
  psVar14 = gLastArmyTypeForCostMap;
  piVar13 = gCostMapPtr;
  puVar12 = gDistGridDeref;
  piVar11 = gDistGridHandle;
  piVar10 = gCostMapHandle;
  piVar9 = piRam1011735c;
  piVar8 = piRam10117358;
  piVar7 = piRam10117354;
  iVar19 = gFlyingMoveCostTable;
  iVar16 = gStdMoveCostTable;
  ppuVar15 = 0 /* TVect base */;
  if (*gPathfindingInitialized == 0) {
    ppuVar15 = 0 /* TVect base */;
    FUN_100424e4();
  }
  if ((param_2 != 0) || (*psVar14 != param_1)) {
    *psVar14 = param_1;
    *(unsigned short *)(*(int*)((char*)ppuVar15 - 0x67b)) = (unsigned short)(param_1 == 0xe);
    puVar3 = (int *)*piVar11;
    if (puVar3 != (int *)0x0) {
      DetachResource(puVar3);
      ppuVar15 = local_5c;
    }
    *puVar12 = *puVar3;
    *(int **)(*(int*)((char*)ppuVar15 - 0xc1)) = puVar12;
    piVar4 = (int *)*piVar10;
    if (piVar4 != (int *)0x0) {
      DetachResource(piVar4);
    }
    *piVar13 = *piVar4;
    uVar22 = 0x6f;
    do {
      uVar23 = 0x9b;
      do {
        uVar18 = (unsigned long long)
                 *(char *)(*piVar9 + (*(unsigned int *)(*piVar8 + (int)(((uVar23 & 0x1fffffff) * 8 - uVar23
                                                                & 0xffffffff) << 5) +
                                               (int)((uVar22 & 0xffffffff) << 1)) >> 0x18) + 0x711);
        iVar20 = (int)uVar22;
        iVar24 = (int)uVar23;
        if (param_1 == 0xe) {
          if ((*(unsigned int *)(*piVar7 + (int)(((uVar23 & 0x1fffffff) * 8 - uVar23 & 0xffffffff) << 4) +
                        iVar20) >> 0x18 & 0x1f) == 0) {
            uVar17 = (char)*(short *)((int)((uVar18 & 0xffffffff) << 1) + iVar19);
          }
          else {
            uVar17 = 1;
          }
          *(char *)(*piVar13 + iVar20 * 0x9c + iVar24) = uVar17;
        }
        else {
          if ((*(unsigned int *)(*piVar7 + (int)(((uVar23 & 0x1fffffff) * 8 - uVar23 & 0xffffffff) << 4) +
                        iVar20) >> 0x18 & 0x1f) == 0) {
            uVar17 = (char)*(short *)((int)((uVar18 & 0xffffffff) << 1) + iVar16);
          }
          else {
            uVar17 = 1;
          }
          *(char *)(*piVar13 + iVar20 * 0x9c + iVar24) = uVar17;
        }
        if ((uVar18 - 1 & 0xffffffff) < 10) {
          return;
        }
        if ((*(unsigned int *)(*piVar8 + (int)(((uVar23 & 0x1fffffff) * 8 - uVar23 & 0xffffffff) << 5) +
                      (int)((uVar22 & 0xffffffff) << 1)) >> 0x17 & 1) != 0) {
          iVar5 = *piVar13 + iVar20 * 0x9c;
          *(unsigned char *)(iVar5 + iVar24) = *(unsigned char *)(iVar5 + iVar24) | 0x10;
        }
        uVar23 = (unsigned long long)(short)((short)uVar23 + -1);
      } while (iVar24 != 0);
      uVar22 = (unsigned long long)(short)((short)uVar22 + -1);
    } while (iVar20 != 0);
    sVar1 = *(short *)(*piVar9 + 0x1602);
    while (sVar1 != 0) {
      sVar1 = sVar1 + -1;
      iVar16 = *piVar9 + sVar1 * 0x42;
      iVar19 = (int)*(short *)(iVar16 + 0x1604);
      iVar16 = (int)*(short *)(iVar16 + 0x1606);
      lVar6 = -(long long)
               *(char *)(*piVar9 + (*(unsigned int *)(*piVar8 + iVar16 * 0xe0 + iVar19 * 2) >> 0x18) + 0x711
                        );
      lVar21 = lVar6 + 10;
      if (((lVar21 + (-(unsigned long long)(lVar21 == 0) - (lVar6 + 9)) & 0xff) != 0) &&
         (iVar16 = FUN_10042a24(iVar19,iVar16), iVar16 != 0)) {
        iVar16 = sVar1 * 0x42 + *piVar9;
        sVar2 = *(short *)(iVar16 + 0x1606);
        iVar19 = (int)sVar2;
        iVar16 = *(short *)(iVar16 + 0x1604) * 0x9c;
        *(unsigned char *)(*piVar13 + iVar16 + (int)sVar2) = *(unsigned char *)(*piVar13 + iVar16 + (int)sVar2) | 0x18
        ;
        iVar20 = iVar16 + *piVar13 + iVar19;
        *(unsigned char *)(iVar20 + 0x9c) = *(unsigned char *)(iVar20 + 0x9c) | 0x18;
        iVar20 = iVar16 + *piVar13 + iVar19;
        *(unsigned char *)(iVar20 + 1) = *(unsigned char *)(iVar20 + 1) | 0x18;
        iVar19 = iVar16 + *piVar13 + iVar19;
        *(unsigned char *)(iVar19 + 0x9d) = *(unsigned char *)(iVar19 + 0x9d) | 0x18;
      }
    }
    iVar16 = *piVar11;
    if (iVar16 != 0) {
      DisposeHandle_Thunk(iVar16);
    }
    iVar16 = *piVar10;
    if (iVar16 != 0) {
      DisposeHandle_Thunk(iVar16);
    }
  }
  return;
}
















































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100445a8 Size: 84 bytes */
void FUN_100445a8(short *param_1,int param_2)

{
  int *piVar1;
  
  piVar1 = piRam101176e0;
  *param_1 = *(short *)*piRam101176e0;
  param_1[1] = *(short *)(*piVar1 + 2);
  param_1[2] = *(short *)(*piVar1 + 0x12);
  param_1[3] = *(short *)(*piVar1 + 0x14);
  *(int *)(param_1 + 6) = param_2;
  param_1[5] = *gCurrentMoveMode;
  param_1[4] = *gCurrentPathFlags;
  return;
}



/* Address: 0x10045ac8 Size: 1092 bytes */
void FUN_10045ac8()

{
  int uVar1;
  unsigned int uVar3;
  char cVar4;
  int iVar5;
  int iVar6;
  int bVar7;
  char *puVar8;
  char *puVar9;
  int *puVar10;
  int iVar12;
  long long uVar11;
  int iVar13;
  short sVar14;
  int iVar15;
  int local_30c;
  char auStack_2e8 [72];
  char auStack_2a0 [256];
  char auStack_1a0 [72];
  char auStack_158 [256];
  char local_58 [7];
  char local_51;
  int uVar2;
  
  puVar10 = puRam101175f8;
  iVar13 = gSpriteTable;
  puVar9 = puRam10115ee0;
  puVar8 = puRam10115edc;
  iVar5 = *(int *)(*piRam1011734c + 0x88);
  ResourceRead_Dispatch((int)*(int **)(iVar5 + 0x148) + (int)*(short *)(**(int **)(iVar5 + 0x148) + 0x2b8));
  iVar15 = local_30c;
  FUN_100099cc();
  iVar15 = *(int *)(iVar15 + -0x19c8);
  uVar1 = *(int *)(iVar15 + 0x13c);
  uVar2 = *(int *)(iVar15 + 0x140);
  uVar3 = *(unsigned int *)(iVar15 + 0x144);
  *puVar10 = *(int *)(iVar15 + 0x138);
  puVar10[1] = uVar1;
  puVar10[2] = uVar2;
  puVar10[3] = uVar3 & 0xff000000;
  ResourceRead_Dispatch((int)*(int **)(iVar5 + 0x148) + (int)*(short *)(**(int **)(iVar5 + 0x148) + 0x2b8));
  iVar15 = local_30c;
  iVar12 = FUN_1005f36c(0x44415420,puVar10);
  bVar7 = false;
  if ((iVar12 != 0) && (uVar11 = FUN_10051f98(0x44415420,puVar10,1), (int)uVar11 != 0)) {
    StreamRead(uVar11,local_58,8);
    CloseStream(uVar11,1);
    local_51 = 0;
    iVar12 = 0;
    do {
      cVar4 = local_58[iVar12];
      if (((cVar4 == ' ') || (cVar4 == '\r')) || (cVar4 == '\t')) {
        local_58[iVar12] = '\0';
      }
      sVar14 = (short)iVar12 + 1;
      iVar12 = (int)sVar14;
    } while (sVar14 < 8);
    iVar12 = FUN_100001e0(local_58,(unsigned long long)*(unsigned int *)(iVar15 + -0x19c8) + 0x148);
    iVar15 = local_30c;
    if (iVar12 != 0) {
      bVar7 = true;
    }
  }
  if (bVar7) {
    iVar15 = 0;
    do {
      iVar12 = iVar15 * 0x14 + iVar13;
      *(unsigned int *)(iVar12 + 0x140) = *(unsigned int *)(iVar12 + 0x140) | 0xc0000000;
      sVar14 = (short)iVar15 + 1;
      iVar15 = (int)sVar14;
    } while (sVar14 < 9);
    *(unsigned int *)(iVar13 + 0x348) = *(unsigned int *)(iVar13 + 0x348) | 0xc0000000;
    BlockMoveData_Thunk(puVar8,local_58);
    if (*(int *)(iVar5 + 0x150) == 0) {
      BuildPascalString(auStack_158,local_58);
      FUN_10027a58(2,auStack_158,0,auStack_1a0);
      FUN_1007edac(iVar5,auStack_1a0);
      ResourceRead_Dispatch((int)*(int **)(iVar5 + 0x150) +
                   (int)*(short *)(**(int **)(iVar5 + 0x150) + 0x118));
    }
  }
  else {
    iVar12 = 0;
    do {
      iVar6 = iVar12 * 0x14 + iVar13;
      *(unsigned int *)(iVar6 + 0x140) = *(unsigned int *)(iVar6 + 0x140) & 0x3fffffff | 0x80000000;
      sVar14 = (short)iVar12 + 1;
      iVar12 = (int)sVar14;
    } while (sVar14 < 9);
    *(unsigned int *)(iVar13 + 0x348) = *(unsigned int *)(iVar13 + 0x348) & 0x3fffffff | 0x80000000;
    iVar15 = *(int *)(iVar15 + -0x19c8);
    *puVar8 = *(char *)(iVar15 + 0x14c);
    DrawNumber(puVar10,iVar15 + 0x150,local_58);
  }
  iVar15 = *(int *)(local_30c + -0x19c8);
  uVar1 = *(int *)(iVar15 + 0x160);
  uVar2 = *(int *)(iVar15 + 0x164);
  uVar3 = *(unsigned int *)(iVar15 + 0x168);
  *puVar10 = *(int *)(iVar15 + 0x15c);
  puVar10[1] = uVar1;
  puVar10[2] = uVar2;
  puVar10[3] = uVar3 & 0xff000000;
  ResourceRead_Dispatch((int)*(int **)(iVar5 + 0x148) + (int)*(short *)(**(int **)(iVar5 + 0x148) + 0x2b8));
  iVar15 = local_30c;
  iVar13 = FUN_1005f36c(0x44415420,puVar10);
  bVar7 = false;
  if ((iVar13 != 0) && (uVar11 = FUN_10051f98(0x44415420,puVar10,1), (int)uVar11 != 0)) {
    StreamRead(uVar11,local_58,8);
    CloseStream(uVar11,1);
    local_51 = 0;
    iVar13 = 0;
    do {
      cVar4 = local_58[iVar13];
      if (((cVar4 == ' ') || (cVar4 == '\r')) || (cVar4 == '\t')) {
        local_58[iVar13] = '\0';
      }
      sVar14 = (short)iVar13 + 1;
      iVar13 = (int)sVar14;
    } while (sVar14 < 8);
    iVar13 = FUN_100001e0(local_58,(unsigned long long)*(unsigned int *)(iVar15 + -0x19c8) + 0x16c);
    iVar15 = local_30c;
    if (iVar13 != 0) {
      DrawNumber(puVar10,(unsigned long long)*(unsigned int *)(local_30c + -0x19c8) + 0x170,local_58);
      bVar7 = true;
    }
  }
  if (bVar7) {
    BlockMoveData_Thunk(puVar9,local_58);
    if (*(int *)(iVar5 + 0x158) == 0) {
      BuildPascalString(auStack_2a0,local_58);
      FUN_10027a58(4,auStack_2a0,0,auStack_2e8);
      FUN_1007ef9c(iVar5,auStack_2e8);
      ResourceRead_Dispatch((int)*(int **)(iVar5 + 0x158) +
                   (int)*(short *)(**(int **)(iVar5 + 0x158) + 0x118));
    }
  }
  else {
    *puVar9 = *(char *)(*(int *)(iVar15 + -0x19c8) + 0x180);
  }
  ResourceRead_Dispatch((int)*(int **)(iVar5 + 0x148) + (int)*(short *)(**(int **)(iVar5 + 0x148) + 0x2b8));
  FUN_100453f0();
  return;
}

















































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100472f4 Size: 1792 bytes */
void FUN_100472f4(long long param_1,long long param_2,long long param_3,short param_4)

{
  short sVar1;
  short *psVar2;
  int *piVar3;
  int *puVar4;
  long long uVar5;
  int *piVar8;
  int iVar9;
  int *piVar10;
  long long uVar6;
  long long lVar7;
  int local_724;
  char auStack_700 [256];
  char auStack_600 [256];
  char auStack_500 [256];
  char auStack_400 [256];
  char auStack_300 [256];
  char auStack_200 [80];
  char auStack_1b0 [80];
  char auStack_160 [256];
  short local_60;
  int local_5c;
  int local_58;
  int local_54;
  int local_50;
  
  puVar4 = puRam10117370;
  piVar3 = piRam1011735c;
  psVar2 = psRam10115f00;
  sVar1 = *(short *)(*piRam1011735c + 0x110);
  *puRam10115efc = 1;
  *psRam10115ef8 = param_4;
  uVar5 = FUN_1005f678(0x43,0xffffffffffffffff);
  DrawNumber(auStack_1b0,uVar5,param_1);
  uVar5 = FUN_1005f678(0x44,0xffffffffffffffff);
  DrawNumber(auStack_200,uVar5,param_2);
  piVar8 = (int *)ResourceRead_Dispatch(**(int **)(local_724 + -0x1e4) +
                               (int)*(short *)(*(int *)**(int **)(local_724 + -0x1e4) + 200),0xed8,
                               *(int *)(**(int **)(local_724 + -0x554) + 0x88));
  FUN_100013f8(8,0);
  local_60 = 0;
  local_5c = 0;
  local_54 = 0;
  local_50 = 0;
  local_58 = *puVar4;
  *puVar4 = auStack_160;
  iVar9 = TrySetjmp(auStack_160);
  if (iVar9 == 0) {
    piVar10 = (int *)ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x308),0x68656164);
    if (piVar10 != (int *)0x0) {
      uVar5 = FUN_1005f678(0x42,0);
      BuildPascalString(auStack_300,uVar5);
      ResourceRead_Dispatch((int)*(short *)(*piVar10 + 0x7c8) + (int)piVar10,auStack_300,0);
    }
    piVar10 = (int *)ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x308),0x73747231);
    if (piVar10 != (int *)0x0) {
      BuildPascalString(auStack_400,auStack_1b0);
      ResourceRead_Dispatch((int)*(short *)(*piVar10 + 0x7c8) + (int)piVar10,auStack_400,0);
    }
    piVar10 = (int *)ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x308),0x73747232);
    if (piVar10 != (int *)0x0) {
      BuildPascalString(auStack_500,auStack_200);
      ResourceRead_Dispatch((int)*(short *)(*piVar10 + 0x7c8) + (int)piVar10,auStack_500,0);
    }
    piVar10 = (int *)ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x308),0x73747233);
    if (piVar10 != (int *)0x0) {
      uVar5 = FUN_1005f678(0x42,1);
      BuildPascalString(auStack_600,uVar5);
      ResourceRead_Dispatch((int)piVar10 + (int)*(short *)(*piVar10 + 0x7c8),auStack_600,0);
    }
    piVar10 = (int *)ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x308),0x73747234);
    if (piVar10 != (int *)0x0) {
      uVar5 = FUN_1005f678(0x42,2);
      BuildPascalString(auStack_700,uVar5);
      ResourceRead_Dispatch((int)piVar10 + (int)*(short *)(*piVar10 + 0x7c8),auStack_700,0);
    }
    uVar5 = 0x6f636375;
    if (*(short *)(*piVar3 + 0x11e) != 0) {
      iVar9 = (sVar1 * 4 - (int)sVar1) * 4 + *piVar3;
      if (*(short *)(iVar9 + 0x1142) != 0) {
        if (**(int **)(local_724 + -0x3f0) ==
            **(int **)(local_724 + -0x540) + *(short *)(iVar9 + 0x1146) * 0x16) {
          if ((*(short *)(iVar9 + 0x1144) == 5) && (*(short *)(iVar9 + 0x1148) == param_4)) {
            uVar5 = 0x72617a65;
          }
          else if (*(short *)(*piVar3 + sVar1 * 0xc + 0x1144) == 6) {
            iVar9 = CheckQuestCondition(param_4);
            if (iVar9 == 0) {
              iVar9 = FUN_1004645c(param_4);
              if (iVar9 != 0) {
                uVar5 = 0x70696c6c;
              }
            }
            else {
              uVar5 = 0x7361636b;
            }
          }
        }
      }
    }
    uVar6 = ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x7f8));
    if ((int)uVar6 != 0) {
      FUN_10078c94(uVar6,uVar5,0);
    }
    piVar10 = (int *)ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x308),0x70696c6c);
    if (piVar10 != (int *)0x0) {
      iVar9 = *piVar10;
      lVar7 = FUN_1004645c(param_4);
      ResourceRead_Dispatch((int)piVar10 + (int)*(short *)(iVar9 + 0x738),lVar7 == 0,0);
    }
    piVar10 = (int *)ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x308),0x7361636b);
    if (piVar10 != (int *)0x0) {
      iVar9 = *piVar10;
      lVar7 = CheckQuestCondition(param_4);
      ResourceRead_Dispatch((int)piVar10 + (int)*(short *)(iVar9 + 0x738),lVar7 == 0,0);
    }
    piVar10 = (int *)ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x308),0x72617a65);
    if (piVar10 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar10 + (int)*(short *)(*piVar10 + 0x738),*(short *)(*piVar3 + 0x114) == 0
                   ,0);
    }
    if ((*(short *)(*piVar3 + 0x12e) != 0) && (*psVar2 == 0)) {
      *psVar2 = 1;
      FUN_1005f6b0(0x19,10);
      FUN_10040040();
    }
    iVar9 = ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x810));
    ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x328));
    if (iVar9 == 0x6f636375) {
      FUN_1004653c();
    }
    else if (iVar9 == 0x70696c6c) {
      FUN_10046d7c();
    }
    else if (iVar9 == 0x72617a65) {
      FUN_10047190();
    }
    else if (iVar9 == 0x7361636b) {
      FUN_1004702c();
    }
    ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x738));
    *puVar4 = local_58;
  }
  else {
    if (piVar8 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x738));
    }
    LockHandleRange(local_60,local_5c);
  }
  return;
}









































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10047de8 Size: 320 bytes */
void FUN_10047de8(short param_1,int param_2,short *param_3)

{
  int iVar1;
  int iVar2;
  int *piVar3;
  short sVar4;
  int iVar5;
  
  piVar3 = piRam1011735c;
  *param_3 = 0;
  if ((param_1 == -2) || (99 < param_1)) {
    iVar5 = 0;
    if (0 < *(short *)(*piVar3 + 0x1602)) {
      do {
        iVar1 = *piVar3 + iVar5 * 0x42;
        if (((*(short *)(iVar1 + 0x1636) != 0) &&
            ((int)*(char *)(iVar1 + 0x1619) == (int)*(short *)(*piVar3 + 0x110))) &&
           (*(short *)(iVar1 + 0x1638) == -2)) {
          *(short *)(param_2 + *param_3 * 2) = (short)iVar5;
          *param_3 = *param_3 + 1;
        }
        sVar4 = (short)iVar5 + 1;
        iVar5 = (int)sVar4;
      } while (sVar4 < *(short *)(*piVar3 + 0x1602));
      return;
    }
  }
  else {
    iVar5 = 0;
    if (0 < *(short *)(*piVar3 + 0x1602)) {
      do {
        iVar1 = *piVar3 + iVar5 * 0x42;
        if (((*(short *)(iVar1 + 0x1636) != 0) &&
            (iVar2 = param_1 * 0x42 + *piVar3,
            *(short *)(iVar1 + 0x1638) == *(short *)(iVar2 + 0x1604))) &&
           (*(short *)(iVar1 + 0x163a) == *(short *)(iVar2 + 0x1606))) {
          *(short *)(param_2 + *param_3 * 2) = (short)iVar5;
          *param_3 = *param_3 + 1;
        }
        sVar4 = (short)iVar5 + 1;
        iVar5 = (int)sVar4;
      } while (sVar4 < *(short *)(*piVar3 + 0x1602));
    }
  }
  return;
}



/* Address: 0x1004e384 Size: 496 bytes */







/* Address: stubbed - callers disagree on arg count */
long long ProcessQuestEvent() { return 0; }

/* Address: 0x1004f664 Size: 160 bytes */
void FUN_1004f664()

{
  int iVar1;
  int iVar2;
  int *piVar3;
  int *ppuVar4;
  short sVar5;
  
  piVar3 = piRam1011735c;
  ppuVar4 = 0 /* TVect base */;
  sVar5 = RandomRange(1,0x19,0);
  iVar1 = *piVar3;
  iVar2 = *piVar3;
  *(short *)(iVar2 + *(short *)(iVar2 + 0x110) * 2 + 0x1122) =
       *(short *)(iVar1 + *(short *)(iVar1 + 0x110) * 2 + 0x1122) + sVar5 + 100;
  FUN_1004f620(*(short *)(*(int*)((char*)ppuVar4 - 0xb0)),*(short *)(*(int*)((char*)ppuVar4 - 0xaf)),10);
  GetAIStateSnapshot();
  FUN_1002bbd4();
  FUN_1002bcd8();
  return;
}




































































































































































































































































































































































































































































































/* Address: 0x10050ffc Size: 952 bytes */
void FUN_10050ffc(long long param_1,long long param_2,long long param_3,long long param_4,
                 long long param_5,int param_6,int param_7)

{
  int *puVar1;
  int *piVar2;
  int iVar3;
  int *piVar4;
  char auStack_660 [256];
  char auStack_560 [256];
  char auStack_460 [256];
  char auStack_360 [256];
  char auStack_260 [260];
  char auStack_15c [256];
  short local_5c;
  int local_58;
  int local_54;
  int local_50;
  int local_4c;
  
  puVar1 = puRam10117370;
  piVar2 = (int *)ResourceRead_Dispatch(*piRam101176bc + (int)*(short *)(*(int *)*piRam101176bc + 200),0xd52,
                               *(int *)(*piRam1011734c + 0x88));
  FocusObject();
  local_5c = 0;
  local_58 = 0;
  local_50 = 0;
  local_4c = 0;
  local_54 = *puVar1;
  *puVar1 = auStack_15c;
  iVar3 = TrySetjmp(auStack_15c);
  if (iVar3 == 0) {
    piVar4 = (int *)ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x308),0x68656164);
    if (piVar4 != (int *)0x0) {
      BuildPascalString(auStack_260,param_1);
      ResourceRead_Dispatch((int)*(short *)(*piVar4 + 0x7c8) + (int)piVar4,auStack_260,0);
    }
    piVar4 = (int *)ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x308),0x73747231);
    if (piVar4 != (int *)0x0) {
      BuildPascalString(auStack_360,param_2);
      ResourceRead_Dispatch((int)*(short *)(*piVar4 + 0x7c8) + (int)piVar4,auStack_360,0);
    }
    piVar4 = (int *)ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x308),0x73747232);
    if (piVar4 != (int *)0x0) {
      BuildPascalString(auStack_460,param_3);
      ResourceRead_Dispatch((int)*(short *)(*piVar4 + 0x7c8) + (int)piVar4,auStack_460,0);
    }
    piVar4 = (int *)ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x308),0x73747233);
    if (piVar4 != (int *)0x0) {
      BuildPascalString(auStack_560,param_4);
      ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x7c8),auStack_560,0);
    }
    piVar4 = (int *)ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x308),0x73747234);
    if (piVar4 != (int *)0x0) {
      BuildPascalString(auStack_660,param_5);
      ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x7c8),auStack_660,0);
    }
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 800));
    iVar3 = ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x810));
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x328));
    if (iVar3 == 0x6f6b2020) {
      if (param_6 != 0) {
        ResourceRead_Dispatch();
      }
    }
    else if (param_7 != 0) {
      ResourceRead_Dispatch();
    }
    ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x738));
    *puVar1 = local_54;
  }
  else {
    if (piVar2 != (int *)0x0) {
      ResourceRead_Dispatch((int)piVar2 + (int)*(short *)(*piVar2 + 0x738));
    }
    LockHandleRange(local_5c,local_58);
  }
  return;
}

/* Address: 0x100517f8 Size: 1384 bytes */
void FUN_100517f8(short param_1,short param_2,short param_3,short param_4)

{
  unsigned int uVar1;
  int iVar2;
  int *piVar3;
  unsigned int *puVar4;
  unsigned int *puVar5;
  int *ppuVar6;
  unsigned int *puVar7;
  unsigned int *puVar8;
  int *piVar9;
  short uVar10;
  short sVar11;
  short *puVar12;
  int **local_ac;
  char auStack_88 [72];
  int local_40;
  int local_3c;
  
  puVar5 = puRam101175f4;
  puVar4 = puRam101175f0;
  piVar3 = piRam1011734c;
  ppuVar6 = 0 /* TVect base */;
  local_40 = 0xca;
  local_3c = 0x3c;
  ProgressCreate(*gProgressBar,&local_40,0x3f3);
  puVar7 = (unsigned int *)AllocateHandle(0x5dd6);
  if (puVar7 != (unsigned int *)0x0) {
    DetachResource(puVar7);
    ppuVar6 = local_ac;
  }
  uVar1 = *puVar7;
  *puVar5 = uVar1;
  puVar8 = (unsigned int *)AllocateHandle(0x478a,uVar1,0x478a,0);
  if (puVar8 != (unsigned int *)0x0) {
    DetachResource(puVar8);
    ppuVar6 = local_ac;
  }
  *puVar4 = *puVar8;
  DisposeGeneric(*(short *)(*(int*)((char*)ppuVar6 - 0x2d)));
  piVar9 = (int *)FUN_10051f98(0x44415420,ZEXT48(local_ac[-0x653]) + 0x24,1);
  if (piVar9 == (int *)0x0) {
    FocusObject(0);
  }
  puVar12 = (short *)*puVar5;
  sVar11 = 0;
  do {
    ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0x100));
    uVar10 = ByteSwap16();
    *puVar12 = uVar10;
    sVar11 = sVar11 + 1;
    puVar12 = puVar12 + 1;
  } while (sVar11 < 0x132);
  ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0xe0),(unsigned long long)*puVar5 + 0x264,0x200);
  puVar12 = (short *)(*puVar5 + 0x464);
  sVar11 = 0;
  do {
    ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0x100));
    uVar10 = ByteSwap16();
    *puVar12 = uVar10;
    sVar11 = sVar11 + 1;
    puVar12 = puVar12 + 1;
  } while (sVar11 < 0x180);
  ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0xe0),(unsigned long long)*puVar5 + 0x764,800);
  ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0x100));
  uVar10 = ByteSwap16();
  *(short *)(*puVar5 + 0xa84) = uVar10;
  ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0xe0),(unsigned long long)*puVar5 + 0xa86,0x44c);
  ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0x100));
  uVar10 = ByteSwap16();
  *(short *)(*puVar5 + 0xed2) = uVar10;
  ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0xe0),(unsigned long long)*puVar5 + 0xed4,400);
  ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0x100));
  uVar10 = ByteSwap16();
  *(short *)(*puVar5 + 0x1064) = uVar10;
  ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0x100));
  uVar10 = ByteSwap16();
  *(short *)(*puVar5 + 0x1066) = uVar10;
  ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0xe0),(unsigned long long)*puVar5 + 0x1068,0x848);
  ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0x100));
  uVar10 = ByteSwap16();
  *(short *)(*puVar5 + 0x18b0) = uVar10;
  ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0xe0),(unsigned long long)*puVar5 + 0x18b2,0x4b0);
  puVar12 = (short *)(*puVar5 + 0x1d62);
  sVar11 = 0;
  do {
    ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0x100));
    uVar10 = ByteSwap16();
    *puVar12 = uVar10;
    sVar11 = sVar11 + 1;
    puVar12 = puVar12 + 1;
  } while (sVar11 < 0xfa);
  ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0xe0),(unsigned long long)*puVar5 + 0x1f56,16000);
  puVar12 = (short *)*puVar4;
  sVar11 = 0;
  do {
    ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0x100));
    uVar10 = ByteSwap16();
    *puVar12 = uVar10;
    sVar11 = sVar11 + 1;
    puVar12 = puVar12 + 1;
  } while (sVar11 < 0x1a5);
  ResourceRead_Dispatch((int)piVar9 + (int)*(short *)(*piVar9 + 0xe0),(unsigned long long)*puVar4 + 0x34a,0x4440);
  CloseStream(piVar9,1);
  uVar1 = *puVar5;
  *(short *)(uVar1 + 0x2a) = *(short *)(uVar1 + 0x2a) + *(short *)(uVar1 + param_3 * 2 + 0x6e);
  uVar1 = *puVar5;
  *(short *)(uVar1 + 0x3c) = *(short *)(uVar1 + 0x3c) + *(short *)(uVar1 + param_4 * 2 + 0x60);
  uVar1 = *puVar5;
  *(short *)(uVar1 + 0x36) = *(short *)(uVar1 + 0x36) + *(short *)(uVar1 + param_2 * 2 + 0x44);
  uVar1 = *puVar5;
  *(short *)(uVar1 + 0x34) = *(short *)(uVar1 + 0x34) + *(short *)(param_2 * 2 + uVar1 + 0x44);
  uVar1 = *puVar5;
  *(short *)(uVar1 + 0x38) = *(short *)(uVar1 + 0x38) + *(short *)(uVar1 + param_1 * 2 + 0x52);
  uVar1 = *puVar5;
  *(short *)(uVar1 + 0x3a) = *(short *)(uVar1 + 0x3a) + *(short *)(param_1 * 2 + uVar1 + 0x52);
  GenerateRandomMap();
  if (puVar7 != (unsigned int *)0x0) {
    DisposeHandle_Thunk(puVar7);
  }
  ReleaseHandle_Mapgen(puVar7);
  if (puVar8 != (unsigned int *)0x0) {
    DisposeHandle_Thunk(puVar8);
  }
  ReleaseHandle_Mapgen(puVar8);
  iVar2 = *(int *)(*piVar3 + 0x88);
  ResourceRead_Dispatch((int)*(int **)(iVar2 + 0x148) + (int)*(short *)(**(int **)(iVar2 + 0x148) + 0x178),
               auStack_88);
  FUN_1007ec7c(iVar2,auStack_88);
  ResourceRead_Dispatch((int)*(int **)(iVar2 + 0x148) + (int)*(short *)(**(int **)(iVar2 + 0x148) + 0x118));
  return;
}































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1005310c Size: 408 bytes */
int FUN_1005310c(int param_1)

{
  short *psVar1;
  int iVar2;
  int *piVar3;
  int *piVar4;
  int *piVar5;
  int iVar6;
  short sVar7;
  int iVar8;
  short sVar9;
  short sVar10;
  
  iVar6 = iRam1011773c;
  psVar1 = psRam1011760c;
  piVar5 = piRam101174b0;
  piVar4 = piRam10117360;
  piVar3 = piRam1011735c;
  FUN_1003aeb0(2,0,0,*piRam101174b0);
  iVar8 = 0;
  sVar10 = 0;
  if (0 < *psVar1) {
    do {
      iVar2 = *piVar3 + *(short *)(iVar8 * 2 + iVar6) * 0x1e;
      if (*(char *)(iVar2 + 0xd26) == '\x01') {
        sVar10 = sVar10 + *(char *)(iVar2 + 0xd27);
      }
      sVar7 = (short)iVar8 + 1;
      iVar8 = (int)sVar7;
    } while (sVar7 < *psVar1);
  }
  sVar7 = *(short *)(*piVar3 + 0x182);
  sVar9 = 0;
  while (sVar7 != 0) {
    sVar7 = sVar7 + -1;
    psVar1 = (short *)*piVar5;
    if ((*(short *)(*piVar4 + sVar7 * 0x16) == *psVar1) &&
       (*(short *)(*piVar4 + sVar7 * 0x16 + 2) == psVar1[1])) {
      sVar9 = sVar9 + 1;
    }
  }
  sVar7 = RandomRange(1,100,0);
  return (short)(sVar9 * 3 +
                 ((*(char *)(*piVar5 + 8) + sVar10) -
                 *(short *)(*piVar3 + *(char *)(param_1 + 0x1a) * 2 + 0x1046)) * 5 + 0x5a) < sVar7;
}



































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10053838 Size: 432 bytes */
short * FUN_10053838(short param_1,short param_2,short param_3)

{
  short sVar1;
  int iVar2;
  int iVar3;
  int iVar4;
  short sVar5;
  int bVar6;
  int *piVar7;
  int *piVar8;
  short sVar10;
  short sVar11;
  short *puVar9;
  short sVar12;
  unsigned long long uVar13;
  unsigned long long uVar14;
  unsigned long long uVar15;
  unsigned long long uVar16;
  unsigned long long uVar17;
  unsigned long long uVar18;
  char auStack_70 [23];
  char uStack_59;
  short local_54;
  char uStack_53;
  short local_30 [24];
  
  piVar8 = piRam1011735c;
  piVar7 = piRam10117358;
  GetUnitStats(param_1,auStack_70);
  sVar12 = 10;
  bVar6 = false;
  uVar13 = (long long)param_2;
  uVar15 = (long long)param_3;
  uVar17 = (long long)param_3;
  uVar18 = (long long)param_2;
  do {
    uVar14 = uVar18;
    uVar16 = uVar17;
    FUN_1002122c(uVar14,uVar16,local_30);
    if ((local_30[0] < 8) &&
       (((*(unsigned int *)(*piVar7 + (int)(((uVar16 & 0x1fffffff) * 8 - uVar16 & 0xffffffff) << 5) +
                   (int)((uVar14 & 0xffffffff) << 1)) >> 0x10 & 0xf) ==
         (int)*(short *)(*piVar8 + 0x110) || (local_30[0] == 0)))) {
      bVar6 = true;
      uVar17 = uVar16;
      uVar18 = uVar14;
    }
    else {
      sVar10 = RandomRange(1,3,0xfffffffffffffffe);
      sVar1 = (short)uVar14;
      sVar11 = RandomRange(1,3,0xfffffffffffffffe);
      sVar5 = (short)uVar16;
      sVar12 = sVar12 + -1;
      uVar14 = uVar13;
      uVar16 = uVar15;
      uVar17 = (long long)(short)(sVar5 + sVar11);
      uVar18 = (long long)(short)(sVar1 + sVar10);
      if (sVar12 == 0) {
        return (short *)0x0;
      }
    }
    uVar13 = uVar14;
    uVar15 = uVar16;
  } while (!bVar6);
  puVar9 = (short *)FUN_10021434();
  if (puVar9 != (short *)0x0) {
    *puVar9 = (short)uVar14;
    puVar9[1] = (short)uVar16;
    *(char *)(puVar9 + 2) = (char)param_1;
    *(char *)((int)puVar9 + 5) = (char)*(short *)(*piVar8 + 0x110);
    *(char *)(puVar9 + 3) = uStack_53;
    *(char *)((int)puVar9 + 7) = uStack_53;
    *(char *)(puVar9 + 4) = uStack_59;
    *(char *)((int)puVar9 + 9) = 0;
    *(char *)(puVar9 + 5) = 0;
    *(char *)((int)puVar9 + 0xb) = 0;
    iVar4 = (int)(((uVar16 & 0x1fffffff) * 8 - uVar16 & 0x7ffffff) << 5);
    iVar2 = *piVar7 + iVar4;
    iVar3 = (int)((uVar14 & 0x7fffffff) << 1);
    *(unsigned int *)(iVar2 + iVar3) = *(unsigned int *)(iVar2 + iVar3) | 0x100000;
    iVar4 = iVar4 + *piVar7;
    *(unsigned int *)(iVar4 + iVar3) =
         ((int)*(short *)(*piVar8 + 0x110) & 0xfU) << 0x10 | *(unsigned int *)(iVar4 + iVar3) & 0xfff0ffff;
  }
  return puVar9;
}

/* Address: 0x100539e8 Size: 2708 bytes */
void FUN_100539e8(int param_1,short param_2,short param_3)

{
  char uVar1;
  short *puVar2;
  int *piVar3;
  int iVar4;
  short *psVar5;
  int *piVar6;
  unsigned int *puVar7;
  unsigned int *puVar8;
  int uVar9;
  int *piVar10;
  int *ppuVar12;
  unsigned long long uVar11;
  int iVar13;
  long long uVar14;
  int iVar17;
  long long lVar15;
  long long uVar16;
  short *psVar18;
  char cVar19;
  int bVar20;
  short sVar21;
  int iVar22;
  unsigned long long uVar23;
  unsigned long long uVar24;
  int **local_6c;
  short local_48 [36];
  
  iVar22 = iRam1011773c;
  piVar10 = piRam101176e0;
  psVar18 = psRam1011760c;
  uVar9 = uRam101175f8;
  puVar8 = puRam101174b0;
  puVar7 = puRam1011735c;
  piVar6 = piRam10117358;
  piVar3 = piRam10115f64;
  psVar5 = gAutoModeFlag;
  ppuVar12 = 0 /* TVect base */;
  uVar24 = (unsigned long long)param_3;
  uVar23 = (unsigned long long)param_2;
  if ((*gAutoModeFlag == 0) &&
     (*(short *)(*puRam1011735c + *(short *)(*puRam1011735c + 0x110) * 2 + 0xd0) != 1)) {
    bVar20 = false;
  }
  else {
    bVar20 = true;
  }
  uVar14 = LookupRuinAtPos(uVar23,uVar24);
  FUN_1002c85c();
  if (!bVar20) {
    FUN_10030c3c(*(short *)*puVar8,((short *)*puVar8)[1],0);
    FUN_10052eb4(uVar14);
    PlayUISound(0);
  }
  FUN_1003357c(*puVar8,3);
  if (*(char *)(param_1 + 0x18) != '\x05') {
    if (*(char *)(param_1 + 0x1a) == '\0') {
      if (!bVar20) {
        FUN_1005f678(0x33,0);
        FUN_10052f6c();
        ResourceRead_Dispatch(*piVar3 + (int)*(short *)(*(int *)*piVar3 + 0x810));
        ppuVar12 = local_6c;
      }
    }
    else {
      if (!bVar20) {
        cVar19 = *(char *)(*puVar7 + *(char *)(param_1 + 0x1a) * 0x10 + 0xfa6);
        if (('@' < cVar19) && (cVar19 < '[')) {
          cVar19 = cVar19 + ' ';
        }
        if ((((cVar19 == 'a') || (cVar19 == 'e')) || (cVar19 == 'o')) ||
           ((cVar19 == 'i' || (cVar19 == 'u')))) {
          uVar14 = FUN_1005f678(0x34,3);
          DrawNumber(uVar9,uVar14,
                       (unsigned long long)*puVar7 +
                       (((long long)*(char *)(*puVar8 + 10) & 0x3fffffffU) * 4 +
                        (long long)*(char *)(*puVar8 + 10) & 0x3fffffff) * 4 + 0x224,
                       (unsigned long long)*puVar7 +
                       ((long long)*(char *)(param_1 + 0x1a) & 0xfffffffU) * 0x10 + 0xfa6);
        }
        else {
          uVar14 = FUN_1005f678(0x34,0);
          DrawNumber(uVar9,uVar14,
                       (unsigned long long)*puVar7 +
                       (((long long)*(char *)(*puVar8 + 10) & 0x3fffffffU) * 4 +
                        (long long)*(char *)(*puVar8 + 10) & 0x3fffffff) * 4 + 0x224,
                       (unsigned long long)*puVar7 +
                       ((long long)*(char *)(param_1 + 0x1a) & 0xfffffffU) * 0x10 + 0xfa6);
        }
        FUN_10052f6c(uVar9);
        ResourceRead_Dispatch(*piVar3 + (int)*(short *)(*(int *)*piVar3 + 0x810));
        ppuVar12 = local_6c;
        PlayUISound(1);
      }
      iVar17 = FUN_1005310c(param_1);
      if (iVar17 != 0) {
        FUN_10038c60(*(short *)(*puVar7 + 0x110),1,0xfffffffffffffffe,0,
                     *puVar7 + *(char *)(*puVar8 + 10) * 0x14 + 0x224);
        uVar23 = (unsigned long long)*(short *)(*puVar7 + 0x110);
        iVar17 = *puVar7 + (int)(((uVar23 & 0x3fffffff) * 4 + uVar23 & 0xffffffff) << 4) +
                 (int)(((uVar23 & 0x3fffffff) * 4 + uVar23 & 0xffffffff) << 1);
        *(short *)(iVar17 + 0x11a6) = *(short *)(iVar17 + 0x11a6) + 1;
        FUN_1003aeb0(2,0,0,*puVar8);
        iVar17 = 0;
        if (0 < *psVar18) {
          do {
            if (*(short *)(iVar17 * 2 + iVar22) < 8) {
              uVar23 = (unsigned long long)*(short *)(*puVar7 + 0x110);
              iVar4 = *puVar7 + (int)(((uVar23 & 0x3fffffff) * 4 + uVar23 & 0xffffffff) << 4) +
                      (int)(((uVar23 & 0x3fffffff) * 4 + uVar23 & 0xffffffff) << 1);
              *(short *)(iVar4 + 0x11aa) = *(short *)(iVar4 + 0x11aa) + 1;
            }
            sVar21 = (short)iVar17 + 1;
            iVar17 = (int)sVar21;
          } while (sVar21 < *psVar18);
        }
        puVar2 = (short *)*puVar8;
        HandleHeroDefeat(puVar2,*puVar2,puVar2[1]);
        RemoveBattlePiece(*puVar8);
        *puVar8 = 0;
        *piVar10 = 0;
        UpdateDisplayState();
        if (bVar20) {
          return;
        }
        FUN_1005f678(0x34,1);
        FUN_10052f6c();
        FUN_100534c8(0);
        FUN_1003fe5c();
        FUN_1005cc8c();
        piVar3 = *(int **)(*(int*)((char*)ppuVar12 - 0x53d));
        if (piVar3 == (int *)0x0) {
          return;
        }
        ResourceRead_Dispatch((int)piVar3 + (int)*(short *)(*piVar3 + 0x4d0));
        return;
      }
      if (!bVar20) {
        FUN_1005f678(0x34,2);
        FUN_10052f6c();
        ResourceRead_Dispatch(*piVar3 + (int)*(short *)(*(int *)*piVar3 + 0x810));
        ppuVar12 = local_6c;
      }
      *(char *)(param_1 + 0x1a) = 0;
    }
  }
  cVar19 = *(char *)(param_1 + 0x18);
  if (cVar19 == '\x02') {
    if (bVar20) {
      *(char *)(*puVar7 + *(char *)(param_1 + 0x19) * 0x1e + 0xd28) = 3;
      *(short *)(*puVar7 + *(char *)(param_1 + 0x19) * 0x1e + 0xd2a) =
           (short)(((unsigned long long)*puVar8 - (unsigned long long)*(unsigned int *)(*(int*)((char*)ppuVar12 - 0x150)) & 0xffffffff) / 0x16);
      *(short *)(*puVar7 + *(char *)(param_1 + 0x19) * 0x1e + 0xd2c) = 0;
      *(short *)(*puVar7 + *(char *)(param_1 + 0x19) * 0x1e + 0xd2e) = 0;
    }
    else {
      uVar14 = FUN_1005f678(0x35,0);
      DrawNumber(uVar9,uVar14,
                   (unsigned long long)*puVar7 +
                   (((long long)*(char *)(*puVar8 + 10) & 0x3fffffffU) * 4 +
                    (long long)*(char *)(*puVar8 + 10) & 0x3fffffff) * 4 + 0x224,
                   (unsigned long long)*puVar7 +
                   (((long long)*(char *)(param_1 + 0x19) & 0xfffffffU) * 0x10 -
                    (long long)*(char *)(param_1 + 0x19) & 0x7fffffff) * 2 + 0xd12);
      FUN_10052f6c(uVar9);
      *(char *)(*puVar7 + *(char *)(param_1 + 0x19) * 0x1e + 0xd28) = 1;
      *(short *)(*puVar7 + *(char *)(param_1 + 0x19) * 0x1e + 0xd2a) = 0;
      *(short *)(*puVar7 + *(char *)(param_1 + 0x19) * 0x1e + 0xd2c) = param_2;
      *(short *)(*puVar7 + *(char *)(param_1 + 0x19) * 0x1e + 0xd2e) = param_3;
      FUN_100534c8(1);
    }
    FUN_10038c60(*(short *)(*puVar7 + 0x110),6,*(char *)(param_1 + 0x19),0,
                 *puVar7 + *(char *)(*puVar8 + 10) * 0x14 + 0x224);
    iVar22 = *piVar6 + (int)(((uVar24 & 0x1fffffff) * 8 - uVar24 & 0xffffffff) << 5);
    iVar17 = (int)((uVar23 & 0xffffffff) << 1);
    *(unsigned int *)(iVar22 + iVar17) = *(unsigned int *)(iVar22 + iVar17) | 0x400000;
    *(char *)(param_1 + 0x18) = 0;
  }
  else if (cVar19 == '\x04') {
    if (*(short *)(param_1 + 0x1c) == 0) {
      uVar14 = RandomRange(3,500,500);
    }
    else {
      uVar14 = RandomRange(3,1000,1000);
    }
    iVar22 = *piVar6 + (int)(((uVar24 & 0x1fffffff) * 8 - uVar24 & 0xffffffff) << 5);
    iVar17 = (int)((uVar23 & 0xffffffff) << 1);
    *(unsigned int *)(iVar22 + iVar17) = *(unsigned int *)(iVar22 + iVar17) | 0x400000;
    iVar22 = *puVar7 + *(short *)(*puVar7 + 0x110) * 0x14;
    *(short *)(iVar22 + 0x186) = (short)uVar14 + *(short *)(iVar22 + 0x186);
    if (!bVar20) {
      uVar16 = FUN_1005f678(0x36,0);
      DrawNumber(uVar9,uVar16,
                   (unsigned long long)*puVar7 +
                   (((long long)*(char *)(*puVar8 + 10) & 0x3fffffffU) * 4 +
                    (long long)*(char *)(*puVar8 + 10) & 0x3fffffff) * 4 + 0x224,uVar14);
      FUN_10052f6c(uVar9);
      FUN_100534c8(0);
    }
    FUN_10038c60(*(short *)(*puVar7 + 0x110),6,0x66,0,
                 *puVar7 + *(char *)(*puVar8 + 10) * 0x14 + 0x224);
  }
  else if (cVar19 == '\x05') {
    iVar22 = *piVar6 + (int)(((uVar24 & 0x1fffffff) * 8 - uVar24 & 0xffffffff) << 5);
    iVar17 = (int)((uVar23 & 0xffffffff) << 1);
    *(unsigned int *)(iVar22 + iVar17) = *(unsigned int *)(iVar22 + iVar17) | 0x400000;
    lVar15 = RandomRange(1,2,0);
    if (*(short *)(param_1 + 0x1c) != 0) {
      lVar15 = (long long)(short)((short)lVar15 + 2);
    }
    iVar22 = (int)lVar15;
    if (iVar22 < 2) {
      uVar14 = FUN_1004a21c(*(char *)(param_1 + 0x1a));
      uVar16 = FUN_1005f678(0x37,0);
      DrawNumber(uVar9,uVar16,uVar14,
                   (unsigned long long)*puVar7 +
                   (((long long)*(char *)(*puVar8 + 10) & 0x3fffffffU) * 4 +
                    (long long)*(char *)(*puVar8 + 10) & 0x3fffffff) * 4 + 0x224);
    }
    else {
      uVar14 = FUN_1004a21c(*(char *)(param_1 + 0x1a));
      uVar16 = FUN_1005f678(0x37,1);
      DrawNumber(uVar9,uVar16,lVar15,uVar14,
                   (unsigned long long)*puVar7 +
                   (((long long)*(char *)(*puVar8 + 10) & 0x3fffffffU) * 4 +
                    (long long)*(char *)(*puVar8 + 10) & 0x3fffffff) * 4 + 0x224);
    }
    uVar11 = ZEXT48(local_6c);
    if (!bVar20) {
      FUN_10052f6c(uVar9);
      FUN_100534c8(0);
    }
    iVar13 = (int)uVar11;
    iVar17 = *piVar6 + (int)(((uVar24 & 0x1fffffff) * 8 - uVar24 & 0xffffffff) << 5);
    iVar4 = (int)((uVar23 & 0xffffffff) << 1);
    *(unsigned int *)(iVar17 + iVar4) = *(unsigned int *)(iVar17 + iVar4) | 0x400000;
    uVar1 = *(char *)(param_1 + 0x1a);
    sVar21 = 0;
    if (0 < iVar22) {
      do {
        psVar18 = (short *)FUN_10053838(uVar1,uVar23,uVar24);
        iVar13 = (int)uVar11;
        if (psVar18 != (short *)0x0) {
          iVar17 = *piVar6 + psVar18[1] * 0xe0;
          *(unsigned int *)(iVar17 + *psVar18 * 2) = *(unsigned int *)(iVar17 + *psVar18 * 2) | 0x100000;
        }
        sVar21 = sVar21 + 1;
      } while (sVar21 < iVar22);
    }
    FUN_10038c60(*(short *)(*puVar7 + 0x110),6,100,0,
                 *puVar7 + *(char *)(*puVar8 + 10) * 0x14 + 0x224);
    if (*piVar10 != 0) {
      if ((*psVar5 == 0) && (*(short *)(*puVar7 + *(short *)(*puVar7 + 0x110) * 2 + 0xd0) != 1)) {
        bVar20 = false;
      }
      else {
        bVar20 = true;
      }
      if (!bVar20) {
        FUN_1002122c(uVar23,uVar24,local_48);
        FUN_10021a3c(local_48[0],*(int *)(iVar13 + -0x3f4),0);
        PrepareDefender(*piVar10);
        FUN_1005cc8c();
      }
    }
  }
  return;
}












































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10054824 Size: 260 bytes */
void FUN_10054824()

{
  int iVar1;
  int *piVar2;
  int uVar3;
  long long lVar4;
  long long uVar5;
  long long lVar6;
  int local_34;
  
  uVar3 = uRam101175f8;
  piVar2 = piRam1011735c;
  lVar4 = RandomRange(3,500,500);
  iVar1 = *piVar2;
  lVar6 = lVar4 + *(short *)(iVar1 + *(short *)(iVar1 + 0x110) * 0x14 + 0x186);
  if (30000 < (int)lVar6) {
    lVar6 = 30000;
  }
  iVar1 = *piVar2;
  *(short *)(iVar1 + *(short *)(iVar1 + 0x110) * 0x14 + 0x186) = (short)lVar6;
  FUN_1005f678(0x3c,0);
  FUN_100547b4();
  uVar5 = FUN_1005f678(0x3c,1);
  DrawNumber(uVar3,uVar5,lVar4);
  FUN_100547b4(uVar3);
  uVar5 = ResourceRead_Dispatch(**(int **)(local_34 + -0x193c) +
                       (int)*(short *)(*(int *)**(int **)(local_34 + -0x193c) + 0x7f8));
  FocusObject();
  FUN_10096744(uVar5);
  return;
}
























































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10057334 Size: 276 bytes */
void FUN_10057334()

{
  int *piVar1;
  int *piVar2;
  int *piVar3;
  long long uVar4;
  int iVar5;
  
  piVar3 = piRam10117358;
  piVar2 = piRam10117354;
  piVar1 = *(int **)(*(int *)(*piRam1011734c + 0x88) + 0x148);
  ResourceRead_Dispatch((int)piVar1 + (int)*(short *)(*piVar1 + 0x2b8));
  uVar4 = FUN_10052184(0x4d415020,10000,1);
  if (*piVar3 == 0) {
    iVar5 = AllocateBlock(0x8880);
    *piVar3 = iVar5;
  }
  StreamRead(uVar4,*piVar3,0x8880);
  CloseStream(uVar4,1);
  uVar4 = FUN_10052184(0x52442020,10000,1);
  if (*piVar2 == 0) {
    iVar5 = AllocateBlock(0x4440);
    *piVar2 = iVar5;
  }
  StreamRead(uVar4,*piVar2,0x4440);
  CloseStream(uVar4,1);
  return;
}

/* Address: 0x10057448 Size: 936 bytes */
long long FUN_10057448(short param_1)

{
  int uVar1;
  unsigned int uVar2;
  int *piVar3;
  int iVar4;
  int *piVar5;
  int *ppuVar6;
  long long uVar7;
  long long uVar8;
  long long uVar9;
  long long uVar10;
  long long uVar11;
  long long uVar12;
  long long uVar13;
  int *piVar14;
  int iVar15;
  int *puVar16;
  short sVar17;
  int local_1f4;
  char auStack_1d0 [72];
  char auStack_188 [256];
  int local_88;
  int local_84;
  char auStack_80 [128];
  
  uVar1 = uRam1011772c;
  piVar5 = piRam1011735c;
  piVar3 = piRam1011734c;
  puVar16 = gProgressBar;
  ppuVar6 = 0 /* TVect base */;
  if (param_1 != 0) {
    ppuVar6 = 0 /* TVect base */;
    uVar7 = FUN_1005f678(0x88,6);
    uVar8 = FUN_1005f678(0x88,5);
    uVar9 = FUN_1005f678(0x88,4);
    uVar10 = FUN_1005f678(0x88,3);
    uVar11 = FUN_1005f678(0x88,2);
    uVar12 = FUN_1005f678(0x88,1);
    uVar13 = FUN_1005f678(0x88,0);
    InitDataStructures(uVar13,uVar12,uVar11,uVar10,uVar9,uVar8,uVar7);
    local_88 = 0xca;
    local_84 = 0x3c;
    ProgressCreate(*puVar16,&local_88,0x3f3);
    ProgressUpdate(*puVar16,0);
  }
  piVar14 = (int *)FUN_100fb260(0);
  SetResourceType(piVar14,0x5343454e,10000);
  ResourceRead_Dispatch((int)piVar14 + (int)*(short *)(*piVar14 + 0xe0),(*(int*)((char*)ppuVar6 - 0xa7)),0x54);
  ResourceRead_Dispatch((int)piVar14 + (int)*(short *)(*piVar14 + 0x90));
  uVar7 = FUN_10052184(0x53434e20,10000,1);
  if (*piVar5 == 0) {
    iVar15 = AllocateBlock(0x2fcc);
    *piVar5 = iVar15;
  }
  ReadSCNResource(uVar7,*piVar5);
  CloseStream(uVar7,1);
  iVar15 = *(int *)(*piVar3 + 0x88);
  FocusObject(iVar15);
  piVar3 = *(int **)(iVar15 + 0x148);
  FocusObject(piVar3);
  ResourceRead_Dispatch((int)piVar3 + (int)*(short *)(*piVar3 + 0x178),auStack_80);
  DisposeGeneric(**(short **)(local_1f4 + -0xb4));
  DrawNumber(uVar1,*(int *)(local_1f4 + -0x1928),*(short *)(*piVar5 + 0x162));
  BuildPascalString(auStack_188,uVar1);
  FUN_10027a58(1,auStack_188,0,auStack_1d0);
  FUN_1007ed14(iVar15,auStack_1d0);
  ResourceRead_Dispatch((int)*(int **)(iVar15 + 0x14c) + (int)*(short *)(**(int **)(iVar15 + 0x14c) + 0x118))
  ;
  if (param_1 != 0) {
    ProgressUpdate(*puVar16,0x14);
  }
  if (**(short **)(local_1f4 + -0x16a4) != 0) {
    *(short *)(*piVar5 + 0x162) = **(short **)(local_1f4 + -0x143c);
  }
  InitCityNames();
  InitArmyData();
  LoadMapResources();
  if (param_1 != 0) {
    ProgressUpdate(*puVar16,0x3c);
  }
  InitSpecialLocations();
  if (param_1 != 0) {
    ProgressUpdate(*puVar16,100);
    ProgressDelay(10);
  }
  PostLoadInitialization();
  iVar15 = 0;
  do {
    if (*(short *)(*piVar5 + iVar15 * 2 + 0x138) == 0) {
      iVar4 = *(int *)(local_1f4 + -0x1928);
      puVar16 = (int *)(*piVar5 + iVar15 * 0x14);
      uVar1 = *(int *)(iVar4 + 0x10);
      uVar2 = *(unsigned int *)(iVar4 + 0x14);
      *puVar16 = *(int *)(iVar4 + 0xc);
      puVar16[1] = uVar1;
      puVar16[2] = uVar2 & 0xff000000;
      *(short *)(iVar15 * 2 + *piVar5 + 0xc0) = 3;
    }
    sVar17 = (short)iVar15 + 1;
    iVar15 = (int)sVar17;
  } while (sVar17 < 8);
  return 1;
}

























































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1005bbe8 Size: 320 bytes */
void FUN_1005bbe8()

{
  int *piVar1;
  int *piVar4;
  short uVar5;
  long long uVar2;
  long long uVar3;
  
  piVar1 = piRam1011734c;
  piVar4 = *(int **)(*(int *)(*piRam1011734c + 0x88) + 0x148);
  ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0x2b8));
  piVar4 = (int *)FUN_10052184(0x53474e20,10000,1);
  if (piVar4 != (int *)0x0) {
    uVar5 = ResourceRead_Dispatch((int)piVar4 + (int)*(short *)(*piVar4 + 0xb8));
    uVar2 = AllocateBlock(uVar5);
    if ((int)uVar2 != 0) {
      StreamRead(piVar4,uVar2,uVar5);
      CloseStream(piVar4,1);
      uVar3 = FUN_100522dc(*(int *)(*(int *)(*piVar1 + 0x88) + 0x15c),6,0);
      if ((int)uVar3 == 0) {
        FreeBlock(uVar2);
      }
      else {
        StreamWrite(uVar3,uVar2,uVar5);
        CloseStream(uVar3,1);
        FreeBlock(uVar2);
      }
    }
  }
  return;
}










































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x10060078 Size: 1384 bytes */
void FUN_10060078(int *param_1,short param_2)

{
  int *piVar1;
  int *puVar2;
  long long lVar3;
  int bVar4;
  int bVar5;
  int *ppuVar6;
  int iVar7;
  int *piVar8;
  int *piVar9;
  int *piVar10;
  int *piVar11;
  unsigned short *puVar12;
  unsigned long long uVar13;
  char cVar15;
  short sVar14;
  short sVar16;
  int iVar17;
  short sVar18;
  long long lVar19;
  int *puVar20;
  short sVar21;
  unsigned short uVar22;
  int iVar23;
  long long uVar24;
  unsigned long long uVar25;
  unsigned long long uVar26;
  int **local_7c;
  short local_4c;
  short sStack_4a;
  short local_48;
  short local_46;
  
  puVar12 = gViewportOffset;
  piVar11 = piRam1011735c;
  piVar10 = piRam10117354;
  piVar9 = gActiveView;
  piVar8 = gScreenGWorld;
  iVar7 = gPlayerColorTable;
  ppuVar6 = 0 /* TVect base */;
  iVar23 = *piRam1011735c;
  if ((((*(short *)(iVar23 + 0x124) == 0) ||
       (*(short *)(*(short *)(iVar23 + 0x110) * 2 + iVar23 + 0xd0) == 0)) ||
      (*(short *)(iVar23 + 0x15a) != 0)) || (*gScrollLockFlag != 0)) {
    if (*gActiveView != 0) {
      *gScreenGWorld = *gActiveView + 0x80;
    }
    piVar1 = (int *)*piVar9;
    if (piVar1 == (int *)0x0) {
      cVar15 = '\0';
    }
    else {
      cVar15 = ResourceRead_Dispatch((int)piVar1 + (int)*(short *)(*piVar1 + 0x3a8));
      ppuVar6 = local_7c;
    }
    uVar13 = ZEXT48(ppuVar6);
    if (cVar15 != '\0') {
      if ((*(int *)(*piVar9 + 0xa4) == 0) && (*(int *)(*(int *)(*(int*)((char*)ppuVar6 - 0x5a7)) + 0x38) < 0x71)) {
        bVar4 = true;
      }
      else {
        bVar4 = false;
      }
      if (((param_1 == (int *)0x0) || (8 < *(short *)(param_1 + 1))) ||
         (8 < *(short *)((int)param_1 + 6))) {
        bVar5 = false;
      }
      else {
        bVar5 = true;
      }
      if (bVar5) {
        puVar2 = (*(int*)((char*)ppuVar6 - 0x62c));
      }
      else {
        puVar2 = (int *)*piVar8;
      }
      puVar20 = param_1;
      if (param_1 == (int *)0x0) {
        puVar20 = (int *)(*(int*)((char*)ppuVar6 - 0x62b));
      }
      local_4c = (short)((unsigned int)*puVar20 >> 0x10);
      sStack_4a = (short)*puVar20;
      local_48 = (short)((unsigned int)puVar20[1] >> 0x10);
      local_46 = (short)puVar20[1];
      uVar25 = 0;
      if (0 < *(short *)(*piVar11 + 0x1602)) {
        do {
          iVar23 = *piVar11 + (int)(((uVar25 & 0x7ffffff) * 0x20 + uVar25 & 0xffffffff) << 1);
          sVar14 = *(short *)(iVar23 + 0x1604);
          sVar21 = *(short *)(iVar23 + 0x1606);
          uVar26 = (unsigned long long)sVar21;
          if (*(short *)(*piVar11 + 0x124) == 0) goto LAB_10060350;
          if ((param_1 == (int *)0x0) ||
             (((((int)(short)(local_4c + -8) <= (int)sVar14 &&
                ((int)sVar14 <= (int)(short)(local_4c + -8) + (int)(short)(local_48 + 0xc) + -1)) &&
               ((int)(short)(sStack_4a + -8) <= (int)sVar21)) &&
              ((int)sVar21 <= (int)(short)(sStack_4a + -8) + (int)(short)(local_46 + 0xc) + -1)))) {
            if (**(short **)((int)uVar13 + -0x18a8) == 0) {
              iVar23 = *piVar10 + (int)(((uVar26 & 0x1fffffff) * 8 - uVar26 & 0xffffffff) << 4);
              if ((((*(unsigned int *)(iVar23 + sVar14) >> 0x1d & 1) != 0) ||
                  ((*(unsigned int *)(iVar23 + (short)(sVar14 + 1)) >> 0x1d & 1) != 0)) ||
                 (((*(unsigned int *)(iVar23 + sVar14 + 0x70) >> 0x1d & 1) != 0 ||
                  ((*(unsigned int *)(iVar23 + (short)(sVar14 + 1) + 0x70) >> 0x1d & 1) != 0)))) {
LAB_10060350:
                if (!bVar4) {
                  sVar14 = sVar14 << 1;
                }
                sVar14 = sVar14 + -1;
                if (!bVar4) {
                  sVar21 = (short)((uVar26 & 0xffffffff) << 1);
                }
                sVar21 = sVar21 + -1;
                if (bVar5) {
                  if (bVar4) {
                    uVar22 = *puVar12;
                    uVar22 = ((short)uVar22 >> 1) + (unsigned short)((short)uVar22 < 0 && (uVar22 & 1) != 0)
                    ;
                  }
                  else {
                    uVar22 = *puVar12;
                  }
                  sVar14 = sVar14 - uVar22;
                  if (bVar4) {
                    uVar22 = puVar12[1];
                    uVar22 = ((short)uVar22 >> 1) + (unsigned short)((short)uVar22 < 0 && (uVar22 & 1) != 0)
                    ;
                  }
                  else {
                    uVar22 = puVar12[1];
                  }
                  sVar21 = sVar21 - uVar22;
                }
                iVar23 = *piVar11 + (int)(((uVar25 & 0x7ffffff) * 0x20 + uVar25 & 0xffffffff) << 1);
                lVar3 = -(long long)
                         *(char *)(*piVar11 +
                                   (*(unsigned int *)(**(int **)((int)uVar13 + -0x548) +
                                              *(short *)(iVar23 + 0x1606) * 0xe0 +
                                             *(short *)(iVar23 + 0x1604) * 2) >> 0x18) + 0x711);
                lVar19 = lVar3 + 10;
                if ((lVar19 + (-(unsigned long long)(lVar19 == 0) - (lVar3 + 9)) & 0xff) == 0) {
                  if (*(char *)(*piVar11 +
                                (int)(((uVar25 & 0x7ffffff) * 0x20 + uVar25 & 0xffffffff) << 1) +
                               0x1619) == '\x0f') {
                    iVar23 = 8;
                  }
                  else {
                    iVar23 = (int)*(char *)(*piVar11 +
                                            (int)(((uVar25 & 0x7ffffff) * 0x20 + uVar25 & 0xffffffff
                                                  ) << 1) + 0x1619);
                  }
                }
                else {
                  iVar23 = -1;
                }
                if (-1 < iVar23) {
                  if (bVar4) {
                    uVar24 = 0x50;
                  }
                  else {
                    uVar24 = 9;
                  }
                  EnsureSpriteLoaded(uVar24);
                  EnsureSpriteAndLock(uVar24);
                  iVar17 = iVar23 * 8 + iVar7;
                  BlitSpriteWithMask(*(int *)((int)uVar13 + -0x238),
                               *(short *)(iVar23 * 8 + iVar7),*(short *)(iVar17 + 2),
                               *(short *)(iVar17 + 4),*(short *)(iVar17 + 6),puVar2,sVar14
                               ,sVar21);
                  UnlockAndReleaseSprite(uVar24);
                }
                if ((int)uVar25 == (int)param_2) {
                  sVar16 = sVar14 + -1;
                  sVar18 = sVar21 + -1;
                  DrawHLine(*piVar8,sVar16,sVar18,10,0);
                  DrawHLine(*piVar8,sVar16,sVar21 + 9,10,0);
                  DrawVLine(*piVar8,sVar16,sVar18,10,0);
                  DrawVLine(*piVar8,sVar14 + 9,sVar18,10,0);
                }
              }
            }
            else {
              iVar23 = *piVar10 + (int)(((uVar26 & 0x1fffffff) * 8 - uVar26 & 0xffffffff) << 4);
              if (((*(int *)(iVar23 + sVar14) < 0) || (*(int *)(iVar23 + (short)(sVar14 + 1)) < 0))
                 || ((*(int *)(iVar23 + sVar14 + 0x70) < 0 ||
                     (*(int *)(iVar23 + (short)(sVar14 + 1) + 0x70) < 0)))) goto LAB_10060350;
            }
          }
          sVar14 = (short)uVar25 + 1;
          uVar25 = (unsigned long long)sVar14;
        } while (sVar14 < *(short *)(*piVar11 + 0x1602));
      }
    }
  }
  return;
}





























































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100632a0 Size: 832 bytes */
void FUN_100632a0(short param_1,short param_2,short param_3)

{
  int iVar1;
  unsigned int uVar2;
  short sVar3;
  int *piVar4;
  int iVar5;
  int *ppuVar6;
  short sVar7;
  short sVar8;
  int bVar9;
  unsigned long long uVar10;
  int local_40;
  int local_3c;
  short local_38;
  short local_36;
  short local_34;
  short local_32;
  short local_2e;
  short local_2c;
  short local_2a;
  
  iVar5 = gMinimapGWorld;
  piVar4 = piRam1011735c;
  ppuVar6 = 0 /* TVect base */;
  if (*(short *)(*piRam1011735c + 0x124) != 0) {
    uVar10 = (unsigned long long)gColorPalette;
    iVar1 = *piRam1011735c;
    if (((*(short *)(iVar1 + *(short *)(iVar1 + 0x110) * 2 + 0xd0) == 0) ||
        (*(short *)(iVar1 + 0x15a) != 0)) &&
       (FUN_10080030(*(int *)(*piRam1011734c + 0x88)), *(short *)(*(int*)((char*)ppuVar6 - 0x62a)) == 0)) {
      uVar2 = *(unsigned int *)(*(int *)(*(int*)((char*)ppuVar6 - 0x152)) + param_2 * 0xe0 + param_1 * 2);
      if (((*(char *)(*piVar4 + (uVar2 >> 0x18) + 0x711) == '\n') || ((uVar2 >> 0x15 & 1) != 0)) ||
         (*(short *)ppuVar6[-100] == 2)) {
        bVar9 = true;
      }
      else {
        bVar9 = false;
      }
      if (bVar9) {
        SetRect4(&local_38,param_1 + -2,param_2 + -2,5,5);
      }
      else {
        SetRect4(&local_38,param_1 + -1,param_2 + -1,3,3);
      }
      if (local_38 < 0) {
        local_34 = local_38 + local_34;
        local_38 = 0;
      }
      if (local_36 < 0) {
        local_32 = local_36 + local_32;
        local_36 = 0;
      }
      if (0x6f < (int)local_38 + (int)local_34) {
        local_34 = (local_34 - (short)((int)local_38 + (int)local_34)) + 0x6f;
      }
      if (0x9b < (int)local_36 + (int)local_32) {
        local_32 = (local_32 - (short)((int)local_36 + (int)local_32)) + 0x9b;
      }
      sVar3 = local_38 << 1;
      local_2e = local_36 << 1;
      local_2c = local_34 << 1;
      local_2a = local_32 << 1;
      GetGWorld_Wrapper(&local_3c,&local_40);
      SetGWorld_Wrapper(*(int *)(iVar5 + 0x10),0);
      RGBForeColor_Thunk(uVar10);
      SetForeColor_Thunk();
      RGBForeColor_Thunk(uVar10 + 0x5fa);
      SetForeColor_Thunk();
      if (*(int *)(iVar5 + 0x10) != 0) {
        LockPixels_Wrapper(*(int *)(iVar5 + 0x10));
      }
      sVar8 = sVar3;
      if ((int)sVar3 <= (int)sVar3 + (int)local_2c) {
        do {
          sVar7 = local_2e;
          if ((int)local_2e <= (int)local_2e + (int)local_2a) {
            do {
              BlitTerrainPixel(iVar5,sVar8,sVar7,0xff);
              sVar7 = sVar7 + 1;
            } while ((int)sVar7 <= (int)local_2e + (int)local_2a);
          }
          sVar8 = sVar8 + 1;
        } while ((int)sVar8 <= (int)sVar3 + (int)local_2c);
      }
      if (*(int *)(iVar5 + 0x10) != 0) {
        UnlockPixels_Wrapper(*(int *)(iVar5 + 0x10));
      }
      SetGWorld_Wrapper(local_3c,local_40);
      HideMinimapCursor();
      RefreshDisplay(param_3,1,&local_38);
    }
  }
  return;
}





































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1007d168 Size: 3436 bytes */
void FUN_1007d168(int *param_1,long long param_2)

{
  int *piVar1;
  int *piVar2;
  int *piVar3;
  int *piVar4;
  int *ppuVar5;
  int *puVar6;
  int iVar7;
  int *piVar8;
  
  piVar4 = gAppObject;
  piVar3 = gSidePanelWindow;
  piVar2 = gMainGameWindow;
  piVar1 = gInfoWindow;
  piVar8 = gOverviewWindow;
  ppuVar5 = 0 /* TVect base */;
  iVar7 = (int)param_2;
  if (iVar7 == 0x57e) goto LAB_1007d654;
  if (iVar7 < 0x57f) {
    if (iVar7 == 0x401) {
      PlayerSetup_Cmd6();
      return;
    }
    if (iVar7 < 0x402) {
      if (iVar7 == 0x3f6) {
        iVar7 = *piRam1011735c + *(short *)((int)param_1 + 0x1de) * 0x42;
        ShowCityInfo(0,*(short *)(iVar7 + 0x1604),*(short *)(iVar7 + 0x1606));
        return;
      }
      if (iVar7 < 0x3f7) {
        if (iVar7 == 0x3f1) {
          MoveToCoordinates(*(short *)(param_1 + 0x78),*(short *)((int)param_1 + 0x1e2),
                       *(short *)(param_1 + 0x79),*(short *)((int)param_1 + 0x1e6));
          return;
        }
        if (iVar7 < 0x3f2) {
          if (iVar7 == 0x3ee) {
            SelectArmyByIndex(*(short *)(*piRam1011735c + 0x110));
            return;
          }
          if (iVar7 < 0x3ef) {
            if (iVar7 == 1000) {
              if (((*(short *)(*piRam1011735c + *(short *)(*piRam1011735c + 0x110) * 2 + 0xd0) == 0)
                  && (*(char *)((int)param_1 + 0x16e) != '\0')) &&
                 (*(char *)((int)param_1 + 0x16f) == '\0')) {
                *(char *)((int)param_1 + 0x1e9) = 1;
                *(char *)((int)param_1 + 0x16f) = 1;
                ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x110),0x1e);
                return;
              }
              *(char *)((int)param_1 + 0x16f) = 0;
              HandleEndTurn();
              return;
            }
          }
          else {
            if (iVar7 == 0x3ef) {
              DeselectArmy(*(short *)(param_1 + 0x77));
              return;
            }
            if (iVar7 == 0x3f0) {
              iVar7 = *piRam1011735c + *(short *)((int)param_1 + 0x1de) * 0x42;
              ShowCityInfo(2,*(short *)(iVar7 + 0x1604),*(short *)(iVar7 + 0x1606));
              return;
            }
          }
        }
        else {
          if (iVar7 == 0x3f3) {
            piVar8 = (int *)ResourceRead_Dispatch(*piRam101176bc +
                                         (int)*(short *)(*(int *)*piRam101176bc + 200),3000,param_1)
            ;
            FocusObject();
            ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 800));
            return;
          }
          if (iVar7 < 0x3f4) {
            if (iVar7 == 0x3f2) {
              HandleStackManagement(param_1);
              return;
            }
          }
          else {
            if (iVar7 == 0x3f4) {
              DefinePath();
              return;
            }
            if (iVar7 == 0x3f5) {
              CancelUndoAction(param_1,1,1);
              return;
            }
          }
        }
      }
      else {
        if (iVar7 == 0x3fb) {
          SearchRuinsTemples();
          return;
        }
        if (iVar7 < 0x3fc) {
          if (iVar7 == 0x3f8) {
            InspectArmyDetails(*(short *)((int)param_1 + 0x1de));
            return;
          }
          if (iVar7 < 0x3f9) {
            if (iVar7 == 0x3f7) {
              HandleGuardCommand(*(char *)(param_1 + 0x7a));
              return;
            }
          }
          else {
            if (iVar7 == 0x3f9) {
              ShowProductionDialog();
              return;
            }
            if (iVar7 == 0x3fa) {
              ResourceRead_Dispatch(*gMainGameWindow + (int)*(short *)(*(int *)*gMainGameWindow + 0x4c8));
              ResourceRead_Dispatch(*piVar3 + (int)*(short *)(*(int *)*piVar3 + 0x4c8));
              ResourceRead_Dispatch(*piVar8 + (int)*(short *)(*(int *)*piVar8 + 0x4c8));
              ResourceRead_Dispatch(*piVar1 + (int)*(short *)(*(int *)*piVar1 + 0x4c8));
              return;
            }
          }
        }
        else {
          if (iVar7 == 0x3fe) {
            PlayerSetup_Cmd3();
            return;
          }
          if (iVar7 < 0x3ff) {
            if (iVar7 == 0x3fc) {
              PlayerSetup_Cmd1();
              return;
            }
            if (iVar7 == 0x3fd) {
              PlayerSetup_Cmd2();
              return;
            }
          }
          else {
            if (iVar7 == 0x3ff) {
              PlayerSetup_Cmd4();
              return;
            }
            if (iVar7 == 0x400) {
              PlayerSetup_Cmd5();
              return;
            }
          }
        }
      }
    }
    else {
      if (iVar7 == 0x40b) {
        GameOption_Toggle8();
        return;
      }
      if (iVar7 < 0x40c) {
        if (iVar7 == 0x406) {
          GameOption_Toggle3();
          return;
        }
        if (iVar7 < 0x407) {
          if (iVar7 == 0x403) {
            PlayerSetup_Cmd8();
            return;
          }
          if (iVar7 < 0x404) {
            if (iVar7 == 0x402) {
              PlayerSetup_Cmd7();
              return;
            }
          }
          else {
            if (iVar7 == 0x404) {
              GameOption_Toggle1();
              return;
            }
            if (iVar7 == 0x405) {
              GameOption_Toggle2();
              return;
            }
          }
        }
        else {
          if (iVar7 == 0x408) {
            GameOption_Toggle5();
            return;
          }
          if (iVar7 < 0x409) {
            if (iVar7 == 0x407) {
              GameOption_Toggle4();
              return;
            }
          }
          else {
            if (iVar7 == 0x409) {
              GameOption_Toggle6();
              return;
            }
            if (iVar7 == 0x40a) {
              GameOption_Toggle7();
              return;
            }
          }
        }
      }
      else {
        if (iVar7 == 0x578) {
LAB_1007d530:
          puVar6 = (int *)NewPtr_Thunk(0xb4);
          if (puVar6 != (int *)0x0) {
            ConstructCommand(puVar6);
            *puVar6 = (*(int*)((char*)ppuVar5 - 0x437));
            *(short *)(puVar6 + 0x2c) = 0;
          }
          InitCommand(puVar6,param_2,param_1,1,1,param_1);
          *(short *)(puVar6 + 0x2c) = 0;
          ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x200),puVar6);
          return;
        }
        if (iVar7 < 0x579) {
          if (iVar7 == 0x40d) {
            GameOption_Toggle10();
            return;
          }
          if (iVar7 < 0x40e) {
            if (iVar7 == 0x40c) {
              GameOption_Toggle9();
              return;
            }
          }
          else {
            if (iVar7 == 0x40e) {
              GameOption_Toggle11();
              return;
            }
            if (iVar7 == 0x40f) {
              StartOfTurnProcessing();
              return;
            }
          }
        }
        else {
          if (iVar7 == 0x57b) {
            ProcessEndTurn();
            return;
          }
          if (iVar7 < 0x57c) {
            if (iVar7 == 0x579) goto LAB_1007d530;
            if (iVar7 == 0x57a) {
              MoveToOverviewTarget(param_1,*(short *)(*piRam101176e0 + 0x12),
                           *(short *)(*piRam101176e0 + 0x14));
              return;
            }
          }
          else {
            if (iVar7 == 0x57c) {
              puVar6 = (int *)NewPtr_Thunk(0x108);
              if (puVar6 != (int *)0x0) {
                ConstructCommand(puVar6);
                *puVar6 = (*(int*)((char*)ppuVar5 - 0x43a));
                *(short *)(puVar6 + 0xc) = 0xffff;
                *(short *)((int)puVar6 + 0x32) = 0xffff;
                *(short *)(puVar6 + 0x41) = 0;
              }
              InitCommand(puVar6,0x57c,param_1,1,1,param_1);
              ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x200),puVar6);
              return;
            }
            if (iVar7 == 0x57d) goto LAB_1007d654;
          }
        }
      }
    }
    goto LAB_1007deb0;
  }
  if (iVar7 == 0x6a4) {
    ToggleMapZoom();
    return;
  }
  if (iVar7 < 0x6a5) {
    if (iVar7 == 0x5dd) goto LAB_1007d7b4;
    if (iVar7 < 0x5de) {
      if (iVar7 == 0x583) {
        HandleAIStrategy();
        return;
      }
      if (iVar7 < 0x584) {
        if (iVar7 == 0x580) {
LAB_1007d654:
          puVar6 = (int *)NewPtr_Thunk(0x1e0);
          if (puVar6 != (int *)0x0) {
            ConstructCommand(puVar6);
            *puVar6 = (*(int*)((char*)ppuVar5 - 0x43c));
            puVar6[0xc] = 0;
            *(short *)(puVar6 + 0x41) = 0;
            *(short *)(puVar6 + 0x76) = 0;
            *(short *)((int)puVar6 + 0x1da) = 0xffff;
            *(short *)(puVar6 + 0x77) = 0xffff;
          }
          InitCommand(puVar6,param_2,param_1,1,1,param_1);
          puVar6[0xc] = param_1;
          *(short *)((int)puVar6 + 0x1da) = 0xffff;
          *(short *)(puVar6 + 0x77) = 0xffff;
          ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x200),puVar6);
          return;
        }
        if (iVar7 < 0x581) {
          if (iVar7 == 0x57f) goto LAB_1007d654;
        }
        else {
          if (iVar7 == 0x581) {
            ToggleFogOfWar();
            return;
          }
          if (iVar7 == 0x582) {
            iVar7 = *piRam101176e0;
            if (iVar7 == 0) {
              return;
            }
            if (*(short *)(iVar7 + 0x12) < 0) {
              return;
            }
            if (*(short *)(iVar7 + 0x14) < 0) {
              return;
            }
            CenterMapOnLocation(*(short *)(iVar7 + 0x12),*(short *)(iVar7 + 0x14));
            FullDisplayRefresh();
            return;
          }
        }
      }
      else {
        if (iVar7 == 0x585) {
          ShowReports();
          return;
        }
        if (iVar7 < 0x586) {
          if (iVar7 == 0x584) {
            ShowGameHistory();
            return;
          }
        }
        else {
          if (iVar7 == 0x586) {
            HandleDisbandAction();
            return;
          }
          if (iVar7 == 0x5dc) {
            SelectArmySlot((short)param_1[0x59] + -0x5dd);
            return;
          }
        }
      }
    }
    else {
      if (iVar7 == 0x5e2) {
        ShowArmyBonus();
        return;
      }
      if (iVar7 < 0x5e3) {
        if (iVar7 == 0x5df) {
LAB_1007d7b4:
          SelectArmySlot((short)param_2 + -0x5dd);
          return;
        }
        if (iVar7 < 0x5e0) {
          if (iVar7 == 0x5de) goto LAB_1007d7b4;
        }
        else if ((iVar7 == 0x5e0) || (iVar7 == 0x5e1)) goto LAB_1007d7b4;
      }
      else {
        if (iVar7 == 0x641) {
          StartNewGame();
          return;
        }
        if (iVar7 < 0x642) {
          if (iVar7 == 0x5e3) {
            ShowItemsDialog();
            return;
          }
          if (iVar7 == 0x640) {
            ShowScenarioSelect();
            return;
          }
        }
        else {
          if (iVar7 == 0x642) {
            StartRandomMap();
            return;
          }
          if (iVar7 == 0x643) {
            ScanDefenseGrid();
            return;
          }
        }
      }
    }
  }
  else {
    if (iVar7 == 0x70a) goto LAB_1007d830;
    if (iVar7 < 0x70b) {
      if (iVar7 == 0x6a9) {
LAB_1007d7f8:
        ScrollMapInDirection(param_1,param_2);
        return;
      }
      if (iVar7 < 0x6aa) {
        if (iVar7 == 0x6a6) goto LAB_1007d7f8;
        if (iVar7 < 0x6a7) {
          if (iVar7 == 0x6a5) {
            SaveGameToFile();
            return;
          }
        }
        else if ((iVar7 == 0x6a7) || (iVar7 == 0x6a8)) goto LAB_1007d7f8;
      }
      else {
        if (iVar7 == 0x6ab) {
          ToggleSoundEnabled();
          return;
        }
        if (iVar7 < 0x6ac) {
          if (iVar7 == 0x6aa) goto LAB_1007d7f8;
        }
        else {
          if (iVar7 == 0x708) {
            SwitchToPlayer((short)param_1[0x5a] + -0x709);
            return;
          }
          if (iVar7 == 0x709) goto LAB_1007d830;
        }
      }
    }
    else {
      if (iVar7 == 0x76d) {
        InspectHero(param_1);
        return;
      }
      if (0x76d < iVar7) {
        if (iVar7 == 0x771) {
          piVar1 = (int *)*gOverviewWindow;
          if (piVar1 == (int *)0x0) {
            return;
          }
          iVar7 = ResourceRead_Dispatch((int)*(short *)(*piVar1 + 0x668) + (int)piVar1);
          if (iVar7 != 0) {
            ResourceRead_Dispatch(*piVar8 + (int)*(short *)(*(int *)*piVar8 + 0x328));
            return;
          }
          ResourceRead_Dispatch(*piVar8 + (int)*(short *)(*(int *)*piVar8 + 800));
          piVar8 = (int *)*piVar2;
          if (piVar8 == (int *)0x0) {
            return;
          }
          ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x780));
          ResourceRead_Dispatch(*piVar4 + (int)*(short *)(*(int *)*piVar4 + 0x338),
                       *(int *)(*piVar2 + 0x98));
          return;
        }
        if (iVar7 < 0x772) {
          if (iVar7 == 0x76f) {
            piVar8 = (int *)*gSidePanelWindow;
            if (piVar8 == (int *)0x0) {
              return;
            }
            iVar7 = ResourceRead_Dispatch((int)*(short *)(*piVar8 + 0x668) + (int)piVar8);
            if (iVar7 != 0) {
              ResourceRead_Dispatch(*piVar3 + (int)*(short *)(*(int *)*piVar3 + 0x328));
              return;
            }
            ResourceRead_Dispatch(*piVar3 + (int)*(short *)(*(int *)*piVar3 + 800));
            piVar8 = (int *)*piVar2;
            if (piVar8 == (int *)0x0) {
              return;
            }
            ResourceRead_Dispatch((int)piVar8 + (int)*(short *)(*piVar8 + 0x780));
            ResourceRead_Dispatch(*piVar4 + (int)*(short *)(*(int *)*piVar4 + 0x338),
                         *(int *)(*piVar2 + 0x98));
            return;
          }
          if (iVar7 == 0x770) {
            piVar8 = (int *)*gInfoWindow;
            if (piVar8 == (int *)0x0) {
              return;
            }
            iVar7 = ResourceRead_Dispatch((int)*(short *)(*piVar8 + 0x668) + (int)piVar8);
            if (iVar7 != 0) {
              ResourceRead_Dispatch(*piVar1 + (int)*(short *)(*(int *)*piVar1 + 0x328));
              return;
            }
            ResourceRead_Dispatch(*piVar1 + (int)*(short *)(*(int *)*piVar1 + 800));
            piVar8 = (int *)*piVar2;
            if (piVar8 == (int *)0x0) {
              return;
            }
            ResourceRead_Dispatch((int)*(short *)(*piVar8 + 0x780) + (int)piVar8);
            ResourceRead_Dispatch(*piVar4 + (int)*(short *)(*(int *)*piVar4 + 0x338),
                         *(int *)(*piVar2 + 0x98));
            return;
          }
        }
        else {
          if (iVar7 == 0x772) {
            SaveWindowPositions(param_1);
            return;
          }
          if (iVar7 == 0x773) {
            iVar7 = ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x298));
            if (iVar7 != 0) {
              ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x110),0x1e);
            }
            DispatchNextPhase(param_1,0x76c);
            return;
          }
        }
        goto LAB_1007deb0;
      }
      if (iVar7 == 0x70c) {
LAB_1007d830:
        SwitchToPlayer((short)param_2 + -0x709);
        return;
      }
      if (iVar7 < 0x70d) {
        if (iVar7 == 0x70b) goto LAB_1007d830;
      }
      else {
        if (iVar7 == 0x70d) goto LAB_1007d830;
        if (iVar7 == 0x76c) {
          ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x228));
          ResourceRead_Dispatch((int)param_1 + (int)*(short *)(*param_1 + 0x38),0x76c,0);
          RefreshAfterMenus();
          return;
        }
      }
    }
  }
LAB_1007deb0:
  SuperDoMenuCommand(param_1,param_2);
  return;
}










































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1007ded4 Size: 1076 bytes */
void FUN_1007ded4(int param_1,int *param_2,char param_3)

{
  int bVar1;
  int *piVar2;
  int *piVar3;
  int *piVar4;
  int *piVar5;
  int *piVar6;
  int *puVar7;
  int iVar10;
  int *puVar11;
  int uVar12;
  long long uVar8;
  long long lVar9;
  short sVar13;
  char uVar14;
  char auStack_158 [256];
  short local_58;
  int local_54;
  int local_50;
  int local_4c;
  int local_48;
  int *local_3c;
  char local_38;
  
  puVar7 = puRam10117370;
  piVar6 = piRam10117360;
  piVar5 = piRam1011735c;
  piVar4 = piRam10117358;
  piVar3 = piRam10117354;
  piVar2 = piRam1011734c;
  SuperDoRead(param_1,param_2,param_3);
  *(char *)(param_1 + 0x144) = 1;
  if (*piVar5 == 0) {
    iVar10 = AllocateBlock(0x2fcc);
    *piVar5 = iVar10;
  }
  if (*piVar4 == 0) {
    iVar10 = AllocateBlock(0x8880);
    *piVar4 = iVar10;
  }
  if (*piVar6 == 0) {
    iVar10 = AllocateBlock(22000);
    *piVar6 = iVar10;
  }
  if (*piVar3 == 0) {
    iVar10 = AllocateBlock(0x4440);
    *piVar3 = iVar10;
  }
  bVar1 = false;
  uVar14 = 1;
  if (*(char *)(param_1 + 0x16c) != '\0') {
    puVar11 = (int *)GetResourceByTypeId(0x54797065,1000);
    if (puVar11 != (int *)0x0) {
      if (1 < *(short *)*puVar11) {
        PostMenuCommand(0x3fc,0);
        LockHandleRange(0,0x820000);
      }
      ReleaseHandle_Sound(puVar11);
    }
    ResourceRead_Dispatch((int)param_2 + (int)*(short *)(*param_2 + 0x2b8));
    puVar11 = (int *)GetResourceByTypeId(0x5475726e,1000);
    if (puVar11 != (int *)0x0) {
      iVar10 = GetHandleByteCount(puVar11);
      if ((iVar10 != 0) && (*(char *)*puVar11 == '\x01')) {
        bVar1 = true;
      }
      if (!bVar1) {
        iVar10 = *piVar2;
        *(char *)(iVar10 + 0x9a) = 1;
        uVar12 = GetTickCount();
        *(int *)(iVar10 + 0x9c) = uVar12;
      }
      ReleaseHandle_Sound(puVar11);
      uVar14 = 0;
    }
    ResourceRead_Dispatch((int)param_2 + (int)*(short *)(*param_2 + 0x2b8));
    local_58 = 0;
    local_54 = 0;
    local_4c = 0;
    local_48 = 0;
    local_50 = *puVar7;
    *puVar7 = auStack_158;
    iVar10 = TrySetjmp(auStack_158);
    if (iVar10 == 0) {
      iVar10 = 0;
      do {
        puVar11 = (int *)GetResourceByTypeId(0x4d61696c,(short)iVar10 + 1000);
        if (puVar11 != (int *)0x0) {
          uVar8 = GetHandleByteCount(puVar11);
          uVar12 = AllocateBlock();
          local_3c = puVar11;
          local_38 = GetHandleFlags(puVar11);
          DetachResource(local_3c);
          CopyDataToBuffer(*puVar11,uVar12,uVar8);
          *(int *)(param_1 + iVar10 * 4 + 0x170) = uVar12;
          *(char *)(param_1 + 0x16e) = 1;
          *(char *)(param_1 + 0x16f) = uVar14;
          if (((char*)0) != (char *)0x3c) {
            RestoreHandleFlags(local_3c,local_38);
          }
          if (puVar11 != (int *)0x0) {
            ReleaseHandle_Sound(puVar11);
          }
        }
        sVar13 = (short)iVar10 + 1;
        iVar10 = (int)sVar13;
      } while (sVar13 < 8);
      lVar9 = GetResourceByTypeId(0x4d4b696b,1000);
      *(int *)(param_1 + 0x16f) = lVar9 != 0;
      if ((int)lVar9 != 0) {
        ReleaseHandle_Sound(lVar9);
      }
      *puVar7 = local_50;
    }
    else {
      FreeBlock(0);
      LockHandleRange(local_58,local_54);
    }
  }
  LoadAndInitializeGame(param_2,0,*(char *)(param_1 + 0x16c),!bVar1);
  iVar10 = *piVar5;
  if ((*(short *)(iVar10 + 0x124) == 0) && (*(char *)(*piVar2 + 0x9a) == '\0')) {
    CenterMapOnLocation(*(short *)(iVar10 + 0x17e),*(short *)(iVar10 + 0x180));
  }
  if (bVar1) {
    RestoreDisplayState();
  }
  else if ((*(char *)(param_1 + 0x16e) != '\0') &&
          (iVar10 = *piVar2, *(char *)(iVar10 + 0x9a) != '\0')) {
    *(char *)(iVar10 + 0x9a) = 1;
    uVar12 = GetTickCount();
    *(int *)(iVar10 + 0x9c) = uVar12;
  }
  return;
}

/* Address: 0x1007e314 Size: 1408 bytes */
void FUN_1007e314(int param_1)

{
  int *piVar1;
  int *piVar2;
  int *piVar3;
  int bVar4;
  short *psVar5;
  int *piVar6;
  int *piVar7;
  int iVar8;
  short sVar9;
  int iVar10;
  char cVar11;
  short sVar12;
  short sVar13;
  short uVar16;
  long long uVar14;
  long long uVar15;
  int local_14c;
  char local_128 [296];
  
  iVar8 = gDisplayState;
  piVar7 = piRam101176e0;
  piVar6 = piRam1011734c;
  piVar1 = gSidePanelWindow;
  psVar5 = gUIShortGlobal;
  piVar2 = gInfoWindow;
  piVar3 = gOverviewWindow;
  SuperDoSetupMenus(param_1);
  if ((*(char *)(param_1 + 0x16c) == '\0') && (*(char *)(*piVar6 + 0x9a) != '\0')) {
    EnableMenuItem(0x1f,0);
    EnableMenuItem(0x1e,0);
    EnableMenuItem(0x20,0);
    EnableMenuItem(0x21,0);
    EnableMenuItem(0x24,0);
  }
  if (*(char *)(*piVar6 + 0x99) == '\0') {
    local_128[0] = 0;
    SetPanelVisibility(*piVar6,0x76f,*piVar1 != 0);
    piVar1 = (int *)*piVar1;
    if ((piVar1 == (int *)0x0) ||
       (iVar10 = ResourceRead_Dispatch((int)*(short *)(*piVar1 + 0x668) + (int)piVar1), iVar10 == 0)) {
      bVar4 = true;
    }
    else {
      bVar4 = false;
    }
    if (bVar4) {
      uVar16 = 1;
    }
    else {
      uVar16 = 2;
    }
    FUN_100009d8(local_128,0x7da,uVar16);
    FUN_100f3284(0x76f,local_128);
    SetPanelVisibility(*piVar6,0x770,*piVar2 != 0);
    piVar2 = (int *)*piVar2;
    if ((piVar2 == (int *)0x0) ||
       (iVar10 = ResourceRead_Dispatch((int)*(short *)(*piVar2 + 0x668) + (int)piVar2), iVar10 == 0)) {
      bVar4 = true;
    }
    else {
      bVar4 = false;
    }
    if (bVar4) {
      uVar16 = 3;
    }
    else {
      uVar16 = 4;
    }
    FUN_100009d8(local_128,0x7da,uVar16);
    FUN_100f3284(0x770,local_128);
    SetPanelVisibility(*piVar6,0x771,*piVar3 != 0);
    piVar3 = (int *)*piVar3;
    if ((piVar3 == (int *)0x0) ||
       (iVar10 = ResourceRead_Dispatch((int)*(short *)(*piVar3 + 0x668) + (int)piVar3), iVar10 == 0)) {
      bVar4 = true;
    }
    else {
      bVar4 = false;
    }
    if (bVar4) {
      uVar16 = 5;
    }
    else {
      uVar16 = 6;
    }
    FUN_100009d8(local_128,0x7da,uVar16);
    iVar10 = local_14c;
    FUN_100f3284(0x771,local_128);
    if (*(char *)(param_1 + 0x144) != '\0') {
      SetPanelVisibility(*piVar6,0x772,1);
      SetPanelVisibility(*piVar6,0x76d,1);
      if (((*(short *)(**(int **)(iVar10 + -0x544) +
                       *(short *)(**(int **)(iVar10 + -0x544) + 0x110) * 2 + 0xd0) == 0) &&
          (piVar1 = (int *)*piVar6, *(char *)((int)piVar1 + 0x9a) == '\0')) &&
         (iVar10 = ResourceRead_Dispatch((int)*(short *)(*piVar1 + 0x318) + (int)piVar1), iVar10 == 0)) {
        FUN_1003f6e8();
        cVar11 = FUN_10056534();
        iVar10 = *piVar7;
        if (((iVar10 == 0) || (*(short *)(iVar10 + 0x12) < 0)) || (*(short *)(iVar10 + 0x14) < 0)) {
          uVar14 = 0;
        }
        else {
          uVar14 = 1;
        }
        if ((*piVar7 == 0) || (iVar10 = FUN_1005cd18(), iVar10 != 0)) {
          uVar15 = 0;
        }
        else {
          uVar15 = 1;
        }
        SetPanelVisibility(*piVar6,0x578,uVar15);
        sVar12 = 0;
        iVar10 = 0;
        sVar9 = 0;
        if (0 < *psVar5) {
          do {
            sVar12 = sVar9;
            if (*(short *)(iVar10 * 2 + iVar8) != 0) {
              sVar12 = sVar12 + 1;
            }
            sVar13 = (short)iVar10 + 1;
            iVar10 = (int)sVar13;
            sVar9 = sVar12;
          } while (sVar13 < *psVar5);
        }
        if ((*piVar7 == 0) || (sVar12 < 2)) {
          uVar15 = 0;
        }
        else {
          uVar15 = 1;
        }
        SetPanelVisibility(*piVar6,0x579,uVar15);
        if ((*piVar7 == 0) ||
           (**(short **)(local_14c + -0x1a10) <= **(short **)(local_14c + -0x1a04))) {
          uVar15 = 0;
        }
        else {
          uVar15 = 1;
        }
        SetPanelVisibility(*piVar6,0x57a,uVar15);
        SetPanelVisibility(*piVar6,0x57b,1);
        SetPanelVisibility(*piVar6,0x57c,uVar14);
        SetPanelVisibility(*piVar6,0x57d,cVar11);
        if ((*piVar7 == 0) || (cVar11 == '\0')) {
          uVar15 = 0;
        }
        else {
          uVar15 = 1;
        }
        SetPanelVisibility(*piVar6,0x57e,uVar15);
        SetPanelVisibility(*piVar6,0x57f,*piVar7 != 0);
        SetPanelVisibility(*piVar6,0x580,*piVar7 != 0);
        SetPanelVisibility(*piVar6,0x581,*piVar7 != 0);
        SetPanelVisibility(*piVar6,0x582,uVar14);
      }
    }
  }
  else {
    EnableMenuItem(0x1f,0);
    EnableMenuItem(0x1e,*(char *)(param_1 + 0x16c));
    EnableMenuItem(0x22,0);
  }
  return;
}











































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1007e894 Size: 1000 bytes */
void FUN_1007e894(int param_1,int *param_2,char param_3)

{
  int iVar1;
  int uVar2;
  int *piVar3;
  int *puVar4;
  int *ppuVar5;
  int iVar8;
  int *puVar9;
  long long uVar6;
  long long uVar7;
  short sVar10;
  char auStack_250 [264];
  int local_148;
  char auStack_144 [256];
  short local_44;
  int local_40;
  int local_3c;
  int local_38;
  int local_34;
  
  puVar4 = puRam10117370;
  piVar3 = piRam1011734c;
  uVar2 = uRam10116a54;
  ppuVar5 = 0 /* TVect base */;
  SuperDoWrite(param_1,param_2,param_3);
  BuildPascalString(auStack_250,ZEXT48((*(int*)((char*)ppuVar5 - 0x56a))) + 0x14);
  if (*(char *)(param_1 + 0x16c) == '\0') {
    WriteGameStateCrossFormat(param_2);
  }
  else {
    WriteGameState(param_2);
    local_44 = 0;
    local_40 = 0;
    local_38 = 0;
    local_34 = 0;
    local_3c = *puVar4;
    *puVar4 = auStack_144;
    iVar8 = TrySetjmp(auStack_144);
    if (iVar8 == 0) {
      puVar9 = (int *)AllocateHandle(2);
      *(short *)*puVar9 = 1;
      uVar6 = ConvertToString(uVar2);
      AddResource(puVar9,0x54797065,1000,uVar6);
      MarkChanged();
      ReleaseResource_Thunk(puVar9);
      *puVar4 = local_3c;
    }
    else {
      ReleaseHandle_Mapgen(0);
      LockHandleRange(local_44,local_40);
    }
    if (*(char *)(param_1 + 0x16e) != '\0') {
      ResourceRead_Dispatch((int)param_2 + (int)*(short *)(*param_2 + 0x2b8));
      local_148 = 0;
      local_44 = 0;
      local_40 = 0;
      local_38 = 0;
      local_34 = 0;
      local_3c = *puVar4;
      *puVar4 = auStack_144;
      iVar8 = TrySetjmp(auStack_144);
      if (iVar8 == 0) {
        iVar8 = 0;
        do {
          iVar1 = *(int *)(param_1 + iVar8 * 4 + 0x170);
          if (iVar1 != 0) {
            uVar6 = GetHandleDataSize(iVar1);
            CopyHandleData(iVar1,&local_148,uVar6);
            EndFocus();
            uVar6 = ConvertToString(uVar2);
            AddResource(local_148,0x4d61696c,(short)iVar8 + 1000,uVar6);
            MarkChanged();
            ReleaseResource_Thunk(local_148);
            local_148 = 0;
          }
          sVar10 = (short)iVar8 + 1;
          iVar8 = (int)sVar10;
        } while (sVar10 < 8);
        if (*(char *)(param_1 + 0x16f) != '\0') {
          uVar6 = AllocateHandle(0);
          local_148 = (int)uVar6;
          uVar7 = ConvertToString(uVar2);
          AddResource(uVar6,0x4d4b696b,1000,uVar7);
          MarkChanged();
          ReleaseResource_Thunk(local_148);
          local_148 = 0;
          *(char *)(param_1 + 0x16f) = 0;
        }
        *puVar4 = local_3c;
      }
      else {
        local_148 = ReleaseHandle_Mapgen(local_148);
        LockHandleRange(local_44,local_40);
      }
    }
    iVar8 = *piVar3;
    if ((*(char *)(iVar8 + 0x9a) != '\0') || (*(char *)(iVar8 + 0x99) != '\0')) {
      local_44 = 0;
      local_40 = 0;
      local_38 = 0;
      local_34 = 0;
      local_3c = *puVar4;
      *puVar4 = auStack_144;
      iVar8 = TrySetjmp(auStack_144);
      if (iVar8 == 0) {
        puVar9 = (int *)AllocateHandle(2);
        *(char *)*puVar9 = *(char *)(*piVar3 + 0x99);
        uVar6 = ConvertToString(uVar2);
        AddResource(puVar9,0x5475726e,1000,uVar6);
        MarkChanged();
        ReleaseResource_Thunk(puVar9);
        *puVar4 = local_3c;
      }
      else {
        ReleaseHandle_Mapgen(0);
        LockHandleRange(local_44,local_40);
      }
    }
  }
  return;
}





















































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x100a6708 Size: 216 bytes */
void ScanForExistingCities()

{
  int iVar1;
  int *piVar2;
  int *piVar3;
  int *piVar4;
  short sVar5;
  int iVar6;
  int iVar7;
  
  piVar4 = gMapGenConfig;
  piVar3 = gMapGenData;
  piVar2 = piRam1011735c;
  iVar6 = 0;
  do {
    iVar7 = 0;
    do {
      iVar1 = *piVar3 + iVar7 * 0x70 + iVar6;
      if (((*(char *)(iVar1 + 0x34a) == '\n') && (*(char *)(iVar1 + 0x2da) != '\n')) &&
         (*(char *)(iVar1 + 0x349) != '\n')) {
        *(short *)(*piVar4 + 0xa84) = *(short *)(*piVar4 + 0xa84) + 1;
        *(short *)(*piVar2 + *(short *)(*piVar2 + 0x1602) * 0x42 + 0x1604) = (short)iVar6;
        *(short *)(*piVar2 + *(short *)(*piVar2 + 0x1602) * 0x42 + 0x1606) = (short)iVar7;
        *(short *)(*piVar2 + 0x1602) = *(short *)(*piVar2 + 0x1602) + 1;
      }
      sVar5 = (short)iVar7 + 1;
      iVar7 = (int)sVar5;
    } while (sVar5 < 0x9c);
    sVar5 = (short)iVar6 + 1;
    iVar6 = (int)sVar5;
  } while (sVar5 < 0x70);
  return;
}

/* Address: 0x100a9d34 Size: 580 bytes */
int FUN_100a9d34(int param_1,short param_2)

{
  short sVar1;
  int iVar2;
  int *piVar3;
  int iVar4;
  int iVar5;
  short sVar6;
  short sVar7;
  int iVar8;
  int iVar9;
  
  piVar3 = piRam1011735c;
  if ((*(short *)(param_1 + 400) == -1) && (sVar7 = param_2, param_2 != 0)) {
    do {
      sVar7 = sVar7 + -1;
      iVar9 = (int)sVar7;
      sVar1 = *(short *)(*piVar3 + 0x1602);
      iVar2 = 10000;
      while (sVar1 != 0) {
        sVar1 = sVar1 + -1;
        iVar4 = *piVar3 + sVar1 * 0x42;
        iVar4 = CalcDistance(*(short *)(param_1 + iVar9 * 2),
                             *(short *)(param_1 + iVar9 * 2 + 100),
                             *(short *)(iVar4 + 0x1604),*(short *)(iVar4 + 0x1606));
        if (iVar4 < iVar2) {
          iVar2 = iVar4;
        }
      }
      *(short *)(param_1 + iVar9 * 2 + 400) = (short)iVar2;
    } while (iVar9 != 0);
  }
  sVar7 = param_2;
  iVar2 = -1;
  sVar1 = -1;
  if (param_2 != 0) {
    do {
      sVar7 = sVar7 + -1;
      iVar4 = (int)sVar7;
      iVar9 = 10000;
      if (*(short *)(param_1 + iVar4 * 2 + 300) == 0) {
        sVar6 = param_2;
        if (param_2 != 0) {
          do {
            sVar6 = sVar6 + -1;
            iVar8 = sVar6 * 2;
            iVar5 = param_1 + iVar8;
            if (*(short *)(iVar5 + 300) != 0) {
              iVar5 = CalcDistance(*(short *)(param_1 + iVar4 * 2),
                                   *(short *)(param_1 + iVar4 * 2 + 100),
                                   *(short *)(param_1 + iVar8),*(short *)(iVar5 + 100));
              if (iVar5 < iVar9) {
                iVar9 = iVar5;
              }
            }
          } while (sVar6 != 0);
        }
        *(short *)(param_1 + iVar4 * 2 + 500) = (short)iVar9;
      }
    } while (iVar4 != 0);
  }
  while (param_2 != 0) {
    param_2 = param_2 + -1;
    iVar9 = param_1 + param_2 * 2;
    if ((*(short *)(iVar9 + 300) == 0) && (9 < *(short *)(iVar9 + 500))) {
      sVar7 = 0x1e - *(short *)(iVar9 + 400);
      if (sVar7 < 0) {
        sVar7 = 0;
      }
      sVar6 = RandomRange(1,0xf,1);
      if (sVar1 < (short)(sVar7 + sVar6)) {
        iVar2 = (int)param_2;
        sVar1 = sVar7 + sVar6;
      }
    }
  }
  return iVar2;
}

/* Address: 0x100a9f78 Size: 972 bytes */
void FUN_100a9f78(int param_1)

{
  int bVar1;
  int iVar2;
  unsigned int uVar3;
  int bVar4;
  int iVar5;
  int *piVar6;
  int *piVar7;
  int *piVar8;
  int iVar9;
  int iVar10;
  unsigned long long uVar11;
  short sVar12;
  unsigned long long uVar13;
  short sVar14;
  
  piVar8 = gMapGenData;
  piVar7 = piRam10117358;
  piVar6 = piRam10117354;
  iVar10 = 0;
  uVar11 = 1;
  do {
    uVar13 = 1;
    do {
      uVar3 = *(unsigned int *)(*piVar7 + (int)(((uVar11 & 0x1fffffff) * 8 - uVar11 & 0xffffffff) << 5) +
                       (int)((uVar13 & 0xffffffff) << 1)) >> 0x18;
      sVar14 = (short)uVar13;
      sVar12 = (short)uVar11;
      if ((((uVar3 == 0x26) || (uVar3 == 0x16)) &&
          ((uVar3 = *(unsigned int *)(*piVar7 + (int)(((uVar11 & 0x1fffffff) * 8 - uVar11 & 0xffffffff) << 5
                                             ) + (int)((uVar13 & 0xffffffff) << 1) + 2) >> 0x18,
           uVar3 == 0x28 || (uVar3 == 0x18)))) ||
         (((uVar3 = *(unsigned int *)(*piVar7 + (int)(((uVar11 & 0x1fffffff) * 8 - uVar11 & 0xffffffff) << 5
                                             ) + (int)((uVar13 & 0xffffffff) << 1)) >> 0x18,
           uVar3 == 0x21 || (uVar3 == 0x11)) &&
          ((uVar3 = *(unsigned int *)((int)(((uVar11 & 0x1fffffff) * 8 - uVar11 & 0xffffffff) << 5) +
                              *piVar7 + (int)((uVar13 & 0xffffffff) << 1) + 0xe0) >> 0x18,
           uVar3 == 0x24 || (uVar3 == 0x14)))))) {
        bVar4 = false;
        bVar1 = *(unsigned int *)(*piVar7 + (int)(((uVar11 & 0x1fffffff) * 8 - uVar11 & 0xffffffff) << 5) +
                         (int)((uVar13 & 0xffffffff) << 1)) >> 0x18 == 0x26;
        if (bVar1) {
          iVar9 = FUN_100a9ce0(sVar14 + -1,uVar11);
          if ((iVar9 == 0) || (iVar9 = FUN_100a9ce0(sVar14 + 2,uVar11), iVar9 == 0)) {
            iVar9 = FUN_100a9ce0(uVar13,sVar12 + -1);
            if ((iVar9 != 0) && (iVar9 = FUN_100a9ce0(uVar13,sVar12 + 2), iVar9 != 0)) {
              bVar4 = true;
            }
          }
          else {
            bVar4 = true;
          }
        }
        if ((bVar4) && (iVar10 < 0x32)) {
          *(short *)(param_1 + iVar10 * 2) = sVar14;
          iVar9 = param_1 + iVar10 * 2;
          *(short *)(iVar9 + 100) = sVar12;
          *(unsigned short *)(iVar9 + 200) = (unsigned short)bVar1;
          iVar10 = (int)(short)((short)iVar10 + 1);
        }
      }
      uVar13 = (unsigned long long)(short)(sVar14 + 1);
    } while ((short)(sVar14 + 1) < 0x6e);
    uVar11 = (unsigned long long)(short)(sVar12 + 1);
  } while ((short)(sVar12 + 1) < 0x9a);
  if (iVar10 != 0) {
    iVar9 = FindNextRoadSegment(param_1,iVar10);
    while (iVar9 != -1) {
      iVar2 = param_1 + iVar9 * 2;
      *(short *)(iVar2 + 300) = 1;
      sVar12 = *(short *)(param_1 + iVar9 * 2);
      uVar11 = (unsigned long long)*(short *)(iVar2 + 100);
      if (*(short *)(iVar2 + 200) == 0) {
        iVar5 = (int)(((uVar11 & 0x1fffffff) * 8 - uVar11 & 0x7ffffff) << 5);
        iVar9 = *piVar7 + iVar5;
        iVar2 = (int)(((long long)sVar12 & 0x7fffffffU) << 1);
        *(unsigned int *)(iVar9 + iVar2) = *(unsigned int *)(iVar9 + iVar2) & 0xffffff | 0x84000000;
        iVar2 = iVar5 + *piVar7 + iVar2;
        *(unsigned int *)(iVar2 + 0xe0) = *(unsigned int *)(iVar2 + 0xe0) & 0xffffff | 0x94000000;
        iVar9 = (int)(((uVar11 & 0x1fffffff) * 8 - uVar11 & 0xfffffff) << 4);
        *(char *)(iVar9 + *piVar8 + (int)sVar12 + 0x3ba) = 1;
        iVar2 = iVar9 + *piVar8 + (int)sVar12;
        *(char *)(iVar2 + 0x34a) = *(char *)(iVar2 + 0x3ba);
        iVar2 = iVar9 + *piVar6 + (int)sVar12;
        *(unsigned int *)(iVar2 + 0xe0) = *(unsigned int *)(iVar2 + 0xe0) & 0xe0ffffff | 0x1000000;
        iVar9 = iVar9 + *piVar6 + (int)sVar12;
        *(unsigned int *)(iVar9 + -0x70) = *(unsigned int *)(iVar9 + -0x70) & 0xe0ffffff | 0x1000000;
      }
      else {
        iVar5 = (int)(((uVar11 & 0x1fffffff) * 8 - uVar11 & 0x7ffffff) << 5);
        iVar9 = *piVar7 + iVar5;
        iVar2 = (int)(((long long)sVar12 & 0x7fffffffU) << 1);
        *(unsigned int *)(iVar9 + iVar2) = *(unsigned int *)(iVar9 + iVar2) & 0xffffff | 0x85000000;
        iVar2 = iVar5 + *piVar7 + iVar2;
        *(unsigned int *)(iVar2 + 2) = *(unsigned int *)(iVar2 + 2) & 0xffffff | 0x86000000;
        iVar9 = (int)(((uVar11 & 0x1fffffff) * 8 - uVar11 & 0xfffffff) << 4);
        *(char *)(*piVar8 + iVar9 + (int)sVar12 + 0x34b) = 1;
        iVar2 = iVar9 + *piVar8 + (int)sVar12;
        *(char *)(iVar2 + 0x34a) = *(char *)(iVar2 + 0x34b);
        *(unsigned int *)(iVar9 + *piVar6 + (int)(short)(sVar12 + 2)) =
             *(unsigned int *)(iVar9 + *piVar6 + (int)(short)(sVar12 + 2)) & 0xe0ffffff | 0x1000000;
        *(unsigned int *)(iVar9 + *piVar6 + (int)(short)(sVar12 + -1)) =
             *(unsigned int *)(iVar9 + *piVar6 + (int)(short)(sVar12 + -1)) & 0xe0ffffff | 0x1000000;
      }
      iVar9 = FindNextRoadSegment(param_1,iVar10);
    }
  }
  return;
}




/* Address: 0x100eb91c Size: 308 bytes */
void FUN_100eb91c(int param_1,int *param_2,long long param_3,long long param_4,char param_5)

{
  int *piVar1;
  long long uVar2;
  long long lVar3;
  int iVar4;
  int uVar5;
  int local_3c;
  
  piVar1 = piRam10117370;
  *(int *)(param_1 + 0x130) = param_1;
  *(int *)(param_1 + 300) = param_1;
  *(int *)(param_1 + 8) = 0;
  *(int *)(param_1 + 0xc) = 0;
  *(int *)(param_1 + 4) = 0;
  *(char *)(param_1 + 0x14) = param_5;
  *(int **)(param_1 + 0x10) = param_2;
  if (param_2 != (int *)0x0) {
    uVar2 = ResourceRead_Dispatch((int)param_2 + (int)*(short *)(*param_2 + 0xa8));
    lVar3 = FUN_100f0334(0,param_4,uVar2);
    *(int *)(param_1 + 0xc) = (int)lVar3;
    iVar4 = FUN_100f0334(1,param_3,lVar3 + 1);
    *(int *)(param_1 + 8) = iVar4;
    if (*(int *)(param_1 + 0xc) < iVar4) {
      *(int *)(param_1 + 8) = 0;
      *(int *)(param_1 + 0xc) = 0;
    }
    if (param_5 == '\0') {
      *(int *)(param_1 + 4) = *(int *)(param_1 + 0xc);
    }
    else {
      *(int *)(param_1 + 4) = *(int *)(param_1 + 8);
    }
    uVar5 = FUN_100eb8b0(param_1,*(int *)(*(int *)(param_1 + 0x10) + 4));
    *(int *)(*(int *)(param_1 + 0x10) + 4) = uVar5;
    *(short *)(param_1 + 0x118) = 0;
    *(int *)(param_1 + 0x11c) = 0;
    *(int *)(param_1 + 0x120) = 0;
    *(int *)(param_1 + 0x124) = 0;
    *(int *)(param_1 + 0x128) = 0;
    uVar5 = *(int *)(local_3c + -0x824);
    *(int *)(param_1 + 0x120) = *piVar1;
    *piVar1 = param_1 + 0x18;
    *(int *)(param_1 + 0x124) = uVar5;
    *(int *)(param_1 + 0x128) = param_1;
  }
  return;
}




/* Address: stubbed - had compile errors on 64-bit */
void FUN_10114e58() { }




































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* === Recovered from stubs.c === */

/* Address: 0x1002770c Size: 16 bytes */
char FUN_1002770c()

{
  return *(char *)(*piRam1011734c + 0x1a4);
}

/* Address: 0x1003c368 Size: 1232 bytes */
void FUN_1003c368()

{
  int bVar1;
  int *piVar2;
  int *piVar3;
  int *piVar4;
  int *piVar5;
  unsigned long long uVar6;
  int iVar7;
  int iVar8;
  short uVar9;
  int iVar10;
  unsigned short uVar11;
  unsigned short uVar12;
  short sVar14;
  int iVar13;
  short sVar15;
  unsigned long long uVar16;
  unsigned long long uVar17;
  
  iVar13 = gCombatDisplayPieces;
  piVar5 = piRam10117360;
  piVar4 = piRam1011735c;
  piVar3 = piRam10117358;
  piVar2 = piRam10117354;
  uVar17 = ZEXT48(0 /* TVect base */);
  InitArmyProduction();
  InitPlayerData();
  sVar15 = 0;
  uVar16 = 0;
  uVar11 = 1;
  do {
    iVar7 = *piVar4 + (int)(((uVar16 & 0x3fffffff) * 4 + uVar16 & 0xffffffff) << 2);
    iVar8 = LookupCityAtPos(*(short *)(iVar7 + 0x18a),*(short *)(iVar7 + 0x18c));
    iVar7 = (int)uVar17;
    if (iVar8 < 0) {
      iVar8 = (int)((uVar16 & 0xffffffff) << 1);
      *(short *)(*piVar4 + iVar8 + 0xd0) = 1;
      *(short *)(iVar8 + *piVar4 + 0x138) = 0;
    }
    sVar14 = (short)uVar16 + 1;
    uVar16 = (unsigned long long)sVar14;
  } while (sVar14 < 8);
  *(short *)(*piVar4 + 0x118) = 0;
  *(short *)(*piVar4 + 0x15e) = 0;
  *(short *)(*piVar4 + 0x136) = 1;
  *(short *)(*piVar4 + 0x182) = 10;
  *(short *)(*piVar4 + 0x158) = 0;
  iVar8 = 0;
  do {
    if (*(short *)(*piVar4 + iVar8 * 2 + 0xd0) == 0) {
      sVar15 = sVar15 + 1;
    }
    sVar14 = (short)iVar8 + 1;
    iVar8 = (int)sVar14;
  } while (sVar14 < 8);
  if (((*(short *)(*piVar4 + 0x124) != 0) &&
      (uVar11 = (unsigned short)(sVar15 < 1), *(short *)(*piVar4 + 0x124) != 0)) && (sVar15 == 1)) {
    **(short **)(iVar7 + -0x1a18) = 1;
  }
  uVar17 = 0;
  do {
    iVar8 = (int)((uVar17 & 0x7fffffff) << 1);
    *(unsigned short *)(*piVar4 + iVar8 + 0x148) = uVar11;
    if (*(short *)(iVar8 + *piVar4 + 0x138) == 0) {
      *(short *)
       (*piVar4 + (int)(((uVar17 & 0x3fffffff) * 4 + uVar17 & 0xffffffff) << 2) + 0x186) = 0;
    }
    sVar15 = (short)uVar17 + 1;
    uVar17 = (unsigned long long)sVar15;
  } while (sVar15 < 8);
  FUN_1003956c();
  if (*piVar5 == 0) {
    iVar8 = AllocateBlock(22000);
    *piVar5 = iVar8;
  }
  iVar8 = 999;
  do {
    iVar10 = iVar8 * 0x16;
    *(char *)(*piVar5 + iVar10 + 5) = 0xff;
    *(short *)(iVar10 + *piVar5) = 0xffff;
    *(short *)(iVar10 + *piVar5 + 2) = 0xffff;
    bVar1 = iVar8 != 0;
    iVar8 = (int)(short)((short)iVar8 + -1);
  } while (bVar1);
  **(int **)(iVar7 + -0x3f0) = 0;
  **(int **)(iVar7 + -0x1c0) = 0;
  iVar8 = 0;
  do {
    *(int *)(iVar8 * 4 + iVar13) = 0;
    sVar15 = (short)iVar8 + 1;
    iVar8 = (int)sVar15;
  } while (sVar15 < 8);
  FUN_1003c068();
  FUN_100099cc();
  FUN_100090f0();
  FUN_10025f2c();
  FUN_1002cbbc();
  uVar16 = (unsigned long long)*(short *)(*piVar4 + 0x110);
  uVar17 = uVar16;
  if (*(short *)(*piVar4 + 0x124) != 0) {
    uVar6 = 0;
    do {
      uVar17 = uVar6;
      if (*(short *)(*piVar4 + (int)((uVar17 & 0xffffffff) << 1) + 0xd0) == 0) break;
      sVar15 = (short)uVar17 + 1;
      uVar17 = uVar16;
      uVar6 = (long long)sVar15;
    } while (sVar15 < 8);
    CenterMapOnArmy(uVar17);
  }
  iVar13 = (int)(((uVar17 & 0x3fffffff) * 4 + uVar17 & 0xffffffff) << 2);
  *(short *)(*piVar4 + 0x17e) = *(short *)(*piVar4 + iVar13 + 0x18a);
  *(short *)(*piVar4 + 0x180) = *(short *)(iVar13 + *piVar4 + 0x18c);
  uVar11 = **(short **)(iVar7 + -0x1510) / 0x28;
  uVar12 = **(short **)(iVar7 + -0x1514) / 0x28;
  sVar15 = *(short *)(*piVar4 + 0x17e) -
           (((short)uVar11 >> 1) + (unsigned short)((short)uVar11 < 0 && (uVar11 & 1) != 0));
  if (sVar15 < 0) {
    sVar15 = 0;
  }
  *(short *)(*piVar4 + 0x178) = sVar15;
  sVar15 = *(short *)(*piVar4 + 0x180) -
           (((short)uVar12 >> 1) + (unsigned short)((short)uVar12 < 0 && (uVar12 & 1) != 0));
  if (sVar15 < 0) {
    sVar15 = 0;
  }
  *(short *)(*piVar4 + 0x176) = sVar15;
  sVar15 = *(short *)(*piVar4 + 0x178) + uVar11 + -1;
  if (0x6f < sVar15) {
    sVar15 = 0x6f;
  }
  *(short *)(*piVar4 + 0x17c) = sVar15;
  sVar15 = *(short *)(*piVar4 + 0x176) + uVar12 + -1;
  if (0x9b < sVar15) {
    sVar15 = 0x9b;
  }
  *(short *)(*piVar4 + 0x17a) = sVar15;
  iVar13 = 0;
  do {
    uVar17 = 0;
    do {
      iVar7 = *piVar3 + (int)(((uVar17 & 0x1fffffff) * 8 - uVar17 & 0xffffffff) << 5);
      *(unsigned int *)(iVar7 + iVar13 * 2) = *(unsigned int *)(iVar7 + iVar13 * 2) & 0xffdfffff;
      iVar7 = *piVar2 + (int)(((uVar17 & 0x1fffffff) * 8 - uVar17 & 0xffffffff) << 4);
      *(unsigned int *)(iVar7 + iVar13) = *(unsigned int *)(iVar7 + iVar13) & 0xbfffffff;
      sVar15 = (short)uVar17 + 1;
      uVar17 = (unsigned long long)sVar15;
    } while (sVar15 < 0x9c);
    sVar15 = (short)iVar13 + 1;
    iVar13 = (int)sVar15;
  } while (sVar15 < 0x70);
  FUN_1005bbe8();
  iVar13 = 0;
  do {
    uVar9 = RandomRange(1,8,0);
    *(short *)(*piVar4 + iVar13 * 2 + 0x1122) = uVar9;
    if ((*(short *)(*piVar4 + 0x116) != 0) &&
       (iVar7 = *piVar4 + iVar13 * 2, *(short *)(iVar7 + 0xd0) == 0)) {
      *(short *)(iVar7 + 0x1122) = *(short *)(iVar7 + 0x1122) + 400;
    }
    sVar15 = (short)iVar13 + 1;
    iVar13 = (int)sVar15;
  } while (sVar15 < 8);
  FUN_100275ec();
  return;
}






































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































/* Address: 0x1003c838 Size: 256 bytes */
void FUN_1003c838()

{
  short uVar1;
  int *piVar2;
  int iVar3;
  short sVar4;
  int iVar5;
  
  piVar2 = piRam1011735c;
  if (*(short *)(*piRam1011735c + 0x122) == 0) {
    iVar5 = 0;
    do {
      *(short *)(*piVar2 + iVar5 * 2 + 0x164) = (short)iVar5;
      sVar4 = (short)iVar5 + 1;
      iVar5 = (int)sVar4;
    } while (sVar4 < 8);
  }
  else {
    iVar5 = 0;
    do {
      *(short *)(*piVar2 + iVar5 * 2 + 0x164) = (short)iVar5;
      sVar4 = (short)iVar5 + 1;
      iVar5 = (int)sVar4;
    } while (sVar4 < 8);
    sVar4 = 0;
    do {
      iVar3 = RandomRange(1,8,0xffffffffffffffff);
      iVar5 = RandomRange(1,8,0xffffffffffffffff);
      iVar5 = *piVar2 + iVar5 * 2;
      uVar1 = *(short *)(iVar5 + 0x164);
      *(short *)(iVar5 + 0x164) = *(short *)(*piVar2 + iVar3 * 2 + 0x164);
      *(short *)(iVar3 * 2 + *piVar2 + 0x164) = uVar1;
      sVar4 = sVar4 + 1;
    } while (sVar4 < 0x14);
  }
  *(short *)(*piVar2 + 0x174) = 0;
  return;
}

/* Address: 0x10064850 Size: 1588 bytes */
void FUN_10064850(short param_1,short param_2)

{
  unsigned short uVar1;
  int iVar2;
  int *piVar3;
  int *piVar4;
  unsigned int uVar5;
  int *piVar6;
  int iVar7;
  unsigned long long uVar8;
  unsigned long long uVar9;
  short sVar10;
  unsigned long long uVar11;
  int iVar12;
  int iVar13;
  int iVar14;
  unsigned int local_1fc;
  char auStack_1d8 [8];
  short local_1d0;
  short local_1ce;
  short local_1cc;
  short local_1ca;
  short local_1c8;
  short local_1c6;
  short local_1c4;
  short local_1c2;
  int local_1c0;
  int local_1bc;
  int local_1b8;
  int local_1b4;
  short local_1b0 [7];
  unsigned short auStack_1a2 [9];
  int local_190;
  int local_18c;
  int local_188 [2];
  unsigned short local_180 [8];
  short local_170 [8];
  short local_160;
  char auStack_15c [24];
  char auStack_144 [324];
  
  piVar4 = piRam1011735c;
  piVar3 = piRam10116bc0;
  piVar6 = (int *)*gMainGameWindow;
  if (piVar6 == (int *)0x0) {
    piVar6 = (int *)0x0;
  }
  else {
    piVar6 = (int *)ResourceRead_Dispatch((int)*(short *)(*piVar6 + 0x308) + (int)piVar6,0x7475726e);
  }
  if (piVar6 == (int *)0x0) {
    return;
  }
  iVar7 = ResourceRead_Dispatch((int)*(short *)(*piVar6 + 0x3a8) + (int)piVar6);
  if (iVar7 == 0) {
    return;
  }
  iVar7 = piVar6[0xe];
  uVar11 = 0;
  iVar12 = 0;
  uVar8 = 0;
  do {
    if (*(short *)(*piVar4 + iVar12 * 2 + 0x138) != 0) {
      uVar11 = (unsigned long long)(short)((short)uVar11 + 1);
    }
    sVar10 = (short)iVar12 + 1;
    iVar12 = (int)sVar10;
  } while (sVar10 < 8);
  iVar12 = (int)uVar11;
  if (iVar12 == 0) {
    return;
  }
  iVar13 = 0;
  do {
    local_170[iVar13] = 0;
    local_180[iVar13] = 0xffff;
    auStack_1a2[iVar13 + 1] = 0;
    local_1b0[iVar13] = 0;
    sVar10 = (short)iVar13 + 1;
    iVar13 = (int)sVar10;
  } while (sVar10 < 8);
  iVar13 = 0;
  uVar9 = 0;
  do {
    iVar14 = *piVar4;
    sVar10 = *(short *)(iVar14 + iVar13 * 2 + 0x164);
    if (*(short *)(iVar14 + sVar10 * 2 + 0x138) != 0) {
      iVar2 = (int)((uVar9 & 0xffffffff) << 1);
      *(short *)((int)local_180 + iVar2) = sVar10;
      *(short *)((int)local_170 + iVar2) =
           (short)iVar7 + (short)((uVar9 - uVar11 & 0xffffffff) << 4);
      if (sVar10 == *(short *)(iVar14 + 0x110)) {
        uVar8 = uVar9;
      }
      uVar9 = (unsigned long long)(short)((short)uVar9 + 1);
    }
    sVar10 = (short)iVar13 + 1;
    iVar13 = (int)sVar10;
  } while (sVar10 < 8);
  if ((int)uVar8 == 0) {
    if (iVar12 == 1) goto LAB_10064a08;
    auStack_1a2[1] = 1;
  }
  else {
    if (iVar12 != 1) {
      if ((int)uVar8 == iVar12 + -1) {
        iVar7 = (int)((uVar11 & 0x7fffffff) << 1);
        *(unsigned short *)((int)auStack_1a2 + iVar7) = (unsigned short)(1 < iVar12);
        *(short *)((int)local_1b0 + iVar7 + -4) = 1;
      }
      else {
        iVar7 = (int)((uVar8 & 0xffffffff) << 1);
        *(unsigned short *)((int)auStack_1a2 + iVar7 + 2) = (unsigned short)(1 < iVar12);
        *(short *)((int)local_1b0 + iVar7 + -2) = 1;
      }
      goto LAB_10064a90;
    }
LAB_10064a08:
    auStack_1a2[1] = 0;
  }
  *(short *)((int)local_1b0 + (int)((uVar11 & 0xffffffff) << 1) + -2) = 1;
LAB_10064a90:
  uVar5 = local_1fc;
  if (param_1 != 0) {
    local_1bc = piVar3[1];
    local_1b8 = piVar6[0xd];
    local_1c0 = *piVar3 + 1;
    local_1b4 = piVar6[0xe] + -0x80;
    ResourceRead_Dispatch((int)piVar6 + (int)*(short *)(*piVar6 + 0x288),&local_1c0,auStack_1d8);
    GetBitMapPtr(auStack_1d8);
    Render_GetPixBaseAddr();
    local_160 = *(short *)(*piVar4 + 0x136);
    sVar10 = local_160;
    if (local_160 < 2) {
      sVar10 = 1;
    }
    DrawNumber(auStack_15c,*(int *)(local_1fc - 0x188c),sVar10);
    BuildPascalString(auStack_144,auStack_15c);
    MoveTo_Thunk(2,0xc);
    ConvertToString(auStack_144);
    FUN_10000b88();
    uVar8 = (unsigned long long)local_1fc;
    iVar7 = 0;
    if (0 < iVar12) {
      do {
        StoreCoords(local_188,local_170[iVar7],2);
        uVar1 = local_180[iVar7];
        uVar11 = (long long)(short)uVar1 +
                 ((long long)((int)(short)uVar1 >> 2) +
                  (unsigned long long)((short)uVar1 < 0 && (uVar1 & 3) != 0) & 0x3fffffff) * -4;
        SetRect4(&local_190,
                     (short)(((long long)((int)(short)uVar1 >> 2) +
                              (unsigned long long)((short)uVar1 < 0 && (uVar1 & 3) != 0) & 0xffffffff) << 4)
                     + 0x70,(short)(((uVar11 & 0x1fffffff) * 8 - uVar11 & 0xffffffff) << 1) + 0x5e,
                     0x10,0xe);
        FUN_1004615c(9,local_190,local_18c,local_188[0]);
        uVar5 = (unsigned int)uVar8;
        sVar10 = (short)iVar7 + 1;
        iVar7 = (int)sVar10;
      } while (sVar10 < iVar12);
    }
  }
  iVar7 = -1;
  iVar13 = -1;
  iVar14 = 0;
  if (0 < iVar12) {
    do {
      if (local_1b0[iVar14] != 0) {
        iVar7 = iVar14;
      }
      sVar10 = (short)iVar14 + 1;
      iVar14 = (int)sVar10;
    } while (sVar10 < iVar12);
  }
  iVar14 = 0;
  if (0 < iVar12) {
    do {
      if (auStack_1a2[iVar14 + 1] != 0) {
        iVar13 = iVar14;
      }
      sVar10 = (short)iVar14 + 1;
      iVar14 = (int)sVar10;
    } while (sVar10 < iVar12);
  }
  if (param_1 == 0) {
    if ((-1 < iVar7) && (param_2 == 0)) {
      SetDrawingColor(0);
      sVar10 = local_170[iVar7];
      local_1c6 = sVar10 + -3;
      local_1c2 = sVar10 + 0xe;
      local_1c8 = 1;
      local_1c4 = 0x10;
      GetBitMapPtr(&local_1c8);
      PaintRect_Thunk();
      uVar5 = local_1fc;
      StoreCoords(local_188,sVar10,2);
      uVar1 = local_180[iVar7];
      uVar8 = (long long)(short)uVar1 +
              ((long long)((int)(short)uVar1 >> 2) +
               (unsigned long long)((short)uVar1 < 0 && (uVar1 & 3) != 0) & 0x3fffffff) * -4;
      SetRect4(&local_190,
                   (short)(((long long)((int)(short)uVar1 >> 2) +
                            (unsigned long long)((short)uVar1 < 0 && (uVar1 & 3) != 0) & 0xffffffff) << 4) +
                   0x70,(short)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 1) + 0x5e,0x10,
                   0xe);
      FUN_1004615c(9,local_190,local_18c,local_188[0]);
    }
    if ((-1 < iVar13) && (param_2 == 0)) {
      SetDrawingColor(0xff);
      sVar10 = local_170[iVar13];
      local_1ce = sVar10 + -3;
      local_1ca = sVar10 + 0xe;
      local_1d0 = 1;
      local_1cc = 0x10;
      GetBitMapPtr(&local_1d0);
      PaintRect_Thunk();
      uVar5 = local_1fc;
      StoreCoords(local_188,sVar10,2);
      uVar1 = local_180[iVar13];
      uVar8 = (long long)(short)uVar1 +
              ((long long)((int)(short)uVar1 >> 2) +
               (unsigned long long)((short)uVar1 < 0 && (uVar1 & 3) != 0) & 0x3fffffff) * -4;
      SetRect4(&local_190,
                   (short)(((long long)((int)(short)uVar1 >> 2) +
                            (unsigned long long)((short)uVar1 < 0 && (uVar1 & 3) != 0) & 0xffffffff) << 4) +
                   0x70,(short)(((uVar8 & 0x1fffffff) * 8 - uVar8 & 0xffffffff) << 1) + 0x5e,0x10,
                   0xe);
      FUN_1004615c(9,local_190,local_18c,local_188[0]);
    }
  }
  RGBForeColor_Thunk(*(int *)(uVar5 - 0x1d4));
  Render_PenNormal();
  RGBForeColor_Thunk((unsigned long long)*(unsigned int *)(local_1fc - 0x1d4) + 0x5fa);
  SetForeColor_Thunk();
  return;
}
























































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































